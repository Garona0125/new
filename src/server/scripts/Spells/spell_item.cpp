...
{
    SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_1 = 16595,
    SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_2 = 16593,
    SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_3 = 16591,
};
...
class spell_item_noggenfogger_elixir : public SpellScriptLoader
{
    public:
        ...
        class spell_item_noggenfogger_elixir_SpellScript : public SpellScript
        {
            ...
            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_1) || !sSpellMgr->GetSpellInfo(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_2) || !sSpellMgr->GetSpellInfo(SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_3))
                    return false;
                return true;
            }
            ...
            {
                Unit* caster = GetCaster();
                uint32 spellId = SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_3;
                switch (urand(1, 3))
                {
                    case 1:
						spellId = SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_1;
						break;
                    case 2:
						spellId = SPELL_NOGGENFOGGER_ELIXIR_TRIGGERED_2;
						break;
                }
        ...
        class spell_item_savory_deviate_delight_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                switch (urand(1, 2))
                {
                    // Flip Out - ninja
                    case 1:
						spellId = (caster->GetCurrentGender() == GENDER_MALE ? SPELL_FLIP_OUT_MALE : SPELL_FLIP_OUT_FEMALE);
						break;
                    // Yaaarrrr - pirate
                    case 2:
						spellId = (caster->GetCurrentGender() == GENDER_MALE ? SPELL_YAAARRRR_MALE : SPELL_YAAARRRR_FEMALE);
						break;
                }
                caster->CastSpell(caster, spellId, true, NULL);
            }
...
{
    SPELL_UNDERBELLY_ELIXIR_TRIGGERED_1 = 59645,
    SPELL_UNDERBELLY_ELIXIR_TRIGGERED_2 = 59831,
    SPELL_UNDERBELLY_ELIXIR_TRIGGERED_3 = 59843,
};
...
class spell_item_underbelly_elixir : public SpellScriptLoader
{
    public:
        ...
        class spell_item_underbelly_elixir_SpellScript : public SpellScript
        {
            ...
            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_UNDERBELLY_ELIXIR_TRIGGERED_1) || !sSpellMgr->GetSpellInfo(SPELL_UNDERBELLY_ELIXIR_TRIGGERED_2) || !sSpellMgr->GetSpellInfo(SPELL_UNDERBELLY_ELIXIR_TRIGGERED_3))
                    return false;
                return true;
            }
            ...
            {
                Unit* caster = GetCaster();
                uint32 spellId = SPELL_UNDERBELLY_ELIXIR_TRIGGERED_3;
                switch (urand(1, 3))
                {
                    case 1:
						spellId = SPELL_UNDERBELLY_ELIXIR_TRIGGERED_1;
						break;
                    case 2:
						spellId = SPELL_UNDERBELLY_ELIXIR_TRIGGERED_2;
						break;
                }
                caster->CastSpell(caster, spellId, true, NULL);
            }
...
class spell_item_map_of_the_geyser_fields : public SpellScriptLoader
{
    public:
        ...
        class spell_item_map_of_the_geyser_fields_SpellScript : public SpellScript
        {
            ...
            {
                Unit* caster = GetCaster();
                if (caster->FindCreatureWithDistance(NPC_SOUTH_SINKHOLE, 30.0f, true) ||
                    caster->FindCreatureWithDistance(NPC_NORTHEAST_SINKHOLE, 30.0f, true) ||
                    caster->FindCreatureWithDistance(NPC_NORTHWEST_SINKHOLE, 30.0f, true))
                    return SPELL_CAST_OK;
...
class spell_item_crystal_prison_dummy_dnd : public SpellScriptLoader
{
    public:
        ...
        class spell_item_crystal_prison_dummy_dnd_SpellScript : public SpellScript
        {
            ...
            bool Validate(SpellInfo const* /*spell*/)
            {
                if (!sObjectMgr->GetGameObjectTemplate(GO_IMPRISONED_DOOMGUARD))
                    return false;
                return true;
            }
            ...
            void HandleDummy(SpellEffIndex /* effIndex */)
            {
                if (Creature* target = GetHitCreature())
				{
                    ...
                    {
                        GetCaster()->SummonGameObject(GO_IMPRISONED_DOOMGUARD, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), 0, 0, 0, 0, uint32(target->GetRespawnTime()-time(NULL)));
    ...
    public:
        ...
        class spell_item_demon_broiled_surprise_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckRequirement()
            {
                ...
                if (Creature* creature = player->FindCreatureWithDistance(NPC_ABYSSAL_FLAMEBRINGER, 10, false))
                    if (creature->IsDead())
                        return SPELL_CAST_OK;
    ...
    public:
        ...
        class spell_item_refocus_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (!caster || caster->GetCurrentClass() != CLASS_HUNTER)
                    return;
    ...
    public:
        ...
        class spell_item_mind_control_cap_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /* effIndex */)
            {
                ...
                if (roll_chance_i(95))
                    caster->CastSpell(target, roll_chance_i(30) ? SPELL_DULLARD : SPELL_GNOMISH_MIND_CONTROL_CAP, true, GetCastItem()); // 30% chance unsuccessful Mind Control
                else
                    target->CastSpell(caster, SPELL_GNOMISH_MIND_CONTROL_CAP, true); // 5% chance backfire
            }
...
enum AretesGate
{
	SPELL_ARETES_GATES = 26560,
	SPELL_SUMMON_ARETE = 18280,

	QUEST_SECOND_TRIAL = 12847,
};

class spell_item_aretes_gates : public SpellScriptLoader
{
    public:
        spell_item_aretes_gates() : SpellScriptLoader("spell_item_aretes_gates") { }

        class spell_item_aretes_gates_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_item_aretes_gates_SpellScript);

            bool Validate(SpellInfo const* /*spell*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_ARETES_GATES))
                    return false;
                return true;
            }
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
				...
				player->CastSpell(player, SPELL_SUMMON_ARETE, true);
            }

            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_item_aretes_gates_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SUMMON_OBJECT_WILD);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_item_aretes_gates_SpellScript();
        }
};
