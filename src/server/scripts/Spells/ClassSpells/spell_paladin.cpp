...
enum PaladinSpells
{
	...
	SPELL_PALADIN_RIGHTEOUS_DEFENCE          = 31789,
	SPELL_PALADIN_RIGHTEOUS_DEFENCE_EFFECT_1 = 31790,
    ...
    public:
        ...
        class spell_pal_ardent_defender_AuraScript : public AuraScript
        {
            ...
            void Absorb(AuraEffect* aurEff, DamageInfo & dmgInfo, uint32 & absorbAmount)
            {
                ...
                uint32 allowedHealth = victim->CountPctFromMaxHealth(HEALTH_PERCENT_35);
                // If damage kills us
                if (remainingHealth <= 0 && !victim->ToPlayer()->HasSpellCooldown(SPELL_PALADIN_ARGENT_DEFENDER_HEAL))
                {
                    ...
                    uint32 reqDefForMaxHeal  = victim->GetCurrentLevel() * 5 + 140;
                    float pctFromDefense = (defenseSkillValue >= reqDefForMaxHeal)
                        ? 1.0f
                        : float(defenseSkillValue) / float(reqDefForMaxHeal);

                    int32 healAmount = int32(victim->CountPctFromMaxHealth(uint32(healPct * pctFromDefense)));
                    victim->CastCustomSpell(victim, SPELL_PALADIN_ARGENT_DEFENDER_HEAL, &healAmount, NULL, NULL, true, NULL, aurEff);
                    victim->ToPlayer()->AddSpellCooldown(SPELL_PALADIN_ARGENT_DEFENDER_HEAL, 0, time(NULL) + 120);
                }
    ...
    public:
        ...
        class spell_pal_blessing_of_faith_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    uint32 spell_id = 0;
                    switch (unitTarget->GetCurrentClass())
                    {
                        case CLASS_DRUID:
...
class spell_pal_righteous_defense : public SpellScriptLoader
{
    public:
        ...
        class spell_pal_righteous_defense_SpellScript : public SpellScript
        {
            ...
            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_PALADIN_RIGHTEOUS_DEFENCE))
                    return false;
                return true;
            }
            ...
            void HandleSpellEffectTriggerSpell(SpellEffIndex /*effIndex*/)
            {
                if (Unit* caster = GetCaster())
				{
                    if (Unit* targetUnit = GetHitUnit())
                        caster->CastSpell(targetUnit, SPELL_PALADIN_RIGHTEOUS_DEFENCE_EFFECT_1, true);
				}
            }
...
public:
    ...
    class spell_pal_sacred_shield_AuraScript : public AuraScript
    {
        ...
        bool Validate(SpellInfo const* /*entry*/)
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_PALADIN_SACRED_SHIELD_EFFECT))
                return false;
            return true;
        }
        ...
        void HandleEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
			...
			{
                if (caster->ToPlayer())
                    caster->ToPlayer()->AddSpellCooldown(SPELL_PALADIN_SACRED_SHIELD_EFFECT, 0, time(NULL) + 6);
			}
        }
...
class spell_pal_avenging_wrath : public SpellScriptLoader
{
    public:
        ...
        class spell_pal_avenging_wrath_AuraScript : public AuraScript
        {
            ...
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_PALADIN_SANCTIFIED_WRATH) || !sSpellMgr->GetSpellInfo(SPELL_PALADIN_SANCTIFIED_WRATH_TALENT_R1))
                    return false;
                return true;
            }
