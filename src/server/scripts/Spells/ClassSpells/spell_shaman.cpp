...
enum ShamanSpells
{
    ...
    SPELL_SHAMAN_EARTHBIND_TOTEM         = 6474,
    SPELL_SHAMAN_EARTHEN_POWER           = 59566,
	...
	SPELL_REINCARNATION_1                = 21169,
	SPELL_REINCARNATION_2                = 27740,
	SPELL_REINCARNATION_VISUAL           = 58854,
};
...
class spell_sha_fire_nova : public SpellScriptLoader
{
    public:
        ...
        class spell_sha_fire_nova_SpellScript : public SpellScript
        {
            ...
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FIRE_NOVA_R1) || sSpellMgr->GetFirstSpellInChain(SPELL_SHAMAN_FIRE_NOVA_R1) != sSpellMgr->GetFirstSpellInChain(spellEntry->Id))
                    return false;
...
class spell_sha_earthbind_totem : public SpellScriptLoader
{
    public:
        ...
        class spell_sha_earthbind_totem_AuraScript : public AuraScript
        {
            ...
            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_SHAMAN_EARTHBIND_TOTEM) || !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_EARTHEN_POWER))
                    return false;
                return true;
            }
			...
			void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
            {
				...
				{
					if (AuraEffect* aur = owner->GetDummyAuraEffect(SPELL_FAMILY_SHAMAN, 2289, 0))
					{
						if (roll_chance_i(aur->GetBaseAmount()))
							GetTarget()->CastSpell((Unit*)NULL, SPELL_SHAMAN_EARTHEN_POWER, true);
					}
				}
            }
            ...
            void Apply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (AuraEffect* aurEff = owner->GetAuraEffectOfRankedSpell(SPELL_SHAMAN_STORM_EARTH_AND_FIRE, EFFECT_1))
                {
                    if (roll_chance_i(aurEff->GetAmount()))
                        GetCaster()->CastSpell(GetCaster(), SPELL_SHAMAN_EARTHGRAB, false);
                }
            }
                ...
                if (Unit* target = GetHitUnit())
                    if (Unit* caster = GetCaster())
                    {
                        if (Unit* owner = caster->GetOwner())
                        {
                            ...
                            // Restorative Totems
                            if (AuraEffect* dummy = owner->GetAuraEffect(SPELL_AURA_DUMMY, SPELL_FAMILY_SHAMAN, ICON_ID_RESTORATIVE_TOTEMS, 1))
        ...
        class spell_sha_lava_lash_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /* effIndex */)
            {
                if (Player* caster = GetCaster()->ToPlayer())
                {
                    ...
                    if (caster->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND))
                    {
                        // Damage is increased by 25% if your off-hand weapon is enchanted with Flametongue.
                        if (caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELL_FAMILY_SHAMAN, 0x200000, 0, 0))
        ...
        class spell_sha_chain_heal_SpellScript : public SpellScript
        {
            ...
            void HandleHeal(SpellEffIndex /*effIndex*/)
            {
                if (firstHeal)
                {
                    ...
                    if (AuraEffect* aurEff = GetHitUnit()->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELL_FAMILY_SHAMAN, 0, 0, 0x10, GetCaster()->GetGUID()))
                    {
                        riptide = true;
                        // Consume it
    ...
    public:
        ...
        class spell_sha_flame_shock_AuraScript : public AuraScript
        {
            ...
            void HandleDispel(DispelInfo* /*dispelInfo*/)
            {
				...
				{
					// Lava Flows
					if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELL_FAMILY_SHAMAN, ICON_ID_SHAMAN_LAVA_FLOW, EFFECT_0))
                    {
...
class spell_sha_reincarnation : public SpellScriptLoader
{
    public:
        spell_sha_reincarnation() : SpellScriptLoader("spell_sha_reincarnation") {}

        class spell_sha_reincarnation_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_sha_reincarnation_SpellScript);

            bool Validate(SpellInfo const* /*spellEntry*/)
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_REINCARNATION_1))
					return false;
				if (!sSpellMgr->GetSpellInfo(SPELL_REINCARNATION_2))
                    return false;
                return true;
            }
            ...
            {
				if (Unit* caster = GetCaster())
					caster->CastSpell(caster, SPELL_REINCARNATION_VISUAL, true);
            }

            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_sha_reincarnation_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SELF_RESURRECT);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_sha_reincarnation_SpellScript();
        }
};
