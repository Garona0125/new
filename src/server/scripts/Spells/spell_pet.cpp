    ...
    class spell_warl_pet_scaling_01_AuraScript : public AuraScript
    {
        ...
        void ApplyEffect(AuraEffect const* /* aurEff */, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pet = GetUnitOwner())
			{
                if (_tempBonus)
                {
                    PetLevelInfo const* pInfo = sObjectMgr->GetPetLevelInfo(pet->GetEntry(), pet->GetCurrentLevel());
                    uint32 healthMod = 0;
            ...
            if (Unit* pet = GetUnitOwner())
            {
                if (pet->IsGuardian())
                {
                    if (Unit* owner = pet->GetOwner())
                    {
                        ...
                        if (AuraEffect const* aurEff = owner->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELL_FAMILY_DEATH_KNIGHT, 3010, 0))
                        {
    ...
    class spell_dk_avoidance_passive_AuraScript : public AuraScript
    {
        ...
        void CalculateAvoidanceAmount(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* pet = GetUnitOwner())
            {
                if (Unit* owner = pet->GetOwner())
                {
					if (pet->GetEntry() == ENTRY_RISEN_GHOUL)
						amount = -90;

                    if (pet->GetEntry() == ENTRY_ARMY_DEAD_GHOUL)
                        amount = -90;

                    else if (Aura* aura = owner->GetAuraOfRankedSpell(SPELL_NIGHT_OF_THE_DEAD))
                        amount = -aura->GetSpellInfo()->Effects[EFFECT_2].CalcValue();
                }
            }
        }
