...
#define SQL_BOSS_HISTORY_LOG "INSERT INTO `character_mod_kill_boss` (`GuildId`, `CreatureEntry`, `KillerCount`, `KillData`, `MapSpawnMode`, `MapId`, `MapRaid`, `CreatureName`, `MapName`, `MapMaxPlayers`) VALUES ('%u', '%u', '%u', '%s', '%u', '%u', '%u', '%s', '%s', '%u')"
...
int32 BossHistoryNormalText = 15006;
int32 BossHistoryHeroicText = 15007;

class mod_boss_history_world_script : public WorldScript
{
public:
	mod_boss_history_world_script() : WorldScript("mod_boss_history_world_script") { }

	void OnConfigLoad(bool /*reload*/)
    {
        BossHistoryEnable = GetConfigSettings::GetBoolState("BossHistory.Enable", false);

        if (!BossHistoryEnable)
            return;

        BossHistoryInstance = GetConfigSettings::GetBoolState("BossHistory.Instance.Only", false);
        BossHistoryAnnounce = GetConfigSettings::GetBoolState("BossHistory.Announce", false);
        BossHistoryLog = GetConfigSettings::GetBoolState("BossHistory.Log", false);
        BossHistoryNormalText = GetConfigSettings::GetIntState("BossHistory.Normal.Text.ID", 15006);
        BossHistoryHeroicText = GetConfigSettings::GetIntState("BossHistory.Heroic.Text.ID", 15007);
    }
};

class mod_boss_history_creature_script : public AllCreatureScript
{
public:
	mod_boss_history_creature_script() : AllCreatureScript("mod_boss_history_creature_script") { }
    ...
    void AllCreatureJustDied(Creature* creature)
    {
        ...
        if (Group* group = recipient->GetGroup())
        {
            for (GroupReference *itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                if (BossHistoryLog)
                {
                    ...
                    PeopleData << uint32(Temp->GetCurrentLevel()) << DATA_SEPARATOR;
        ...
        else
        {
            ...
            if (BossHistoryLog)
            {
                ...
                PeopleData << uint32(recipient->GetCurrentLevel()) << DATA_SEPARATOR;
...
{
    new mod_boss_history_creature_script();
    new mod_boss_history_world_script();
}