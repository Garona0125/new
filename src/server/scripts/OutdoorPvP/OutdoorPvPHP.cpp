...
void OPvPCapturePointHP::ChangeState()
{
    ...
    case OBJECTIVE_STATE_ALLIANCE:
        ...
        sWorld->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(HP_LANG_LOSE_A[m_TowerType]));
        break;
    case OBJECTIVE_STATE_HORDE:
        ...
        sWorld->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(HP_LANG_LOSE_H[m_TowerType]));
        break;
    ...
    case OBJECTIVE_STATE_ALLIANCE:
    {
        ...
        if (alliance_towers < 3)
            ((OutdoorPvPHP*)m_PvP)->SetAllianceTowersControlled(++alliance_towers);
        sWorld->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(HP_LANG_CAPTURE_A[m_TowerType]));
        break;
    }
    case OBJECTIVE_STATE_HORDE:
    {
        ...
        if (horde_towers < 3)
            ((OutdoorPvPHP*)m_PvP)->SetHordeTowersControlled(++horde_towers);
        sWorld->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(HP_LANG_CAPTURE_H[m_TowerType]));
        break;
    }
    case OBJECTIVE_STATE_NEUTRAL_ALLIANCE_CHALLENGE:
        field = HP_MAP_N[m_TowerType];
    ...
    if (m_State == OBJECTIVE_STATE_ALLIANCE || m_State == OBJECTIVE_STATE_HORDE)
...
{
    public:
        OutdoorPvP_hellfire_peninsula() : OutdoorPvPScript("outdoor_pvp_hp") { }
