...
{
    if (killed->GetTypeId() == TYPE_ID_PLAYER && killer->GetTeam() != killed->ToPlayer()->GetTeam() && killer->GetAreaId() == 3628)
    {
...
uint32 OPvPCapturePointNA::GetAliveGuardsCount()
{
    ...
    for (std::map<uint32, uint64>::iterator itr = m_Creatures.begin(); itr != m_Creatures.end(); ++itr)
    {
        ...
        case NA_NPC_GUARD_15:
			if (Creature const* const creature = HashMapHolder<Creature>::Find(itr->second))
				if (creature->IsAlive())
					++cnt;
            break;
        default:
...
void OPvPCapturePointNA::FactionTakeOver(uint32 team)
{
    ...
    if (m_ControllingFaction == ALLIANCE)
        sWorld->SendZoneText(NA_HALAA_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_NA_LOSE_A));
    else if (m_ControllingFaction == HORDE)
        sWorld->SendZoneText(NA_HALAA_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_NA_LOSE_H));
    ...
    if (team == ALLIANCE)
    {
        ...
        sWorld->SendZoneText(NA_HALAA_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_NA_CAPTURE_A));
    }
    else
    {
        ...
        m_PvP->SendUpdateWorldState(NA_UI_GUARDS_LEFT, m_GuardsAlive);
        sWorld->SendZoneText(NA_HALAA_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_NA_CAPTURE_H));
    }
...
{
    public:
        OutdoorPvP_nagrand() : OutdoorPvPScript("outdoor_pvp_na") { }
