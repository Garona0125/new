...
void OPvPCapturePointZM_Beacon::ChangeState()
{
    ...
    if (m_OldState == OBJECTIVE_STATE_ALLIANCE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(ZMBeaconLoseA[m_TowerType]));
    }
    // if changing from controlling horde to alliance
    else if (m_OldState == OBJECTIVE_STATE_HORDE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(ZMBeaconLoseH[m_TowerType]));
    }
    ...
    switch (m_State)
    {
        case OBJECTIVE_STATE_ALLIANCE:
        {
            ...
            if (alliance_towers < ZM_NUM_BEACONS)
                ((OutdoorPvPZM*)m_PvP)->SetAllianceTowersControlled(++alliance_towers);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(ZMBeaconCaptureA[m_TowerType]));
            break;
        }
        case OBJECTIVE_STATE_HORDE:
        {
            ...
            if (horde_towers < ZM_NUM_BEACONS)
                ((OutdoorPvPZM*)m_PvP)->SetHordeTowersControlled(++horde_towers);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(ZMBeaconCaptureH[m_TowerType]));
            break;
        }
...
{
    if (killed->GetTypeId() != TYPE_ID_PLAYER)
        return;
    ...
    if (retval >= 0)
    {
        if (player->HasAura(ZM_BATTLE_STANDARD_A) && m_GraveYardState != ZM_GRAVEYARD_A)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_H)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_ZM_LOSE_GY_H));
            ...
            player->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_A);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_ZM_CAPTURE_GY_A));
        }
        else if (player->HasAura(ZM_BATTLE_STANDARD_H) && m_GraveYardState != ZM_GRAVEYARD_H)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_A)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_ZM_LOSE_GY_A));
            ...
            player->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_H);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_ZM_CAPTURE_GY_H));
        }
        UpdateTowerState();
    }
    return retval;
...
{
public:
	OutdoorPvP_zangarmarsh() : OutdoorPvPScript("outdoor_pvp_zm") { }
