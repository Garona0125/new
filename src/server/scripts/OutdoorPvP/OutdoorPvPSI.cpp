    ...
    case SI_AREATRIGGER_A:
        if (player->GetTeam() == ALLIANCE && player->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_A >= SI_MAX_RESOURCES)
            {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_SI_CAPTURE_A));
    ...
    case SI_AREATRIGGER_H:
        if (player->GetTeam() == HORDE && player->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_H >= SI_MAX_RESOURCES)
            {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_SI_CAPTURE_H));
...
{
    public:
        OutdoorPvP_silithus() : OutdoorPvPScript("outdoor_pvp_si") { }
