...
OPvPCapturePointGH::OPvPCapturePointGH(OutdoorPvP *pvp) :OPvPCapturePoint(pvp), m_capturable(true), m_ControllingFaction(0), m_VentureState(VENTURE_N)
{
...
void OPvPCapturePointGH::ChangeState()
{
    ...
    case OBJECTIVE_STATE_ALLIANCE:
        sWorld->SendZoneText(GH_OUTDOORPVP_ZONE,sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_GH_LOOSE_A));
        DeSpawnNPCs();
        break;
    case OBJECTIVE_STATE_HORDE:
        sWorld->SendZoneText(GH_OUTDOORPVP_ZONE,sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_GH_LOOSE_H));
        DeSpawnNPCs();
        break;
    ...
    case OBJECTIVE_STATE_ALLIANCE:
        ...
        artkit = 2;
        sWorld->SendZoneText(GH_OUTDOORPVP_ZONE,sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_GH_CAPTURE_A));
        break;
    case OBJECTIVE_STATE_HORDE:
        m_VentureState = VENTURE_H;
        FactionTakeOver(HORDE);
        artkit = 1;
        sWorld->SendZoneText(GH_OUTDOORPVP_ZONE,sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_GH_CAPTURE_H));
        break;
    case OBJECTIVE_STATE_NEUTRAL_ALLIANCE_CHALLENGE:
...
{
    if (killed->GetTypeId() == TYPE_ID_PLAYER && player->GetTeam() != killed->ToPlayer()->GetTeam())
...
{
public:
    OutdoorPvP_grizzly() : OutdoorPvPScript("outdoor_pvp_gh") { }
