...
void OPvPCapturePointTF::ChangeState()
{
    ...
    if (m_OldState == OBJECTIVE_STATE_ALLIANCE)
    {
        ...
        sWorld->SendZoneText(OutdoorPvPTFBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_TF_LOSE_A));
    }
    ...
    else if (m_OldState == OBJECTIVE_STATE_HORDE)
    {
        ...
        sWorld->SendZoneText(OutdoorPvPTFBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_TF_LOSE_H));
    }
    ...
    case OBJECTIVE_STATE_ALLIANCE:
    {
        ...
        if (alliance_towers < TF_TOWER_NUM)
            ((OutdoorPvPTF*)m_PvP)->SetAllianceTowersControlled(++alliance_towers);
        sWorld->SendZoneText(OutdoorPvPTFBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_TF_CAPTURE_A));
    ...
    case OBJECTIVE_STATE_HORDE:
    {
        ...
        if (horde_towers < TF_TOWER_NUM)
            ((OutdoorPvPTF*)m_PvP)->SetHordeTowersControlled(++horde_towers);
        sWorld->SendZoneText(OutdoorPvPTFBuffZones[0], sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_OPVP_TF_CAPTURE_H));
...
{
    public:
        OutdoorPvP_terokkar_forest() : OutdoorPvPScript("outdoor_pvp_tf") { }
