...
enum Spells
{
	...
	SPELL_MIRROED_SOUL_PLAYER                     = 69034,
...
class boss_devourer_of_souls : public CreatureScript
{
    public:
        ...
        struct boss_devourer_of_soulsAI : public BossAI
        {
            ...
            {
                DoSendQuantumText(RAND(SAY_FACE_ANGER_AGGRO, SAY_FACE_DESIRE_AGGRO), me);

                if (!me->FindCreatureWithDistance(NPC_CRUCIBLE_OF_SOULS, 60.0f)) // Prevent double spawn
                ...
                if (MirroredSoulTarget && me->HasAura(SPELL_MIRRORED_SOUL))
                {
                    if (Player* player = Unit::GetPlayer(*me, MirroredSoulTarget))
                    {
                        if (player->GetAura(SPELL_MIRRORED_SOUL))
                        {
                            ...
                            me->CastCustomSpell(player, SPELL_MIRROED_SOUL_PLAYER, &mirrorDamage, 0, 0, true);
                        }
                        else
                            MirroredSoulTarget = 0;
                    }
            ...
            void JustDied(Unit* /*killer*/)
            {
                ...
                DoSendQuantumText(RAND(SAY_FACE_SORROW_DEATH, SAY_FACE_DESIRE_DEATH), me);
                ...
                for (int8 i = 0; outroPositions[i].entry[entryIndex] != 0; ++i)
                {
                    if (Creature* summon = me->SummonCreature(outroPositions[i].entry[entryIndex], spawnPoint, TEMPSUMMON_DEAD_DESPAWN))
                    {
                        ...
                        if (summon->GetEntry() == NPC_JAINA_PART2)
                            DoSendQuantumText(SAY_JAINA_OUTRO, summon);
                        else if (summon->GetEntry() == NPC_SYLVANAS_PART2)
                            DoSendQuantumText(SAY_SYLVANAS_OUTRO, summon);
                    }
                }
            }
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_MIRRORED_SOUL:
                            if (Unit* target = SelectTarget(TARGET_RANDOM, 0, 0, true))
                            {
                                ...
                                DoSendQuantumText(EMOTE_MIRRORED_SOUL, me);
                            }
                        ...
                        case EVENT_UNLEASHED_SOULS:
                            ...
                            DoSendQuantumText(RAND(SAY_FACE_ANGER_UNLEASH_SOUL, SAY_FACE_SORROW_UNLEASH_SOUL, SAY_FACE_DESIRE_UNLEASH_SOUL), me);
                            DoSendQuantumText(EMOTE_UNLEASH_SOUL, me);
                        ...
                        case EVENT_WAILING_SOULS:
                            ...
                            DoSendQuantumText(RAND(SAY_FACE_ANGER_WAILING_SOUL, SAY_FACE_DESIRE_WAILING_SOUL), me);
                            DoSendQuantumText(EMOTE_WAILING_SOUL, me);
...
class npc_well_of_souls : public CreatureScript
{
public:
    npc_well_of_souls() : CreatureScript("npc_well_of_souls") {}

    struct npc_well_of_soulsAI : public QuantumBasicAI
    {
        npc_well_of_soulsAI(Creature* creature) : QuantumBasicAI(creature)
		{
        ...
        void Reset()
        {
			DoCast(me, SPELL_WF_WELL_OF_SOULS);
		}
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_well_of_soulsAI(creature);
    }
};
