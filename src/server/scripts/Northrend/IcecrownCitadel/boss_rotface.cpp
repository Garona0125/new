...
enum Texts
{
    SAY_PRECIOUS_DIES           = -1659094,
    SAY_AGGRO                   = -1659095,
    EMOTE_SLIME_SPRAY           = -1659096,
    SAY_SLIME_SPRAY             = -1659097,
    EMOTE_UNSTABLE_EXPLOSION    = -1659098,
    SAY_UNSTABLE_EXPLOSION      = -1659099,
    SAY_KILL_1                  = -1659100,
	SAY_KILL_2                  = -1659101,
    SAY_BERSERK                 = -1659102,
    SAY_DEATH                   = -1659103,
    EMOTE_MUTATED_INFECTION     = -1659104,
    EMOTE_PRECIOUS_ZOMBIES      = -1659105,
};
...
enum Spells
{
    ...
    SPELL_GREEN_ABOMINATION_HITTIN_YA_PROC  = 70001,    // prevents getting hit by infection
	...
	SPELL_VILE_GAS_10                       = 72272,
	SPELL_VILE_GAS_25                       = 72273,
	SPELL_VILE_GAS_TRIGGER                  = 72285,
	SPELL_PRECIOUS_RIBBON                   = 70404,
	SPELL_DOUBLE_ATTACK                     = 19818,
};

#define MUTATED_INFECTION RAID_MODE(SPELL_MUTATED_INFECTION_10N, SPELL_MUTATED_INFECTION_25N, SPELL_MUTATED_INFECTION_10H, SPELL_MUTATED_INFECTION_25H)
    ...
    public:
        ...
        struct boss_rotfaceAI : public BossAI
        {
            ...
            void EnterToBattle(Unit* who)
            {
				...
				DoCast(me, SPELL_GREEN_ABOMINATION_HITTIN_YA_PROC, true);
            }
            ...
            void SpellHitTarget(Unit* /*target*/, SpellInfo const* spell)
            {
                if (spell->Id == SPELL_SLIME_SPRAY)
                    DoSendQuantumText(SAY_SLIME_SPRAY, me);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SLIME_SPRAY:
                            ...
                            {
								if (Creature* ooze = me->SummonCreature(NPC_OOZE_SPRAY_STALKER, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 10*IN_MILLISECONDS))
									ooze->GetMotionMaster()->MoveChase(target);

                                DoSendQuantumText(EMOTE_SLIME_SPRAY, me);
                        ...
                        case EVENT_MUTATED_INFECTION:
                            me->CastCustomSpell(RAID_MODE(SPELL_MUTATED_INFECTION_10N, SPELL_MUTATED_INFECTION_25N, SPELL_MUTATED_INFECTION_10H, SPELL_MUTATED_INFECTION_25H), SPELLVALUE_MAX_TARGETS, 1, 0, false);
                            events.ScheduleEvent(EVENT_MUTATED_INFECTION, InfectionCooldown);
                            break;
						case EVENT_VILE_GAS:
							if (Unit* target = SelectTarget(TARGET_RANDOM, 0, 100, true))
							{
								if (Creature* vile = me->SummonCreature(NPC_VILE_GAS_STALKER, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 10*IN_MILLISECONDS))
								{
									if (Player* player = vile->FindPlayerWithDistance(2.0f))
										player->CastSpell(player, RAID_MODE(SPELL_VILE_GAS_10, SPELL_VILE_GAS_25, SPELL_VILE_GAS_10, SPELL_VILE_GAS_25), true);
								}
							}
...
class npc_little_ooze : public CreatureScript
{
    public:
        ...
        struct npc_little_oozeAI : public QuantumBasicAI
        {
            npc_little_oozeAI(Creature* creature) : QuantumBasicAI(creature){}
...
class npc_big_ooze : public CreatureScript
{
    public:
        ...
        struct npc_big_oozeAI : public QuantumBasicAI
        {
            npc_big_oozeAI(Creature* creature) : QuantumBasicAI(creature)
			{
...
class npc_precious_icc : public CreatureScript
{
    public:
        ...
        struct npc_precious_iccAI : public QuantumBasicAI
        {
            npc_precious_iccAI(Creature* creature) : QuantumBasicAI(creature), _summons(me)
            {
                instance = creature->GetInstanceScript();
            }
            ...
            void Reset()
            {
				...
				DoCast(me, SPELL_PRECIOUS_RIBBON, true);
            ...
            void JustDied(Unit* killer)
            {
				...
				{
                    if (rotface->IsAlive())
						DoSendQuantumText(SAY_PRECIOUS_DIES, rotface);
				}
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_ZOMBIES:
                            DoSendQuantumText(EMOTE_PRECIOUS_ZOMBIES, me);
    ...
    public:
        ...
        class spell_rotface_ooze_flood_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                GetHitUnit()->GetCreatureListWithEntryInGrid(triggers, GetHitUnit()->GetEntry(), 20.5f); // 12.5

				if (triggers.empty())
					return;

                triggers.sort(Trinity::ObjectDistanceOrderPred(GetHitUnit()));
                GetHitUnit()->CastSpell(triggers.back(), uint32(GetEffectValue()), false, 0, 0, GetOriginalCaster() ? GetOriginalCaster()->GetGUID() : 0);
            }
            ...
            void NotifyTargets()
            {
                if (Creature* caster = GetCaster()->ToCreature())
                    if (Unit* target = GetHitUnit())
						DoSendQuantumText(EMOTE_MUTATED_INFECTION, caster);
            }
    ...
    public:
        ...
        class spell_rotface_little_ooze_combine_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                GetHitCreature()->DespawnAfterAction();
            }
                    ...
                    if (newStack >= 5)
                    {
                        ...
                        if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                            if (Creature* rotface = Unit::GetCreature(*GetCaster(), instance->GetData64(DATA_ROTFACE)))
                                if (rotface->IsAlive())
                                {
									DoSendQuantumText(SAY_UNSTABLE_EXPLOSION, rotface);
									DoSendQuantumText(EMOTE_UNSTABLE_EXPLOSION, rotface);
                                }

                        if (Creature* cre = GetCaster()->ToCreature())
                            cre->AI()->DoAction(EVENT_STICKY_OOZE);
                        GetCaster()->CastSpell(GetCaster(), SPELL_UNSTABLE_OOZE_EXPLOSION, false, 0, 0, GetCaster()->GetGUID());
                        if (InstanceScript* instance = GetCaster()->GetInstanceScript())
    ...
    public:
        ...
        class spell_rotface_unstable_ooze_explosion_init_SpellScript : public SpellScript
        {
            ...
            void HandleCast(SpellEffIndex effIndex)
            {
                ...
                Creature* dummy = GetCaster()->SummonCreature(NPC_UNSTABLE_EXPLOSION_STALKER, x, y, z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 1*MINUTE*IN_MILLISECONDS);
            ...
            void CheckTarget(SpellEffIndex effIndex)
            {
                ...
                if (InstanceScript* script = GetCaster()->GetInstanceScript())
				{
                    if (Creature* rotface = script->instance->GetCreature(script->GetData64(DATA_ROTFACE)))
                        rotface->CastSpell(x, y, z, triggered_spell_id, true, 0, 0, GetCaster()->GetGUID());
				}
            }
    ...
    public:
        ...
        class spell_rotface_unstable_ooze_explosion_suicide_AuraScript : public AuraScript
        {
            ...
            void DespawnSelf(AuraEffect const* /*aurEff*/)
            {
                ...
                target->ToCreature()->DespawnAfterAction(60000);
            }
    ...
    public:
        ...
        class spell_rotface_slime_spray_SpellScript : public SpellScript
        {
            ...
            void HandleResidue()
            {
                ...
                if (player->HasAura(SPELL_GREEN_BLIGHT_RESIDUE))
                    return;

				player->CastSpell(player, SPELL_GREEN_BLIGHT_RESIDUE, TRIGGERED_FULL_MASK);
            }
			...
			void ChangeOrientation()
			{
				...
				if (Creature* target = caster->FindCreatureWithDistance(NPC_OOZE_SPRAY_STALKER, 200.0f))
    ...
    public:
        ...
        class spell_rotface_vile_gas_trigger_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<Unit*>& targetList)
            {
                ...
                while (itr != targetList.end() && (*itr)->GetDistance(GetCaster()) < 5.0f)
                {
                ...
                if (!ranged.empty())
                    Quantum::DataPackets::RandomResizeList(ranged, GetCaster()->GetMap()->Is25ManRaid() ? 3 : 1);

                targetList.swap(ranged);
            }
            ...
            void Register()
            {
                OnUnitTargetSelect += SpellUnitTargetFn(spell_rotface_vile_gas_trigger_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
