...
enum Spells
{
    ...
    SPELL_DEVOURING_FLAME_TRIGGER_10             = 64709,
	SPELL_DEVOURING_FLAME_TRIGGER_25             = 64734,
    // HarpoonSpells
...
const Position PosEngRepair[4] =
{
	{607.709f, -130.658f, GROUND_Z, 4.67849f},
	{588.939f, -128.862f, GROUND_Z, 5.17198f},
	{573.045f, -130.893f, GROUND_Z, 4.85804f},
	{559.428f, -134.935f, GROUND_Z, 4.68131f},
};
...
class npc_razorscale_controller : public CreatureScript
{
    public:
        ...
        struct npc_razorscale_controllerAI : public BossAI
        {
            npc_razorscale_controllerAI(Creature* creature) : BossAI(creature, DATA_RAZORSCALE_CONTROLLER)
            {
            ...
            {
                if (me->IsInCombatActive() && instance->GetBossState(DATA_RAZORSCALE) != IN_PROGRESS)
                    EnterEvadeMode();
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BUILD_HARPOON_1:
							me->MonsterTextEmote(EMOTE_HARPOON, LANG_UNIVERSAL, true);
                            ...
                            {
                                if (GameObject* BrokenHarpoon = Harpoon->FindGameobjectWithDistance(GO_RAZOR_BROKEN_HARPOON, 5.0f)) //only nearest broken harpoon
                        ...
                        case EVENT_BUILD_HARPOON_2:
                            ...
                            {
                                if (GameObject* BrokenHarpoon = Harpoon->FindGameobjectWithDistance(GO_RAZOR_BROKEN_HARPOON, 5.0f))
                                    BrokenHarpoon->RemoveFromWorld();
...
class boss_razorscale : public CreatureScript
{
    public:
        ...
        struct boss_razorscaleAI : public BossAI
        {
            ...
            void Reset()
            {
                if (Creature* controller = ObjectAccessor::GetCreature(*me, instance ? instance->GetData64(DATA_RAZORSCALE_CONTROLLER) : 0))
                {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (HealthBelowPct(HEALTH_PERCENT_50) && !PermaGround)
                    EnterPermaGround();
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            ...
                            case EVENT_BREATH:
                                ...
                                DoSendQuantumText(EMOTE_BREATH, me, 0);
                                DoCastAOE(RAID_MODE(SPELL_FLAME_BREATH_10, SPELL_FLAME_BREATH_25));
            ...
            void EnterPermaGround()
            {
                ...
                me->SetCanFly(false);
                me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_STUNNED | UNIT_FLAG_PACIFIED | UNIT_FLAG_NOT_SELECTABLE);
				me->SetUInt32Value(UNIT_NPC_EMOTE_STATE, EMOTE_STATE_NONE);
                me->SetReactState(REACT_AGGRESSIVE);
                me->RemoveAurasDueToSpell(SPELL_HARPOON_TRIGGER);
            ...
            void SummonMoleMachines()
            {
                ...
                {
                    float x = float(irand(540, 640));
                    float y = float(irand(-230, -195));
                    float z = GROUND_Z;
                    me->SummonCreature(NPC_MOLE_MACHINE_TRIGGER, x, y, z, 0);
                }
            }
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    case ACTION_EVENT_START:
                        ...
                        if (Player* player = me->FindPlayerWithDistance(150.0f))
                            AttackStart(player);
...
class npc_expedition_commander : public CreatureScript
{
    public:
		...
		bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if (instance->GetBossState(DATA_RAZORSCALE) == NOT_STARTED)
            {
        ...
        struct npc_expedition_commanderAI : public QuantumBasicAI
        {
            npc_expedition_commanderAI(Creature* creature) : QuantumBasicAI(creature), summons(me)
            {
            ...
            {
                if (!Greet && me->IsWithinDistInMap(who, 10.0f) && who->GetTypeId() == TYPE_ID_PLAYER)
                {
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    case ACTION_GROUND_PHASE:
                        DoSendQuantumText(SAY_GROUND_PHASE, me);
                        break;
            ...
            void UpdateAI(uint32 const diff)
            {
                if (AttackStartTimer <= diff)
                {
                    switch (Phase)
                    {
                        ...
                        case 4:
                            for (uint8 n = 0; n < RAID_MODE<uint8>(2, 4); ++n)
                                Engineer[n]->SetUInt32Value(UNIT_NPC_EMOTE_STATE, EMOTE_STATE_USE_STANDING);
                            for (uint8 n = 0; n < 4; ++n)
                                Defender[n]->SetUInt32Value(UNIT_NPC_EMOTE_STATE, EMOTE_STATE_READY_2H);
...
class npc_mole_machine_trigger : public CreatureScript
{
    public:
        ...
        struct npc_mole_machine_triggerAI : public QuantumBasicAI
        {
            npc_mole_machine_triggerAI(Creature* creature) : QuantumBasicAI(creature), Summons(me)
            {
...
class npc_devouring_flame : public CreatureScript
{
    public:
        ...
        struct npc_devouring_flameAI : public QuantumBasicAI
        {
            npc_devouring_flameAI(Creature* creature) : QuantumBasicAI(creature)
            {
				SetCombatMovement(false);
                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_PACIFIED);
                me->SetDisplayId(me->GetCreatureTemplate()->Modelid2);
...
class npc_darkrune_watcher : public CreatureScript
{
    public:
        ...
        struct npc_darkrune_watcherAI : public QuantumBasicAI
        {
            npc_darkrune_watcherAI(Creature* creature) : QuantumBasicAI(creature){}
...
class npc_darkrune_guardian : public CreatureScript
{
    public:
        ...
        struct npc_darkrune_guardianAI : public QuantumBasicAI
        {
            npc_darkrune_guardianAI(Creature* creature) : QuantumBasicAI(creature){}
...
class npc_darkrune_sentinel : public CreatureScript
{
    public:
        ...
        struct npc_darkrune_sentinelAI : public QuantumBasicAI
        {
            npc_darkrune_sentinelAI(Creature* creature) : QuantumBasicAI(creature){}
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (HeroicStrikeTimer <= diff)
                {
                    ...
                    HeroicStrikeTimer = urand(4000, 6000);
                }
                else HeroicStrikeTimer -= diff;

				if (WhirlwindTimer <= diff && Is25ManRaid())
				{
					DoCast(me, SPELL_WHIRLWIND);
					WhirlwindTimer = urand(10000, 11000);
				}
                ...
                if (BattleShoutTimer <= diff)
                {
                    ...
                    BattleShoutTimer = urand(7000, 8000);
                }
