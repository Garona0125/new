...
#define EMOTE_GUARDIAN "A Guardian of Icecrown enters the fight."
#define EMOTE_START_ATTACK "Kel'Thuzad strikes"
...
public:
    ...
    struct boss_kelthuzadAI : public BossAI
    {
        ...
        uint32 Phase;
        uint32 GuardiansOfIcecrownTimer;
        uint32 Faction;
        uint32 AbosKilled;
        ...
        void Reset()
        {
            ...
            for (uint8 i = 0; i <= 3; ++i)
            {
                if (GameObject* portal = me->GetMap()->GetGameObject(PortalsGUID[i]))
                {
        ...
        void DoAction(const int32 action)
        {
            switch(action)
            {
            case ACTION_ABOMINATION_KILLED:
                AbosKilled++;
                break;
            }
        }
        ...
        void EnterToBattle(Unit* /*who*/)
        {
            ...
            DoSendQuantumText(SAY_SUMMON_MINIONS, me);
            Phase = 1;
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_NOT_SELECTABLE);
        ...
        {
            PortalsGUID[0] = instance ? instance->GetData64(DATA_KELTHUZAD_PORTAL_1) : 0;
            PortalsGUID[1] = instance ? instance->GetData64(DATA_KELTHUZAD_PORTAL_2) : 0;
            PortalsGUID[2] = instance ? instance->GetData64(DATA_KELTHUZAD_PORTAL_3) : 0;
            PortalsGUID[3] = instance ? instance->GetData64(DATA_KELTHUZAD_PORTAL_4) : 0;
        ...
        void UpdateAI(const uint32 diff)
        {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_WEAVER:
                            if (nWeaver < 8)
                            {
                                DoSummon(NPC_SOUL_WEAVER, Pos[RAND(0, 3, 6, 9)]);
                                nWeaver++;
                                events.Repeat(25000);
                            }
                            break;
                        ...
                        case EVENT_PHASE:
                            events.Reset();
							me->MonsterTextEmote(EMOTE_START_ATTACK, LANG_UNIVERSAL, true);
            ...
            else
            {
                ...
                if (Phase != 3)
                {
                    ...
                    {
                        Phase = 3;
						DoSendQuantumText(SAY_REQUEST_AID, me);

						if (Creature* lich = Unit::GetCreature(*me, instance ? instance->GetData64(DATA_LICH_KING) : 0))
							DoSendQuantumText(SAY_ANSWER_REQUEST, lich);
                ...
                {
                    if (GuardiansOfIcecrownTimer <= diff)
                    {
                    ...
                    else GuardiansOfIcecrownTimer -= diff;
                }
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHAIN:
                        {
                            ...
                            {
                                Unit* target = SelectTarget(TARGET_RANDOM, 1, 200, true);
                            ...
                            if (!chained.empty())
                                DoSendQuantumText(RAND(SAY_CHAIN_1, SAY_CHAIN_2), me);
                        ...
                        case EVENT_CHAINED_SPELL:
                        {
                            ...
                            for (itr = chained.begin(); itr != chained.end();)
                            {
                                if (Unit* player = Unit::GetPlayer(*me, (*itr).first))
                                {
                                    ...
                                    if (Unit* target = SelectTarget(TARGET_TOP_AGGRO, 0, NotCharmedTargetSelector()))
                                    {
                        ...
                        case EVENT_BLAST:
                            if (!me->IsNonMeleeSpellCasted(false))
                            {
                                if (Unit* target = SelectTarget(TARGET_RANDOM, RAID_MODE(1, 0), 0, true))
                                    DoCast(target, SPELL_FROST_BLAST);
                                if (rand()%2)
                                    DoSendQuantumText(SAY_FROST_BLAST, me);
...
public:
    ...
    bool OnTrigger(Player* player, const AreaTriggerEntry* /*at*/)
    {
        ...
        if (!instance || instance->IsEncounterInProgress() || instance->GetData(BOSS_KELTHUZAD) == DONE)
            return false;
        ...
        if (GameObject* trigger = instance->instance->GetGameObject(instance->GetData64(DATA_KELTHUZAD_TRIGGER)))
        {
            ...
            {
                if (Creature* sum = trigger->SummonCreature(NPC_SOUL_WEAVER, PosWeavers[i]))
                {
                    pKelthuzadAI->spawns.Summon(sum);
                    sum->GetMotionMaster()->MoveRandom(3.0f);
                    //sum->SetReactState(REACT_DEFENSIVE);
...
public:
    ...
    struct npc_unstoppable_abominationAI : QuantumBasicAI
    {
        npc_unstoppable_abominationAI(Creature* creature) : QuantumBasicAI(creature)
        {
        ...
        void JustDied(Unit* /*killer*/)
        {
            if (!instance || instance->GetData(BOSS_KELTHUZAD) != IN_PROGRESS)
                return;
		...
		void UpdateAI(const uint32 diff)
        {
            ...
            if (MortalWoundTimer <= diff)
            {
                ...
                MortalWoundTimer = urand(6000, 7000);
            }
	...
	CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_unstoppable_abominationAI(creature);
    }
};
...
public:
    ...
    struct npc_mr_bigglesworthAI : public QuantumBasicAI
    {
        npc_mr_bigglesworthAI(Creature* creature) : QuantumBasicAI(creature)
        {
            instance = creature->GetInstanceScript();
        }
        ...
        void JustDied(Unit* /*killer*/)
        {
            if (instance->GetData(BOSS_KELTHUZAD) == DONE)
                return;

			if (Creature* kel = me->FindCreatureWithDistance(NPC_MB_KEL_THUZAD, 500.0f))
				DoSendQuantumText(SAY_CAT_DIED, kel);
        }
    };
