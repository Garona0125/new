...
#define EMOTE_GATE "The central gate opens!"
...
const Waves hardwaves[] =
{
    ...
    {NPC_LIVE_KNIGHT, 10000, 1},
    {NPC_LIVE_TRAINEE, 10000, 1},
    {NPC_LIVE_KNIGHT, 15000, 1},
    {NPC_LIVE_TRAINEE, 5000, 1},
    {NPC_LIVE_TRAINEE, 20000, 1},
    {NPC_LIVE_KNIGHT, 0, 1},
    {NPC_LIVE_RIDER, 10000, 1},
    {NPC_LIVE_TRAINEE, 10000, 1},
...
class boss_gothik_the_harvester : public CreatureScript
{
    public:
        boss_gothik_the_harvester() : CreatureScript("boss_gothik_the_harvester") { }

        struct boss_gothik_the_harvesterAI : public BossAI
        {
			boss_gothik_the_harvesterAI(Creature* creature) : BossAI(creature, BOSS_GOTHIK) {}
            ...
            void EnterToBattle(Unit* /*who*/)
            {
                ...
                DoSendQuantumText(SAY_SPEECH, me);
            ...
            void JustDied(Unit* /*Killer*/)
            {
				...
				me->MonsterTextEmote(EMOTE_GATE, 0, true);
				instance->SetData(DATA_GOTHIK_GATE, GO_STATE_ACTIVE);
            }
                    ...
                    switch (entry)
                    {
                        case NPC_LIVE_TRAINEE:
                        {
                            if (Creature* LiveTrigger0 = Unit::GetCreature(*me, LiveTriggerGUID[1]))
                                DoSummon(NPC_LIVE_TRAINEE, LiveTrigger0, 1);
            ...
            void SpellHit(Unit* /*caster*/, SpellInfo const* spell)
            {
                ...
                if (spellId && me->IsInCombatActive())
                {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SUMMON:
                            ...
                            {
                                PhaseTwo = true;
                                DoSendQuantumText(SAY_TELEPORT, me);
                            ...
                            break;
                        case EVENT_BOLT:
                            DoCastVictim(RAID_MODE(SPELL_SHADOW_BOLT_10, SPELL_SHADOW_BOLT_25));
                            events.ScheduleEvent(EVENT_BOLT, 1000);
                            break;
                        case EVENT_HARVEST:
        ...
        {
            return new boss_gothik_the_harvesterAI(creature);
        }
};
