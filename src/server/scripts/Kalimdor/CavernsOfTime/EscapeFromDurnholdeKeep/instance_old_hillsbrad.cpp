...
public:
    ...
    struct instance_old_hillsbrad_InstanceMapScript : public InstanceScript
    {
        ...
        Player* GetPlayerInMap()
        {
            ...
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: GetPlayerInMap, but PlayerList is empty!");
            return NULL;
        }
        ...
        void SetData(uint32 type, uint32 data)
        {
            ...
            if (!player)
            {
                sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: SetData (Type: %u Data %u) cannot find any player.", type, data);
                return;
            }
            ...
            switch (type)
            {
                case TYPE_BARREL_DIVERSION:
                {
                    if (data == IN_PROGRESS)
                    {
                        ...
                        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: go_barrel_old_hillsbrad count %u", BarrelCount);
                ...
                case TYPE_THRALL_EVENT:
                {
                    if (data == FAIL)
                    {
                        if (ThrallEventCount <= 20)
                        {
                            ...
                            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: Thrall event failed %u times. Resetting all sub-events.", ThrallEventCount);
                        ...
                        Encounter[1] = data;
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: Thrall escort event adjusted to data %u.", data);
                    break;
                }
                case TYPE_THRALL_PART1:
                    Encounter[2] = data;
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: Thrall event part I adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART2:
                    Encounter[3] = data;
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: Thrall event part II adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART3:
                    Encounter[4] = data;
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: Thrall event part III adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART4:
                    Encounter[5] = data;
                     sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Old Hillsbrad: Thrall event part IV adjusted to data %u.", data);
                    break;
            }
        }
