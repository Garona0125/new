...
public:
    ...
    struct instance_mount_hyjal_InstanceMapScript : public InstanceScript
    {
        ...
        std::list<uint64> AncientGemGUID;
        ...
        void OnGameObjectCreate(GameObject* go)
        {
            switch (go->GetEntry())
            {
                ...
                case 182061:
                    ...
                    break;
                case GO_ANCIENT_GEM:
                    AncientGemGUID.push_back(go->GetGUID());
                    break;
            }
        }
        ...
        void Load(const char* in)
        {
            ...
            loadStream >> Encounter[0] >> Encounter[1] >> Encounter[2] >> Encounter[3] >> Encounter[4] >> allianceRetreat >> hordeRetreat >> RaidDamage;
            for (uint8 i = 0; i < MAX_ENCOUNTER; ++i)
                if (Encounter[i] == IN_PROGRESS)                // Do not load an encounter as IN_PROGRESS - reset it instead.
