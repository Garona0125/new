...
public:
    ...
    struct boss_azgalorAI : public hyjal_trashAI
    {
        ...
        {
            if (instance && IsEvent)
                instance->SetData(DATA_AZGALOR_EVENT, IN_PROGRESS);
        ...
        void JustDied(Unit* victim)
        {
            ...
            if (instance && IsEvent)
                instance->SetData(DATA_AZGALOR_EVENT, DONE);
			Talk(SAY_DEATH);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (RainTimer <= diff)
            {
                DoCast(SelectTarget(TARGET_RANDOM, 0, 30, true), SPELL_RAIN_OF_FIRE);
            ...
            if (DoomTimer <= diff)
            {
                DoCast(SelectTarget(TARGET_RANDOM, 1, 100, true), SPELL_DOOM); // never on tank
...
public:
    ...
    struct mob_lesser_doomguardAI : public hyjal_trashAI
    {
        ...
        {
            if (me->IsWithinDist(who, 50) && !me->IsInCombatActive() && me->IsValidAttackTarget(who))
                AttackStart(who);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                DoCast(SelectTarget(TARGET_RANDOM, 0, 100, true), SPELL_CRIPPLE);
