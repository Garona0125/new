...
void hyjalAI::Reset()
{
    ...
    if (instance)
    {
        ...
        {
            //Reset World States
            instance->DoUpdateWorldState(WORLD_STATE_WAVES_COUNT, 0);
            instance->DoUpdateWorldState(WORLD_STATE_ENEMY, 0);
            instance->DoUpdateWorldState(WORLD_STATE_ENEMY_COUNT, 0);
...
void hyjalAI::EnterToBattle(Unit* /*who*/)
{
    if (IsDummy)return;
...
void hyjalAI::SummonNextWave(const Wave wave[18], uint32 Count, float Base[4][3])
{
    ...
    if (!wave[Count].IsBoss)
    {
        ...
        instance->DoUpdateWorldState(WORLD_STATE_WAVES_COUNT, stateValue);    // Set world state to our current wave number
        // Enable world state
        ...
        {
            NextWaveTimer = 15000;
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: HyjalAI: debug mode is enabled. Next Wave in 15 seconds");
        }
    }
    else
    {
        ...
        instance->DoUpdateWorldState(WORLD_STATE_WAVES_COUNT, 0);
        instance->DoUpdateWorldState(WORLD_STATE_ENEMY_COUNT, 1);
...
void hyjalAI::JustDied(Unit* /*killer*/)
{
    ...
    if (instance)
    {
		if (instance->GetData(DATA_RAGE_WINTERCHILL_EVENT) == IN_PROGRESS)
            instance->SetData(DATA_RAGE_WINTERCHILL_EVENT, NOT_STARTED);

        if (instance->GetData(DATA_ANETHERON_EVENT) == IN_PROGRESS)
            instance->SetData(DATA_ANETHERON_EVENT, NOT_STARTED);

        if (instance->GetData(DATA_KAZROGAL_EVENT) == IN_PROGRESS)
            instance->SetData(DATA_KAZROGAL_EVENT, NOT_STARTED);

        if (instance->GetData(DATA_AZGALOR_EVENT) == IN_PROGRESS)
            instance->SetData(DATA_AZGALOR_EVENT, NOT_STARTED);

        instance->SetData(DATA_RESET_RAIDDAMAGE, 0);
    }
}
...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    if (!creatures.empty())
    {
        for (std::list<Creature*>::const_iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
        {
            ...
            (*itr)->SetCurrentFaction(35);//make them friendly so mobs won't attack them
        }
    }
}
