...
{
    if (done_by->GetTypeId() == TYPE_ID_PLAYER || (done_by->GetTypeId() == TYPE_ID_UNIT && CAST_CRE(done_by)->IsPet()))
    {
        damageTaken += damage;
        if (instance)
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (!meteor)
            {
                ...
                if (trigger)
                {
                    ...
                    trigger->SetCurrentFaction(me->GetFaction());
...
public:
    ...
    struct mob_ghoulAI : public hyjal_trashAI
    {
        ...
        void WaypointReached(uint32 waypointId)
        {
            ...
            if (waypointId == LastOverronPos && IsOverrun)
            {
                me->SetUInt32Value(UNIT_NPC_EMOTE_STATE, EMOTE_ONESHOT_ATTACK_UNARMED);
                if ((faction == 0 && LastOverronPos == 17) || (faction == 1 && LastOverronPos == 21))
                {
                    me->setDeathState(DEAD);
                    me->RemoveCorpse();
    ...
    struct mob_necromancerAI : public hyjal_trashAI
    {
        ...
        void WaypointReached(uint32 waypointId)
        {
            if (waypointId == 7 && instance && !IsOverrun)
            {
				...
				{
                    if (Unit* target = Unit::GetUnit(*me, instance->GetData64(DATA_JAINAPROUDMOORE)))
					{
						if (target && target->IsAlive())
							me->AddThreat(target, 0.0f);
					}
...
public:
    ...
    struct alliance_riflemanAI : public QuantumBasicAI
    {
        alliance_riflemanAI(Creature* creature) : QuantumBasicAI(creature)
        {
            Reset();
