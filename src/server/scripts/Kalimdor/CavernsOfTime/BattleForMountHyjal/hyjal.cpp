...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        uint32 RageEncounter = ai->GetInstanceData(DATA_RAGE_WINTERCHILL_EVENT);
        uint32 AnetheronEncounter = ai->GetInstanceData(DATA_ANETHERON_EVENT);
        ...
        switch (action)
        {
            ...
            case GOSSIP_ACTION_INFO_DEF + 3:
             ...
             case GOSSIP_ACTION_INFO_DEF:
                ai->Debug = !ai->Debug;
                sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: HyjalAI - Debug mode has been toggled");
                break;
        }
        return true;
    }
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        uint32 ANETHERON_EVENT = ai->GetInstanceData(DATA_ANETHERON_EVENT);

        if (ANETHERON_EVENT == DONE && ai->GetInstanceData(DATA_ALLIANCE_RETREAT))
        {
            uint32 KAZROGAL_EVENT = ai->GetInstanceData(DATA_KAZROGAL_EVENT);
            uint32 AZGALOR_EVENT  = ai->GetInstanceData(DATA_AZGALOR_EVENT);
            if (KAZROGAL_EVENT == NOT_STARTED)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_BEGIN_HORDE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            else if (KAZROGAL_EVENT == DONE && AZGALOR_EVENT == NOT_STARTED)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_AZGALOR, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
            else if (AZGALOR_EVENT == DONE)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_RETREAT, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
        }
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (AZGALOR_EVENT == DONE && !player->HasItemCount(ITEM_TEAR_OF_GODDESS, 1))
