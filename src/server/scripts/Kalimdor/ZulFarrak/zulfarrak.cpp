...
public:
    ...
    struct npc_sergeant_blyAI : public QuantumBasicAI
    {
        npc_sergeant_blyAI(Creature* creature) : QuantumBasicAI(creature)
        {
        ...
        uint32 RevengeTimer;                                   //this is wrong, spell should never be used unless me->GetVictim() dodge, parry or block attack. Trinity support required.
        uint64 PlayerGUID;

        void Reset()
        {
            ShieldBashTimer = 5000;
            RevengeTimer = 8000;
        ...
        void switchFactionIfAlive(InstanceScript* instance, uint32 entry)
        {
		   ...
		   {
               if (crew->IsAlive())
                   crew->SetCurrentFaction(FACTION_HOSTILE);
		   }
        }
    };
...
private:
	void initBlyCrewMember(InstanceScript* instance, uint32 entry, float x, float y, float z)
	{
		if (Creature* crew = instance->instance->GetCreature(instance->GetData64(entry)))
		{
			...
			crew->SetCurrentFaction(FACTION_FREED);
		}
	}
};
...
public:
    ...
    struct npc_weegli_blastfuseAI : public QuantumBasicAI
    {
        npc_weegli_blastfuseAI(Creature* creature) : QuantumBasicAI(creature)
        {
            instance = creature->GetInstanceScript();
        ...
        void MovementInform(uint32 /*type*/, uint32 /*id*/)
        {
            if (instance)
            {
                if (instance->GetData(EVENT_PYRAMID) == PYRAMID_CAGES_OPEN)
                {
                    ...
                    DoSendQuantumText(SAY_WEEGLI_OHNO, me);
        ...
        void DestroyDoor()
        {
            if (me->IsAlive())
            {
                ...
                DoSendQuantumText(SAY_WEEGLI_OK_I_GO, me);
                destroyingDoor = true;
            }
        }
    };
...
public:
    ...
    {
        Creature* zumrah = player->FindCreatureWithDistance(NPC_ZUMRAH, 30.0f);

        if (!zumrah)
            return false;

        zumrah->SetCurrentFaction(ZUMRAH_HOSTILE_FACTION);
        return true;
    }
};
