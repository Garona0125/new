...
public:
    ...
    struct instance_zulfarrak_InstanceMapScript : public InstanceScript
    {
        ...
        uint64 EndDoorGUID;
        uint32 PyramidPhase;
        uint32 MajorWaveTimer;
        uint32 MinorWaveTimer;
        uint32 addGroupSize;
        uint32 waypoint;
            ...
            switch (PyramidPhase)
            {
                ...
                case PYRAMID_WAVE_1:
                    if (IsWaveAllDead())
                    {
                        ...
                        MajorWaveTimer = 10000; //give players a few seconds before wave 2 starts to rebuff
                    }
                    else
                        if (MinorWaveTimer<diff)
                        {
                            SendAddsUpStairs(addGroupSize++);
                            MinorWaveTimer = 10000;
                        }
                        else
                            MinorWaveTimer -= diff;
                    break;
                case PYRAMID_PRE_WAVE_2:
                    if (MajorWaveTimer<diff)
                    {
                        // beginning 2nd wave!
                        SpawnPyramidWave(2);
                ...
                case PYRAMID_WAVE_2:
                    if (IsWaveAllDead())
                    {
                        ...
                        MajorWaveTimer = 5000; //give NPCs time to return to their home spots
                    }
                    else
                        if (MinorWaveTimer<diff)
                        {
