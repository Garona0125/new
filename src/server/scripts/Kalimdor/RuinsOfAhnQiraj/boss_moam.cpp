...
class boss_moam : public CreatureScript
{
    public:
        ...
        struct boss_moamAI : public BossAI
        {
            ...
            {
                if (!IsStonePhase && HealthBelowPct(HEALTH_PERCENT_45))
                {
                    IsStonePhase = true;
            ...
            void DoAction(int32 const action)
            {
                switch (action)
                {
                    ...
                    {
                        DoCast(me, SPELL_SUMMON_MANA_FIEND_1, true);
                        DoCast(me, SPELL_SUMMON_MANA_FIEND_2, true);
                        DoCast(me, SPELL_SUMMON_MANA_FIEND_3, true);
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_DRAIN_MANA:
                        {
                            std::list<Unit*> targetList;
                            {
								...
								{
									if ((*itr)->getTarget()->GetTypeId() == TYPE_ID_PLAYER && (*itr)->getTarget()->GetPowerType() == POWER_MANA)
                                        targetList.push_back((*itr)->getTarget());
								}
                            }

                            Quantum::DataPackets::RandomResizeList(targetList, 5);
