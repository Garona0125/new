...
enum SkornWhitecloud
{
	QUEST_THE_DEMON_SCARRED_CLOACK = 770,
};
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (!player->GetQuestRewardStatus(QUEST_THE_DEMON_SCARRED_CLOACK))
...
public:
    ...
    struct npc_kyle_frenziedAI : public QuantumBasicAI
    {
        npc_kyle_frenziedAI(Creature* creature) : QuantumBasicAI(creature) {}
        ...
        void SpellHit(Unit* Caster, SpellInfo const* spell)
        {
            if (!me->GetVictim() && !EventActive && spell->Id == SPELL_LUNCH)
            {
                ...
                EventActive = true;
                DoSendQuantumText(EMOTE_SEE_LUNCH, me);
                ...
                if (EventTimer <= diff)
                {
                    ...
                    switch (EventPhase)
                    {
                        case 1:
                            ...
                            break;
                        case 2:
                            DoSendQuantumText(EMOTE_EAT_LUNCH, me);
                        ...
                        case 4:
                            EventTimer = 30000;
                            DoSendQuantumText(EMOTE_DANCE, me);
                            me->SetUInt32Value(UNIT_NPC_EMOTE_STATE, EMOTE_STATE_DANCE_SPECIAL);
                            break;
                        case 5:
...
Position const PlainVisionWP[50] =
{
...
public:
    ...
    struct npc_plains_visionAI  : public QuantumBasicAI
    {
        npc_plains_visionAI(Creature* creature) : QuantumBasicAI(creature) {}
        ...
        void UpdateAI(const uint32 /*diff*/)
        {
            if (NewWaypoint)
            {
                me->GetMotionMaster()->MovePoint(WayPointId, PlainVisionWP[WayPointId]);
                NewWaypoint = false;
            }
        }
    };
