...
public:
    ...
    struct netherspite_infernalAI : public QuantumBasicAI
    {
        netherspite_infernalAI(Creature* creature) : QuantumBasicAI(creature),
...
public:
    ...
    struct boss_malchezaarAI : public QuantumBasicAI
    {
        boss_malchezaarAI(Creature* creature) : QuantumBasicAI(creature)
        {
        ...
        void ClearWeapons()
        {
            ...
            me->SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, cinfo->mindmg);
            me->SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, cinfo->maxdmg);
        ...
        void SummonInfernal(const uint32 /*diff*/)
        {
            ...
            {
                infernal->SetDisplayId(MODEL_ID_INVISIBLE);
                infernal->SetCurrentFaction(me->GetFaction());
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (phase == 1)
            {
                if (HealthBelowPct(60))
                {
                    ...
                    const CreatureTemplate* cinfo = me->GetCreatureTemplate();
                    me->SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, 2*cinfo->mindmg);
                    me->SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, 2*cinfo->maxdmg);
                    me->UpdateDamagePhysical(BASE_ATTACK);

                    me->SetBaseWeaponDamage(OFF_ATTACK, MIN_DAMAGE, cinfo->mindmg);
                    me->SetBaseWeaponDamage(OFF_ATTACK, MAX_DAMAGE, cinfo->maxdmg);
                ...
                if (HealthBelowPct(30))
                {
                    ...
                    for (uint8 i = 0; i < 2; ++i)
                    {
                        ...
                        if (axe)
                        {
                            ...
                            axe->SetCurrentFaction(me->GetFaction());
                            axes[i] = axe->GetGUID();
                            if (target)
                            {
