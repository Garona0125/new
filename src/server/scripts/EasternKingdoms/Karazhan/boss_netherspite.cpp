...
public:
    ...
    struct boss_netherspiteAI : public QuantumBasicAI
    {
        boss_netherspiteAI(Creature* creature) : QuantumBasicAI(creature)
        {
        ...
        void UpdatePortals() // Here we handle the beams' behavior
        {
            for (int j=0; j<3; ++j) // j = color
                if (Creature* portal = Unit::GetCreature(*me, PortalGUID[j]))
                {
                    ...
                    if (j == RED_PORTAL && me->GetVictim() != target && target->GetTypeId() == TYPE_ID_PLAYER)
					{
        ...
        void SwitchToPortalPhase()
        {
            ...
            DoSendQuantumText(EMOTE_PHASE_PORTAL, me);
        }
        ...
        void SwitchToBanishPhase()
        {
            ...
            PhaseTimer = 30000;
            PortalPhase = false;
			DoSendQuantumText(EMOTE_PHASE_BANISH, me);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                DoCast(SelectTarget(TARGET_RANDOM, 1, 45, true), SPELL_VOIDZONE, true);
                VoidZoneTimer = 15000;
            }
