...
#define OZ_GM_GOSSIP1  "Change event to EVENT_OZ"
#define OZ_GM_GOSSIP2  "Change event to EVENT_HOOD"
#define OZ_GM_GOSSIP3  "Change event to EVENT_RAJ"
...
public:
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        switch (action)
        {
            ...
            case GOSSIP_ACTION_INFO_DEF+3:
                ...
                sLog->OutConsole("QUANTUMCORE SCRIPTS: player (GUID " UI64FMTD ") manually set Opera event to EVENT_OZ", player->GetGUID());
                break;
            case GOSSIP_ACTION_INFO_DEF+4:
                player->CLOSE_GOSSIP_MENU();
                pBarnesAI->EventId = EVENT_HOOD;
                sLog->OutConsole("QUANTUMCORE SCRIPTS: player (GUID " UI64FMTD ") manually set Opera event to EVENT_HOOD", player->GetGUID());
                break;
            case GOSSIP_ACTION_INFO_DEF+5:
                player->CLOSE_GOSSIP_MENU();
                pBarnesAI->EventId = EVENT_RAJ;
                sLog->OutConsole("QUANTUMCORE SCRIPTS: player (GUID " UI64FMTD ") manually set Opera event to EVENT_RAJ", player->GetGUID());
                break;
        }
    ...
    struct npc_barnesAI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 i)
        {
            ...
            switch (i)
            {
                ...
                case 4:
                    ...
                    if (Creature* pSpotlight = me->SummonCreature(NPC_SPOTLIGHT, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000))
                    {
        ...
        {
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Barnes Opera Event - Introduction complete - preparing encounter %d", EventId);
            uint8 index = 0;
            uint8 count = 0;
...
public:
    ...
    struct npc_image_of_medivhAI : public QuantumBasicAI
    {
        npc_image_of_medivhAI(Creature* creature) : QuantumBasicAI(creature)
        {
            instance = creature->GetInstanceScript();
        }
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        {
            player->CLOSE_GOSSIP_MENU();
            creature->MonsterSay("You are in combat!", LANG_UNIVERSAL, 0);
            return true;
        }
        else
    ...
    struct npc_hastingsAI : public QuantumBasicAI
    {
        npc_hastingsAI(Creature* creature) : QuantumBasicAI(creature)
        {
        ...
        void Reset()
        {
            events.ScheduleEvent(EVENT_SUMMON_BONUS_BOSS, 60 *IN_MILLISECONDS);
            Spawn = false;
        }
        ...
        void DoAction(const int32 actionId)
        {
            switch(actionId)
            {
                case ACTION_START_SUMMON:
                    events.ScheduleEvent(EVENT_SUMMON_BONUS_BOSS, 6 *IN_MILLISECONDS);
                    Spawn = true;
                    break;
                default:
                    break;
                    ...
                    case EVENT_SUMMON_BONUS_BOSS:
                    {
                        switch(rand()%3)
                        {
                            case 0:
                                ...
                                me->MonsterYell("Rokad the Ravager howls through the halls", LANG_UNIVERSAL, 0);
                                break;
                            case 1:
                                me->SummonCreature(NPC_HYAKISS_THE_LURKER, -10896.1f, -2075.4f, 49.5f, 1.4f, TEMPSUMMON_TIMED_DESPAWN, 600 * IN_MILLISECONDS);
                                me->MonsterYell("Hyakiss the Lurker skitters out from the shadows...!", LANG_UNIVERSAL, 0);
                                break;
                            case 2:
                                ...
                                me->MonsterYell("Shadikith the Glider flaps down from the ceiling...!", LANG_UNIVERSAL, 0);
                                break;
                        }
                        break;
                    }
...
public:
    ...
    struct npc_rokad_the_ravagerAI : public QuantumBasicAI
    {
        npc_rokad_the_ravagerAI(Creature* creature) : QuantumBasicAI(creature) {}

		EventMap events;

        void Reset()
        {
            events.ScheduleEvent(EVENT_RAVAGE, 15 * IN_MILLISECONDS);
        }
...
public:
    ...
    struct npc_shadikith_the_gliderAI : public QuantumBasicAI
    {
        npc_shadikith_the_gliderAI(Creature* creature) : QuantumBasicAI(creature) {}

		EventMap events;

        void Reset()
        {
            events.ScheduleEvent(EVENT_DIVE, 8 * IN_MILLISECONDS);
            events.ScheduleEvent(EVEMT_SONIC_BURST, 10 * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_WING_BUFFET, 15 * IN_MILLISECONDS);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                {
                    case EVENT_DIVE:
                        if (Unit* target = SelectTarget(TARGET_FARTHEST, 0, 45.0f, true))
						{
                    ...
                    case EVEMT_SONIC_BURST:
                        if (Unit* target = SelectTarget(TARGET_NEAREST, 0, 0, true))
						{
...
public:
    ...
    struct npc_hyakiss_the_lurkerAI : public QuantumBasicAI
    {
        npc_hyakiss_the_lurkerAI(Creature* creature) : QuantumBasicAI(creature) {}

		EventMap events;

        void Reset()
        {
            events.ScheduleEvent(EVENT_ACID_FANG, 8*IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_HYAKISS_WEB, 15*IN_MILLISECONDS);
        }
