...
class boss_janalai : public CreatureScript
{
    public:
        ...
        struct boss_janalaiAI : public QuantumBasicAI
        {
            boss_janalaiAI(Creature* creature) : QuantumBasicAI(creature)
            {
            ...
            {
                if (instance)
                    instance->SetData(DATA_JANALAI_EVENT, NOT_STARTED);
            ...
            void JustDied(Unit* /*Killer*/)
            {
                ...
                if (instance)
                    instance->SetData(DATA_JANALAI_EVENT, DONE);
            }
            ...
            {
                if (instance)
                    instance->SetData(DATA_JANALAI_EVENT, IN_PROGRESS);
            ...
            bool HatchAllEggs(uint32 uiAction) //1: reset, 2: isHatching all
            {
                ...
                for (std::list<Creature*>::const_iterator i = templist.begin(); i != templist.end(); ++i)
                {
                    ...
                    else if (uiAction == 2 &&(*i)->GetDisplayId() != MODEL_ID_INVISIBLE)
...
class npc_janalai_firebomb : public CreatureScript
{
    public:
        ...
        struct npc_janalai_firebombAI : public QuantumBasicAI
        {
            npc_janalai_firebombAI(Creature* creature) : QuantumBasicAI(creature){}
...
class mob_janalai_hatcher : public CreatureScript
{
    public:
        ...
        struct mob_janalai_hatcherAI : public QuantumBasicAI
        {
            mob_janalai_hatcherAI(Creature* creature) : QuantumBasicAI(creature)
            {
                instance =creature->GetInstanceScript();
            }
            ...
            bool HatchEggs(uint32 num)
            {
				...
				{
                    if ((*i)->GetDisplayId() != MODEL_ID_INVISIBLE)
                    {
                       (*i)->CastSpell(*i, SPELL_HATCH_EGG, false);
                        num--;
                    }
            ...
            void UpdateAI(const uint32 diff)
            {
                if (!instance || !(instance->GetData(DATA_JANALAI_EVENT) == IN_PROGRESS))
                {
                    me->DisappearAndDie();
                    return;
                }
...
class mob_janalai_hatchling : public CreatureScript
{
    public:
        ...
        struct mob_janalai_hatchlingAI : public QuantumBasicAI
        {
            mob_janalai_hatchlingAI(Creature* creature) : QuantumBasicAI(creature)
            {
                instance =creature->GetInstanceScript();
            }
            ...
            void EnterToBattle(Unit* /*who*/) {/*DoZoneInCombat();*/}
...
public:
    ...
    struct mob_janalai_eggAI : public QuantumBasicAI
    {
        mob_janalai_eggAI(Creature* creature) : QuantumBasicAI(creature){}
