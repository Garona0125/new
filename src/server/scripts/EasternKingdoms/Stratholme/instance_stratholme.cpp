...
public:
    ...
    struct instance_stratholme_InstanceMapScript : public InstanceScript
    {
        ...
        uint32 BaronRunTimer;
        uint32 SlaugtherSquareTimer;
        ...
        bool StartSlaugtherSquare()
        {
            ...
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Stratholme: Cannot open slaugther square yet.");
            return false;
        }
            ...
            case TYPE_BARON_RUN:
                ...
                case IN_PROGRESS:
                    if (Encounter[0] == IN_PROGRESS || Encounter[0] == FAIL)
                        break;
                    Encounter[0] = data;
                    BaronRunTimer = 2700000;
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Stratholme: Baron run in progress.");
                    break;
                case FAIL:
            ...
            case TYPE_RAMSTEIN:
                if (data == IN_PROGRESS)
                {
                    ...
                    if (!count)
                    {
                        ...
                        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Stratholme: Ramstein spawned.");
                    }
                    else
                        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Stratholme: %u Abomnation left to kill.", count);
                }
                ...
                if (data == DONE)
                {
                    SlaugtherSquareTimer = 300000;
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Stratholme: Slaugther event will continue in 1 minute.");
                    ...
                    if (GetData(TYPE_BARON_RUN) == IN_PROGRESS)
                    {
                        ...
                        if (!players.isEmpty())
                        {
                            for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                            {
                                ...
                                {
                                    if (player->HasAura(SPELL_BARON_ULTIMATUM))
        ...
        void Update(uint32 diff)
        {
            if (BaronRunTimer)
            {
                if (BaronRunTimer <= diff)
                {
                    if (GetData(TYPE_BARON_RUN) != DONE)
                        SetData(TYPE_BARON_RUN, FAIL);
                    BaronRunTimer = 0;
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Stratholme: Baron run event reached end. Event has state %u.", GetData(TYPE_BARON_RUN));
                }
				else BaronRunTimer -= diff;
            }

            if (SlaugtherSquareTimer)
            {
                if (SlaugtherSquareTimer <= diff)
                {
                    if (Creature* pBaron = instance->GetCreature(baronGUID))
                    {
                        for (uint8 i = 0; i < 4; ++i)
                        ...
                        HandleGameObject(ziggurat4GUID, true);
                        HandleGameObject(ziggurat5GUID, true);
                        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Stratholme: Black guard sentries spawned. Opening gates to baron.");
                    }
                    SlaugtherSquareTimer = 0;
                }
				else SlaugtherSquareTimer -= diff;
            }
        }
    };
