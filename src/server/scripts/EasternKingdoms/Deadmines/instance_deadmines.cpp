...
class instance_deadmines : public InstanceMapScript
{
    public:
        ...
        struct instance_deadmines_InstanceMapScript : public InstanceScript
        {
            ...
            uint32 State;
            uint32 CannonBlastTimer;
            uint32 PiratesDelayTimer;
            ...
            virtual void Update(uint32 diff)
            {
                ...
                switch (State)
                {
                    case CANNON_GUNPOWDER_USED:
                        CannonBlastTimer = DATA_CANNON_BLAST_TIMER;
                    ...
                    case CANNON_BLAST_INITIATED:
                        PiratesDelayTimer = DATA_PIRATES_DELAY_TIMER;
                        if (CannonBlastTimer <= diff)
                        {
                            SummonCreatures();
                            ShootCannon();
                            BlastOutDoor();
						...
						else CannonBlastTimer -= diff;
                        break;
                    case PIRATES_ATTACK:
                        if (PiratesDelayTimer <= diff)
                        {
                            MoveCreaturesInside();
                            State = EVENT_DONE;
                        }
						else PiratesDelayTimer -= diff;
                        break;
                }
            }
