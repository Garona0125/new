...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF+1)
        {
            ...
            creature->SetCurrentFaction(player->GetFaction());
        ...
        void WaypointReached(uint32 waypointId)
        {
            ...
            if (GetPlayerForEscort())
			{
                if (me->GetFaction() != GetPlayerForEscort()->GetFaction())
                    me->SetCurrentFaction(GetPlayerForEscort()->GetFaction());
			}
...
public:
    ...
    struct boss_grubbisAI : public QuantumBasicAI
    {
        boss_grubbisAI(Creature* creature) : QuantumBasicAI(creature)
        {
            SetDataSummoner();
        }
