        ...
        void WaypointReached(uint32 i)
        {
            ...
            case 5:
                if (instance)
                {
                    ...
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: npc_grimstone: event reached end and set complete.");
                }
                break;
            }
        }
...
public:
    ...
    struct mob_phalanxAI : public QuantumBasicAI
    {
        mob_phalanxAI(Creature* creature) : QuantumBasicAI(creature) {}
        ...
        void Reset()
        {
            ...
            FireballVolleyTimer = 0;
            MightyBlowTimer = 15000;
        }
...
public:
    ...
    struct npc_rocknotAI : public npc_escortAI
    {
        ...
        uint32 BreakKegTimer;
        uint32 BreakDoorTimer;
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (BreakKegTimer)
            {
                if (BreakKegTimer <= diff)
                {
                    DoGo(DATA_GO_BAR_KEG, 0);
                    BreakKegTimer = 0;
                    BreakDoorTimer = 1000;
                }
				else BreakKegTimer -= diff;
            }

            if (BreakDoorTimer)
            {
                if (BreakDoorTimer <= diff)
                {
                    ...
                    if (Unit* tmp = Unit::GetUnit(*me, instance->GetData64(DATA_PHALANX)))
                        tmp->SetCurrentFaction(16);

                    instance->SetData(TYPE_BAR, DONE);

                    BreakDoorTimer = 0;
                }
				else BreakDoorTimer -= diff;
            }
