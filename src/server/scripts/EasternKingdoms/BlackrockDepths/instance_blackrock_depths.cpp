...
public:
    ...
    struct instance_blackrock_depths_InstanceMapScript : public InstanceScript
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Blackrock Depths: SetData64 update (Type: %u Data " UI64FMTD ")", type, data);
        ...
        {
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: Instance Blackrock Depths: SetData update (Type: %u Data %u)", type, data);
        ...
        void TombOfSevenEvent()
        {
            if (GhostKillCount < 7 && TombBossGUIDs[TombEventCounter])
            {
                ...
                {
                    boss->SetCurrentFaction(FACTION_HOSTILE);
        ...
        void TombOfSevenReset()
        {
            ...
            for (uint8 i = 0; i < 7; ++i)
            {
                if (Creature* boss = instance->GetCreature(TombBossGUIDs[i]))
                {
                    ...
                    boss->SetCurrentFaction(FACTION_FRIEND);
                }
            }
            GhostKillCount = 0;
            TombEventStarterGUID = 0;
