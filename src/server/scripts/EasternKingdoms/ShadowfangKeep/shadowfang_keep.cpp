            ...
            switch (id)
            {
                case 0:
                    if (me->GetEntry() == NPC_ASH)
						DoSendQuantumText(SAY_FREE_AS, me);
                    else
						DoSendQuantumText(SAY_FREE_AD, me);
                    break;
                case 10:
                    if (me->GetEntry() == NPC_ASH)
						DoSendQuantumText(SAY_OPEN_DOOR_AS, me);
                    else
                        DoSendQuantumText(SAY_OPEN_DOOR_AD, me);
                    break;
                case 11:
                    if (me->GetEntry() == NPC_ASH)
                ...
                case 12:
                    if (me->GetEntry() == NPC_ASH)
                        DoSendQuantumText(SAY_POST_DOOR_AS, me);
					else
						DoSendQuantumText(SAY_POST1_DOOR_AD, me);
                ...
                case 13:
                    if (me->GetEntry() != NPC_ASH)
						DoSendQuantumText(SAY_POST2_DOOR_AD, me);
                    break;
            }
        }
    };
...
public:
    ...
    struct npc_arugal_voidwalkerAI : public QuantumBasicAI
    {
        npc_arugal_voidwalkerAI(Creature* creature) : QuantumBasicAI(creature)
        {
            instance = creature->GetInstanceScript();
        }
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            {
                if (Creature* Friend = me->FindCreatureWithDistance(me->GetEntry(), 25.0f, true))
                {
                    if (Friend)
                        DoCast(Friend, SPELL_DARK_OFFERING);
                }
                else
