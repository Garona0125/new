        ...
        struct boss_majordomoAI : public BossAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                if (instance && instance->GetBossState(BOSS_MAJORDOMO_EXECUTUS) != DONE)
                {
                    ...
                    if (!me->FindCreatureWithDistance(NPC_FLAMEWAKER_HEALER, 100.0f) && !me->FindCreatureWithDistance(NPC_FLAMEWAKER_ELITE, 100.0f))
                    {
                        instance->UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, me->GetEntry(), me);
                        me->SetCurrentFaction(35);
                        me->AI()->EnterEvadeMode();
						DoSendQuantumText(SAY_DEFEAT, me);
                        _JustDied();
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            ...
                            case EVENT_OUTRO_2:
                                ...
                                break;
                            case EVENT_OUTRO_3:
								DoSendQuantumText(SAY_ARRIVAL_2_MAJ, me);
                                break;
                            default:
                                break;
                        }
            ...
            void DoAction(const int32 action)
            {
                if (action == ACTION_START_RAGNAROS)
                {
					...
					DoSendQuantumText(SAY_SUMMON_MAJ, me);
