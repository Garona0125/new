...
public:
    ...
    struct boss_illidan_stormrageAI : public QuantumBasicAI
    {
        boss_illidan_stormrageAI(Creature* c) : QuantumBasicAI(c), Summons(me)
        {
        ...
        void HandleFlightSequence()
        {
            ...
            case 1://lift off
                ...
                me->SetUnitMovementFlags(MOVEMENTFLAG_ON_TRANSPORT);
                me->StopMoving();
            ...
            case 3://throw one glaive
                {
                    ...
                    if (Glaive)
                    {
                        ...
                        Glaive->SetDisplayId(MODEL_ID_INVISIBLE);
                        Glaive->SetCurrentFaction(me->GetFaction());
                ...
                for(uint8 i = 0; i < 2; i++)
                {
                    if (GlaiveGUID[i])
                    {
                        ...
                        if (Glaive)
                        {
                            ...
                            Glaive->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, MODEL_ID_INVISIBLE); // disappear but not die for now
                        }
                    }
                }
        ...
        void HandleTransformSequence()
        {
            ...
            if (DemonTransformation[TransformCount].displayid)
                me->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, DemonTransformation[TransformCount].displayid); // It's morphin time!
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (Phase == PHASE_FLIGHT)
            {
                ...
                case EVENT_FIREBALL:
                    if (!me->IsNonMeleeSpellCasted(false))
                    {
                        DoCast(SelectTarget(TARGET_RANDOM, 0), SPELL_FIREBALL);
                ...
                case EVENT_DARK_BARRAGE:
                    if (!me->IsNonMeleeSpellCasted(false))
                    {
                        DoCast(SelectTarget(TARGET_RANDOM, 0), SPELL_DARK_BARRAGE);
...
public:
    ...
    struct  flame_of_azzinothAI : public QuantumBasicAI
    {
        flame_of_azzinothAI(Creature *c) : QuantumBasicAI(c) {}
        ...
        void ChargeCheck()
        {
            Unit* target = SelectTarget(TARGET_FARTHEST, 0, 200, false);
...
public:
    ...
    struct  npc_akama_illidanAI : public QuantumBasicAI
    {
        npc_akama_illidanAI(Creature* c) : QuantumBasicAI(c)
        {
            instance = c->GetInstanceScript();
        }
        ...
        void MovementInform(uint32 MovementType, uint32 Data)
        {
			...
			me->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP); // TEST
        ...
        void BeginChannel()
        {
            ...
            if (Creature* Channel = me->SummonCreature(ILLIDAN_DOOR_TRIGGER, x, y, z+5, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 360000))
            {
                ...
                Channel->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, MODEL_ID_INVISIBLE); // Invisible but spell visuals can still be seen.
...
public:
    ...
    struct  boss_maievAI : public QuantumBasicAI
    {
        boss_maievAI(Creature *c) : QuantumBasicAI(c) {};
        ...
        void BlinkToPlayer()
        {
            if (GETCRE(Illidan, IllidanGUID))
            {
                Unit* target = CAST_AI(boss_illidan_stormrage::boss_illidan_stormrageAI,Illidan->AI())->SelectTarget(TARGET_RANDOM, 0);
...
public:
    ...
    struct cage_trap_triggerAI : public QuantumBasicAI
    {
        cage_trap_triggerAI(Creature *c) : QuantumBasicAI(c) {}
...
public:
    ...
    bool OnGossipHello (Player* pPlayer, GameObject* pGo)
    {
        ...
        if (Creature* pTrigger = pGo->FindCreatureWithDistance(23304, 10.0f))
...
public:
    ...
    struct  shadow_demonAI : public QuantumBasicAI
    {
        shadow_demonAI(Creature *c) : QuantumBasicAI(c) {}
...
public:
    ...
    struct  mob_parasitic_shadowfiendAI : public QuantumBasicAI
    {
        mob_parasitic_shadowfiendAI(Creature* creature) : QuantumBasicAI(creature)
        {
...
public:
    ...
    struct mob_blade_of_azzinothAI : public NullCreatureAI
    {
        mob_blade_of_azzinothAI(Creature* c) : NullCreatureAI(c) {}

        void SpellHit(Unit *caster, const SpellInfo *spell)
        {
            if (spell->Id == SPELL_THROW_GLAIVE2 || spell->Id == SPELL_THROW_GLAIVE)
                me->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, 21431);//appear when hit by Illidan's glaive
        }
    };

	CreatureAI* GetAI(Creature* _Creature) const
    {
        return new mob_blade_of_azzinothAI(_Creature);
    }
};
...
void boss_illidan_stormrage::boss_illidan_stormrageAI::Reset()
{
    ...
    me->SetUInt32Value(UNIT_FIELD_DISPLAY_ID, 21135);
    ...
    case PARASITIC_SHADOWFIEND:
        {
            ...
            Unit* target = SelectTarget(TARGET_TOP_AGGRO, 0, 999, true);
            if (!target || target->HasAura(SPELL_PARASITIC_SHADOWFIEND, 0)
                || target->HasAura(SPELL_PARASITIC_SHADOWFIEND2, 0))
                target = SelectTarget(TARGET_RANDOM, 0, 999, true);
            if (target)
                summon->AI()->AttackStart(target);
        }
		break;
    case SHADOW_DEMON:
        if (Unit* target = SelectTarget(TARGET_RANDOM, 0, 999, true)) // only on players.
        {
...
void boss_illidan_stormrage::boss_illidan_stormrageAI::SummonFlamesOfAzzinoth()
{
    ...
    for(uint8 i = 0; i < 2; ++i)
    {
        if (GETUNIT(Glaive, GlaiveGUID[i]))
        {
            ...
            if (Flame)
            {
                Flame->SetCurrentFaction(me->GetFaction()); // Just in case the database has it as a different faction
...
void boss_illidan_stormrage::boss_illidan_stormrageAI::SummonMaiev()
{
    ...
    {
        EnterEvadeMode();
        me->MonsterTextEmote("is unable to summon Maiev Shadowsong and enter Phase 4. Resetting Encounter.", LANG_UNIVERSAL, true);
    }
}
