...
public:
    ...
    struct boss_hydross_the_unstableAI : public QuantumBasicAI
    {
        boss_hydross_the_unstableAI(Creature* creature) : QuantumBasicAI(creature), Summons(me)
        {
        ...
        uint64 beams[2];
        uint32 PosCheckTimer;
        uint32 MarkOfHydrossTimer;
        uint32 MarkOfCorruptionTimer;
        uint32 WaterTombTimer;
        uint32 VileSludgeTimer;
        ...
        void SummonBeams()
        {
            Creature* beamer = me->SummonCreature(NPC_BEAM_DUMMY, -258.333f, -356.34f, 22.0499f, 5.90835f, TEMPSUMMON_CORPSE_DESPAWN, 0);
            if (beamer)
            {
                beamer->CastSpell(me, SPELL_BLUE_BEAM, true);
                beamer->SetDisplayId(MODEL_ID_INVISIBLE);
                beamer->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                beams[0]=beamer->GetGUID();
            }
            beamer = me->SummonCreature(NPC_BEAM_DUMMY, -219.918f, -371.308f, 22.0042f, 2.73072f, TEMPSUMMON_CORPSE_DESPAWN, 0);
            if (beamer)
            {
                beamer->CastSpell(me, SPELL_BLUE_BEAM, true);
                beamer->SetDisplayId(MODEL_ID_INVISIBLE);
        ...
        {
            if (CorruptedForm)
                DoSendQuantumText(RAND(SAY_CORRUPT_SLAY1, SAY_CORRUPT_SLAY2), me);
			else
				DoSendQuantumText(RAND(SAY_CLEAN_SLAY1, SAY_CLEAN_SLAY2), me);
        }

        void JustSummoned(Creature* summoned)
        {
            if (summoned->GetEntry() == NPC_PURE_SPAWN)
            {
            ...
            if (summoned->GetEntry() == NPC_TAINTED_SPAWN)
            {
        ...
        {
            if (CorruptedForm)
                DoSendQuantumText(SAY_CORRUPT_DEATH, me);
            else
                DoSendQuantumText(SAY_CLEAN_DEATH, me);
