...
public:
    ...
    struct boss_the_lurker_belowAI : public QuantumBasicAI
    {
        boss_the_lurker_belowAI(Creature* creature) : QuantumBasicAI(creature), Summons(me)
        {
        ...
        void EnterToBattle(Unit* who)
        {
            ...
            QuantumBasicAI::EnterToBattle(who);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            else // submerged
            {
                if (PhaseTimer <= diff)
                {
                    ...
                    me->RemoveAllAuras();
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER);
                    me->RemoveFlag(UNIT_NPC_EMOTE_STATE, EMOTE_STATE_SUBMERGED);
                ...
                if (!Spawned)
                {
                    me->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER);
                    // spawn adds
                    for (uint8 i = 0; i < 9; ++i)
...
public:
    ...
    struct mob_coilfang_ambusherAI : public QuantumBasicAI
    {
        mob_coilfang_ambusherAI(Creature* creature) : QuantumBasicAI(creature)
		{
			SetCombatMovement(false);
		}
