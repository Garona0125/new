    ...
    for (uint8 i = 0; i < 3; ++i)
    {
        ...
        for (uint8 j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j)
        {
                ...
                if (pItem->GetTemplate()->RequiredSpell == reqSpell)
            {
                ...
                sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUMCORE SCRIPTS: player attempt to unlearn spell %u, but item %u is equipped.", reqSpell, pItem->GetTemplate()->ItemId);
                return false;
            }
        }
    }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->HasSkill(SKILL_ALCHEMY) && player->GetBaseSkillValue(SKILL_ALCHEMY) >= 350 && player->GetCurrentLevel() > 67)
        {
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        //WEAPONSMITH SPEC
        if (player->HasSpell(S_WEAPON) && player->GetCurrentLevel() > 49 && player->GetBaseSkillValue(SKILL_BLACKSMITHING) >= 250)
        {
            switch (eCreature)
            {
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->HasSkill(SKILL_LEATHERWORKING) && player->GetBaseSkillValue(SKILL_LEATHERWORKING) >= 250 && player->GetCurrentLevel() > 49)
        {
            switch (eCreature)
            {
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        uint32 eCreature = creature->GetEntry();
                                                                //TAILORING SPEC
        if (player->HasSkill(SKILL_TAILORING) && player->GetBaseSkillValue(SKILL_TAILORING) >= 350 && player->GetCurrentLevel() > 59)
        {
