...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand accountSetCommandTable[] =
        {
            ...
            { "gmlevel",        SEC_MAIN_ADMINISTRATOR, true,  &HandleAccountSetGmLevelCommand,   "", NULL },
            { "password",       SEC_MAIN_ADMINISTRATOR, true,  &HandleAccountSetPasswordCommand,  "", NULL },
            { NULL,             0,                      false, NULL,                              "", NULL }
        };
        static ChatCommand accountCommandTable[] =
        {
            { "addon",          SEC_MODERATOR,          false, &HandleAccountAddonCommand,        "", NULL },
            { "create",         SEC_MAIN_ADMINISTRATOR, true,  &HandleAccountCreateCommand,       "", NULL },
            { "delete",         SEC_MAIN_ADMINISTRATOR, true,  &HandleAccountDeleteCommand,       "", NULL },
            { "onlinelist",     SEC_MAIN_ADMINISTRATOR, true,  &HandleAccountOnlineListCommand,   "", NULL },
	...
	static bool HandleAccountCreateCommand(ChatHandler* handler, char const* args)
    {
        ...
        switch (result)
        {
            case AOR_OK:
				...
				{
					sLog->OutCharacters("Account: %d (IP: %s) Character:[%s] (GUID: %u) created Account %s",
    ...
    static bool HandleAccountPasswordCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!AccountMgr::CheckPassword(handler->GetSession()->GetAccountId(), std::string(oldPassword)))
        {
			...
			sLog->OutCharacters("Account: %u (IP: %s) Character:[%s] (GUID: %u) Tried to change password.",
				handler->GetSession()->GetAccountId(),
				handler->GetSession()->GetRemoteAddress().c_str(),
				handler->GetSession()->GetPlayer()->GetName(),
				handler->GetSession()->GetPlayer()->GetGUIDLow());
        ...
        switch (result)
        {
            case AOR_OK:
				...
				sLog->OutCharacters("Account: %u (IP: %s) Character:[%s] (GUID: %u) Changed Password.",
					handler->GetSession()->GetAccountId(),
					handler->GetSession()->GetRemoteAddress().c_str(),
					handler->GetSession()->GetPlayer()->GetName(),
					handler->GetSession()->GetPlayer()->GetGUIDLow());
    ...
    static bool HandleAccountSetGmLevelCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (handler->GetSession())
            ...
            playerSecurity = SEC_MAIN_ADMINISTRATOR;
        ...
        if (gmRealmID == -1 && !AccountMgr::IsMainAdministratorAccount(playerSecurity))
        {
            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_SECURITY_GM_LEVEL_TEST);
        ...
        if (gmRealmID < -1)
        {
            handler->SendSysMessage(LANG_INVALID_REALM_ID);
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (gmRealmID == -1)
        {
            stmt = LoginDatabase.GetPreparedStatement(LOGIN_DEL_ACCOUNT_SECURITY);
            stmt->setUInt32(0, targetAccountId);
        }
        else
        {
            stmt = LoginDatabase.GetPreparedStatement(LOGIN_DEL_ACCOUNT_SECURITY_BY_REALM);
            stmt->setUInt32(0, targetAccountId);
        ...
        if (gm != 0)
        {
            stmt = LoginDatabase.GetPreparedStatement(LOGIN_INS_ACCOUNT_SECURITY);
