...
public:
		    ...
		    case BAN_SUCCESS:
            {
                if (atoi(durationStr) > 0)
                {
                    if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
                        sWorld->SendWorldText(LANG_BAN_CHARACTER_WORLD, (handler->GetSession() ? handler->GetSession()->GetPlayerName() : "Server"), name.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
                    else
                ...
                {
                    if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
                        sWorld->SendWorldText(LANG_BAN_CHARACTER_PERMAMENT, (handler->GetSession() ? handler->GetSession()->GetPlayerName() : "Server"), name.c_str(), reasonStr);
                    else
    ...
    static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        ...
        switch (sWorld->BanAccount(mode, nameOrIP, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName() : ""))
        {
			case BAN_SUCCESS:
                ...
                {
                    if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
                        sWorld->SendWorldText(LANG_BAN_ACCOUNT_WORLD, (handler->GetSession() ? handler->GetSession()->GetPlayerName() : "Server"), nameOrIP.c_str(), secsToTimeString(TimeStringToSecs(durationStr), true).c_str(), reasonStr);
                    else
                ...
                {
					if (sWorld->getBoolConfig(CONFIG_SHOW_BAN_IN_WORLD))
						sWorld->SendWorldText(LANG_BAN_ACCOUNT_PERMAMENT, (handler->GetSession() ? handler->GetSession()->GetPlayerName() : "Server"), nameOrIP.c_str(), reasonStr);
					else
    ...
    static bool HandleBanInfoHelper(uint32 accountId, char const* accountName, ChatHandler* handler)
    {
        ...
        do
        {
            ...
            std::string banTime = permanent ? handler->GetQuantumSystemText(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
            handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
                fields[0].GetCString(), banTime.c_str(), active ? handler->GetQuantumSystemText(LANG_BANINFO_YES) : handler->GetQuantumSystemText(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
        }
    ...
    static bool HandleBanInfoCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        do
        {
            ...
            std::string banTime = permanent ? handler->GetQuantumSystemText(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
            handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
                fields[0].GetCString(), banTime.c_str(), active ? handler->GetQuantumSystemText(LANG_BANINFO_YES) : handler->GetQuantumSystemText(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
        }
        while (result->NextRow());
    ...
    static bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_BANINFO_IPENTRY,
            fields[0].GetCString(), fields[1].GetCString(), permanent ? handler->GetQuantumSystemText(LANG_BANINFO_NEVER) : fields[2].GetCString(),
            permanent ? handler->GetQuantumSystemText(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
