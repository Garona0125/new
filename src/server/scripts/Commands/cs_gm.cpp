...
public:
    ...
    static bool HandleGMVisibleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_YOU_ARE, handler->GetSession()->GetPlayer()->IsGMVisible() ? handler->GetQuantumSystemText(LANG_VISIBLE) : handler->GetQuantumSystemText(LANG_INVISIBLE));
            return true;
        }
