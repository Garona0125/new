...
NpcFlagText const npcFlagTexts[NPCFLAG_COUNT] =
{
    ...
    { UNIT_NPC_FLAG_BATTLE_MASTER,      LANG_NPCINFO_BATTLE_MASTER      },
    { UNIT_NPC_FLAG_FLIGHT_MASTER,      LANG_NPCINFO_FLIGHT_MASTER      },
    ...
    { UNIT_NPC_FLAG_QUEST_GIVER,        LANG_NPCINFO_QUEST_GIVER        },
    { UNIT_NPC_FLAG_REPAIR,             LANG_NPCINFO_REPAIR             },
    { UNIT_NPC_FLAG_SPELL_CLICK,        LANG_NPCINFO_SPELL_CLICK        },
    { UNIT_NPC_FLAG_SPIRIT_GUIDE,       LANG_NPCINFO_SPIRIT_GUIDE       },
    { UNIT_NPC_FLAG_SPIRIT_HEALER,      LANG_NPCINFO_SPIRIT_HEALER      },
    { UNIT_NPC_FLAG_STABLE_MASTER,      LANG_NPCINFO_STABLE_MASTER      },
    { UNIT_NPC_FLAG_TABARD_DESIGNER,    LANG_NPCINFO_TABARD_DESIGNER    },
...
UnitFlag const unitFlags[MAX_UNIT_FLAGS] =
{
    ...
    { UNIT_FLAG_NO_AGGRO_FOR_PLAYER   , "UNIT_FLAG_NO_AGGRO_FOR_PLAYER"   },
    { UNIT_FLAG_NO_AGGRO_FOR_CREATURE , "UNIT_FLAG_NO_AGGRO_FOR_CREATURE" },
    { UNIT_FLAG_LOOTING               , "UNIT_FLAG_LOOTING"               },
    { UNIT_FLAG_PET_IN_COMBAT         , "UNIT_FLAG_PET_IN_COMBAT"         },
    { UNIT_FLAG_PLAYER_VS_PLAYER      , "UNIT_FLAG_PLAYER_VS_PLAYER"      },
...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand npcAddCommandTable[] =
        {
            ...
            { "item",           SEC_GAME_MASTER,    false, &HandleNpcAddVendorItemCommand,     "", NULL },
            { "move",           SEC_GAME_MASTER,    false, &HandleNpcAddMoveCommand,           "", NULL },
            { "temp",           SEC_GAME_MASTER,    false, &HandleNpcAddTempSpawnCommand,      "", NULL },
            //{ TODO: fix or remove this command
            { "weapon",         SEC_ADMINISTRATOR,  false, &HandleNpcAddWeaponCommand,         "", NULL },
            //}
            { "",               SEC_GAME_MASTER,    false, &HandleNpcAddCommand,               "", NULL },
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand npcDeleteCommandTable[] =
        {
            { "item",           SEC_GAME_MASTER,    false, &HandleNpcDeleteVendorItemCommand,  "", NULL },
            { "",               SEC_GAME_MASTER,    false, &HandleNpcDeleteCommand,            "", NULL },
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand npcFollowCommandTable[] =
        {
            { "stop",           SEC_GAME_MASTER,    false, &HandleNpcUnFollowCommand,          "", NULL },
            { "",               SEC_GAME_MASTER,    false, &HandleNpcFollowCommand,            "", NULL },
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand npcSetCommandTable[] =
        {
            ...
            { "factionid",      SEC_GAME_MASTER,    false, &HandleNpcSetFactionIdCommand,      "", NULL },
			{ "emote",          SEC_ADMINISTRATOR,  false, &HandleNpcSetEmoteCommand,          "", NULL },
            { "flag",           SEC_GAME_MASTER,    false, &HandleNpcSetFlagCommand,           "", NULL },
            { "level",          SEC_GAME_MASTER,    false, &HandleNpcSetLevelCommand,          "", NULL },
            { "link",           SEC_GAME_MASTER,    false, &HandleNpcSetLinkCommand,           "", NULL },
            { "model",          SEC_GAME_MASTER,    false, &HandleNpcSetModelCommand,          "", NULL },
            { "movetype",       SEC_GAME_MASTER,    false, &HandleNpcSetMoveTypeCommand,       "", NULL },
            { "phase",          SEC_GAME_MASTER,    false, &HandleNpcSetPhaseCommand,          "", NULL },
            { "spawndist",      SEC_GAME_MASTER,    false, &HandleNpcSetSpawnDistCommand,      "", NULL },
            { "spawntime",      SEC_GAME_MASTER,    false, &HandleNpcSetSpawnTimeCommand,      "", NULL },
            { "data",           SEC_ADMINISTRATOR,  false, &HandleNpcSetDataCommand,           "", NULL },
            //{ TODO: fix or remove these commands
            { "name",           SEC_GAME_MASTER,    false, &HandleNpcSetNameCommand,           "", NULL },
            { "subname",        SEC_GAME_MASTER,    false, &HandleNpcSetSubNameCommand,        "", NULL },
            //}
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand npcCommandTable[] =
        {
			...
			{ "near",           SEC_GAME_MASTER,    false, &HandleNpcNearCommand,              "", NULL },
            { "move",           SEC_GAME_MASTER,    false, &HandleNpcMoveCommand,              "", NULL },
            ...
            { "tame",           SEC_GAME_MASTER,    false, &HandleNpcTameCommand,              "", NULL },
            { "add",            SEC_GAME_MASTER,    false, NULL,                               "", npcAddCommandTable },
            { "delete",         SEC_GAME_MASTER,    false, NULL,                               "", npcDeleteCommandTable },
            { "follow",         SEC_GAME_MASTER,    false, NULL,                               "", npcFollowCommandTable },
            { "set",            SEC_GAME_MASTER,    false, NULL,                               "", npcSetCommandTable },
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
    ...
    static bool HandleNpcSetEntryCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!unit || unit->GetTypeId() != TYPE_ID_UNIT)
        {
            handler->SendSysMessage(LANG_SELECT_CREATURE);
            handler->SetSentErrorMessage(true);
            return false;
    ...
    static bool HandleNpcSetLevelCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (creature->IsPet())
        {
            ...
            {
                creature->SetUInt32Value(UNIT_FIELD_PET_NEXT_LEVEL_EXP, sObjectMgr->GetXPForLevel(lvl)/4);
    ...
    static bool HandleNpcSetFactionIdCommand(ChatHandler* handler, const char* args)
    {
        ...
        creature->SetCurrentFaction(factionId);
    ...
    static bool HandleNpcInfoCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        handler->PSendSysMessage(LANG_NPCINFO_LEVEL, target->GetCurrentLevel());
    ...
    static bool HandleNpcSetEmoteCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->SetUInt32Value(UNIT_NPC_EMOTE_STATE, emote);
        return true;
    }
    ...
    */
    static bool HandleNpcSetMoveTypeCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (dontdel_str)
        {
            //sLog->OutErrorConsole("DEBUG: All 3 params are set");
            ...
            {
                //sLog->OutErrorConsole("DEBUG: doNotDelete = true;");
                doNotDelete = true;
            }
        }
        else
        {
            ...
            if (type_str)
            {
                sLog->OutErrorConsole("DEBUG: Only 2 params ");
                if (stricmp(type_str, "NODEL") == 0)
                {
                    //sLog->OutErrorConsole("DEBUG: type_str, NODEL ");
                    doNotDelete = true;
                    type_str = NULL;
                }
            }
    ...
    static bool HandleNpcTameCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        uint8 level = (creatureTarget->GetCurrentLevel() < (player->GetCurrentLevel() - 5)) ? (player->GetCurrentLevel() - 5) : creatureTarget->GetCurrentLevel();
