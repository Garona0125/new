...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand arenaCommandTable[] =
        {
            ...
            { "info",           SEC_GAME_MASTER,    true,  &HandleArenaInfoCommand,     "", NULL },
            { "lookup",         SEC_GAME_MASTER,    false, &HandleArenaLookupCommand,   "", NULL },
            { NULL,             SEC_GAME_MASTER,    false, NULL,                        "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "arena",          SEC_GAME_MASTER,    false, NULL,                       "", arenaCommandTable },
    ...
    static bool HandleArenaCreateCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (type == 2 || type == 3 || type == 5 )
        {
            ...
            {
                handler->PSendSysMessage(LANG_ARENA_ERROR_SIZE, target->GetName());
                handler->SetSentErrorMessage(true);
                return false;
            }
    ...
    static bool HandleArenaDisbandCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (handler->GetSession())
            sLog->outArena("GameMaster: %s [GUID: %u] disbanded arena team type: %u [Id: %u].",
                handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUIDLow(), arena->GetType(), teamId);
        else
            sLog->outArena("Console: disbanded arena team type: %u [Id: %u].", arena->GetType(), teamId);
    ...
    static bool HandleArenaRenameCommand(ChatHandler* handler, char const* _args)
    {
        ...
        if (handler->GetSession())
            sLog->outArena("GameMaster: %s [GUID: %u] rename arena team \"%s\"[Id: %u] to \"%s\"",
                handler->GetSession()->GetPlayer()->GetName(), handler->GetSession()->GetPlayer()->GetGUIDLow(), oldArenaStr, arena->GetId(), newArenaStr);
        else
            sLog->outArena("Console: rename arena team \"%s\"[Id: %u] to \"%s\"", oldArenaStr, arena->GetId(), newArenaStr);
	...
	static bool HandleArenaCaptainCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_ARENA_CAPTAIN, arena->GetName().c_str(), arena->GetId(), oldCaptainNameData->m_name.c_str(), target->GetName());
        if (handler->GetSession())
            sLog->outArena("GameMaster: %s [GUID: %u] promoted player: %s [GUID: %u] to leader of arena team \"%s\"[Id: %u]",
                handler->GetSession()->GetPlayer()->GetName(),
				handler->GetSession()->GetPlayer()->GetGUIDLow(),
				target->GetName(), target->GetGUIDLow(),
				arena->GetName().c_str(), arena->GetId());
        else
            sLog->outArena("Console: promoted player: %s [GUID: %u] to leader of arena team \"%s\"[Id: %u]",
                target->GetName(), target->GetGUIDLow(), arena->GetName().c_str(), arena->GetId());
