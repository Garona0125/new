...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "delete",         SEC_MAIN_ADMINISTRATOR, true,  &HandleCharacterDeletedDeleteCommand,   "", NULL },
            ...
            { "old",            SEC_MAIN_ADMINISTRATOR, true,  &HandleCharacterDeletedOldCommand,      "", NULL },
            { NULL,             0,                      false, NULL,                                   "", NULL }
        };

        static ChatCommand characterCommandTable[] =
        {
            { "customize",      SEC_GAME_MASTER,        true,  &HandleCharacterCustomizeCommand,       "", NULL },
            { "changefaction",  SEC_GAME_MASTER,        true,  &HandleCharacterChangeFactionCommand,   "", NULL },
            { "changerace",     SEC_GAME_MASTER,        true,  &HandleCharacterChangeRaceCommand,      "", NULL },
            { "deleted",        SEC_GAME_MASTER,        true,  NULL,                                   "", characterDeletedCommandTable },
            { "erase",          SEC_MAIN_ADMINISTRATOR, true,  &HandleCharacterEraseCommand,           "", NULL },
            { "level",          SEC_ADMINISTRATOR,      true,  &HandleCharacterLevelCommand,           "", NULL },
            { "rename",         SEC_GAME_MASTER,        true,  &HandleCharacterRenameCommand,          "", NULL },
            { "reputation",     SEC_GAME_MASTER,        true,  &HandleCharacterReputationCommand,      "", NULL },
            { "titles",         SEC_GAME_MASTER,        true,  &HandleCharacterTitlesCommand,          "", NULL },
            { NULL,             0,                      false, NULL,                                   "", NULL }
        };

        static ChatCommand commandTable[] =
        {
            { "character",      SEC_GAME_MASTER,    true,  NULL,                                   "", characterCommandTable },
    ...
    */
    ...
    static bool HandleCharacterTitlesCommand(ChatHandler* handler, char const* args)
    {
        ...
        char const* knownStr = handler->GetQuantumSystemText(LANG_KNOWN);
        ...
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
        {
            ...
            {
                std::string name = target->GetCurrentGender() == GENDER_MALE ? titleInfo->nameMale[loc] : titleInfo->nameFemale[loc];
                if (name.empty())
                    continue;
    ...
    static bool HandleCharacterLevelCommand(ChatHandler* handler, char const* args)
    {
        ...
        int32 oldlevel = target ? target->GetCurrentLevel() : Player::GetLevelFromDB(targetGuid);
    ...
    static bool HandleCharacterReputationCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
        {
            ...
            std::string rankName = handler->GetQuantumSystemText(ReputationRankStrIndex[rank]);
    ...
    {
        int32 keepDays = sWorld->getIntConfig(CONFIG_CHAR_DELETE_KEEP_DAYS);
