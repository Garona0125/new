...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "mask",           SEC_GAME_MASTER,    false, &HandleTitlesSetMaskCommand,        "", NULL },
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand titlesCommandTable[] =
        {
            { "add",            SEC_GAME_MASTER,    false, &HandleTitlesAddCommand,            "", NULL },
            { "current",        SEC_GAME_MASTER,    false, &HandleTitlesCurrentCommand,        "", NULL },
            { "remove",         SEC_GAME_MASTER,    false, &HandleTitlesRemoveCommand,         "", NULL },
            { "set",            SEC_GAME_MASTER,    false, NULL,              "", titlesSetCommandTable },
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "titles",         SEC_GAME_MASTER,    false, NULL,                 "", titlesCommandTable },
            { NULL,             0,                  false, NULL,                               "", NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleTitlesCurrentCommand(ChatHandler* handler, const char* args)
    {
        ...
        handler->PSendSysMessage(LANG_TITLE_CURRENT_RES, id, target->GetCurrentGender() == GENDER_MALE ? titleInfo->nameMale[handler->GetSessionDbcLocale()] : titleInfo->nameFemale[handler->GetSessionDbcLocale()], tNameLink.c_str());
        return true;
    }
    ...
    static bool HandleTitlesAddCommand(ChatHandler* handler, const char* args)
    {
		...
		snprintf(titleNameStr, 80, target->GetCurrentGender() == GENDER_MALE ? titleInfo->nameMale[handler->GetSessionDbcLocale()] : titleInfo->nameFemale[handler->GetSessionDbcLocale()], target->GetName());
