...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "create",             SEC_GAME_MASTER,        false, &HandleCreatePetCommand,             "", NULL },
            { "learn",              SEC_GAME_MASTER,        false, &HandlePetLearnCommand,              "", NULL },
            { "unlearn",            SEC_GAME_MASTER,        false, &HandlePetUnlearnCommand,            "", NULL },
            { NULL,                 0,                      false, NULL,                                "", NULL }
        };
        ...
        static ChatCommand commandTable[] =
        {
            ...
            { "guid",               SEC_GAME_MASTER,        false, &HandleGUIDCommand,                  "", NULL },
            { "help",               SEC_PLAYER,             true,  &HandleHelpCommand,                  "", NULL },
            { "itemmove",           SEC_GAME_MASTER,        false, &HandleItemMoveCommand,              "", NULL },
            ...
            { "kick",               SEC_GAME_MASTER,        true,  &HandleKickPlayerCommand,            "", NULL },
            ...
            { "pinfo",              SEC_GAME_MASTER,        true,  &HandlePInfoCommand,                 "", NULL },
            { "respawn",            SEC_ADMINISTRATOR,      false, &HandleRespawnCommand,               "", NULL },
            { "send",               SEC_MODERATOR,          true,  NULL,                                "", sendCommandTable },
            { "pet",                SEC_GAME_MASTER,        false, NULL,                                "", petCommandTable },
            ...
            { "combatstop",         SEC_GAME_MASTER,        true,  &HandleCombatStopCommand,            "", NULL },
            { "flusharenapoints",   SEC_ADMINISTRATOR,      false, &HandleFlushArenaPointsCommand,      "", NULL },
            { "repairitems",        SEC_GAME_MASTER,        true,  &HandleRepairitemsCommand,           "", NULL },
			...
			{ "pvpstats",           SEC_GAME_MASTER,        true,  &HandlePvPstatsCommand,              "", NULL },
            ...
            { "playall",            SEC_GAME_MASTER,        false, HandlePlayAllCommand,                "", NULL },
			{ "nameannounce",       SEC_GAME_MASTER,        false, HandleNameAnnounceCommand,           "", NULL },
            { NULL,                 0,                      false, NULL,                                "", NULL }
        };
	...
	static bool HandlePvPstatsCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_PVP_STATS_FACTIONS_OVERALL);
            ...
            if (result)
            {
                ...
                handler->PSendSysMessage(LANG_PVP_STATS, alliance_victories, horde_victories);
            }
            else
                return false;
        }
        else
            handler->PSendSysMessage(LANG_PVP_STATS_DISABLED);
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (*args)
        {
            ...
            if (guid)
                object = (WorldObject*)ObjectAccessor::GetObjectByTypeMask(*handler->GetSession()->GetPlayer(), guid, TYPE_MASK_UNIT | TYPE_MASK_GAMEOBJECT);
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetQuantumSystemText(LANG_OFFLINE));
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            {
                handler->PSendSysMessage(LANG_UNKNOWN_SPELL, target == handler->GetSession()->GetPlayer() ? handler->GetQuantumSystemText(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            target->RemoveSpellCooldown(spellIid, true);
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid, target == handler->GetSession()->GetPlayer() ? handler->GetQuantumSystemText(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleGetDistanceCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (*args)
        {
            uint64 guid = handler->extractGuidFromLink((char*)args);
            if (guid)
                obj = (WorldObject*)ObjectAccessor::GetObjectByTypeMask(*handler->GetSession()->GetPlayer(), guid, TYPE_MASK_UNIT | TYPE_MASK_GAMEOBJECT);
    ...
    static bool HandleKickPlayerCommand(ChatHandler* handler, char const* args)
    {
		...
		if (sWorld->getBoolConfig(CONFIG_SHOW_KICK_IN_WORLD))
			sWorld->SendWorldText(LANG_COMMAND_KICK_WORLD, (handler->GetSession() ? handler->GetSession()->GetPlayerName() : "Server"), playerName.c_str(), kickReasonStr.c_str());
		else
	...
	static bool HandleUnstuckCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (player->IsInFlight() || player->IsInCombatActive())
        {
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (graveyard)
        {
            ...
            std::string team_name = handler->GetQuantumSystemText(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
                team_name = handler->GetQuantumSystemText(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
                team_name = handler->GetQuantumSystemText(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
                team_name = handler->GetQuantumSystemText(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
		...
		sLog->outDetail(handler->GetQuantumSystemText(LANG_ADDITEM), itemId, count);
    ...
    static bool HandleAddItemSetCommand(ChatHandler* handler, char const* args)
    {
		...
		sLog->outDetail(handler->GetQuantumSystemText(LANG_ADDITEMSET), itemSetId);
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
            ...
            level           = target->GetCurrentLevel();
            latency         = target->GetSession()->GetLatency();
            race            = target->GetCurrentRace();
            Class           = target->GetCurrentClass();
        ...
        std::string userName  = handler->GetQuantumSystemText(LANG_ERROR);
        std::string eMail     = handler->GetQuantumSystemText(LANG_ERROR);
        std::string lastIp    = handler->GetQuantumSystemText(LANG_ERROR);
        uint32 security       = 0;
        std::string lastLogin = handler->GetQuantumSystemText(LANG_ERROR);
        ...
        handler->PSendSysMessage(LANG_PINFO_ACCOUNT, (target ? "" : handler->GetQuantumSystemText(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(targetGuid), userName.c_str(), accId, eMail.c_str(), security, lastIp.c_str(), lastLogin.c_str(), latency);
    ...
    static bool HandleRespawnCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        {
            if (target->GetTypeId() != TYPE_ID_UNIT || target->IsPet())
            {
    ...
    static bool HandleMuteCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
			...
			{
				sWorld->SendWorldText(LANG_COMMAND_MUTE_WORLD, (handler->GetSession() ? handler->GetSession()->GetPlayerName() : "Server"), nameLink.c_str(), notSpeakTime, muteReasonStr.c_str());
		...
		if (sWorld->getBoolConfig(CONFIG_SHOW_MUTE_IN_WORLD) && !target)
			sWorld->SendWorldText(LANG_COMMAND_MUTE_WORLD, handler->GetSession()->GetPlayerName(), nameLink.c_str(), notSpeakTime, muteReasonStr.c_str());
		else
    ...
    static bool HandleMovegensCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        handler->PSendSysMessage(LANG_MOVEGENS_LIST, (unit->GetTypeId() == TYPE_ID_PLAYER ? "Player" : "Creature"), unit->GetGUIDLow());
        ...
        for (uint8 i = 0; i < MAX_MOTION_SLOT; ++i)
        {
            ...
            switch (movementGenerator->GetMovementGeneratorType())
            {
                ...
                case CHASE_MOTION_TYPE:
                {
                    ...
                    else if (target->GetTypeId() == TYPE_ID_PLAYER)
                ...
                {
                    if (unit->GetTypeId() == TYPE_ID_UNIT)
    ...
    */
    ...
    static bool HandleDamageCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!schoolStr)
        {
            ...
            if (target != handler->GetSession()->GetPlayer())
                handler->GetSession()->GetPlayer()->SendAttackStateUpdate (HITINFO_AFFECTS_VICTIM, target, 1, SPELL_SCHOOL_MASK_NORMAL, damage, 0, 0, VICTIM_STATE_HIT, 0);
            return true;
        }
        ...
        if (!spellStr)
        {
            ...
            handler->GetSession()->GetPlayer()->SendAttackStateUpdate (HITINFO_AFFECTS_VICTIM, target, 1, schoolmask, damage, absorb, resist, VICTIM_STATE_HIT, 0);
            return true;
        }
    ...
    static bool HandleCreatePetCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        if (!creatureTarget || creatureTarget->IsPet() || creatureTarget->GetTypeId() == TYPE_ID_PLAYER)
        {
        ...
        if (!pet->InitStatsForLevel(creatureTarget->GetCurrentLevel()))
        {
			sLog->OutErrorConsole("InitStatsForLevel() in EffectTameCreature failed! Pet deleted.");
        ...
        pet->SetUInt32Value(UNIT_FIELD_LEVEL, creatureTarget->GetCurrentLevel()-1);
        ...
        pet->SetUInt32Value(UNIT_FIELD_LEVEL, creatureTarget->GetCurrentLevel());
	...
	static bool HandleListFreezeCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        do
        {
            ...
            if (Player* frozen = sObjectAccessor->FindPlayerByName(player.c_str()))
				frozen->SaveToDB();
	...
	static bool HandleNameAnnounceCommand(ChatHandler* handler, char const* args)
	{
		...
		if (sWorld->getIntConfig(CONFIG_NAME_COMMAND) == 1)
		{
			...
			{
			    case SEC_MODERATOR:
					sWorld->SendWorldText(LANG_MODERATOR_ANNOUNCE, handler->GetSession()->GetPlayerName(), args);
					break;
				case SEC_EVENT_MASTER:
					sWorld->SendWorldText(LANG_EVENT_MASTER_ANNOUNCE,handler->GetSession()->GetPlayerName(), args);
					break;
				case SEC_GAME_MASTER:
					sWorld->SendWorldText(LANG_GAME_MASTER_ANNOUNCE, handler->GetSession()->GetPlayerName(), args);
					break;
				case SEC_ADMINISTRATOR:
					sWorld->SendWorldText(LANG_ADMINISTRATOR_ANNOUNCE, handler->GetSession()->GetPlayerName(), args);
					break;
				case SEC_MAIN_ADMINISTRATOR:
					sWorld->SendWorldText(LANG_MAIN_ADMINISTRATOR_ANNOUNCE, handler->GetSession()->GetPlayerName(), args);
					break;
				case SEC_DEVELOPER:
					sWorld->SendWorldText(LANG_DEVELOPER_ANNOUNCE, handler->GetSession()->GetPlayerName(), args);
					break;
				case SEC_TECHNICAL_MASTER:
					sWorld->SendWorldText(LANG_TECH_SUPPORT_ANNOUNCE, handler->GetSession()->GetPlayerName(), args);
					break;
			}
		}

		if (sWorld->getIntConfig(CONFIG_NAME_COMMAND) == 0)
			sWorld->SendWorldText(LANG_ANNOUNCE_COLOR, handler->GetSession()->GetPlayerName(), args);
