...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand modifyCommandTable[] =
        {
            ...
            { "reputation",     SEC_GAME_MASTER,    false, &HandleModifyRepCommand,           "", NULL },
            { "arenapoints",    SEC_MODERATOR,      false, &HandleModifyArenaCommand,         "", NULL },
            { "drunk",          SEC_MODERATOR,      false, &HandleModifyDrunkCommand,         "", NULL },
            { "standstate",     SEC_GAME_MASTER,    false, &HandleModifyStandStateCommand,    "", NULL },
            { "phase",          SEC_ADMINISTRATOR,  false, &HandleModifyPhaseCommand,         "", NULL },
            { "gender",         SEC_GAME_MASTER,    false, &HandleModifyGenderCommand,        "", NULL },
            { "speed",          SEC_MODERATOR,      false, NULL,           "", modifyspeedCommandTable },
			{ "aspeed",         SEC_MODERATOR,      false, &HandleModifyASpeedCommand,        "", NULL },
			{ "xp",             SEC_MODERATOR,      false, &HandleModifyXPCommand,            "", NULL },
            { NULL,             0,                  false, NULL,                                           "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "morph",          SEC_GAME_MASTER,    false, &HandleModifyMorphCommand,          "", NULL },
            { "demorph",        SEC_GAME_MASTER,    false, &HandleDeMorphCommand,              "", NULL },
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
        ...
        sLog->outDetail(handler->GetQuantumSystemText(LANG_CURRENT_ENERGY), target->GetMaxPower(POWER_ENERGY));
    ...
    static bool HandleModifyFactionCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->SetCurrentFaction(factionid);
    ...
    static bool HandleModifyTalentCommand (ChatHandler* handler, const char* args)
    {
        ...
        else if (target->ToCreature()->IsPet())
        {
            ...
            if (owner && owner->GetTypeId() == TYPE_ID_PLAYER && ((Pet*)target)->IsPermanentPetFor(owner->ToPlayer()))
            {
                // check online security
    ...
    static bool HandleModifyMountCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_VS_PLAYER);
        target->Mount(mId);
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (moneyToAdd < 0)
        {
            ...
            sLog->outDetail(handler->GetQuantumSystemText(LANG_CURRENT_MONEY), targetMoney, moneyToAdd, newmoney);
            if (newmoney <= 0)
            {
        ...
        sLog->outDetail(handler->GetQuantumSystemText(LANG_NEW_MONEY), targetMoney, moneyToAdd, target->GetMoney());
    ...
    static bool HandleModifyBitCommand(ChatHandler* handler, const char* args)
    {
        ...
        // check online security
        if (target->GetTypeId() == TYPE_ID_PLAYER && handler->HasLowerSecurity(target->ToPlayer(), 0))
            return false;
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetQuantumSystemText(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
    ...
    static bool HandleModifyMorphCommand(ChatHandler* handler, const char* args)
    {
        ...
        // check online security
        else if (target->GetTypeId() == TYPE_ID_PLAYER && handler->HasLowerSecurity(target->ToPlayer(), 0))
            return false;
    ...
    static bool HandleModifyStandStateCommand(ChatHandler* handler, const char* args)
    {
        ...
        handler->GetSession()->GetPlayer()->SetUInt32Value(UNIT_NPC_EMOTE_STATE, anim_id);
    ...
    static bool HandleModifyGenderCommand(ChatHandler* handler, const char* args)
    {
        ...
        PlayerInfo const* info = sObjectMgr->GetPlayerInfo(target->GetCurrentRace(), target->GetCurrentClass());
        if (!info)
            return false;
        ...
        {
            if (target->GetCurrentGender() == GENDER_FEMALE)
                return true;
