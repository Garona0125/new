...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand resetCommandTable[] =
        {
			...
			{ "arenapoints",    SEC_ADMINISTRATOR,  true,  &HandleResetArenaPointsCommand,      "", NULL },
    ...
    {
        ChrClassesEntry const* classEntry = sChrClassesStore.LookupEntry(player->GetCurrentClass());
        if (!classEntry)
        {
            sLog->OutErrorConsole("Class %u not found in DBC (Wrong DBC files?)", player->GetCurrentClass());
            return false;
        }
        ...
        player->SetUInt32Value(UNIT_FIELD_BYTES_0, ((player->GetCurrentRace()) | (player->GetCurrentClass() << 8) | (player->GetCurrentGender() << 16) | (powerType << 24)));
    ...
    static bool HandleResetLevelCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint8 oldLevel = target->GetCurrentLevel();

        // set starting level
        uint32 startLevel = target->GetCurrentClass() != CLASS_DEATH_KNIGHT
    ...
    static bool HandleResetTalentsCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!handler->extractPlayerTarget((char*)args, &target, &targetGuid, &targetName))
        {
            ...
            if (!*args && creature && creature->IsPet())
            {
                ...
                if (owner && owner->GetTypeId() == TYPE_ID_PLAYER && creature->ToPet()->IsPermanentPetFor(owner->ToPlayer()))
                {
