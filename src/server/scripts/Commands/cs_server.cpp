...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "difftime",       SEC_MAIN_ADMINISTRATOR, true,  &HandleServerSetDiffTimeCommand,     "", NULL },
            { "loglevel",       SEC_MAIN_ADMINISTRATOR, true,  &HandleServerSetLogLevelCommand,     "", NULL },
            { "logfilelevel",   SEC_MAIN_ADMINISTRATOR, true,  &HandleServerSetLogFileLevelCommand, "", NULL },
        ...
        {
            { "corpses",        SEC_GAME_MASTER,        true,  &HandleServerCorpsesCommand,         "", NULL },
            { "exit",           SEC_MAIN_ADMINISTRATOR, true,  &HandleServerExitCommand,            "", NULL },
            ...
            { "togglequerylog", SEC_MAIN_ADMINISTRATOR, true,  &HandleServerToggleQueryLogging,     "", NULL },
            { NULL,             0,                      false, NULL,                                "", NULL }
        };
    ...
    static bool HandleServerPLimitCommand(ChatHandler* handler, char const* args)
    {
        if (*args)
        {
			...
			else if (strncmp(paramStr, "eventmaster", limit) == 0)
                sWorld->SetPlayerSecurityLimit(SEC_EVENT_MASTER);
			else if (strncmp(paramStr, "gamemaster", limit) == 0)
                sWorld->SetPlayerSecurityLimit(SEC_GAME_MASTER);
            else if (strncmp(paramStr, "administrator", limit) == 0)
                sWorld->SetPlayerSecurityLimit(SEC_ADMINISTRATOR);
			else if (strncmp(paramStr, "mainadministrator", limit) == 0)
                sWorld->SetPlayerSecurityLimit(SEC_MAIN_ADMINISTRATOR);
			else if (strncmp(paramStr, "developer", limit) == 0)
				sWorld->SetPlayerSecurityLimit(SEC_DEVELOPER);
			else if (strncmp(paramStr, "technicalmaster", limit) == 0)
				sWorld->SetPlayerSecurityLimit(SEC_TECHNICAL_MASTER);
            else if (strncmp(paramStr, "reset", limit) == 0)
            {
                sWorld->SetPlayerAmountLimit(GetConfigSettings::GetIntState("PlayerLimit", 100));
        ...
        switch (allowedAccountType)
        {
			...
			case SEC_MAIN_ADMINISTRATOR:
				secName = "Console"; // Main Administrator
				break;
			case SEC_DEVELOPER:
				secName = "Developer";
				break;
			case SEC_TECHNICAL_MASTER:
				secName = "Technicalmaster";
				break;
            default:
                secName = "<unknown>";
                break;
