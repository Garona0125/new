...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand learnAllCommandTable[] =
        {
            ...
            { "gm",             SEC_GAME_MASTER,    false, &HandleLearnAllGMCommand,            "", NULL },
            { "crafts",         SEC_GAME_MASTER,    false, &HandleLearnAllCraftsCommand,        "", NULL },
            { "default",        SEC_MODERATOR,      false, &HandleLearnAllDefaultCommand,       "", NULL },
            { "lang",           SEC_MODERATOR,      false, &HandleLearnAllLangCommand,          "", NULL },
            { "recipes",        SEC_GAME_MASTER,    false, &HandleLearnAllRecipesCommand,       "", NULL },
            { NULL,             0,                  false, NULL,                                "", NULL }
        };
    ...
    {
        ChrClassesEntry const* classEntry = sChrClassesStore.LookupEntry(handler->GetSession()->GetPlayer()->GetCurrentClass());
        if (!classEntry)
            return true;
