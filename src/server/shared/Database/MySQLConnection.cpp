...
bool MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        sLog->OutErrorConsole("Could not initialize Mysql connection to database `%s`", m_connectionInfo.database.c_str());
        return false;
    }
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
			sLog->OutConsole("MySQL client library: %s", mysql_get_client_info()); // OutSQLConnection
			sLog->OutConsole("MySQL server ver: %s ", mysql_get_server_info(m_Mysql)); // OutSQLConnection
        }
    ...
    {
        sLog->OutErrorConsole("Could not connect to MySQL database at %s: %s\n", m_connectionInfo.host.c_str(), mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(const char* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            sLog->OutSQLConnection("SQL: %s", sql);
            sLog->OutSQLConnection("ERROR: [%u] %s", lErrno, mysql_error(m_Mysql));
        ...
        {
            sLog->OutSQLConnection("[%u ms] SQL: %s", getMSTimeDiff(_s, getMSTime()), sql);
        }
    }
...
bool MySQLConnection::Execute(PreparedStatement* stmt)
{
    ...
    uint32 index = stmt->m_index;
    {
        ...
        if (sLog->GetSQLDriverQueryLogging())
            sLog->OutSQLConnection("[%u ms] SQL(p): %s", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString(m_queries[index].first).c_str());
    ...
    for (itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    sLog->OutSQLConnection("[Warning] Transaction aborted. %u queries not executed.", (uint32)queries.size());
                    RollbackTransaction();
                    return false;
                }
            }
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    if (!ret)
        sLog->OutSQLConnection("ERROR: Could not fetch prepared statement %u on database `%s`, connection type: %s.",
...
void MySQLConnection::PrepareStatement(uint32 index, const char* sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        sLog->OutSQLConnection("[ERROR]: In mysql_stmt_init() id: %u, sql: \"%s\"", index, sql);
        sLog->OutSQLConnection("[ERROR]: %s", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            sLog->OutSQLConnection("[ERROR]: In mysql_stmt_prepare() id: %u, sql: \"%s\"", index, sql);
            sLog->OutSQLConnection("[ERROR]: %s", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo)
{
    switch (errNo)
    {
        ...
        case 2055:  // "Lost connection to MySQL server at '%s', system error: %d"
        {
            ...
            {
                sLog->OutSQLConnection("Connection to the MySQL server is active.");
                if (oldThreadId != mysql_thread_id(GetHandle()))
                    sLog->OutSQLConnection("Successfully reconnected to %s @%s:%s (%s).",
        ...
        default:
            sLog->OutSQLConnection("Unhandled MySQL errno %u. Unexpected behaviour possible.", errNo);
            return false;
    }
}