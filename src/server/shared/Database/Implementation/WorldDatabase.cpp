...
{
    if (!m_reconnecting)
        m_stmts.resize(MAX_WORLD_DATA_BASE_STATEMENTS);

    PREPARE_STATEMENT(WORLD_SEL_QUEST_POOLS, "SELECT entry, pool_entry FROM pool_quest", CONNECTION_SYNCH);
    PREPARE_STATEMENT(WORLD_DEL_CRELINKED_RESPAWN, "DELETE FROM creature_linked_respawn WHERE guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_REP_CREATURE_LINKED_RESPAWN, "REPLACE INTO creature_linked_respawn (guid, linkedGuid) VALUES (?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(WORLD_INS_GAME_TELE, "INSERT INTO game_teleportation_zones (id, position_x, position_y, position_z, orientation, map, name) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_DEL_GAME_TELE, "DELETE FROM game_teleportation_zones WHERE name = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_INS_NPC_VENDOR, "INSERT INTO npc_vendor (entry, item, maxcount, incrtime, extendedcost) VALUES(?, ?, ?, ?, ?)", CONNECTION_ASYNC);
