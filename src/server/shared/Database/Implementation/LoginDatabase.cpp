...
{
    if (!m_reconnecting)
        m_stmts.resize(MAX_LOGIN_DATA_BASE_STATEMENTS);

    PREPARE_STATEMENT(LOGIN_SEL_REALM_LIST, "SELECT id, name, address, port, icon, flag, timezone, allowedSecurityLevel, population, gamebuild FROM realmlist WHERE flag <> 3 ORDER BY name", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_SESSION_KEY, "SELECT a.sessionkey, a.id, aa.PrivilegeLevel  FROM account a LEFT JOIN account_security aa ON (a.id = aa.id) WHERE username = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_UPD_VS, "UPDATE account SET v = ?, s = ? WHERE username = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_UPD_LOGON_PROOF, "UPDATE account SET sessionkey = ?, last_ip = ?, last_login = NOW(), locale = ?, failed_logins = 0, os = ? WHERE username = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_LOGON_CHALLENGE, "SELECT a.sha_pass_hash, a.id, a.locked, a.last_ip, aa.PrivilegeLevel, a.v, a.s FROM account a LEFT JOIN account_security aa ON (a.id = aa.id) WHERE a.username = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_UPD_FAILED_LOGINS, "UPDATE account SET failed_logins = failed_logins + 1 WHERE username = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_FAILED_LOGINS, "SELECT id, failed_logins FROM account WHERE username = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_NUM_CHARS_ON_REALM, "SELECT charcount FROM account_char_per_realm WHERE realmid = ? AND accountid= ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_DEL_REALM_CHARACTERS_BY_REALM, "DELETE FROM account_char_per_realm WHERE accountid = ? AND realmid = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(LOGIN_DEL_REALM_CHARACTERS, "DELETE FROM account_char_per_realm WHERE accountid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_REALM_CHARACTERS, "INSERT INTO account_char_per_realm (charcount, accountid, realmid) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_SUM_REALM_CHARACTERS, "SELECT SUM(charcount) FROM account_char_per_realm WHERE accountid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT, "INSERT INTO account(username, sha_pass_hash, joindate) VALUES(?, ?, NOW())", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_REALM_CHARACTERS_INIT, "INSERT INTO account_char_per_realm (realmid, accountid, charcount) SELECT realmlist.id, account.id, 0 FROM realmlist, account LEFT JOIN account_char_per_realm ON accountid=account.id WHERE accountid IS NULL", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(LOGIN_INS_LOG, "INSERT INTO logs (time, realm, type, string) VALUES (?, ? , ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(LOGIN_UPD_UPTIME_PLAYERS, "UPDATE uptime SET uptime = ?, max_players = ? WHERE realm_id = ? AND start_time = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_OLD_LOGS, "DELETE FROM logs WHERE (time + ?) < ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_SECURITY, "DELETE FROM account_security WHERE id = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_SECURITY_BY_REALM, "DELETE FROM account_security WHERE id = ? AND (RealmID = ? OR RealmID = -1)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_SECURITY, "INSERT INTO account_security (id, PrivilegeLevel, RealmID, CheckoutCode) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
	PREPARE_STATEMENT(LOGIN_GET_ACCOUNT_ID_BY_USERNAME, "SELECT id FROM account WHERE username = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_GET_ACCOUNT_SECURITY_GM_LEVEL, "SELECT PrivilegeLevel FROM account_security WHERE id = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_GET_ACCOUNT_SECURITY_GM_LEVEL_BY_REALM_ID, "SELECT PrivilegeLevel FROM account_security WHERE id = ? AND (RealmID = ? OR RealmID = -1)", CONNECTION_SYNCH);
	...
	PREPARE_STATEMENT(LOGIN_SEL_PINFO, "SELECT a.username, aa.PrivilegeLevel, a.email, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), a.mutetime FROM account a LEFT JOIN account_security aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) WHERE a.id = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_GM_ACCOUNTS, "SELECT a.username, aa.PrivilegeLevel FROM account a, account_security aa WHERE a.id = aa.id AND aa.PrivilegeLevel >= ? AND (aa.realmid = -1 OR aa.realmid = ?)", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_INFO, "SELECT a.username, a.last_ip, aa.PrivilegeLevel, a.expansion FROM account a LEFT JOIN account_security aa ON (a.id = aa.id) WHERE a.id = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_SECURITY_GM_LEVEL_TEST, "SELECT 1 FROM account_security WHERE id = ? AND PrivilegeLevel > ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_SECURITY, "SELECT a.id, aa.PrivilegeLevel, aa.RealmID FROM account a LEFT JOIN account_security aa ON (a.id = aa.id) WHERE a.username = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_REALM_LIST_SECURITY_LEVEL, "SELECT allowedSecurityLevel from realmlist WHERE id = ?", CONNECTION_SYNCH);
	...
	PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_PREMIUM, "SELECT 1 FROM account_premium WHERE id = ? AND active = 1", CONNECTION_SYNCH);
}