...
void PreparedStatement::BindParameters()
{
    ...
    if (i < m_stmt->m_paramCount)
        sLog->OutSQLConnection("[WARNING]: BindParameters() for statement %u did not bind all allocated parameters", m_index);
    #endif
}
...
MySQLPreparedStatement::MySQLPreparedStatement(MYSQL_STMT* stmt) : m_Mstmt(stmt), m_bind(NULL)
{
...
bool MySQLPreparedStatement::CheckValidIndex(uint8 index)
{
    ...
    if (m_paramsSet[index])
        sLog->OutSQLConnection("[WARNING] Prepared Statement (id: %u) trying to bind value on already bound index (%u).", m_stmt->m_index, index);
    return true;
}
