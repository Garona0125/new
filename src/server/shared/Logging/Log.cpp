...
void Log::SetLogLevel(char *Level)
{
    ...
    OutConsole("LogLevel is %u", m_logLevel);
}
...
void Log::SetLogFileLevel(char *Level)
{
    ...
    OutConsole("LogFileLevel is %u", m_logFileLevel);
}
...
void Log::SetDBLogLevel(char *Level)
{
    ...
    OutConsole("DBLogLevel is %u", m_dbLogLevel);
}

void Log::Initialize()
{
    /// Check whether we'll log GM commands/RA events/character outputs/chat stuffs
    m_dbChar = GetConfigSettings::GetBoolState("LogDB.Char", false);
    m_dbRA = GetConfigSettings::GetBoolState("LogDB.RA", false);
    m_dbGM = GetConfigSettings::GetBoolState("LogDB.GM", false);
    m_dbChat = GetConfigSettings::GetBoolState("LogDB.Chat", false);

    /// Realm must be 0 by default
    SetRealmID(0);

    /// Common log files data
    m_logsDir = GetConfigSettings::GetStringDefault("LogsDir", "");
    if (!m_logsDir.empty())
    ...
    InitColors(GetConfigSettings::GetStringDefault("LogColors", ""));

    m_gmlog_per_account = GetConfigSettings::GetBoolState("GmLogPerAccount", false);
    if (!m_gmlog_per_account)
        gmLogfile = openLogFile("GMLogFile", "GmLogTimestamp", "a");
    else
    {
        // GM log settings for per account case
        m_gmlog_filename_format = GetConfigSettings::GetStringDefault("GMLogFile", "");
        if (!m_gmlog_filename_format.empty())
        {
            bool m_gmlog_timestamp = GetConfigSettings::GetBoolState("GmLogTimestamp", false);
    ...
    m_logLevel = GetConfigSettings::GetIntState("LogLevel", LOGL_NORMAL);
    m_logFileLevel = GetConfigSettings::GetIntState("LogFileLevel", LOGL_NORMAL);
    m_dbLogLevel = GetConfigSettings::GetIntState("DBLogLevel", LOGL_NORMAL);
    m_sqlDriverQueryLogging  = GetConfigSettings::GetBoolState("SQLDriverQueryLogging", false);

    m_DebugLogMask = DebugLogFilters(GetConfigSettings::GetIntState("DebugLogMask", LOG_FILTER_NONE));

    // Char log settings
    m_charLog_Dump = GetConfigSettings::GetBoolState("CharLogDump", false);
    m_charLog_Dump_Separate = GetConfigSettings::GetBoolState("CharLogDump.Separate", false);
    if (m_charLog_Dump_Separate)
    {
        m_dumpsDir = GetConfigSettings::GetStringDefault("CharLogDump.SeparateDir", "");
        if (!m_dumpsDir.empty())
		{
...
{
    std::string logfn=GetConfigSettings::GetStringDefault(configFileName, "");
    if (logfn.empty())
        return NULL;

    if (configTimeStampFlag && GetConfigSettings::GetBoolState(configTimeStampFlag, false))
    {
...
void Log::OutConsole(const char * str, ...)
{
    if (!str)
        return;
...
void Log::OutErrorConsole(const char * err, ...)
{
    if (!err)
        return;
...
void Log::OutSQLConnection(const char* str, ...)
{
    if (!str)
        return;
...
void Log::OutErrorDatabase(const char * err, ...)
{
    if (!err)
        return;
...
void Log::OutConsoleInLine(const char * str, ...)
{
    if (!str)
        return;
...
void Log::OutCharacters(const char * str, ...)
{
    if (!str)
        return;
...
void Log::OutCharactersDump(const char * str, uint32 account_id, uint32 guid, const char * name)
{
    FILE* file = NULL;
    if (m_charLog_Dump_Separate)
    {
...
void Log::outErrorST(const char * str, ...)
{
    ...
    ACE_Stack_Trace st;
    sLog->OutErrorConsole("%s [Stacktrace: %s]", nnew_str, st.c_str());
}
