...
void TCSoapRunnable::run()
{
    ...
    {
        sLog->OutErrorConsole("TCSoap: couldn't bind to %s:%d", m_host.c_str(), m_port);
        exit(-1);
    }

    sLog->OutConsole("TCSoap: bound to http://%s:%d", m_host.c_str(), m_port);
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    if (acc)
    {
        sLog->OutErrorConsole("TCSoap: Error while acquiring lock, acc = %i, errno = %u", acc, errno);
    }
