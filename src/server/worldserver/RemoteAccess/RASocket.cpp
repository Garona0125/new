...
RASocket::RASocket()
{
    _minLevel = GetConfigSettings::GetIntState("Remote.Access.MinLevel", 3);
}
...
int RASocket::open(void *)
{
    ...
    {
        sLog->OutErrorConsole("RASocket::open: peer().get_remote_addr error is %s", ACE_OS::strerror(errno));
        return -1;
    }
