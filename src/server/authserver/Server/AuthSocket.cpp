...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        {
            sLog->OutErrorConsole("[Auth] got unknown packet from '%s'", socket().getRemoteAddress().c_str());
            socket().shutdown();
            return;
        }
    }
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    else
    {
        ...
        stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_LOGON_CHALLENGE);
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
        ...
        PreparedStatement *stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_LOGON_PROOF);
    ...
    else
    {
        ...
        uint32 MaxWrongPassCount = GetConfigSettings::GetIntState("WrongPass.MaxCount", 0);
        if (MaxWrongPassCount > 0)
        {
            //Increment number of failed logins by one and if it reaches the limit temporarily ban that account or IP
            PreparedStatement *stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_FAILED_LOGINS);
            stmt->setString(0, _login);
            LoginDatabase.Execute(stmt);

            stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_FAILED_LOGINS);
            stmt->setString(0, _login);
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                {
                    uint32 WrongPassBanTime = GetConfigSettings::GetIntState("WrongPass.BanTime", 600);
                    bool WrongPassBanType = GetConfigSettings::GetBoolState("WrongPass.BanType", false);
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_SESSION_KEY);
    stmt->setString(0, _login);
    PreparedQueryResult result = LoginDatabase.Query(stmt);
    ...
    if (!result)
    {
        sLog->OutErrorConsole("'%s:%d' [ERROR] user %s tried to login and we cannot find his session key in the database.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
...
bool AuthSocket::_HandleReconnectProof()
{
    ...
    {
        sLog->OutErrorConsole("'%s:%d' [ERROR] user %s tried to login, but session is invalid.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
}
...
bool AuthSocket::_HandleRealmList()
{
    ...
    if (!result)
    {
        sLog->OutErrorConsole("'%s:%d' [ERROR] user %s tried to login but we cannot find him in the database.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
...
bool AuthSocket::_HandleXferResume()
{
    ...
    {
        sLog->OutErrorConsole("Error while resuming patch transfer (wrong packet)");
        return false;
    }
...
bool AuthSocket::_HandleXferAccept()
{
    ...
    if (!pPatch)
    {
        sLog->OutErrorConsole("Error while accepting patch transfer (wrong packet)");
        return false;
    }
...
void Patcher::LoadPatchMD5(char *szFileName)
{
    ...
    if (!pPatch)
    {
        sLog->OutErrorConsole("Error loading patch %s\n", path.c_str());
        return;
    }
