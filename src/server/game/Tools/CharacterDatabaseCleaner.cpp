...
void CharacterDatabaseCleaner::CleanDatabase()
{
    ...
    sLog->OutConsole("QUANTUM CORE SERVER: Cleaning character database...");
    ...
    sLog->OutConsole(">> Cleaned character database in %u ms", GetMSTimeDiffToNow(oldMSTime));
    sLog->OutConsole();
}
...
void CharacterDatabaseCleaner::CheckUnique(const char* column, const char* table, bool (*check)(uint32))
{
    ...
    if (!result)
    {
        sLog->OutConsole("Table %s is empty.", table);
        return;
    }
