...
DumpReturn PlayerDumpReader::LoadDump(const std::string& file, uint32 account, std::string name, uint32 guid)
{
    ...
    while (!feof(fin))
    {
        ...
        */
        ...
        if (tn.empty())
        {
            sLog->OutErrorConsole("LoadPlayerDump: Can't extract table name from line: '%s'!", line.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        {
            sLog->OutErrorConsole("LoadPlayerDump: Unknown table: '%s'!", tn.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        switch (type)
        {
            ...
            default:
                sLog->OutErrorConsole("Unknown dump table type: %u", type);
                break;
        }
