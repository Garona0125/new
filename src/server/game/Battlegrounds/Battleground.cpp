...
namespace Trinity
{
    class BattlegroundChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetQuantumSystemText(_textId, loc_idx);
                if (_args)
                {
    ...
    class Battleground2ChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetQuantumSystemText(_textId, loc_idx);
                char const* arg1str = _arg1 ? sObjectMgr->GetQuantumSystemText(_arg1, loc_idx) : "";
                char const* arg2str = _arg2 ? sObjectMgr->GetQuantumSystemText(_arg2, loc_idx) : "";
...
inline void Battleground::_ProcessOfflineQueue()
{
    ...
    if (!m_OfflineQueue.empty())
    {
        ...
        if (itr != m_Players.end())
        {
            ...
            {
				if (isBattleground() && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_TRACK_DESERTERS) && (GetStatus() == STATUS_IN_PROGRESS || GetStatus() == STATUS_WAIT_JOIN))
				{
					PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_DESERTER_TRACK);
					stmt->setUInt32(0, player->GetGUIDLow()); // need test
    ...
    else if (!sBattlegroundMgr->isTesting())
    {
        ...
        if (newtime > (MINUTE * IN_MILLISECONDS))
        {
            if (newtime / (MINUTE * IN_MILLISECONDS) != m_PrematureCountDownTimer / (MINUTE * IN_MILLISECONDS))
                PSendMessageToAll(LANG_BATTLEGROUND_FINISH_WARNING_MIN, CHAT_MSG_SYSTEM, NULL, (uint32)(m_PrematureCountDownTimer / (MINUTE * IN_MILLISECONDS)));
        }
        else
        {
            //announce every 15 seconds
            if (newtime / (15 * IN_MILLISECONDS) != m_PrematureCountDownTimer / (15 * IN_MILLISECONDS))
                PSendMessageToAll(LANG_BATTLEGROUND_FINISH_WARNING_SEC, CHAT_MSG_SYSTEM, NULL, (uint32)(m_PrematureCountDownTimer / IN_MILLISECONDS));
        }
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    if (!(m_Events & BG_STARTING_EVENT_1))
    {
        ...
        if (!FindBgMap())
        {
            sLog->OutErrorConsole("Battleground::_ProcessJoin: map (map id: %u, instance id: %u) is not created!", m_MapId, m_InstanceID);
            EndNow();
            return;
        }
		...
		else
        {
            ...
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
			{
                if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                {
					...
					if (player->HasAura(SPELL_FREEZE_CHEATER)) // For Battleground Anticheat
						player->RemoveAurasDueToSpell(SPELL_FREEZE_CHEATER);
                }
    ...
    if (m_ValidStartPositionTimer > 1*IN_MILLISECONDS)
    {
        m_ValidStartPositionTimer = 0;
        ...
        if (maxDist > 0.0f)
        {
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
            {
                if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                {
                    ...
                    if (dist >= maxDist) // Battleground Anticheat System by Crispi
                    {
						if (GetConfigSettings::GetBoolState("Battleground.Anticheat.Enable", true))
						{
							if (player->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_ON_TRANSPORT) && player->GetMapId() == 607)
								return;

							player->TeleportTo(GetMapId(), x, y, z, o);
							player->RemoveAurasDueToSpell(SPELL_PREPARATION);
							player->CastSpell(player, SPELL_FREEZE_CHEATER, true);
							player->MonsterTextEmote("%s Where are you going?", LANG_UNIVERSAL, player->GetGUID());
							player->PlayDirectSound(11179);
							// Sender Data
							sLog->OutConsole("BATTLEGROUND ANTICHEAT SYSTEM: Sending player %s back to start location (map: %u) (Opened The Gate Before The Start Battleground!)", player->GetName(), GetMapId());
							std::string sText = ("Player:|CFFFF33FF " + std::string(player->GetName()) + "|r Used Cheat On The Battleground!");
...
inline void Battleground::_ProcessLeave(uint32 diff)
{
    ...
    if (m_EndTime <= 0)
    {
        ...
        for (itr = m_Players.begin(); itr != m_Players.end(); itr = next)
        {
            next = itr;
            ++next;
            //itr is erased here!remove player
            RemovePlayerAtLeave(itr->first, true, true); //  from BG
...
inline Player* Battleground::_GetPlayer(uint64 guid, bool offlineRemove, const char* context) const
{
    ...
    if (!offlineRemove)
    {
        ...
        if (!player)
            sLog->OutErrorConsole("Battleground::%s: player (GUID: %u) not found for BG (map: %u, instance id: %u)!",
...
void Battleground::EndBattleground(uint32 winner)
{
	...
	{
		stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_PVP_STATS_MAX_ID);
		result = CharacterDatabase.Query(stmt);

		if (result)
		{
			Field* fields = result->Fetch();
			battlegroundId = fields[0].GetInt64() + 1;
		}

		stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_PVP_STATS_BATTLEGROUND);
	}
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (isArena() && isRated() && winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team)
        {
            if (team == winner)
            {
				...
				player->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA, GetMapId());
		...
		if (team == winner)
		{
			if (GetConfigSettings::GetBoolState("Simple.Battleground.Rewards.Enable", true))
				RewardItemId(player, GetConfigSettings::GetIntState("Battleground.Winner.Mark.Count", 0));
		}
		else
			if (winner)
				if (GetConfigSettings::GetBoolState("Simple.Battleground.Rewards.Enable", true))
					RewardItemId(player, GetConfigSettings::GetIntState("Battleground.Loser.Mark.Count", 0));
		...
		if (isBattleground() && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_STORE_STATISTICS_ENABLE))
        {
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_PVP_STATS_PLAYER);
        ...
        if (team == winner)
        {
            if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetTypeID()))
            {
				...
				if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetTypeID())) // Test Emblems
				{
					if (GetConfigSettings::GetBoolState("Battleground.Reward.System.Enable", true))
						player->AddItem(GetConfigSettings::GetIntState("Random.Battleground.Reward.Item.ID", 0), GetConfigSettings::GetIntState("Battleground.Winner.Mark.Count", 0));
				}
            }
        ...
        else
        {
			...
			{
				if (GetConfigSettings::GetBoolState("Battleground.Reward.System.Enable", true))
					player->AddItem(GetConfigSettings::GetIntState("Random.Battleground.Reward.Item.ID", 0), GetConfigSettings::GetIntState("Battleground.Loser.Mark.Count", 0));
			}
        }
...
void Battleground::RewardItemId(Player *player, uint32 count)
{
	if (GetConfigSettings::GetBoolState("Simple.Battleground.Rewards.Enable", true))
	{
		switch(GetTypeID())
		{
		    case BATTLEGROUND_AV:
				RewardItem(player, GetConfigSettings::GetIntState("Alterac.Valley.Reward.ID", 0), count);
				break;
			case BATTLEGROUND_WS:
				RewardItem(player, GetConfigSettings::GetIntState("Warsong.Gulch.Reward.ID", 0), count);
				break;
			case BATTLEGROUND_AB:
				RewardItem(player, GetConfigSettings::GetIntState("Arathi.Basin.Reward.ID", 0), count);
				break;
			case BATTLEGROUND_EY:
				RewardItem(player, GetConfigSettings::GetIntState("Eye.of.the.Storm.Reward.ID", 0), count);
				break;
			case BATTLEGROUND_SA:
				RewardItem(player, GetConfigSettings::GetIntState("Strand.of.the.Ancients.Reward.ID", 0), count);
				break;
			case BATTLEGROUND_IC:
				RewardItem(player, GetConfigSettings::GetIntState("Isle.of.Conquest.Reward.ID", 0), count);
				break;
			default:
				return;
		}
...
void Battleground::RewardItem(Player *player, uint32 item_id, uint32 count)
{
	...
	uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, item_id, count, &no_space_count);

	if (msg == EQUIP_ERR_ITEM_NOT_FOUND)
	{
		sLog->OutErrorDatabase("Battleground reward item (Entry %u) not exist in `item_template`.", item_id);
		return;
	}
	...
	if (count == 0 || !dest.empty())
		if (Item* item = player->StoreNewItem(dest, item_id, true, 0))
...
void Battleground::Reset()
{
    ...
    if (m_InvitedAlliance > 0 || m_InvitedHorde > 0)
        sLog->OutErrorConsole("Battleground::Reset: one of the counters is not 0 (alliance: %u, horde: %u) for BG (map: %u, instance id: %u)!",
...
void Battleground::UpdatePlayerScore(Player* Source, uint32 type, uint32 value, bool doAddHonor)
{
    ...
    switch (type)
    {
        ...
        case SCORE_HEALING_DONE:                            // Healing Done
            ...
            break;
        default:
            sLog->OutErrorConsole("Battleground::UpdatePlayerScore: unknown score type (%u) for BG (map: %u, instance id: %u)!",
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/)
{
    ...
    {
        sLog->OutErrorDatabase("Battleground::AddObject: cannot create gameobject (entry: %u) for BG (map: %u, instance id: %u)!",
                entry, m_MapId, m_InstanceID);
        sLog->OutErrorConsole("Battleground::AddObject: cannot create gameobject (entry: %u) for BG (map: %u, instance id: %u)!",
                entry, m_MapId, m_InstanceID);
        delete go;
        return false;
    }
...
void Battleground::DoorClose(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        sLog->OutErrorConsole("bg.battleground", "Battleground::DoorClose: door gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
...
void Battleground::DoorOpen(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        sLog->OutErrorConsole("bg.battleground", "Battleground::DoorOpen: door gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
            type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
}
...
GameObject* Battleground::GetBGObject(uint32 type, bool logError)
{
    ...
    {
        if (logError)
            sLog->OutErrorConsole("Battleground::GetBGObject: gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
        else
            sLog->OutConsoleInLine("Battleground::GetBGObject: gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
    }
    return obj;
}
...
Creature* Battleground::GetBGCreature(uint32 type, bool logError)
{
    ...
    {
        if (logError)
            sLog->OutErrorConsole("Battleground::GetBGCreature: creature (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, GUID_LOPART(BgCreatures[type]), m_MapId, m_InstanceID);
        //else
            //sLog->OutConsoleInLine("Battleground::GetBGCreature: creature (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!", type, GUID_LOPART(BgCreatures[type]), m_MapId, m_InstanceID);
    }
    return creature;
}
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, uint32 teamval, float x, float y, float z, float o, uint32 respawntime)
{
    ...
    {
        sLog->OutErrorConsole("Battleground::AddCreature: cannot create creature (entry: %u) for BG (map: %u, instance id: %u)!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return NULL;
    }
    ...
    if (!cinfo)
    {
        sLog->OutErrorConsole("Battleground::AddCreature: creature template (entry: %u) does not exist for BG (map: %u, instance id: %u)!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return NULL;
    }
...
bool Battleground::DelCreature(uint32 type)
{
    ...
    sLog->OutErrorConsole("Battleground::DelCreature: creature (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
...
bool Battleground::DelObject(uint32 type)
{
    ...
    sLog->OutErrorConsole("Battleground::DelObject: gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
        type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
    BgObjects[type] = 0;
    return false;
}
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, uint32 team)
{
    ...
    sLog->OutErrorConsole("Battleground::AddSpiritGuide: cannot create spirit guide (type: %u, entry: %u) for BG (map: %u, instance id: %u)!",
...
void Battleground::SendWarningToAll(int32 entry, ...)
{
    ...
    const char* format = sObjectMgr->GetQuantumSystemTextForDBCLocale(entry);
...
// To be removed
const char* Battleground::GetQuantumSystemText(int32 entry)
{
    // FIXME: now we have different DBC locales and need localized message for each target client
    return sObjectMgr->GetQuantumSystemTextForDBCLocale(entry);
}
...
void Battleground::HandleTriggerBuff(uint64 go_guid)
{
    ...
    if (index < 0)
    {
        sLog->OutErrorConsole("Battleground::HandleTriggerBuff: cannot find buff gameobject (GUID: %u, entry: %u, type: %u) in internal data for BG (map: %u, instance id: %u)!",
...
int32 Battleground::GetObjectType(uint64 guid)
{
    for (uint32 i = 0; i < BgObjects.size(); ++i)
        if (BgObjects[i] == guid)
            return i;
    sLog->OutErrorConsole("Battleground::GetObjectType: player used gameobject (GUID: %u) which is not in internal data for BG (map: %u, instance id: %u), cheating?",
...
{
	sLog->OutErrorConsole("Unhandled AreaTrigger %u in Battleground %u. Player coords (x: %f, y: %f, z: %f)",
