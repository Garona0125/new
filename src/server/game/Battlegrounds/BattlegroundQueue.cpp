...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    {
        sLog->OutErrorConsole("BattlegroundQueue: couldn't find player to remove GUID: %u", GUID_LOPART(guid));
        return;
    }
    ...
    if (bracket_id == -1)
    {
        sLog->OutErrorConsole("BattlegroundQueue: ERROR Cannot find groupinfo for player GUID: %u", GUID_LOPART(guid));
        return;
    }
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    if (!bg_template)
    {
        sLog->OutErrorConsole("Battleground: Update: bg template not found for %u", bgTypeId);
        return;
    }
    ...
    if (!bracketEntry)
    {
        sLog->OutErrorConsole("Battleground: Update: bg bracket entry not found for map %u bracket id %u", bg_template->GetMapId(), bracket_id);
        return;
    }
    ...
    if (bg_template->isBattleground())
    {
        ...
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            if (!bg2)
            {
                sLog->OutErrorConsole("BattlegroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
            }
    ...
    else if (bg_template->isArena())
    {
        ...
        if (found == 2)
        {
            ...
            if (!arena)
            {
                sLog->OutErrorConsole("BattlegroundQueue::Update couldn't create arena instance for rated arena match!");
                return;
            }
