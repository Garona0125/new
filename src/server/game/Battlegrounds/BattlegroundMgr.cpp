    ...
    switch (StatusID)
    {
        ...
        case STATUS_IN_PROGRESS:                            // status_in_progress
            ...
            *data << uint8(uiFrame);
            break;
        default:
            sLog->OutErrorConsole("Unknown Battleground status!");
            break;
    }
}
...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket* data, Battleground* bg)
{
    ...
    *data << uint32(scoreCount);                            // placeholder
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        {
            sLog->OutErrorConsole("Player " UI64FMTD " has scoreboard entry for battleground %u but is not in battleground!", itr->first, bg->GetTypeID(true));
            continue;
        }
        ...
        *data << uint32(itr2->second->HealingDone);             // healing done
        ...
        {
            sLog->OutErrorConsole("Battleground %u scoreboard has more entries (%u) than allowed players in this bg (%u)", bg->GetTypeID(true), bg->GetPlayerScoresSize(), bg->GetMaxPlayers());
            break;
        }
    }
...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId bgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    ...
    if (!bg_template)
    {
        sLog->OutErrorConsole("Battleground: CreateNewBattleground - bg template not found for %u", bgTypeId);
        return NULL;
    }
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    {
		sLog->OutConsole("=====================================================================");
        sLog->OutConsole(">> Loaded 0 battlegrounds. DB table `battleground_template` is empty!");
		sLog->OutConsole("=====================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!bl)
        {
            sLog->OutErrorConsole("Battleground ID %u not found in BattlemasterList.dbc. Battleground not created.", bgTypeID_);
            continue;
        }
        ...
        {
			sLog->OutErrorDatabase("Table `battleground_template` for id %u has bad values for MinPlayersPerTeam (%u) and MaxPlayersPerTeam(%u)",
        ...
        {
			sLog->OutErrorDatabase("Table `battleground_template` for id %u has bad values for LevelMin (%u) and LevelMax(%u)",
        ...
        {
            sLog->OutErrorDatabase("Table `battleground_template` for id %u have non-existed WorldSafeLocs.dbc id %u in field `AllianceStartLoc`. BG not created.", data.bgTypeId, startId);
            continue;
        }
        ...
        {
            sLog->OutErrorDatabase("Table `battleground_template` for id %u have non-existed WorldSafeLocs.dbc id %u in field `HordeStartLoc`. BG not created.", data.bgTypeId, startId);
            continue;
        }
	...
	sLog->OutConsole("=====================================");
    sLog->OutConsole(">> Loaded %u battlegrounds in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=====================================");
    sLog->OutConsole();
}
...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket* data, uint64 guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
    ...
    winner_kills = Trinity::Honor::hk_honor_at_level(player->GetCurrentLevel(), float(winner_kills));
    loser_kills = Trinity::Honor::hk_honor_at_level(player->GetCurrentLevel(), float(loser_kills));
    ...
    else                                                    // battleground
    {
        ...
        *data << uint32(0);                                 // number of bg instances
        ...
        if (Battleground* bgTemplate = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId))
        {
            ...
            if (PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(bgTemplate->GetMapId(), player->GetCurrentLevel()))
            {
                uint32 count = 0;
...
void BattlegroundMgr::SendToBattleground(Player* player, uint32 instanceId, BattlegroundTypeId bgTypeId)
{
    ...
    {
        sLog->OutErrorConsole("player %u is trying to port to non-existent bg instance %u", player->GetGUIDLow(), instanceId);
    }
}
...
void BattlegroundMgr::LoadBattleMastersEntry()
{
    ...
    {
		sLog->OutConsole("=========================================================================");
        sLog->OutConsole(">> Loaded 0 battlemaster entries. DB table `battlemaster_entry` is empty!");
		sLog->OutConsole("=========================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
		...
		{
			if ((cInfo->npcflag & UNIT_NPC_FLAG_BATTLE_MASTER) == 0)
				sLog->OutErrorDatabase("Creature (Entry: %u) listed in `battlemaster_entry` is not a battlemaster.", entry);
		}
		else
		{
			sLog->OutErrorDatabase("Creature (Entry: %u) listed in `battlemaster_entry` does not exist.", entry);
			continue;
		}
        uint32 bgTypeId = fields[1].GetUInt32();
        if (!sBattlemasterListStore.LookupEntry(bgTypeId))
        {
            sLog->OutErrorDatabase("Table `battlemaster_entry` contain entry %u for not existed battleground type %u, ignored.", entry, bgTypeId);
            continue;
        }
	...
	sLog->OutConsole("============================================");
    sLog->OutConsole(">> Loaded %u battlemaster entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("============================================");
    sLog->OutConsole();
}
...
void BattlegroundMgr::CheckBattleMasters()
{
	...
	{
		if ((itr->second.npcflag & UNIT_NPC_FLAG_BATTLE_MASTER) && mBattleMastersMap.find(itr->second.Entry) == mBattleMastersMap.end())
		{
			sLog->OutErrorDatabase("CreatureTemplate (Entry: %u) has UNIT_NPC_FLAG_BATTLEMASTER but no data in `battlemaster_entry` table. Removing flag!", itr->second.Entry);
			const_cast<CreatureTemplate*>(&itr->second)->npcflag &= ~UNIT_NPC_FLAG_BATTLE_MASTER;
		}
	}
}
