...
bool ArenaTeam::AddMember(uint64 playerGuid)
{
    ...
    if (player)
    {
        playerClass = player->GetCurrentClass();
    ...
    {
        sLog->OutErrorConsole("Arena: Player %s (guid: %u) already has an arena team of type %u", playerName.c_str(), GUID_LOPART(playerGuid), GetType());
        return false;
    }
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result)
{
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("ArenaTeam %u has member with empty name - probably player %u doesn't exist, deleting him from memberlist!", arenaTeamId, GUID_LOPART(newMember.Guid));
...
void ArenaTeam::Roster(WorldSession* session)
{
    ...
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        ...
        data << uint8((player ? player->GetCurrentLevel() : 0));  // unknown, level?
...
void ArenaTeam::BroadcastEvent(ArenaTeamEvents event, uint64 guid, uint8 strCount, std::string str1, std::string str2, std::string str3)
{
    ...
    switch (strCount)
    {
        ...
        case 3:
            ...
            break;
        default:
            sLog->OutErrorConsole("Unhandled strCount %u in ArenaTeam::BroadcastEvent", strCount);
            return;
    }
...
uint8 ArenaTeam::GetSlotByType(uint32 type)
{
    ...
    sLog->OutErrorConsole("FATAL: Unknown arena team type %u for some arena team", type);
    return 0xFF;
}
...
uint32 ArenaTeam::GetTypeBySlot(uint8 slot)
{
    ...
    sLog->OutErrorConsole("FATAL: Unknown arena team slot %u for some arena team", slot);
    return 0xFFFFFFFF;
}
