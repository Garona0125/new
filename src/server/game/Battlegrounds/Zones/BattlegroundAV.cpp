...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN][0])
    {
        if (!m_CaptainAlive[0])
        {
            sLog->OutErrorConsole("Killed a Captain twice, please report this bug, if you haven't done \".respawn\"");
            return;
        }
        ...
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD, false);
        if (creature)
            YellToAll(creature, GetQuantumSystemText(LANG_BG_AV_A_CAPTAIN_DEAD), LANG_UNIVERSAL);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN][0])
    {
        ...
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD, false);
        if (creature)
            YellToAll(creature, GetQuantumSystemText(LANG_BG_AV_H_CAPTAIN_DEAD), LANG_UNIVERSAL);
...
Creature* BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type)
{
    ...
    if (triggerSpawnID && newFaction)
    {
        ...
        {
            trigger->SetCurrentFaction(newFaction);
        ...
        for (uint8 i=0; i <= 1; i++)//0=alliance, 1=horde
        {
            ...
            else
            {
                if (i == 0)
                {
                    ...
                    Creature* creature = GetBGCreature(AV_CPLACE_MAX + 61, false);
                    if (creature)
                ...
                else
                {
                    ...
                    Creature* creature = GetBGCreature(AV_CPLACE_MAX + 59, false); //TODO: make the captains a dynamic creature
                    if (creature)
...
void BattlegroundAV::AddPlayer(Player* player)
{
    ...
    if (m_MaxLevel == 0)
        m_MaxLevel=(player->GetCurrentLevel()%10 == 0)? player->GetCurrentLevel() : (player->GetCurrentLevel()-(player->GetCurrentLevel()%10))+10; //TODO: just look at the code \^_^/ --but queue-info should provide this information..
...
void BattlegroundAV::RemovePlayer(Player* player, uint64 /*guid*/, uint32 /*team*/)
{
   if (!player)
    {
        sLog->OutErrorConsole("bg_AV no player at remove");
        return;
    }
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        ...
        if (BgCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
            ...
            sLog->OutErrorConsole("BG_AV: playerdestroyedpoint: marshal %i doesn't exist", AV_CPLACE_A_MARSHAL_SOUTH + tmp);
        //spawn destroyed aura
        for (uint8 i=0; i <= 9; i++)
    ...
    char buf[256];
    if (IsTower(node))
        sprintf(buf, GetQuantumSystemText(LANG_BG_AV_TOWER_TAKEN), GetNodeName(node), (owner == ALLIANCE) ? GetQuantumSystemText(LANG_BG_AV_ALLY) : GetQuantumSystemText(LANG_BG_AV_HORDE));
    else
        sprintf(buf, GetQuantumSystemText(LANG_BG_AV_GRAVE_TAKEN), GetNodeName(node), (owner == ALLIANCE) ? GetQuantumSystemText(LANG_BG_AV_ALLY) :GetQuantumSystemText(LANG_BG_AV_HORDE));
...
//    }
    if (team == ALLIANCE || team == HORDE)
    {
        ...
        char buf[256];
        sprintf(buf, GetQuantumSystemText(LANG_BG_AV_MINE_TAKEN), GetQuantumSystemText((mine == AV_NORTH_MINE) ? LANG_BG_AV_MINE_NORTH : LANG_BG_AV_MINE_SOUTH), (team == ALLIANCE) ?  GetQuantumSystemText(LANG_BG_AV_ALLY) : GetQuantumSystemText(LANG_BG_AV_HORDE));
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD, false);
        if (creature)
            YellToAll(creature, buf, LANG_UNIVERSAL);
    }
    else
    {
        ...
        {
            if (Creature* creature = GetBGCreature(AV_CPLACE_MINE_S_3, false))
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    if (m_Team_QuestStatus[team2][0] < 500)
        ...
        if (!AddSpiritGuide(node, BG_AV_CreaturePos[node][0], BG_AV_CreaturePos[node][1], BG_AV_CreaturePos[node][2], BG_AV_CreaturePos[node][3], owner))
            sLog->OutErrorConsole("AV: couldn't spawn spiritguide at node %i", node);
    ...
    if (trigger)
    {
        ...
        trigger->SetCurrentFaction(owner == ALLIANCE ? 84 : 83);
        trigger->CastSpell(trigger, SPELL_HONORABLE_DEFENDER_25Y, false);
    }
}
...
BG_AV_Nodes BattlegroundAV::GetNodeThroughObject(uint32 object)
{
    ...
    sLog->OutErrorConsole("BattlegroundAV: ERROR! GetPlace got a wrong object :(");
    ASSERT(false);
...
{ //this function is the counterpart to GetNodeThroughObject()
    ...
    sLog->OutErrorConsole("BattlegroundAV: Error! GetPlaceNode couldn't resolve node %i", node);
    ASSERT(false);
    return 0;
}
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    {
        sLog->OutErrorConsole("BG_AV: player defends point which doesn't belong to his team %i", node);
        return;
    }
   ...
   //spawn new go :)
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, GetQuantumSystemText((IsTower(node)) ? LANG_BG_AV_TOWER_DEFENDED : LANG_BG_AV_GRAVE_DEFENDED), GetNodeName(node), (team == ALLIANCE) ?  GetQuantumSystemText(LANG_BG_AV_ALLY) : GetQuantumSystemText(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD, false);
    if (creature)
        YellToAll(creature, buf, LANG_UNIVERSAL);
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, (IsTower(node)) ? GetQuantumSystemText(LANG_BG_AV_TOWER_ASSAULTED) : GetQuantumSystemText(LANG_BG_AV_GRAVE_ASSAULTED), GetNodeName(node),  (team == ALLIANCE) ?  GetQuantumSystemText(LANG_BG_AV_ALLY) : GetQuantumSystemText(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD, false);
    if (creature)
        YellToAll(creature, buf, LANG_UNIVERSAL);
...
uint8 BattlegroundAV::GetWorldStateType(uint8 state, uint16 team) //this is used for node worldstates and returns values which fit good into the worldstatesarray
{
    ...
    sLog->OutErrorConsole("BG_AV: should update a strange worldstate state:%i team:%i", state, team);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    {
        sLog->OutErrorDatabase("BatteGroundAV: Failed to spawn some object Battleground not created!1");
        return false;
    }
    ...
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION ; i < BG_AV_NODES_MAX; ++i)
    {
        if (i <= BG_AV_NODES_FROSTWOLF_HUT)
        {
            ...
            {
                sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!2");
                return false;
            }
        }
        else //towers
        {
            if (i <= BG_AV_NODES_STONEHEART_BUNKER) //alliance towers
            {
                ...
                {
                    sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!3");
                    return false;
                }
            }
            else //horde towers
            {
                ...
                {
                    sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!4");
                    return false;
                }
            }
            for (uint8 j=0; j <= 9; j++) //burning aura
            {
                ...
                {
                    sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!5.%i", i);
                    return false;
                }
            }
        }
    ...
    for (uint8 i=0; i<2; i++) //burning aura for buildings
    {
        for (uint8 j=0; j <= 9; j++)
        {
            if (j<5)
            {
                ...
                {
                    sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!6.%i", i);
                    return false;
                }
            }
            else
            {
                ...
                {
                    sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!7.%i", i);
                    return false;
                }
            }
        }
    ...
    for (uint16 i= 0; i <= (BG_AV_OBJECT_MINE_SUPPLY_N_MAX-BG_AV_OBJECT_MINE_SUPPLY_N_MIN); i++)
    {
        ...
        {
            sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.5.%i", i);
            return false;
        }
    }
    for (uint16 i= 0 ; i <= (BG_AV_OBJECT_MINE_SUPPLY_S_MAX-BG_AV_OBJECT_MINE_SUPPLY_S_MIN); i++)
    {
        ...
        {
            sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.6.%i", i);
            return false;
        }
    }
    ...
    {
        sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!8");
        return false;
    }
    ...
    {
        sLog->OutErrorConsole("BatteGroundAV: Failed to spawn Horde Banner not created!8.1");
        return false;
    }
    ...
    {
        sLog->OutErrorConsole("BatteGroundAV: Failed to spawn Alliance Banner not created!8.2");
        return false;
    }
    for (uint8 i = 0; i < 4; i++)
    {
        ...
        {
            sLog->OutErrorConsole("BatteGroundAV: Failed to spawn some object Battleground not created!9.%i", i);
            return false;
        }
    }
...
const char* BattlegroundAV::GetNodeName(BG_AV_Nodes node)
{
    switch (node)
    {
        case BG_AV_NODES_FIRSTAID_STATION:  return GetQuantumSystemText(LANG_BG_AV_NODE_GRAVE_STORM_AID);
        case BG_AV_NODES_DUNBALDAR_SOUTH:   return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_DUN_S);
        case BG_AV_NODES_DUNBALDAR_NORTH:   return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_DUN_N);
        case BG_AV_NODES_STORMPIKE_GRAVE:   return GetQuantumSystemText(LANG_BG_AV_NODE_GRAVE_STORMPIKE);
        case BG_AV_NODES_ICEWING_BUNKER:    return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_ICEWING);
        case BG_AV_NODES_STONEHEART_GRAVE:  return GetQuantumSystemText(LANG_BG_AV_NODE_GRAVE_STONE);
        case BG_AV_NODES_STONEHEART_BUNKER: return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_STONE);
        case BG_AV_NODES_SNOWFALL_GRAVE:    return GetQuantumSystemText(LANG_BG_AV_NODE_GRAVE_SNOW);
        case BG_AV_NODES_ICEBLOOD_TOWER:    return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_ICE);
        case BG_AV_NODES_ICEBLOOD_GRAVE:    return GetQuantumSystemText(LANG_BG_AV_NODE_GRAVE_ICE);
        case BG_AV_NODES_TOWER_POINT:       return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_POINT);
        case BG_AV_NODES_FROSTWOLF_GRAVE:   return GetQuantumSystemText(LANG_BG_AV_NODE_GRAVE_FROST);
        case BG_AV_NODES_FROSTWOLF_ETOWER:  return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_FROST_E);
        case BG_AV_NODES_FROSTWOLF_WTOWER:  return GetQuantumSystemText(LANG_BG_AV_NODE_TOWER_FROST_W);
        case BG_AV_NODES_FROSTWOLF_HUT:     return GetQuantumSystemText(LANG_BG_AV_NODE_GRAVE_FROST_HUT);
        default:
            sLog->OutErrorConsole("tried to get name for node %u", node);
            break;
    }
