...
bool BattlegroundSA::ResetObjs()
{
	...
	for (uint8 i = 0; i < BG_SA_MAX_GY; i++)
    {
        ...
        if (!sg)
        {
            sLog->OutErrorConsole("SOTA: Can't find GY entry %u", BG_SA_GYEntries[i]);
            return false;
        }
        ...
        else
        {
            ...
            if (!AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z, BG_SA_GYOrientation[i], ((Attackers == TEAM_HORDE)? ALLIANCE : HORDE)))
                sLog->OutErrorConsole("SOTA: couldn't spawn GY: %u", i);
        }
    }
...
void BattlegroundSA::EventPlayerDamagedGO(Player* /*player*/, GameObject* go, uint32 eventType)
{
    ...
    {
		uint32 GateId = getGateIdFromEventId(eventType);
...
void BattlegroundSA::OverrideGunFaction()
{
    ...
    for (uint8 i = BG_SA_GUN_1; i <= BG_SA_GUN_10; i++)
    {
        if (Creature* gun = GetBGCreature(i))
            gun->SetCurrentFaction(BG_SA_Factions[Attackers ? TEAM_ALLIANCE : TEAM_HORDE]);
    }

    for (uint8 i = BG_SA_DEMOLISHER_1; i <= BG_SA_DEMOLISHER_4; i++)
    {
        if (Creature* dem = GetBGCreature(i))
            dem->SetCurrentFaction(BG_SA_Factions[Attackers]);
    }
}
...
{
	uint32 id = getGateIdFromEventId(go->GetGOInfo()->building.destroyedEvent);
    if (!GateStatus[id])
        return;

    if (GameObject* go = GetBGObject(id))
    {
        if (go->GetGOValue()->Building.Health == 0)
        {
            GateStatus[id] = BG_SA_GATE_DESTROYED;
            uint32 uws = getWorldStateFromGateId(id);
            if (uws)
                UpdateWorldState(uws, GateStatus[id]);
...
void BattlegroundSA::CaptureGraveyard(BG_SA_Graveyards i, Player* Source)
{
	...
	if (!sg)
	{
		sLog->OutErrorConsole("BattlegroundSA::CaptureGraveyard: non-existant GY entry: %u", BG_SA_GYEntries[i]);
		return;
	}
    ...
    switch (i)
    {
        case BG_SA_LEFT_CAPTURABLE_GY:
            ...
            if (Source->GetTeamId() == TEAM_ALLIANCE)
			{
				...
				PlaySoundToAll(SOUND_GRAVEYARD_TAKEN_ALLIANCE);
			}
            else
			{
                SendWarningToAll(LANG_BG_SA_H_GY_WEST);
				PlaySoundToAll(SOUND_GRAVEYARD_TAKEN_HORDE);
			}
            break;
		...
		(GateStatus[BG_SA_GREEN_GATE] == BG_SA_GATE_DESTROYED || GateStatus[BG_SA_BLUE_GATE] == BG_SA_GATE_DESTROYED))
    {
        if (Source->GetTeamId() == Attackers)
        {
            ...
            if (Status == BG_SA_ROUND_ONE)
            {
                ...
                UpdateWaitTimer = 5*IN_MILLISECONDS;
                SignaledRoundTwo = false;
                SignaledRoundTwoHalfMin = false;
                InitSecondRound = true;
                ResetObjs();
...
void BattlegroundSA::EndBattleground(uint32 winner)
{
    ...
    if (winner == ALLIANCE)
	{
		PlaySoundToAll(SOUND_VICTORY_ALLIANCE);
        RewardHonorToTeam(GetBonusHonorFromKill(1), ALLIANCE);
	}
	else if (winner == HORDE)
	{
		PlaySoundToAll(SOUND_VICTORY_HORDE);
