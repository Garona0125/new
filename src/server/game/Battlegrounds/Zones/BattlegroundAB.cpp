...
void BattlegroundAB::_NodeOccupied(uint8 node, Team team)
{
    if (!AddSpiritGuide(node, BG_AB_SpiritGuidePos[node][0], BG_AB_SpiritGuidePos[node][1], BG_AB_SpiritGuidePos[node][2], BG_AB_SpiritGuidePos[node][3], team))
        sLog->OutErrorConsole("Failed to spawn spirit guide! point: %u, team: %u, ", node, team);
    ...
    if (node >= BG_AB_DYNAMIC_NODES_COUNT)//only dynamic nodes, no start points
        return;
    Creature* trigger = BgCreatures[node+7] ? GetBGCreature(node+7, false) : NULL;//0-6 spirit guides
    if (!trigger)
...
bool BattlegroundAB::SetupBattleground()
{
    for (int i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
    {
        ...
        {
            sLog->OutErrorDatabase("BatteGroundAB: Failed to spawn some object Battleground not created!");
            return false;
        }
    }
    ...
    {
        sLog->OutErrorDatabase("BatteGroundAB: Failed to spawn door object Battleground not created!");
        return false;
    }
    //buffs
    for (int i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i)
    {
        if (!AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i, Buff_Entries[0], BG_AB_BuffPositions[i][0], BG_AB_BuffPositions[i][1], BG_AB_BuffPositions[i][2], BG_AB_BuffPositions[i][3], 0, 0, sin(BG_AB_BuffPositions[i][3]/2), cos(BG_AB_BuffPositions[i][3]/2), RESPAWN_ONE_DAY)
            ...
            sLog->OutErrorDatabase("BatteGroundAB: Failed to spawn buff object!");
    }
    return true;
}
