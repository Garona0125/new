...
void BattlegroundWS::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (_bothFlagsKept)
        {
            ...
            if (_flagDebuffState == 0 && _flagSpellForceTimer >= 10*MINUTE*IN_MILLISECONDS)
            {
            ...
            else if (_flagDebuffState == 1 && _flagSpellForceTimer >= 15*MINUTE*IN_MILLISECONDS)
            {
                if (Player* player = ObjectAccessor::FindPlayer(m_FlagKeepers[0]))
                {
...
void BattlegroundWS::RespawnFlagAfterDrop(uint32 team)
{
    ...
    if (GameObject* obj = GetBgMap()->GetGameObject(GetDroppedFlagGUID(team)))
        ...
        sLog->OutErrorConsole("unknown droped flag bg, guid: %u", GUID_LOPART(GetDroppedFlagGUID(team)));
...
void BattlegroundWS::RemovePlayer(Player* player, uint64 guid, uint32 /*team*/)
{
    ...
    if (IsAllianceFlagPickedup() && m_FlagKeepers[BG_TEAM_ALLIANCE] == guid)
    {
        if (!player)
        {
            sLog->OutErrorConsole("BattlegroundWS: Removing offline player who has the FLAG!!");
...
bool BattlegroundWS::SetupBattleground()
{
    ...
    {
        sLog->OutErrorDatabase("BatteGroundWS: Failed to spawn some object Battleground not created!");
        return false;
    }
    ...
    {
        sLog->OutErrorDatabase("BatteGroundWS: Failed to spawn Alliance spirit guide! Battleground not created!");
        return false;
    }
    ...
    {
        sLog->OutErrorDatabase("BatteGroundWS: Failed to spawn Horde spirit guide! Battleground not created!");
        return false;
    }
