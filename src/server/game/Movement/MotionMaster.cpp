...
{
    if (_owner->GetTypeId() == TYPE_ID_UNIT)
    {
...
void MotionMaster::MoveTargetedHome()
{
    ...
    if (_owner->GetTypeId()==TYPE_ID_UNIT && !((Creature*)_owner)->GetCharmerOrOwnerGUID())
    {
        sLog->outStaticDebug("Creature (Entry: %u GUID: %u) targeted home", _owner->GetEntry(), _owner->GetGUIDLow());
        Mutate(new HomeMovementGenerator<Creature>(), MOTION_SLOT_ACTIVE);
    }
    else if (_owner->GetTypeId()==TYPE_ID_UNIT && ((Creature*)_owner)->GetCharmerOrOwnerGUID())
    {
        ...
        if (target)
        {
            sLog->outStaticDebug("Following %s (GUID: %u)", target->GetTypeId() == TYPE_ID_PLAYER ? "player" : "creature", target->GetTypeId() == TYPE_ID_PLAYER ? target->GetGUIDLow() : ((Creature*)target)->GetDBTableGUIDLow());
            Mutate(new FollowMovementGenerator<Creature>(*target,PET_FOLLOW_DIST,PET_FOLLOW_ANGLE), MOTION_SLOT_ACTIVE);
        }
    }
    else
    {
        sLog->OutErrorConsole("Player (GUID: %u) attempt targeted home", _owner->GetGUIDLow());
    }
}

void MotionMaster::MoveConfused()
{
    if (_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
...
void MotionMaster::MoveChase(Unit* target, float dist, float angle)
{
    ...
    if (_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
        sLog->outStaticDebug("Player (GUID: %u) chase to %s (GUID: %u)",
            _owner->GetGUIDLow(),
            target->GetTypeId() == TYPE_ID_PLAYER ? "player" : "creature",
            target->GetTypeId() == TYPE_ID_PLAYER ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveSeekAssistance(float x, float y, float z)
{
    if (_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
        sLog->OutErrorConsole("Player (GUID: %u) attempt to seek assistance", _owner->GetGUIDLow());
    }
    else
    {
...
void MotionMaster::MoveSeekAssistanceDistract(uint32 time)
{
    if (_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
        sLog->OutErrorConsole("Player (GUID: %u) attempt to call distract after assistance", _owner->GetGUIDLow());
    }
    else
    {
...
void MotionMaster::MoveFleeing(Unit* enemy, uint32 time)
{
    ...
    if (_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
        sLog->outStaticDebug("Player (GUID: %u) flee from %s (GUID: %u)", _owner->GetGUIDLow(),
            enemy->GetTypeId() == TYPE_ID_PLAYER ? "player" : "creature",
            enemy->GetTypeId() == TYPE_ID_PLAYER ? enemy->GetGUIDLow() : enemy->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveTaxiFlight(uint32 path, uint32 pathnode)
{
    if (_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
        ...
        {
            sLog->OutErrorConsole("%s attempt taxi to (not existed Path %u node %u)",
            _owner->GetName(), path, pathnode);
        }
    }
    else
    {
        sLog->OutErrorConsole("Creature (Entry: %u GUID: %u) attempt taxi to (Path %u node %u)",
...
{
    sLog->outCrash("Unit (Entry %u) is trying to delete its updating Movement Generator (Type %u)!", _owner->GetEntry(), curr->GetMovementGeneratorType());
    if (isStatic(curr))
        return;
    if (!_expList)
