...
void MoveSpline::init_spline(const MoveSplineInitArgs& args)
{
    ...
    if (spline.length() < minimal_duration) // sLog->OutErrorConsole Replace to sLog->OutConsole
    {
		//sLog->OutConsole("MoveSpline::init_spline: zero length spline, wrong input data?");
...
MoveSpline::MoveSpline() : m_Id(0), time_passed(0), vertical_acceleration(0.f), initialOrientation(0.f), effect_start_time(0), point_Idx(0), point_Idx_offset(0)
{
...
bool MoveSplineInitArgs::Validate(Unit* unit) const // sLog->OutErrorDatabase Replace to sLog->OutConsole
{
#define CHECK(exp) \
    if (!(exp))\
    {\
		sLog->OutConsole("MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->GetTypeId() == TYPE_ID_PLAYER ? unit->GetGUIDLow() : unit->ToCreature()->GetDBTableGUIDLow(), unit->GetEntry());\
        return false;\
    }
    CHECK(path.size() > 1);
    CHECK(velocity > 0.1f);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            {
                sLog->OutErrorConsole("MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
