...
struct SkillPerfectItemEntry
{
    ...
    SkillPerfectItemEntry(uint32 rS, float pCC, uint32 pIT): requiredSpecialization(rS), perfectCreateChance(pCC), perfectItemType(pIT) {}
};
...
void LoadSkillPerfectItemTable()
{
    ...
    {
		sLog->OutConsole("==========================================================================================");
        sLog->OutConsole(">> Loaded 0 spell perfection definitions. DB table `skill_perfect_item_template` is empty.");
		sLog->OutConsole("==========================================================================================");
		sLog->OutConsole();
        return;
    }
    ...
    do /* fetch data and run sanity checks */
    {
        ...
        {
            sLog->OutErrorDatabase("Skill perfection data for spell %u has non-existent spell id in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            sLog->OutErrorConsole("Skill perfection data for spell %u has non-existent required specialization spell id %u in `skill_perfect_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            sLog->OutErrorConsole("Skill perfection data for spell %u has impossibly low proc chance in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            sLog->OutErrorConsole("Skill perfection data for spell %u references non-existent perfect item id %u in `skill_perfect_item_template`!", spellId, perfectItemType);
            continue;
        }
	...
	sLog->OutConsole("====================================================");
    sLog->OutConsole(">> Loaded %u spell perfection definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("====================================================");
	sLog->OutConsole();
}
...
struct SkillExtraItemEntry
{
    ...
    SkillExtraItemEntry(uint32 rS, float aCC, uint8 aMN): requiredSpecialization(rS), additionalCreateChance(aCC), additionalMaxNum(aMN) {}
};
...
void LoadSkillExtraItemTable()
{
    ...
    {
		sLog->OutConsole("============================================================================================");
        sLog->OutConsole(">> Loaded 0 spell specialization definitions. DB table `skill_extra_item_template` is empty.");
		sLog->OutConsole("============================================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!sSpellMgr->GetSpellInfo(spellId))
        {
            sLog->OutErrorConsole("Skill specialization %u has non-existent spell id in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        if (!sSpellMgr->GetSpellInfo(requiredSpecialization))
        {
            sLog->OutErrorConsole("Skill specialization %u have not existed required specialization spell id %u in `skill_extra_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            sLog->OutErrorConsole("Skill specialization %u has too low additional create chance in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        if (!additionalMaxNum)
        {
            sLog->OutErrorConsole("Skill specialization %u has 0 max number of extra items in `skill_extra_item_template`!", spellId);
            continue;
        }
	...
	sLog->OutConsole("========================================================");
    sLog->OutConsole(">> Loaded %u spell specialization definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("========================================================");
    sLog->OutConsole();
}
