...
void TicketMgr::LoadTickets()
{
    ...
    if (!result)
    {
        sLog->OutConsole(">> Loaded 0 GM tickets. DB table `gm_tickets` is empty!");
        sLog->OutConsole();
        return;
    }
    ...
    sLog->OutConsole(">> Loaded %u GM tickets in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->OutConsole();
}
...
void TicketMgr::LoadSurveys()
{
    ...
    sLog->OutConsole(">> Loaded GM Survey count from database in %u ms", GetMSTimeDiffToNow(oldMSTime));
    sLog->OutConsole();
}
