...
bool ThreatCalcHelper::isValidProcess(Unit* hatedUnit, Unit* hatingUnit, SpellInfo const* threatSpell)
{
    ...
    // not to GM
    if (hatedUnit->GetTypeId() == TYPE_ID_PLAYER && hatedUnit->ToPlayer()->IsGameMaster())
        return false;
    ...
    ASSERT(hatingUnit->GetTypeId() == TYPE_ID_UNIT);
...
void HostileReference::updateOnlineStatus()
{
    ...
    if (isValid()
        && (getTarget()->GetTypeId() != TYPE_ID_PLAYER || !getTarget()->ToPlayer()->IsGameMaster())
...
void ThreatManager::_addThreat(Unit* victim, float threat)
{
    ...
    if (!ref) // there was no ref => create a new one
    {
        ...
        if (victim->GetTypeId() == TYPE_ID_PLAYER && victim->ToPlayer()->IsGameMaster())
