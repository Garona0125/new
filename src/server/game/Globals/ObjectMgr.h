...
// Quantum System Text id ranges
#define MIN_QUANTUM_SYSTEM_TEXT_ID     1                    // 'quantum_system_text'
#define MAX_QUANTUM_SYSTEM_TEXT_ID     2000000000
#define MIN_DB_SCRIPT_STRING_ID        MAX_QUANTUM_SYSTEM_TEXT_ID // 'db_script_string'
...
struct QuantumSystemTextLocale
{
...
typedef UNORDERED_MAP<int32, QuantumSystemTextLocale> QuantumSystemTextLocaleContainer;
...
class ObjectMgr
{
    ...
    public:
        ...
        {
            sLog->OutConsole("QUANTUMCORE SERVER: Loading GO Start Quest Data...");
            LoadGameobjectQuestRelations();
            sLog->OutConsole("QUANTUMCORE SERVER: Loading GO End Quest Data...");
            LoadGameobjectInvolvedRelations();
            sLog->OutConsole("QUANTUMCORE SERVER: Loading Creature Start Quest Data...");
            LoadCreatureQuestRelations();
            sLog->OutConsole("QUANTUMCORE SERVER: Loading Creature End Quest Data...");
        ...
        bool LoadQuantumSystemTexts(char const* table, int32 min_value, int32 max_value);
        bool LoadQuantumSystemTexts() { return LoadQuantumSystemTexts("quantum_system_texts", MIN_QUANTUM_SYSTEM_TEXT_ID, MAX_QUANTUM_SYSTEM_TEXT_ID); }
        ...
        QuantumSystemTextLocale const* GetQuantumSystemTextLocale(int32 entry) const
        {
            QuantumSystemTextLocaleContainer::const_iterator itr = _quantumSystemTextLocaleStore.find(entry);
            if (itr == _quantumSystemTextLocaleStore.end()) return NULL;
            return &itr->second;
        }
        const char *GetQuantumSystemText(int32 entry, LocaleConstant locale_idx) const;
        const char *GetQuantumSystemTextForDBCLocale(int32 entry) const { return GetQuantumSystemText(entry, DBCLocaleIndex); }
        ...
        uint32 AddCreatureData(uint32 entry, uint32 team, uint32 map, float x, float y, float z, float o, uint32 spawntimedelay = 0);
        bool MoveCreatureData(uint32 guid, uint32 map, Position pos);
    ...
    private:
        ...
        QuantumSystemTextLocaleContainer _quantumSystemTextLocaleStore;
...
bool LoadQuantumSystemTexts(char const* table, int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
