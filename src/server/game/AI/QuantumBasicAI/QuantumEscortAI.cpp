...
npc_escortAI::npc_escortAI(Creature* creature) : QuantumBasicAI(creature),
    m_uiPlayerGUID(0),
...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI has left combat and is now returning to last point");
    }
    else
    {
        me->GetMotionMaster()->MoveTargetedHome();
		if (HasImmuneToNPCFlags)
			me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_CREATURE);
        Reset();
    }
}
...
void npc_escortAI::UpdateAI(uint32 const diff)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && !me->GetVictim() && m_uiWPWaitTimer && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiWPWaitTimer <= diff)
        {
            ...
            if (CurrentWP == WaypointList.end())
            {
                if (DespawnAtEnd)
                {
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI reached end of waypoints");
                    ...
                    if (m_bCanReturnToStart)
                    {
                        ...
                        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI are returning home to spawn location: %u, %f, %f, %f", POINT_HOME, fRetX, fRetY, fRetZ);
                        return;
                    }
                ...
                {
                    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI reached end of waypoints with Despawn off");
            ...
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
                ...
                sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI start waypoint %u (%f, %f, %f).", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && m_uiPlayerGUID && !me->GetVictim() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiPlayerCheckTimer <= diff)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI failed because player/group was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 moveType, uint32 pointId)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI has returned to original position before combat");
    ...
    {
        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI has returned to original home location and will continue from beginning of waypoint list.");
    ...
    else
    {
        ...
        {
            sLog->OutErrorConsole("QUANTUM CORE SCRIPTS ERROR: EscortAI reached waypoint out of order %u, expected %u, creature entry %u", pointId, CurrentWP->id, me->GetEntry());
            return;
        }

        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI Waypoint %u reached", CurrentWP->id);
...
void npc_escortAI::SetRun(bool on)
{
    if (on)
    {
        if (!m_bIsRunning)
            ...
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI attempt to set walk mode, but is already walking.");
    }
...
void npc_escortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, uint64 playerGUID /* = 0 */, Quest const* quest /* = NULL */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    if (me->GetVictim())
    {
        sLog->OutErrorConsole("QUANTUM CORE SCRIPTS ERROR: EscortAI (script: %s, creature entry: %u) attempts to Start while in combat", me->GetScriptName().c_str(), me->GetEntry());
        return;
    }

    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        sLog->OutErrorConsole("QUANTUM CORE SCRIPTS: EscortAI (script: %s, creature entry: %u) attempts to Start while already escorting", me->GetScriptName().c_str(), me->GetEntry());
        return;
    }
    ...
    if (WaypointList.empty())
    {
        sLog->OutErrorDatabase("QUANTUM CORE SCRIPTS: EscortAI (script: %s, creature entry: %u) starts with 0 waypoints (possible missing entry in script_waypoint. Quest: %u).",
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");

    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        me->GetMotionMaster()->MovementExpired();
        me->GetMotionMaster()->MoveIdle();
        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }

    //disable npcflags
    me->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);
	if (me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_CREATURE))
	{
		HasImmuneToNPCFlags = true;
		me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_CREATURE);
	}

    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: EscortAI started with " UI64FMTD " waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = " UI64FMTD "", uint64(WaypointList.size()), m_bIsActiveAttacker, m_bIsRunning, m_uiPlayerGUID);
