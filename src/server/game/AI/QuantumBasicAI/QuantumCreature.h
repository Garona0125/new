...
#ifndef QUANTUM_CREATURE_H
#define QUANTUM_CREATURE_H
...
class SummonList : public std::list<uint64>
{
    public:
		...
		template <class Predicate> void DoAction(int32 info, Predicate& predicate, uint16 max = 0)
		{
			...
			Quantum::DataPackets::RandomResizeList<uint64, Predicate>(listCopy, predicate, max);
...
struct QuantumBasicAI : public CreatureAI
{
    explicit QuantumBasicAI(Creature* creature);
    virtual ~QuantumBasicAI() {}
	...
	// Called before EnterToBattle even before the creature is in combat.
    ...
    Unit* SelectTargetHpFriendly(float range, uint32 minHPDiff = 1);
...
class BossAI : public QuantumBasicAI
{
    public:
        ...
        void Reset() { _Reset(); }
        void EnterToBattle(Unit* /*who*/) { _EnterToBattle(); }
...
class WorldBossAI : public QuantumBasicAI
{
    public:
