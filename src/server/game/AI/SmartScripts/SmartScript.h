...
class SmartScript
{
    public:
        ...
        {
            return obj && (obj->GetTypeId() == TYPE_ID_UNIT || obj->GetTypeId() == TYPE_ID_PLAYER);
        }

        bool IsPlayer(WorldObject* obj)
        {
            return obj && obj->GetTypeId() == TYPE_ID_PLAYER;
        }

        bool IsCreature(WorldObject* obj)
        {
            return obj && obj->GetTypeId() == TYPE_ID_UNIT;
        }

        bool IsGameObject(WorldObject* obj)
        {
            return obj && obj->GetTypeId() == TYPE_ID_GAMEOBJECT;
        }

        void OnUpdate(const uint32 diff);
        void OnMoveInLineOfSight(Unit* who);

        Unit* SelectTargetHpFriendly(float range, uint32 MinHPDiff);
        ...
        bool IsSmart(Creature* c = NULL)
        {
            ...
            if (!smart)
                sLog->OutErrorDatabase("SmartScript: Action target Creature(entry: %u) is not using SmartAI, action skipped to prevent crash.", c ? c->GetEntry() : (me ? me->GetEntry() : 0));
        ...
        bool IsSmartGO(GameObject* g = NULL)
        {
            ...
            if (!go || go->GetAIName() != "SmartGameObjectAI")
                smart = false;
            if (!smart)
                sLog->OutErrorDatabase("SmartScript: Action target GameObject(entry: %u) is not using SmartGameObjectAI, action skipped to prevent crash.", g ? g->GetEntry() : (go ? go->GetEntry() : 0));
