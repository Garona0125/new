...
class QuantumSystemTextTextBuilder
{
    public:
        QuantumSystemTextTextBuilder(WorldObject* obj, ChatMsg msgtype, int32 id, uint32 language, uint64 targetGUID): _source(obj), _msgType(msgtype), _textId(id), _language(language), _targetGUID(targetGUID){}

        size_t operator()(WorldPacket* data, LocaleConstant locale) const
        {
            std::string text = sObjectMgr->GetQuantumSystemText(_textId, locale);
...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
            ...
            if (targets)
            {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    if (IsCreature(*itr))
                    {
                        ...
                        {
                            (*itr)->ToCreature()->SetCurrentFaction(e.action.faction.factionID);
                        ...
                        else
                        {
                            if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate((*itr)->ToCreature()->GetEntry()))
                            {
                                ...
                                {
                                    (*itr)->ToCreature()->SetCurrentFaction(ci->faction_A);
                                    sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SET_FACTION: Creature entry %u, GuidLow %u set faction to %u",
        ...
        case SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (IsPlayer(*itr))
                {
					...
					sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS: Player guidLow %u credited quest %u", (*itr)->GetGUIDLow(), e.action.quest.quest);
                }
            }
        ...
        case SMART_ACTION_INVOKER_CAST:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_INVOKER_CAST: Invoker %u casts spell %u on target %u with castflags %u", tempLastInvoker->GetGUIDLow(), e.action.cast.spell, (*itr)->GetGUIDLow(), e.action.cast.flags);
                }
            }
        ...
        case SMART_ACTION_SET_EMOTE_STATE:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    (*itr)->ToUnit()->SetUInt32Value(UNIT_NPC_EMOTE_STATE, e.action.emote.emote);
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST:
        {
            ...
            {
                QuantumSystemTextTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, LANG_FLEE, LANG_UNIVERSAL, 0);
        ...
        case SMART_ACTION_SET_INST_DATA:
        {
            ...
            if (!instance)
            {
                sLog->OutErrorDatabase("SmartScript: Event %u attempt to set instance data without instance script. EntryOrGuid %d", e.GetEventType(), e.entryOrGuid);
                break;
            }
        ...
        case SMART_ACTION_CALL_FOR_HELP:
        {
            if (me)
            {
				...
				{
					QuantumSystemTextTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, LANG_CALL_FOR_HELP, LANG_UNIVERSAL, 0);
					sCreatureTextMgr->SendChatPacket(me, builder, CHAT_MSG_MONSTER_EMOTE);
				}
        ...
        case SMART_ACTION_FORCE_DESPAWN:
        {
			...
			// Otherwise we call the despawn directly
				 ...
				 me->DespawnAfterAction(e.action.forceDespawn.delay);
            break;
        }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (Creature* npc = (*itr)->ToCreature())
                {
                    ...
                    if (e.action.equip.entry)
                    {
                        ...
                        if (!einfo)
                        {
                            sLog->OutErrorDatabase("SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info entry %u", e.action.equip.entry);
                            return;
                        }
        ...
        case SMART_ACTION_CALL_TIMED_ACTIONLIST:
        {
            ...
            {
                sLog->OutErrorDatabase("SmartScript: Entry %d SourceType %u Event %u Action %u is using TARGET_NONE(0) for Script9 target. Please correct target_type in database.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
                break;
            }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    if (SmartAI* ai = CAST_AI(SmartAI, (*itr)->ToCreature()->AI()))
                        ...
                        sLog->OutErrorDatabase("SmartScript: Action target for SMART_ACTION_SEND_TARGET_TO_TARGET is not using SmartAI, skipping");
                }
                else if (IsGameObject(*itr))
                {
                    if (SmartGameObjectAI* ai = CAST_AI(SmartGameObjectAI, (*itr)->ToGameObject()->AI()))
                        ...
                        sLog->OutErrorDatabase("SmartScript: Action target for SMART_ACTION_SEND_TARGET_TO_TARGET is not using SmartGameObjectAI, skipping");
                }
            }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsCreature(*itr))
                {
                    if (e.GetTargetType() == SMART_TARGET_SELF)
						...
						sLog->OutErrorDatabase("SmartScript: Action target for SMART_ACTION_SET_HOME_POS is not using SMART_TARGET_SELF or SMART_TARGET_POSITION, skipping");
                }
		...
		default:
			sLog->OutErrorDatabase("SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Unhandled Action type %u", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
			break;
    }
    ...
    if (e.link && e.link != e.event_id)
    {
        ...
        if (linked.GetActionType() && linked.GetEventType() == SMART_EVENT_LINK)
            ...
            sLog->OutErrorDatabase("SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Link Event %u not found or invalid, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
    }
}
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
    ...
    if (mTemplate)
    {
        sLog->OutErrorDatabase("SmartScript::InstallTemplate: Entry %d SourceType %u AI Template can not be set more then once, skipped.", e.entryOrGuid, e.GetScriptType());
        return;
    }
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= NULL*/)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_HOSTILE_SECOND_AGGRO:
            if (me)
                if (Unit* u = me->AI()->SelectTarget(TARGET_TOP_AGGRO, 1))
                    l->push_back(u);
            break;
        case SMART_TARGET_HOSTILE_LAST_AGGRO:
            if (me)
                if (Unit* u = me->AI()->SelectTarget(TARGET_BOTTOM_AGGRO, 0))
                    l->push_back(u);
            break;
        case SMART_TARGET_HOSTILE_RANDOM:
            if (me)
				if (Unit* u = me->AI()->SelectTarget(TARGET_RANDOM, 0))
					l->push_back(u);
            break;
        case SMART_TARGET_HOSTILE_RANDOM_NOT_TOP:
            if (me)
                if (Unit* u = me->AI()->SelectTarget(TARGET_RANDOM, 1))
                    l->push_back(u);
            break;
        ...
        case SMART_TARGET_CREATURE_GUID:
        {
            ...
            else
            {
                ...
                {
                    sLog->OutErrorDatabase("SMART_TARGET_CREATURE_GUID can not be used without invoker and without entry");
                    break;
                }
        ...
        case SMART_TARGET_GAMEOBJECT_GUID:
        {
            ...
            else
            {
                if (!trigger && !GetBaseObject())
                {
                    sLog->OutErrorDatabase("SMART_TARGET_GAMEOBJECT_GUID can not be used without invoker and without entry");
                    break;
                }
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        {
            if (!me || !me->IsInCombatActive() || !me->GetMaxHealth())
                return;
        ...
        {
            if (!me || !me->IsInCombatActive() || !me->GetVictim() || !me->GetVictim()->GetMaxHealth())
                return;
        ...
        {
            if (!me || !me->IsInCombatActive() || !me->GetMaxPower(POWER_MANA))
                return;
        ...
        {
            if (!me || !me->IsInCombatActive() || !me->GetVictim() || !me->GetVictim()->GetMaxPower(POWER_MANA))
                return;
        ...
        {
            if (!me || !me->IsInCombatActive() || !me->GetVictim())
                return;
        ...
        {
            if (!me || !me->IsInCombatActive() || !me->GetVictim() || !me->GetVictim()->IsNonMeleeSpellCasted(false, false, true))
                return;
            ProcessAction(e, me->GetVictim());
        ...
        case SMART_EVENT_FRIENDLY_HEALTH:
        {
            ...
            Unit* target = SelectTargetHpFriendly((float)e.event.friendlyHealth.radius, e.event.friendlyHealth.hpDeficit);
            if (!target || !target->IsInCombatActive())
                return;
        ...
        case SMART_EVENT_KILL:
        {
            if (!me || !unit)
                return;
            if (e.event.kill.playerOnly && unit->GetTypeId() != TYPE_ID_PLAYER)
                return;
		...
		case SMART_EVENT_FRIENDLY_HEALTH_PCT:
        {
            ...
            {
                if (IsUnit(*itr) && me->IsFriendlyTo((*itr)->ToUnit()) && (*itr)->ToUnit()->IsAlive() && (*itr)->ToUnit()->IsInCombatActive())
                {
		...
		default:
			sLog->OutErrorDatabase("SmartScript::ProcessEvent: Unhandled Event type %u", e.GetEventType());
			break;
    }
}
...
void SmartScript::UpdateTimer(SmartScriptHolder& e, uint32 const diff)
{
    ...
    if (e.GetEventType() == SMART_EVENT_UPDATE_IC && (!me || !me->IsInCombatActive()))
        return;

    if (e.GetEventType() == SMART_EVENT_UPDATE_OOC && (me && me->IsInCombatActive()))//can be used with me=NULL (go script)
        return;
...
void SmartScript::FillScript(SmartAIEventList e, WorldObject* obj, AreaTriggerEntry const* at)
{
    ...
    if (mEvents.empty() && obj)
        sLog->OutErrorDatabase("SmartScript: Entry %u has events but no events added to list because of instance flags.", obj->GetEntry());
    if (mEvents.empty() && at)
        sLog->OutErrorDatabase("SmartScript: AreaTrigger %u has events but no events added to list because of instance flags. NOTE: triggers can not handle any instance flags.", at->id);
}
...
void SmartScript::OnInitialize(WorldObject* obj, AreaTriggerEntry const* at)
{
    if (obj)//handle object based scripts
    {
        switch (obj->GetTypeId())
        {
            ...
            case TYPE_ID_GAMEOBJECT:
                ...
                break;
            default:
                sLog->OutErrorConsole("SmartScript::OnInitialize: Unhandled TypeID !WARNING!");
                return;
        }
    } else if (at)
    {
    ...
    {
        sLog->OutErrorConsole("SmartScript::OnInitialize: !WARNING! Initialized objects are NULL.");
        return;
    }
...
Unit* SmartScript::SelectTargetHpFriendly(float range, uint32 MinHPDiff)
{
    if (!me)
        return NULL;
