...
WayPoint* SmartAI::GetNextWayPoint()
{
    ...
    if (itr != mWayPoints->end())
    {
        ...
        {
            sLog->OutErrorConsole("SmartAI::GetNextWayPoint: Got not expected waypoint id %u, expected %u", mLastWP->id, mCurrentWPID);
        }
        return (*itr).second;
    }
    return NULL;
...
{
    if (me->IsInCombatActive())// no wp movement in combat
    {
        sLog->OutErrorConsole("SmartAI::StartPath: Creature entry %u wanted to start waypoint movement while in combat, ignoring.", me->GetEntry());
        return;
    }
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        sLog->OutErrorConsole("SmartAI::StartPath: Creature entry %u wanted to pause waypoint movement while already paused, ignoring.", me->GetEntry());
        return;
    }
...
void SmartAI::UpdatePath(const uint32 diff)
{
    ...
    if (HasEscortState(SMART_ESCORT_PAUSED))
    {
        ...
        {
            if (!me->IsInCombatActive() && !HasEscortState(SMART_ESCORT_RETURNING) && (mWPReached || mLastWPIDReached == SMART_ESCORT_LAST_OOC_POINT || mForcedPaused))
            {
    ...
    if (me->IsInCombatActive() || HasEscortState(SMART_ESCORT_PAUSED | SMART_ESCORT_RETURNING))
        return;
    // handle next wp
...
void SmartAI::UpdateAI(const uint32 diff)
{
    ...
    if (mFollowGuid)
    {
        ...
        {
            if (me->FindCreatureWithDistance(mFollowArrivedEntry, INTERACTION_DISTANCE, true))
            {
...
void SmartAI::EnterToBattle(Unit* enemy)
{
