...
void CreatureAI::TalkToMap(uint8 id, uint64 whisperTarget /*= NULL*/)
{
...
void CreatureAI::DoZoneInCombat(Creature* creature /*= NULL*/, float maxRangeToNearestTarget /* = 50.0f*/)
{
    ...
    if (!map->IsDungeon())
    {
		...
		sLog->outBasic("DoZoneInCombat called for creature that has empty threat list (creature entry = %u)", creature->GetEntry()); // Replace to sLog->outBasic
        return;
    }
...
void CreatureAI::DoAttackerAreaInCombat(Unit* attacker, float range, Unit* unit)
{
    if (!attacker)
        attacker = me;
    ...
    if (!unit->CanHaveThreatList() || unit->getThreatManager().isThreatListEmpty())
        return;
    ...
    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
    {
        if (Player* player = i->getSource())
		{
            ...
            {
                unit->SetInCombatWith(player);
				player->SetInCombatWith(unit);
				unit->AddThreat(player, 0.0f);
			}
		}
	}
}
    ...
    if (attacker)
    {
        if (Group* group = attacker->GetGroup())
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                if (GroupGuy && GroupGuy->IsAlive() && GroupGuy->GetMapId() == me->GetMapId())
                {
                    me->SetInCombatWith(GroupGuy);
                    GroupGuy->SetInCombatWith(me);
                    me->AddThreat(GroupGuy, 0.0f);
                }
            }
        }
    }
