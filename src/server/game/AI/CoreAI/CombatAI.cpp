...
void CombatAI::EnterToBattle(Unit* who)
{
    for (SpellVct::iterator i = spells.begin(); i != spells.end(); ++i)
    {
...
void CasterAI::EnterToBattle(Unit* who)
{
    if (spells.empty())
        return;
...
{
    if (!me->m_spells[0])
        sLog->OutErrorConsole("ArcherAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
...
{
    if (!me->m_spells[0])
        sLog->OutErrorConsole("TurretAI set for creature (entry = %u) with spell1=0. AI will do nothing", me->GetEntry());
