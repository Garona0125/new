...
PetAI::PetAI(Creature* creature) : CreatureAI(creature), i_tracker(TIME_INTERVAL_LOOK)
{
    m_AllySet.clear();
    UpdateAllies();
}
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            if (spellInfo->IsPositive())
            {
                if (spellInfo->CanBeUsedInCombat())
                {
                    ...
                    if (!me->IsInCombatActive() && !me->GetCharmInfo()->IsCommandAttack())
                        continue;
                }
...
void PetAI::UpdateAllies()
{
    ...
    if (!owner)
        return;
    else if (owner->GetTypeId() == TYPE_ID_PLAYER)
