...
struct DefaultTargetSelector : public std::unary_function<Unit*, bool>
{
    ...
    bool operator()(Unit const* target) const
    {
        ...
        if (m_playerOnly && (target->GetTypeId() != TYPE_ID_PLAYER))
            return false;
...
class UnitAI
{
    ...
    public:
        ...
        template <class PREDICATE> Unit* SelectTarget(SelectAggroTarget targetType, uint32 position, PREDICATE const& predicate)
        {
            ...
            {
                case TARGET_NEAREST:
                case TARGET_TOP_AGGRO:
                {
                    std::list<Unit*>::iterator itr = targetList.begin();
                    std::advance(itr, position);
                    return *itr;
                }
                case TARGET_FARTHEST:
                case TARGET_BOTTOM_AGGRO:
                {
        ...
        template <class PREDICATE> void SelectTargetList(std::list<Unit*>& targetList, PREDICATE const& predicate, uint32 maxTargets, SelectAggroTarget targetType)
        {
            ...
            if (targetType == TARGET_FARTHEST || targetType == TARGET_BOTTOM_AGGRO)
                targetList.reverse();

            if (targetType == TARGET_RANDOM)
				Quantum::DataPackets::RandomResizeList(targetList, maxTargets);
            else
