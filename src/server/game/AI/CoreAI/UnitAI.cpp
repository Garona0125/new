...
void UnitAI::DoCast(uint32 spellId)
{
    ...
    switch (AISpellInfo[spellId].target)
    {
        ...
        case AITARGET_ENEMY:
        {
            ...
            target = SelectTarget(TARGET_RANDOM, 0, spellInfo->GetMaxRange(false), playerOnly);
            break;
        }
        case AITARGET_ALLY:
			target = me;
        ...
        case AITARGET_DEBUFF:
        {
            ...
            if (!(spellInfo->Attributes & SPELL_ATTR0_BREAKABLE_BY_DAMAGE)
                ...
                target = SelectTarget(TARGET_RANDOM, 0, targetSelector);
            break;
        }
    }
...
bool NonTankTargetSelector::operator()(Unit const* target) const
{
    ...
    if (_playerOnly && target->GetTypeId() != TYPE_ID_PLAYER)
        return false;
