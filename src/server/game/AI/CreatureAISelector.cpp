...
namespace FactorySelector
{
    CreatureAI* selectAI(Creature* creature)
    {
        ...
        if ((!creature->IsPet() || !((Pet*)creature)->isControlled()) && !creature->IsCharmed())
            if (CreatureAI* quantumBasicAI = sScriptMgr->GetCreatureAI(creature))
                return quantumBasicAI;
        ...
        if (!ai_factory)
        {
            ...
            else if (creature->HasUnitTypeMask(UNIT_MASK_CONTROLABLE_GUARDIAN) && ((Guardian*)creature)->GetOwner()->GetTypeId() == TYPE_ID_PLAYER)
                ai_factory = ai_registry.GetRegistryItem("PetAI");
            else if (creature->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELL_CLICK))
        ...
        sLog->outDebug(LOG_FILTER_QCSCR, "Creature %u used AI is %s.", creature->GetGUIDLow(), ainame.c_str());
    ...
    GameObjectAI* SelectGameObjectAI(GameObject* go)
    {
		...
		if (GameObjectAI* quantumBasicAI = sScriptMgr->GetGameObjectAI(go))
			return quantumBasicAI;
        ...
        sLog->outDebug(LOG_FILTER_QCSCR, "GameObject %u used AI is %s.", go->GetGUIDLow(), ainame.c_str());
