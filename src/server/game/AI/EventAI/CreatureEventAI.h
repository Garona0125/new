...
#ifndef QUANTUM_CREATURE_EVENTAI_H
#define QUANTUM_CREATURE_EVENTAI_H
...
enum EventAI_ActionType
{
    ...
    ACTION_T_MOUNT_TO_ENTRY_OR_MODEL    = 43,               // Creature_template entry(param1) OR ModelId (param2) (or 0 for both to RemoveMount)
...
struct CreatureEventAI_Action
{
    ...
    union
    {
        ...
        struct
        {
            uint32 creatureId;                              // set one from fields (or 0 for both to RemoveMount)
            uint32 modelId;
        } mount;
        // RAW
        struct
...
class CreatureEventAI : public CreatureAI
{
    ...
    public:
        ...
        void DoSendQuantumText(int32 textEntry, WorldObject* source, Unit* target);
