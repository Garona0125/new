...
bool CreatureEventAIHolder::UpdateRepeatTimer(Creature* creature, uint32 repeatMin, uint32 repeatMax)
{
    ...
    {
        sLog->OutErrorDatabase("CreatureEventAI: Creature %u using Event %u (Type = %u) has RandomMax < RandomMin. Event repeating disabled.", creature->GetEntry(), Event.event_id, Event.event_type);
        Enabled = false;
        return false;
    }
...
CreatureEventAI::CreatureEventAI(Creature* c) : CreatureAI(c)
{
    ...
    if (CreatureEvents != sEventAIMgr->GetCreatureEventAIMap().end())
    {
        ...
        if (m_CreatureEventAIList.empty())
            sLog->OutErrorDatabase("CreatureEventAI: Creature %u has events but no events added to list because of instance flags.", me->GetEntry());
    }
    else
        sLog->OutErrorDatabase("CreatureEventAI: EventMap for Creature %u is empty but creature is using CreatureEventAI.", me->GetEntry());
...
bool CreatureEventAI::ProcessEvent(CreatureEventAIHolder& holder, Unit* actionInvoker /*=NULL*/)
{
    ...
    switch (event.event_type)
    {
        ...
        case EVENT_T_HP:
        {
            if (!me->IsInCombatActive() || !me->GetMaxHealth())
                return false;
        ...
        case EVENT_T_MANA:
        {
            if (!me->IsInCombatActive() || !me->GetMaxPower(POWER_MANA))
                return false;
        ...
        {
            if (!me->IsInCombatActive() || !me->GetVictim() || !me->GetVictim()->GetMaxHealth())
                return false;
        ...
        case EVENT_T_TARGET_CASTING:
            if (!me->IsInCombatActive() || !me->GetVictim() || !me->GetVictim()->IsNonMeleeSpellCasted(false, false, true))
                return false;
        ...
        case EVENT_T_FRIENDLY_HP:
        {
            ...
            Unit* unit = SelectTargetHpFriendly((float)event.friendly_hp.radius, event.friendly_hp.hpDeficit);
            if (!unit)
                return false;
        ...
        case EVENT_T_SUMMONED_UNIT:
        {
            ...
            if (!actionInvoker || actionInvoker->GetTypeId() != TYPE_ID_UNIT)
                return false;
        ...
        {
            if (!me->IsInCombatActive() || !me->GetVictim() || !me->GetVictim()->GetMaxPower(POWER_MANA))
                return false;
        ...
        default:
            sLog->OutErrorDatabase("CreatureEventAI: Creature %u using Event %u has invalid Event Type(%u), missing from ProcessEvent() Switch.", me->GetEntry(), holder.Event.event_id, holder.Event.event_type);
            break;
    }
    ...
    switch (action.type)
    {
        case ACTION_T_TEXT:
        {
            ...
            if (temp)
            {
                ...
                if (actionInvoker)
                {
                    if (actionInvoker->GetTypeId() == TYPE_ID_PLAYER)
                ...
                DoSendQuantumText(temp, me, target);
            }
            break;
        }
        case ACTION_T_SET_FACTION:
        {
            if (action.set_faction.factionId)
                me->SetCurrentFaction(action.set_faction.factionId);
            else
            {
                if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(me->GetEntry()))
                {
                    ...
                    if (me->GetFaction() != ci->faction_A)
                        me->SetCurrentFaction(ci->faction_A);
                }
            }
            break;
        }
        ...
        case ACTION_T_CAST:
        {
            ...
            if (canCast)
            {
                ...
                if (tSpell)
                {
                    ...
                    sLog->OutErrorDatabase("CreatureEventAI: event %d creature %d attempt to cast spell that doesn't exist %d", eventId, me->GetEntry(), action.cast.spellId);
            }
            break;
        }
        ...
        case ACTION_T_INC_PHASE:
        {
            ...
            if (new_phase < 0)
            {
                sLog->OutErrorDatabase("CreatureEventAI: Event %d decrease m_Phase under 0. CreatureEntry = %d", eventId, me->GetEntry());
                m_Phase = 0;
            }
            else if (new_phase >= MAX_PHASE)
            {
                sLog->OutErrorDatabase("CreatureEventAI: Event %d incremented m_Phase above %u. m_Phase mask cannot be used with phases past %u. CreatureEntry = %d", eventId, MAX_PHASE-1, MAX_PHASE-1, me->GetEntry());
                m_Phase = MAX_PHASE-1;
            }
            else
                m_Phase = new_phase;
        ...
        case ACTION_T_QUEST_EVENT_ALL:
            if (actionInvoker && actionInvoker->GetTypeId() == TYPE_ID_PLAYER)
            {
        ...
        case ACTION_T_RANDOM_PHASE_RANGE:
            if (action.random_phase_range.phaseMin <= action.random_phase_range.phaseMax)
                ...
                sLog->OutErrorDatabase("CreatureEventAI: ACTION_T_RANDOM_PHASE_RANGE cannot have Param2 < Param1. Event = %d. CreatureEntry = %d", eventId, me->GetEntry());
            break;
        ...
        case ACTION_T_SET_INST_DATA:
        {
            ...
            if (!instance)
            {
                sLog->OutErrorDatabase("CreatureEventAI: Event %d attempt to set instance data without instance script. Creature %d", eventId, me->GetEntry());
                return;
            }
        ...
        case ACTION_T_SET_INST_DATA64:
        {
            ...
            if (!target)
            {
                sLog->OutErrorDatabase("CreatureEventAI: Event %d attempt to set instance data64 but Target == NULL. Creature %d", eventId, me->GetEntry());
                return;
            }
            ...
            if (!instance)
            {
                sLog->OutErrorDatabase("CreatureEventAI: Event %d attempt to set instance data64 without instance script. Creature %d", eventId, me->GetEntry());
                return;
            }
        ...
        case ACTION_T_UPDATE_TEMPLATE:
            ...
            {
                sLog->OutErrorDatabase("CreatureEventAI: Event %d ACTION_T_UPDATE_TEMPLATE call with param1 == current entry. Creature %d", eventId, me->GetEntry());
                return;
            }
        ...
        case ACTION_T_DIE:
            if (me->IsDead())
            {
                sLog->OutErrorDatabase("CreatureEventAI: Event %d ACTION_T_DIE on dead creature. Creature %d", eventId, me->GetEntry());
                return;
            }
            me->Kill(me);
            break;
        ...
        case ACTION_T_SUMMON_GO:
        {
            ...
            if (!object)
            {
                sLog->OutErrorDatabase("QUANTUM CORE SCRIPTS: EventAI failed to spawn object %u. Spawn event %d is on creature %d", action.raw.param1, eventId, me->GetEntry());
            }
            break;
        }
        ...
        {
            me->DespawnAfterAction(action.forced_despawn.msDelay);
            break;
        }
...
void CreatureEventAI::Reset()
{
    ...
    for (CreatureEventAIList::iterator i = m_CreatureEventAIList.begin(); i != m_CreatureEventAIList.end(); ++i)
    {
        ...
        switch (event.event_type)
        {
            ...
            //default:
            //TODO: enable below code line / verify this is correct to enable events previously disabled (ex. aggro yell), instead of enable this in void EnterToBattle()
            //(*i).Enabled = true;
            //(*i).Time = 0;
            //break;
        }
...
{
    if (m_bEmptyList || victim->GetTypeId() != TYPE_ID_PLAYER)
        return;
...
void CreatureEventAI::EnterToBattle(Unit* enemy)
{
...
inline Unit* CreatureEventAI::GetTargetByType(uint32 target, Unit* actionInvoker)
{
    switch (target)
    {
        ...
        case TARGET_T_HOSTILE_SECOND_AGGRO:
            return SelectTarget(TARGET_TOP_AGGRO, 1);
        case TARGET_T_HOSTILE_LAST_AGGRO:
            return SelectTarget(TARGET_BOTTOM_AGGRO, 0);
        case TARGET_T_HOSTILE_RANDOM:
            return SelectTarget(TARGET_RANDOM, 0);
        case TARGET_T_HOSTILE_RANDOM_NOT_TOP:
            return SelectTarget(TARGET_RANDOM, 1);
...
Unit* CreatureEventAI::SelectTargetHpFriendly(float range, uint32 minHPDiff)
{
...
void CreatureEventAI::DoSendQuantumText(int32 textEntry, WorldObject* source, Unit* target)
{
    if (!source)
    {
        sLog->OutErrorDatabase("CreatureEventAI: DoSendQuantumText entry %i, invalid Source pointer.", textEntry);
        return;
    }

    if (textEntry >= 0)
    {
        sLog->OutErrorDatabase("CreatureEventAI: DoSendQuantumText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but text entry must be negative.", source->GetEntry(), source->GetTypeId(), source->GetGUIDLow(), textEntry);
        return;
    }
    ...
    {
        sLog->OutErrorDatabase("CreatureEventAI: DoSendQuantumText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.", source->GetEntry(), source->GetTypeId(), source->GetGUIDLow(), textEntry);
        return;
    }

    sLog->outDebug(LOG_FILTER_DATABASE_AI, "CreatureEventAI: DoSendQuantumText: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u", textEntry, (*i).second.SoundId, (*i).second.Type, (*i).second.Language, (*i).second.Emote);
    ...
    if ((*i).second.SoundId)
    {
        if (sSoundEntriesStore.LookupEntry((*i).second.SoundId))
            ...
            sLog->OutErrorDatabase("CreatureEventAI: DoSendQuantumText entry %i tried to process invalid sound id %u.", textEntry, (*i).second.SoundId);
    }

    if ((*i).second.Emote)
    {
        if (source->GetTypeId() == TYPE_ID_UNIT || source->GetTypeId() == TYPE_ID_PLAYER)
        {
            ((Unit*)source)->HandleEmoteCommand((*i).second.Emote);
        }
        else
            sLog->OutErrorDatabase("CreatureEventAI: DoSendQuantumText entry %i tried to process emote for invalid TypeId (%u).", textEntry, source->GetTypeId());
    }
    ...
    switch ((*i).second.Type)
    {
        ...
        case CHAT_TYPE_WHISPER:
        {
            ...
            else sLog->OutErrorDatabase("CreatureEventAI: DoSendQuantumText entry %i cannot whisper without target unit (TYPE_ID_PLAYER).", textEntry);
        }
		break;
