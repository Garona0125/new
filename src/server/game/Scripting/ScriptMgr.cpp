    ...
    public:
        ...
        static void AddScript(TScript* const script)
        {
            ...
            for (ScriptMapIterator it = ScriptPointerList.begin(); it != ScriptPointerList.end(); ++it)
            {
                if (it->second == script)
                {
                    sLog->OutErrorConsole("Script '%s' has same memory pointer as '%s'.",
            ...
            if (script->IsDatabaseBound())
            {
                ...
                if (id)
                {
                    ...
                    else
                    {
                        ...
                        sLog->OutErrorConsole("Script '%s' already assigned with the same script name, so the script can't work.",
                ...
                else
                {
                    ...
                    if (script->GetName().find("example") == std::string::npos && script->GetName().find("Smart") == std::string::npos)
                        sLog->OutErrorDatabase("Script named '%s' does not have a script name assigned in database.",
                            script->GetName().c_str());
                }
            }
            else
...
void DoSendQuantumText(int32 iTextEntry, WorldObject* pSource, Unit* target)
{
    if (!pSource)
    {
        sLog->OutErrorConsole("QUANTUM CORE SCRIPTS: DoSendQuantumText entry %i, invalid Source pointer.", iTextEntry);
        return;
    }

    if (iTextEntry >= 0)
    {
        sLog->OutErrorDatabase("QUANTUM CORE SCRIPTS: DoSendQuantumText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but text entry must be negative.", pSource->GetEntry(), pSource->GetTypeId(), pSource->GetGUIDLow(), iTextEntry);
        return;
    }
    ...
    if (!pData)
    {
        sLog->OutErrorDatabase("QUANTUM CORE SCRIPTS: DoSendQuantumText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.", pSource->GetEntry(), pSource->GetTypeId(), pSource->GetGUIDLow(), iTextEntry);
        return;
    }

    sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: DoSendQuantumText: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u", iTextEntry, pData->uiSoundId, pData->uiType, pData->uiLanguage, pData->uiEmote);
    ...
    if (pData->uiSoundId)
    {
		if (sSoundEntriesStore.LookupEntry(pData->uiSoundId))
            ...
            sLog->OutErrorConsole("QUANTUM CORE SCRIPTS: DoSendQuantumText entry %i tried to process invalid sound id %u.", iTextEntry, pData->uiSoundId);
    }

    if (pData->uiEmote)
    {
        if (pSource->GetTypeId() == TYPE_ID_UNIT || pSource->GetTypeId() == TYPE_ID_PLAYER)
            ((Unit*)pSource)->HandleEmoteCommand(pData->uiEmote);
        else
            sLog->OutErrorConsole("QUANTUM CORE SCRIPTS: DoSendQuantumText entry %i tried to process emote for invalid TypeId (%u).", iTextEntry, pSource->GetTypeId());
    }
    ...
    switch (pData->uiType)
    {
        ...
        case CHAT_TYPE_WHISPER:
        {
            if (target && target->GetTypeId() == TYPE_ID_PLAYER)
                ...
                sLog->OutErrorConsole("QUANTUM CORE SCRIPTS: DoSendQuantumText entry %i cannot whisper without target unit (TYPE_ID_PLAYER).", iTextEntry);
...
void ScriptMgr::Initialize()
{
    ...
    sLog->OutConsole("QUANTUMCORE SERVER: LOADING C++ SCRIPTS");

    FillSpellSummary();
    AddScripts();

	sLog->OutConsole("====================================");
    sLog->OutConsole(">> LOADED %u C++ SCRIPTS in %u ms", GetScriptCount(), GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("====================================");
    sLog->OutConsole();
}
...
void ScriptMgr::LoadDatabase()
{
    ...
    sScriptSystemMgr->LoadQuantumCoreTexts();
...
{
    ASSERT(veh);
    ASSERT(veh->GetBase()->GetTypeId() == TYPE_ID_UNIT);
...
{
    FOREACH_SCRIPT(PlayerScript)->OnUpdateTimer(player, diff);
}
    ...
    : ScriptObject(name), MapScript<Map>(mapId)
{
    if (GetEntry() && !GetEntry()->IsWorldMap())
        sLog->OutErrorConsole("WorldMapScript for map %u is invalid.", mapId);
    ...
    : ScriptObject(name), MapScript<InstanceMap>(mapId)
{
    if (GetEntry() && !GetEntry()->IsDungeon())
        sLog->OutErrorConsole("InstanceMapScript for map %u is invalid.", mapId);
    ...
    : ScriptObject(name), MapScript<BattlegroundMap>(mapId)
{
    if (GetEntry() && !GetEntry()->IsBattleground())
        sLog->OutErrorConsole("BattlegroundMapScript for map %u is invalid.", mapId);
