...
{
    m_owner.DespawnAfterAction();    // since we are here, we are not TempSummon as object type cannot change during runtime
    return true;
}
...
Creature::~Creature()
{
    ...
    //if (m_uint32Values)
    //    sLog->OutErrorConsole("Deconstruct Creature Entry = %u", GetEntry());
}
...
bool Creature::InitEntry(uint32 Entry, uint32 /*team*/, const CreatureData* data)
{
    ...
    if (!normalInfo)
    {
        sLog->OutErrorDatabase("Creature::InitEntry creature entry %u does not exist.", Entry);
        return false;
    }
    ...
    {
        sLog->OutErrorDatabase("Creature (Entry: %u) has no model defined in table `creature_template`, can't load. ", Entry);
        return false;
    }
...
bool Creature::UpdateEntry(uint32 Entry, uint32 team, const CreatureData* data)
{
    ...
    SelectLevel();
    if (team == HORDE)
        SetCurrentFaction(cInfo->faction_H);
    else
        SetCurrentFaction(cInfo->faction_A);
    ...
    CreatureBaseStats const* stats = sObjectMgr->GetCreatureBaseStats(GetCurrentLevel(), cInfo->unit_class);
    ...
    if (IsVehicle())
    {
        ...
        {
            SetCurrentFaction(owner->GetFaction()); // vehicles should have same as owner faction
    ...
    {
        ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_MOD_TAUNT, true);
...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
		case JUST_RESPAWNED:
			...
			sLog->OutErrorConsole("Creature (GUID: %u Entry: %u) in wrong state: JUST_RESPAWNED (4)", GetGUIDLow(), GetEntry());
            break;
        case JUST_DIED:
            // Must not be called, see Creature::setDeathState JUST_DIED -> CORPSE promoting.
            sLog->OutErrorConsole("Creature (GUID: %u Entry: %u) in wrong state: JUST_DEAD (1)", GetGUIDLow(), GetEntry());
            break;
        case DEAD:
        {
        ...
        case ALIVE:
        {
            ...
            if (m_combatPulseDelay > 0 && IsInCombatActive() && GetMap()->IsDungeon())
            {
            ...
            bool bInCombat = IsInCombatActive() && (!GetVictim() || !GetVictim()->GetCharmerOrOwnerPlayerOrPlayerItself() ||  !GetVictim()->GetCharmerOrOwnerPlayerOrPlayerItself()->IsGameMaster());
...
void Creature::RegenerateMana()
{
    ...
    if (IsInCombatActive() || GetCharmerOrOwnerGUID())
    {
...
bool Creature::AIM_Initialize(CreatureAI* ai)
{
    ...
    if (m_AI_locked)
    {
        sLog->outDebug(LOG_FILTER_QCSCR, "AIM_Initialize: failed to init, locked.");
        return false;
    }
...
bool Creature::Create(uint32 guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, uint32 team, float x, float y, float z, float ang, const CreatureData* data)
{
    ...
    if (!cinfo)
    {
        sLog->OutErrorDatabase("Creature::Create(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, Entry);
        return false;
    }
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("Creature::Create(): given coordinates for creature (guidlow %d, entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)", guidlow, Entry, x, y, z, ang);
        return false;
    }
	...
	//! Need to be called after LoadCreaturesAddon - MOVEMENT_FLAG_HOVER is set there
...
bool Creature::IsCanTrainingOf(Player* player, bool msg) const
{
    ...
    {
        sLog->OutErrorDatabase("Creature %u (Entry: %u) have UNIT_NPC_FLAG_TRAINER but have empty trainer spell list.",
            GetGUIDLow(), GetEntry());
        return false;
    }
    ...
    switch (GetCreatureTemplate()->trainer_type)
    {
        case TRAINER_TYPE_CLASS:
            if (player->GetCurrentClass() != GetCreatureTemplate()->trainer_class)
            {
                if (msg)
                {
        ...
        case TRAINER_TYPE_MOUNTS:
            if (GetCreatureTemplate()->trainer_race && player->GetCurrentRace() != GetCreatureTemplate()->trainer_race)
            {
                if (msg)
                {
                    player->PlayerTalkClass->ClearMenus();
...
{
    return player->GetCurrentLevel() >= 10 && GetCreatureTemplate()->trainer_type == TRAINER_TYPE_CLASS && player->GetCurrentClass() == GetCreatureTemplate()->trainer_class;
}
...
void Creature::SetLootRecipient(Unit* unit)
{
    ...
    if (unit->GetTypeId() != TYPE_ID_PLAYER && !unit->IsVehicle())
        return;
...
void Creature::SaveToDB()
{
    ...
    if (!data)
    {
        sLog->OutErrorConsole("Creature::SaveToDB failed, cannot get creature data!");
        return;
    }
...
void Creature::SelectLevel()
{
    ...
    SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, cInfo->mindmg * damagemod);
    SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, cInfo->maxdmg * damagemod);
...
bool Creature::CreateFromProto(uint32 guidlow, uint32 Entry, uint32 vehId, uint32 team, const CreatureData* data)
{
    ...
    if (!cinfo)
    {
        sLog->OutErrorDatabase("Creature::CreateFromProto(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, Entry);
        return false;
    }
...
bool Creature::LoadCreatureFromDB(uint32 guid, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        sLog->OutErrorDatabase("Creature (GUID: %u) not found in table `creature`, can't load. ", guid);
        return false;
    }
...
void Creature::LoadEquipment(uint32 equip_entry, bool force)
{
    ...
    EquipmentInfo const* eqinfo = sObjectMgr->GetEquipmentInfo(equip_entry);
    if (!eqinfo)
        return;

    m_equipmentId = equip_entry;
    for (uint8 i = 0; i < 3; ++i)
        SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i, eqinfo->ItemEntry[i]);
}
...
void Creature::DeleteFromDB()
{
    if (!m_DBTableGuid)
    {
        sLog->OutErrorConsole("Trying to delete not saved creature! LowGUID: %u, Entry: %u", GetGUIDLow(), GetEntry());
        return;
    }
...
bool Creature::canStartAttack(Unit const* who, bool force) const
{
	...
	if ((HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_CREATURE) && who->GetTypeId() != TYPE_ID_PLAYER) // flag is valid only for non player characters 
		|| (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER) && who->GetTypeId() == TYPE_ID_PLAYER) // immune to PC and target is a player, return false
		|| (who->GetOwner() && who->GetOwner()->GetTypeId() == TYPE_ID_PLAYER && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER))) // player pets are immune to pc as well
		return false;

    // Do not attack non-combat pets
    if (who->GetTypeId() == TYPE_ID_UNIT && who->GetCreatureType() == CREATURE_TYPE_NON_COMBAT_PET)
        return false;
    ...
    if (!force)
    {
        ...
        if (who->IsInCombatActive() && IsWithinDist(who, ATTACK_DISTANCE))
		{
	...
	// No aggro from gray creatures
	 if (CheckNoGrayAggroConfig(who->GetCurrentLevelForTarget(this), GetCurrentLevelForTarget(who)))
		 return false;
...
float Creature::GetAttackDistance(Unit const* player) const
{
    ...
    uint32 playerlevel = player->GetCurrentLevelForTarget(this);
    uint32 creaturelevel = GetCurrentLevelForTarget(player);
...
void Creature::DespawnAfterAction(uint32 msTimeToDespawn /*= 0*/)
{
...
SpellInfo const* Creature::reachWithSpellAttack(Unit* victim)
{
    ...
    for (uint32 i=0; i < CREATURE_MAX_SPELLS; ++i)
    {
        ...
        if (!spellInfo)
        {
            sLog->OutErrorConsole("WORLD: unknown spell id %i", m_spells[i]);
            continue;
        }
...
Unit* Creature::SelectNearestTargetInAttackDistance(float dist) const
{
    ...
    {
        sLog->OutErrorConsole("Creature (GUID: %u Entry: %u) SelectNearestTargetInAttackDistance called with dist > MAX_VISIBILITY_DISTANCE. Distance set to ATTACK_DISTANCE.", GetGUIDLow(), GetEntry());
...
bool Creature::CanAssistTo(const Unit* u, const Unit* enemy, bool checkfaction /*= true*/) const
{
	...
	if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NO_AGGRO_FOR_CREATURE))
        return false;
...
bool Creature::LoadCreaturesAddon(bool reload)
{
    ...
    if (cainfo->emote != 0)
        SetUInt32Value(UNIT_NPC_EMOTE_STATE, cainfo->emote);
    ...
    if (!cainfo->auras.empty())
    {
        for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin(); itr != cainfo->auras.end(); ++itr)
        {
            ...
            {
                sLog->OutErrorDatabase("Creature (GUID: %u Entry: %u) has wrong spell %u defined in `auras` field.", GetGUIDLow(), GetEntry(), *itr);
                continue;
            }
            ...
            {
                if (!reload)
					sLog->OutErrorDatabase("Creature (GUID: %u Entry: %u) has duplicate aura (spell %u) in `auras` field.", GetGUIDLow(), GetEntry(), *itr);
...
void Creature::SetInCombatWithZone()
{
    if (!CanHaveThreatList())
    {
        sLog->OutErrorConsole("Creature entry %u call SetInCombatWithZone but creature cannot have threat list.", GetEntry());
        return;
    }
    ...
    if (!map->IsDungeon())
    {
        sLog->OutErrorConsole("Creature entry %u call SetInCombatWithZone for map (id: %u) that isn't an instance.", GetEntry(), map->GetId());
        return;
    }
...
uint8 Creature::GetCurrentLevelForTarget(WorldObject const* target) const
{
    if (!isWorldBoss() || !target->ToUnit())
        return Unit::GetCurrentLevelForTarget(target);

    uint16 level = target->ToUnit()->GetCurrentLevel() + sWorld->getIntConfig(CONFIG_WORLD_BOSS_LEVEL_DIFF);
    if (level < 1)
        return 1;
