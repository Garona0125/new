...
void TempSummon::Update(uint32 diff)
{
    ...
    switch (m_type)
    {
        ...
        default:
            UnSummon();
            sLog->OutErrorConsole("Temporary summoned creature (entry: %u) have unknown type %u of ", GetEntry(), m_type);
            break;
    }
}
...
void TempSummon::InitStats(uint32 duration)
{
    ...
    if (owner && isTrigger() && m_spells[0])
    {
        ...
        SetLevel(owner->GetCurrentLevel());
    ...
    if (m_Properties->Faction)
        SetCurrentFaction(m_Properties->Faction);
...
void TempSummon::InitSummon()
{
    ...
    if (owner)
    {
        if (owner->GetTypeId() == TYPE_ID_UNIT && owner->ToCreature()->IsAIEnabled)
...
void TempSummon::UnSummon(uint32 msTime)
{
    ...
    if (owner && owner->GetTypeId() == TYPE_ID_UNIT && owner->ToCreature()->IsAIEnabled)
...
void TempSummon::RemoveFromWorld()
{
    ...
    //if (GetOwnerGUID())
    //    sLog->OutErrorConsole("Unit %u has owner guid when removed from world", GetEntry());
...
void Minion::InitStats(uint32 duration)
{
    ...
    SetCurrentFaction(m_owner->GetFaction());
...
void Guardian::InitStats(uint32 duration)
{
    ...
    InitStatsForLevel(m_owner->GetCurrentLevel());

    if (m_owner->GetTypeId() == TYPE_ID_PLAYER && HasUnitTypeMask(UNIT_MASK_CONTROLABLE_GUARDIAN))
...
{
    ASSERT(owner->GetTypeId() == TYPE_ID_PLAYER);
...
{
    Minion::InitStats(duration);
    SetLevel(m_owner->GetCurrentLevel());
    SetReactState(REACT_PASSIVE);
}
