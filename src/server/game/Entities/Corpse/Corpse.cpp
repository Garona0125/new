...
{
    m_objectType |= TYPE_MASK_CORPSE;
    m_objectTypeId = TYPE_ID_CORPSE;
...
bool Corpse::Create(uint32 guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("Corpse (guidlow %d, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
...
void Corpse::DeleteBonesFromWorld()
{
    ...
    if (!corpse)
    {
        sLog->OutErrorConsole("Bones %u not found in world.", GetGUIDLow());
        return;
    }
...
bool Corpse::LoadCorpseFromDB(uint32 guid, Field* fields)
{
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("Corpse (guid: %u, owner: %u) is not created, given coordinates are not valid (X: %f, Y: %f, Z: %f)",
