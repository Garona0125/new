...
{
    m_objectType |= TYPE_MASK_GAMEOBJECT;
    m_objectTypeId = TYPE_ID_GAMEOBJECT;
...
void GameObject::RemoveFromOwner()
{
	...
	// outCrash Replace to outDebug
   sLog->outDebug(LOG_FILTER_QCSCR, "Removed GameObject (GUID: %u Entry: %u SpellId: %u LinkedGO: %u) that just lost any reference to the owner (GUID: %u Type: '%s') GO list",
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("Gameobject (GUID: %u Entry: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)", guidlow, name_id, x, y);
        return false;
    }
    ...
    if (!goinfo)
    {
        sLog->OutErrorDatabase("Gameobject (GUID: %u Entry: %u) not created: non-existing entry in `gameobject_template`. Map: %u (X: %f Y: %f Z: %f)", guidlow, name_id, map->GetId(), x, y, z);
        return false;
    }
    ...
    {
        sLog->OutErrorDatabase("Gameobject (GUID: %u Entry: %u) not created: non-existing GO type '%u' in `gameobject_template`. It will crash client if created.", guidlow, name_id, goinfo->type);
        return false;
    }
...
void GameObject::Update(uint32 diff)
{
    ...
    {
        if (!AIM_Initialize())
            sLog->OutErrorConsole("Could not initialize GameObjectAI");
    } else
        AI()->UpdateAI(diff);
    ...
    switch (m_lootState)
    {
        ...
        case GO_READY:
        {
            if (m_respawnTime > 0)                          // timer on
            {
                ...
                if (m_respawnTime <= now)            // timer expired
                {
                    ...
                    switch (GetGoType())
                    {
                        case GAMEOBJECT_TYPE_FISHING_NODE:   //  can't fish now
                        {
                            Unit* caster = GetOwner();
                            if (caster && caster->GetTypeId() == TYPE_ID_PLAYER)
                            {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (ok)
                    {
						...
						if (ok->GetTypeId() == TYPE_ID_PLAYER)
                        ...
                        if (IsBattlegroundTrap && ok->GetTypeId() == TYPE_ID_PLAYER)
                        {
        ...
        case GO_JUST_DEACTIVATED:
        {
            ...
            if (GetGoType() == GAMEOBJECT_TYPE_GOOBER)
            {
                ...
                if (GetGOInfo()->flags & GO_FLAG_NO_DESPAWN)
                    return;
            }
...
void GameObject::SaveToDB()
{
    ...
    if (!data)
    {
        sLog->OutErrorConsole("GameObject::SaveToDB failed, cannot get gameobject data!");
        return;
    }
...
bool GameObject::LoadGameObjectFromDB(uint32 guid, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        sLog->OutErrorDatabase("Gameobject (GUID: %u) not found in table `gameobject`, can't load. ", guid);
        return false;
    }
    ...
    if (data->spawntimesecs >= 0)
    {
        ...
        {
            SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NO_DESPAWN);
...
bool GameObject::IsAlwaysVisibleFor(WorldObject const* seer) const
{
	...
	if (uint64 guid = GetOwnerGUID())
	{
		...
		Unit* owner = GetOwner();
		if (owner && seer->isType(TYPE_MASK_UNIT) && owner->IsFriendlyTo(((Unit*)seer)))
			return true;
	}
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        {
            if (user->GetTypeId() != TYPE_ID_PLAYER)
                return;
        ...
        //fishing bobber
        case GAMEOBJECT_TYPE_FISHING_NODE:                   //17
        {
            Player* player = user->ToPlayer();
            if (!player)
                return;
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    if (!zone_skill)
                        sLog->OutErrorDatabase("Fishable areaId %u are not properly defined in `skill_fishing_base_level`.", subzone);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
						if (Player* target = ObjectAccessor::GetPlayer(*this, Quantum::DataPackets::SelectRandomContainerElement(m_unique_users)))
        ...
        case GAMEOBJECT_TYPE_SPELL_CASTER:                   //22
        {
            ...
            if (info->spellcaster.partyOnly)
            {
                ...
                if (user->GetTypeId() != TYPE_ID_PLAYER || !user->ToPlayer()->IsInSameRaidWith(caster->ToPlayer()))
                    return;
            }
        ...
        case GAMEOBJECT_TYPE_MEETING_STONE:                  //23
        {
            GameObjectTemplate const* info = GetGOInfo();
            ...
            if (level < info->meetingstone.minLevel)
                return;
            level = targetPlayer->GetCurrentLevel();
            if (level < info->meetingstone.minLevel)
                return;
        ...
        default:
            if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
                sLog->OutErrorConsole("GameObject::Use(): unit (type: %u, guid: %u, name: %s) tries to use object (guid: %u, entry: %u, name: %s) of unknown type (%u)",
    ...
    if (!spellInfo)
    {
        if (user->GetTypeId() != TYPE_ID_PLAYER || !sOutdoorPvPMgr->HandleCustomSpell(user->ToPlayer(), spellId, this))
            sLog->OutErrorConsole("WORLD: unknown spell id %u at use action for gameobject (Entry: %u GoType: %u)", spellId, GetEntry(), GetGoType());
        else
...
void GameObject::CastSpell(Unit* target, uint32 spellId)
{
    ...
    {
        trigger->SetCurrentFaction(owner->GetFaction());
...
void GameObject::SetLootState(LootState state, Unit* unit)
{
	...
	if (GetGoType() == GAMEOBJECT_TYPE_DOOR) // only set collision for doors on SetGoState (need test)
		return;
