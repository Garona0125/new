...
struct GameObjectTemplate
{
    ...
    uint32 GetLockId() const
    {
        switch (type)
        {
            ...
            case GAMEOBJECT_TYPE_AREA_DAMAGE:
    ...
    uint32 GetCharges() const                               // despawn at uses amount
    {
        switch (type)
        {
            //case GAMEOBJECT_TYPE_TRAP:
				//return trap.charges;
            case GAMEOBJECT_TYPE_GUARD_POST:
				return guardpost.charges;
            case GAMEOBJECT_TYPE_SPELL_CASTER:
...
class GameObject : public WorldObject, public GridObject<GameObject>
{
    public:
        ...
        uint8 GetCurrentLevelForTarget(WorldObject const* target) const
        {
            if (Unit* owner = GetOwner())
                return owner->GetCurrentLevelForTarget(target);
