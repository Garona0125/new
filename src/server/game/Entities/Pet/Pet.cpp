...
bool Pet::LoadPetFromDB(Player* owner, uint32 petentry, uint32 petnumber, bool current)
{
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("Pet (guidlow %d, entry %d) not loaded. Suggested coordinates isn't valid (X: %f Y: %f)",
    ...
    {
        case SUMMON_PET:
            petlevel = owner->GetCurrentLevel();
            SetUInt32Value(UNIT_FIELD_BYTES_0, 0x800);
        ...
        default:
            if (!IsPetGhoul())
                sLog->OutErrorConsole("Pet have incorrect type (%u) for pet loading.", getPetType());
            break;
    }

    SetUInt32Value(UNIT_FIELD_PET_NAME_TIME_STAMP, uint32(time(NULL))); // cast can't be helped here
    ...
    if (owner->GetTypeId() == TYPE_ID_PLAYER && isControlled() && !isTemporarySummoned() && (getPetType() == SUMMON_PET || getPetType() == HUNTER_PET))
...
void Pet::DeleteFromDB(uint32 guidlow)
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_PET_DECLINED_NAME);
    stmt->setUInt32(0, guidlow);
    trans->Append(stmt);
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (isControlled())
            {
                ...
                {
                    sLog->OutErrorConsole("Pet %u is not pet of owner %s, removed", GetEntry(), m_owner->GetName());
...
void Pet::LoseHappiness()
{
    ...
    if (IsInCombatActive())                                        //we know in combat happiness fades faster, multiplier guess
...
void Pet::GivePetXP(uint32 xp)
{
    ...
    uint8 maxlevel = std::min((uint8)sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL), GetOwner()->GetCurrentLevel());
    uint8 petlevel = GetCurrentLevel();

    // If pet is detected to be at, or above(?) the players level, don't hand out XP
    if (petlevel >= maxlevel)
       return;

    uint32 nextLvlXP = GetUInt32Value(UNIT_FIELD_PET_NEXT_LEVEL_EXP);
    ...
    while (newXP >= nextLvlXP && petlevel < maxlevel)
    {
        ...
        nextLvlXP = GetUInt32Value(UNIT_FIELD_PET_NEXT_LEVEL_EXP);
    }
...
void Pet::GivePetLevel(uint8 level)
{
    ...
    if (getPetType()==HUNTER_PET)
    {
        ...
        SetUInt32Value(UNIT_FIELD_PET_NEXT_LEVEL_EXP, uint32(sObjectMgr->GetXPForLevel(level)*PET_XP_FACTOR));
    }
...
bool Pet::CreateBaseAtCreature(Creature* creature)
{
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("Pet (guidlow %d, entry %d) not created base at creature. Suggested coordinates isn't valid (X: %f Y: %f)",
            GetGUIDLow(), GetEntry(), GetPositionX(), GetPositionY());
        return false;
    }
    ...
    if (!cinfo)
    {
        sLog->OutErrorConsole("CreateBaseAtCreature() failed, creatureInfo is missing!");
        return false;
    }
...
bool Pet::CreateBaseAtTamed(CreatureTemplate const* cinfo, Map* map, uint32 phaseMask)
{
    ...
    SetPowerType(POWER_FOCUS);
    SetUInt32Value(UNIT_FIELD_PET_NAME_TIME_STAMP, 0);
    SetUInt32Value(UNIT_FIELD_PET_EXPERIENCE, 0);
    SetUInt32Value(UNIT_FIELD_PET_NEXT_LEVEL_EXP, uint32(sObjectMgr->GetXPForLevel(GetCurrentLevel()+1)*PET_XP_FACTOR));
    SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (IsPet() && m_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
        if ((m_owner->GetCurrentClass() == CLASS_WARLOCK)
			|| (m_owner->GetCurrentClass() == CLASS_SHAMAN)        // Fire Elemental
            || (m_owner->GetCurrentClass() == CLASS_DEATH_KNIGHT)) // Risen Ghoul
            petType = SUMMON_PET;
        else if (m_owner->GetCurrentClass() == CLASS_HUNTER)
        {
            petType = HUNTER_PET;
            m_unitTypeMask |= UNIT_MASK_HUNTER_PET;
        }
        else
            sLog->OutErrorConsole("Unknown type pet %u is summoned by player class %u", GetEntry(), m_owner->GetCurrentClass());
    }
    ...
    {
        float scale;
        if (GetCurrentLevel() >= cFamily->maxScaleLevel)
            scale = cFamily->maxScale;
        else if (GetCurrentLevel() <= cFamily->minScaleLevel)
            scale = cFamily->minScale;
        else
            scale = cFamily->minScale + float(GetCurrentLevel() - cFamily->minScaleLevel) / cFamily->maxScaleLevel * (cFamily->maxScale - cFamily->minScale);
	...
	if (IsPet() && m_owner->GetTypeId() == TYPE_ID_PLAYER && m_owner->GetCurrentClass() == CLASS_DEATH_KNIGHT)
    {
		...
		sLog->OutConsole("MV: %d; MR: %d; MM: %d; RM: %d", hasteMV, hasteRV, hasteMM, hasteRM);
    ...
    switch (petType)
    {
        case SUMMON_PET:
        {
            ...
            SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, float(petlevel - (petlevel / 4)));
            SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, float(petlevel + (petlevel / 4)));
        ...
        case HUNTER_PET:
        {
            SetUInt32Value(UNIT_FIELD_PET_NEXT_LEVEL_EXP, uint32(sObjectMgr->GetXPForLevel(petlevel)*PET_XP_FACTOR));
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case ENTRY_TREANT:
                {
					...
					SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, minDmg);
					SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, maxDmg);
                    break;
                }
                ...
                case ENTRY_FIRE_ELEMENTAL:
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, float(petlevel * 4 - petlevel));
                    SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, float(petlevel * 4 + petlevel));
                    break;
                }
                case ENTRY_SHADOWFIEND:
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, float((petlevel * 4 - petlevel) + bonus_dmg));
                    SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, float((petlevel * 4 + petlevel) + bonus_dmg));

                    break;
                }
                case ENTRY_VENOMOUS_SNAKE:
                {
                    SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, float((petlevel / 2) - 25));
                    SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, float((petlevel / 2) - 18));
                    break;
                }
				case ENTRY_VIPER:
                {
                    SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, float(petlevel / 2 - 10));
                    SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, float(petlevel / 2));
                    break;
                }
                case ENTRY_FERAL_SPIRIT:
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, float((petlevel * 4 - petlevel) + (m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * dmg_multiplier * 2 / 14)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, float((petlevel * 4 + petlevel) + (m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * dmg_multiplier * 2 / 14)));
					...
					if (!HasAura(61783)) // Feral Spirit Pet Scaling 04
						AddAura(61783, this);
                    break;
                }
                ...
                case ENTRY_BLOODWORM:
                {
					...
					SetBaseWeaponDamage(BASE_ATTACK, MIN_DAMAGE, float(petlevel - 30 - (petlevel / 4) + m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.006f));
					SetBaseWeaponDamage(BASE_ATTACK, MAX_DAMAGE, float(petlevel - 30 + (petlevel / 4) + m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.006f));
					break;
                }
				case ENTRY_RISEN_GHOUL:
				{
					CastSpell(this, 62137, true);
					CastSpell(this, 7398, true);
					break;
				}
				case ENTRY_ARMY_DEAD_GHOUL:
				{
					CastSpell(this, 62137, true);
					CastSpell(this, 7398, true);
					break;
...
uint32 Pet::GetCurrentFoodBenefitLevel(uint32 itemlevel)
{
    ...
    if (GetCurrentLevel() <= itemlevel + 5)                         //possible to feed level 60 pet with level 55 level food for full effect
        return 35000;
    // -10..-6
    else if (GetCurrentLevel() <= itemlevel + 10)                   //pure guess, but sounds good
        return 17000;
    // -14..-11
    else if (GetCurrentLevel() <= itemlevel + 14)                   //level 55 food gets green on 70, makes sense to me
        return 8000;
    // -15 or less
    else
...
void Pet::_LoadSpellCooldowns()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                sLog->OutErrorConsole("Pet %u have unknown spell %u in `pet_spell_cooldown`, skipping.", m_charmInfo->GetPetNumber(), spell_id);
                continue;
            }
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            sLog->OutErrorConsole("Pet::addSpell: Non-existed in SpellStore spell #%u request, deleting for all pets in `pet_spell`.", spellId);
            ...
            sLog->OutErrorConsole("Pet::addSpell: Non-existed in SpellStore spell #%u request.", spellId);
    ...
    if (talentCost)
    {
        int32 free_points = GetMaxTalentPointsForLevel(GetCurrentLevel());
...
bool Pet::removeSpell(uint32 spell_id, bool learn_prev, bool clear_ab)
{
    ...
    if (talentCost > 0)
    {
        if (m_usedTalentCount > talentCost)
            ...
            m_usedTalentCount = 0;
        // update free talent points
        int32 free_points = GetMaxTalentPointsForLevel(GetCurrentLevel()) - m_usedTalentCount;
        SetFreeTalentPoints(free_points > 0 ? free_points : 0);
    }
...
void Pet::SynchronizeLevelWithOwner()
{
    ...
    switch (getPetType())
    {
        // always same level
        case SUMMON_PET:
            GivePetLevel(owner->GetCurrentLevel());
            break;
        // can't be greater owner level
        case HUNTER_PET:
            if (GetCurrentLevel() > owner->GetCurrentLevel())
                GivePetLevel(owner->GetCurrentLevel());
            else if (GetCurrentLevel() + 5 < owner->GetCurrentLevel())
                GivePetLevel(owner->GetCurrentLevel() - 5);
            break;
        default:
            break;
    }
