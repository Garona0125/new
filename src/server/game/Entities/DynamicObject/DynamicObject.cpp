...
{
    m_objectType |= TYPE_MASK_DYNAMIC_OBJECT;
    m_objectTypeId = TYPE_ID_DYNAMIC_OBJECT;
...
bool DynamicObject::CreateDynamicObject(uint32 guidlow, Unit* caster, uint32 spellId, Position const& pos, float radius, DynamicObjectType type)
{
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("DynamicObject (spell %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)", spellId, GetPositionX(), GetPositionY());
        return false;
    }
