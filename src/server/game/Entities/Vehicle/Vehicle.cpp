...
Vehicle::~Vehicle()
{
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
    {
        if (itr->second.Passenger)
            sLog->OutErrorConsole("Vehicle::~Vehicle() | passenger (%s), Base (%s), BaseEntry (%u), MapId (%u)", ObjectAccessor::GetUnit(*GetBase(), itr->second.Passenger) ? ObjectAccessor::GetUnit(*GetBase(), itr->second.Passenger)->GetName() : "n/a", GetBase()->GetName(), GetBase()->GetEntry(), GetBase()->GetMapId());
...
void Vehicle::Install()
{
    ...
    if (GetBase()->GetTypeId() == TYPE_ID_UNIT)
        sScriptMgr->OnInstall(this);
}

void Vehicle::InstallAllAccessories(bool evading)
{
    if (GetBase()->GetTypeId() == TYPE_ID_PLAYER || !evading)
...
void Vehicle::Reset(bool evading /*= false*/)
{
    ...
    if (_me->GetTypeId() == TYPE_ID_PLAYER)
    {
        if (_usableSeatNum)
    ...
    else
    {
        ...
        if (_usableSeatNum)
            _me->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELL_CLICK);
    }
...
void Vehicle::ApplyAllImmunities()
{
    ...
    if (_me->ToCreature() && _me->ToCreature()->GetCreatureTemplate()->type == CREATURE_TYPE_MECHANICAL && !_me->ToCreature()->isWorldBoss())
    {
        ...
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_PERIODIC_HEAL, true);
        // ... Shield & Immunity grant spells ...
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_SCHOOL_IMMUNITY, true);
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_MOD_UNATTACKABLE, true);
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_SCHOOL_ABSORB, true);
        ...
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_DAMAGE_SHIELD, true);
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_SPLIT_DAMAGE_PCT, true);
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_MOD_RESISTANCE, true);
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_MOD_STAT, true);
        _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, true);
    }
    ...
    switch (GetVehicleInfo()->m_ID)
    {
		...
		case 554: // Alliance and Horde Gunship Cannon
            _me->SetControlled(true, UNIT_STATE_ROOT);
            // why we need to apply this? we can simple add immunities to slow mechanic in DB
            _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_MOD_DECREASE_SPEED, true);
            break;
        case 303:
        ...
        case 337:
            _me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, false);
            _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_SCHOOL_ABSORB, false);
            _me->ApplySpellImmune(0, IMMUNITY_AURA_TYPE, SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, false);
            break;
        case 335:
        case 338:
            _me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, false);
...
void Vehicle::InstallAccessory(uint32 entry, int8 seatId, bool minion, uint8 type, uint32 summonTime)
{
    ...
    if (Unit* passenger = GetPassenger(seatId))
    {
        ...
        {
            ASSERT(passenger->GetTypeId() == TYPE_ID_UNIT);
            if (_me->GetTypeId() == TYPE_ID_UNIT)
            {
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (seat->second.SeatInfo->CanEnterOrExit())
    {
        ...
        if (!_usableSeatNum)
        {
            if (_me->GetTypeId() == TYPE_ID_PLAYER)
                ...
                _me->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELL_CLICK);
        }
    }
    ...
    unit->AddUnitMovementFlag(MOVEMENTFLAG_ON_TRANSPORT);
    ...
    if (_me->GetTypeId() == TYPE_ID_UNIT && unit->GetTypeId() == TYPE_ID_PLAYER && seat->first == 0 && seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
    {
...
void Vehicle::RemovePassenger(Unit* unit)
{
    ...
    if (_me->IsInWorld())
    {
        unit->RemoveUnitMovementFlag(MOVEMENTFLAG_ON_TRANSPORT);
        unit->m_movementInfo.t_pos.Relocate(0, 0, 0, 0);
        unit->m_movementInfo.t_time = 0;
        unit->m_movementInfo.t_seat = 0;
    }

    if (_me->GetTypeId() == TYPE_ID_UNIT && _me->ToCreature()->IsAIEnabled)
    ...
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
	{
        if (Unit* passenger = ObjectAccessor::GetUnit(*GetBase(), itr->second.Passenger))
        {
            ...
            if (!passenger->IsOnVehicle(GetBase()))
                sLog->OutErrorConsole("Vehicle::RelocatePassengers() | passenger (%s) Base (%s) BaseEntry (%u) MapId (%u)", passenger->GetName(), GetBase()->GetName(), GetBase()->GetEntry(), passenger->GetMapId());
...
void Vehicle::Dismiss()
{
    ...
    Uninstall();
	GetBase()->ToCreature()->DespawnAfterAction();
}
...
void Vehicle::Relocate(Position pos)
{
    ...
    for (std::set<Unit*>::const_iterator itr = vehiclePlayers.begin(); itr != vehiclePlayers.end(); ++itr)
    {
        if (Unit* player = (*itr))
        {
            ...
            //player->TeleportTo(player->GetMapId(), triggerPos.GetPositionX(), triggerPos.GetPositionY(), triggerPos.GetPositionZ(), triggerPos.GetOrientation(), TELE_TO_NOT_LEAVE_COMBAT);
        }
    }
