...
void AddItemsSetItem(Player* player, Item* item)
{
    ...
    if (!set)
    {
        sLog->OutErrorDatabase("Item set %u for item (id %u) not found, mods not applied.", setid, proto->ItemId);
        return;
    }
    ...
    for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; ++x)
    {
        ...
        for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y)
        {
            if (!eff->spells[y])                             // free slot
            {
                ...
                if (!spellInfo)
                {
                    sLog->OutErrorConsole("WORLD: unknown spell id %u in items set %u effects", set->spells[x], setid);
                    break;
                }
...
void RemoveItemsSetItem(Player*player, ItemTemplate const* proto)
{
    ...
    if (!set)
    {
        sLog->OutErrorDatabase("Item set #%u for item #%u not found, mods not removed.", setid, proto->ItemId);
        return;
    }
...
Item::Item()
{
    m_objectType |= TYPE_MASK_ITEM;
    m_objectTypeId = TYPE_ID_ITEM;
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id)
{
    ...
    if ((itemProto->RandomProperty) && (itemProto->RandomSuffix))
    {
        sLog->OutErrorDatabase("Item template %u have RandomProperty == %u and RandomSuffix == %u, but must have one from field =0", itemProto->ItemId, itemProto->RandomProperty, itemProto->RandomSuffix);
        return 0;
    }
    ...
    if (itemProto->RandomProperty)
    {
        ...
        if (!random_id)
        {
            sLog->OutErrorDatabase("Enchantment id #%u used but it doesn't have records in 'ItemRandomProperties.dbc'", randomPropId);
            return 0;
        }
    ...
    else
    {
        ...
        if (!random_id)
        {
            sLog->OutErrorDatabase("Enchantment id #%u used but it doesn't have records in sItemRandomSuffixStore.", randomPropId);
            return 0;
        }
...
bool Item::IsFitToSpellRequirements(SpellInfo const* spellInfo) const
{
    ...
    {
        if (proto->InventoryType == INVENTORY_TYPE_WEAPON &&
            (spellInfo->EquippedItemInventoryTypeMask & (1 << INVENTORY_TYPE_WEAPON_MAIN_HAND) ||
             spellInfo->EquippedItemInventoryTypeMask & (1 << INVENTORY_TYPE_WEAPON_OFF_HAND)))
            return true;
...
bool Item::DeleteFakeEntry()
{
    if (!GetFakeEntry())
        return false;
    GetOwner()->UpdateUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRY_ID + (GetSlot() * 2), GetEntry());
...
{
    GetOwner()->UpdateUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRY_ID + (GetSlot() * 2), entry);
    sObjectMgr->_itemFakeEntryStore[GetGUIDLow()] = entry;
    CharacterDatabase.PExecute("REPLACE INTO character_transmogrification_items (GUID, FakeEntry) VALUES (%u, %u)", GetGUIDLow(), entry);
}
