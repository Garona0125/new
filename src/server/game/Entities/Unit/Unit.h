...
#define BASE_MIN_DAMAGE 1.0f
#define BASE_MAX_DAMAGE 2.0f
...
enum VictimState
{
    VICTIM_STATE_INTACT         = 0, // set when attacker misses
    VICTIM_STATE_HIT            = 1, // victim got clear/blocked hit
    VICTIM_STATE_DODGE          = 2,
    VICTIM_STATE_PARRY          = 3,
    VICTIM_STATE_INTERRUPT      = 4,
    VICTIM_STATE_BLOCKS         = 5, // unused? not set when blocked, even on full block
...
enum UnitMods
{
    ...
    UNIT_MOD_DAMAGE_MAIN_HAND,
    UNIT_MOD_DAMAGE_OFF_HAND,
...
enum UnitState
{
    ...
    UNIT_STATE_UNATTACKABLE    = (UNIT_STATE_IN_FLIGHT | UNIT_STATE_ON_VEHICLE),
...
enum UnitFlags
{
    ...
    UNIT_FLAG_NO_AGGRO_FOR_PLAYER   = 0x00000100,           // disables combat/assistance with PlayerCharacters (PC) - see Unit::_IsValidAttackTarget, Unit::_IsValidAssistTarget
    UNIT_FLAG_NO_AGGRO_FOR_CREATURE = 0x00000200,           // disables combat/assistance with NonPlayerCharacters (NPC) - see Unit::_IsValidAttackTarget, Unit::_IsValidAssistTarget
    UNIT_FLAG_LOOTING               = 0x00000400,           // loot animation
    UNIT_FLAG_PET_IN_COMBAT         = 0x00000800,           // in combat?, 2.0.8
    UNIT_FLAG_PLAYER_VS_PLAYER      = 0x00001000,           // changed in 3.0.3
    UNIT_FLAG_SILENCED              = 0x00002000,           // silenced, 2.1.1
    UNIT_FLAG_UNK_14                = 0x00004000,           // 2.0.8
    UNIT_FLAG_SWIMMING              = 0x00008000,           // For Swimming Creatures in Water
    ...
    UNIT_FLAG_FEIGN_DEATH           = 0x20000000,           // used in Feign Death spell
...
enum UnitFlags2
{
    ...
    UNIT_FLAG2_DISARM_OFF_HAND              = 0x00000080,
...
enum NPCFlags
{
    ...
    UNIT_NPC_FLAG_QUEST_GIVER           = 0x00000002,       // guessed, probably ok
    ...
    UNIT_NPC_FLAG_FLIGHT_MASTER         = 0x00002000,       // 100%
    UNIT_NPC_FLAG_SPIRIT_HEALER         = 0x00004000,       // guessed
    UNIT_NPC_FLAG_SPIRIT_GUIDE          = 0x00008000,       // guessed
    UNIT_NPC_FLAG_INNKEEPER             = 0x00010000,       // 100%
    UNIT_NPC_FLAG_BANKER                = 0x00020000,       // 100%
    UNIT_NPC_FLAG_PETITIONER            = 0x00040000,       // 100% 0xC0000 = guild petitions, 0x40000 = arena team petitions
    UNIT_NPC_FLAG_TABARD_DESIGNER       = 0x00080000,       // 100%
    UNIT_NPC_FLAG_BATTLE_MASTER         = 0x00100000,       // 100%
    UNIT_NPC_FLAG_AUCTIONEER            = 0x00200000,       // 100%
    UNIT_NPC_FLAG_STABLE_MASTER         = 0x00400000,       // 100%
    UNIT_NPC_FLAG_GUILD_BANKER          = 0x00800000,       // cause client to send 997 opcode
    UNIT_NPC_FLAG_SPELL_CLICK           = 0x01000000,       // cause client to send 1015 opcode (spell click)
...
enum MovementFlags
{
    ...
    MOVEMENTFLAG_ON_TRANSPORT          = 0x00000200,               // Used for flying on some creatures
    ...
    MOVEMENTFLAG_WATER_WALKING         = 0x10000000,               // prevent unit from falling through water
    MOVEMENTFLAG_FALLING_SLOW          = 0x20000000,               // active rogue safe fall spell (passive)
    MOVEMENT_FLAG_HOVER                = 0x40000000,               // hover, cannot jump
...
private:
    ...
    SpellSchoolMask const m_schoolMask;
public:
    explicit HealInfo(Unit* _healer, Unit* _target, uint32 _heal, SpellInfo const* _spellInfo, SpellSchoolMask _schoolMask): m_healer(_healer), m_target(_target), m_heal(_heal), m_spellInfo(_spellInfo), m_schoolMask(_schoolMask)
    {
        m_absorb = 0;
    }
...
{
    SpellNonMeleeDamage(Unit* _attacker, Unit* _target, uint32 _SpellID, uint32 _schoolMask): target(_target), attacker(_attacker), SpellID(_SpellID), damage(0), overkill(0), schoolMask(_schoolMask), absorb(0), resist(0), physicalLog(false), unused(false), blocked(0), HitInfo(0), cleanDamage(0){}
...
{
    SpellPeriodicAuraLogInfo(AuraEffect const* _auraEff, uint32 _damage, uint32 _overDamage, uint32 _absorb, uint32 _resist, float _multiplier, bool _critical): auraEff(_auraEff), damage(_damage), overDamage(_overDamage), absorb(_absorb), resist(_resist), multiplier(_multiplier), critical(_critical){}
...
class Unit : public WorldObject
{
    public:
        ...
        uint8 GetCurrentLevel() const { return uint8(GetUInt32Value(UNIT_FIELD_LEVEL)); }
        uint8 GetCurrentLevelForTarget(WorldObject const* /*target*/) const { return GetCurrentLevel(); }
        void SetLevel(uint8 lvl);
        uint8 GetCurrentRace() const { return GetByteValue(UNIT_FIELD_BYTES_0, 0); }
        uint32 GetCurrentRaceMask() const { return 1 << (GetCurrentRace()-1); }
        uint8 GetCurrentClass() const { return GetByteValue(UNIT_FIELD_BYTES_0, 1); }
        uint32 GetCurrentClassMask() const { return 1 << (GetCurrentClass()-1); }
        uint8 GetCurrentGender() const { return GetByteValue(UNIT_FIELD_BYTES_0, 2); }
        ...
        uint32 GetPower(Powers power) const { return GetUInt32Value(UNIT_FIELD_POWER_1 + power); }
        uint32 GetMaxPower(Powers power) const { return GetUInt32Value(UNIT_FIELD_MAX_POWER_1 + power); }
        ...
        {
           float f_BaseAttackTime = GetFloatValue(UNIT_FIELD_BASE_ATTACK_TIME+att) / m_modAttackSpeedPct[att];
           return (uint32)f_BaseAttackTime;
        }

        void SetAttackTime(WeaponAttackType att, uint32 val) { SetFloatValue(UNIT_FIELD_BASE_ATTACK_TIME+att, val*m_modAttackSpeedPct[att]); }
        ...
        void SetCurrentFaction(uint32 faction) { SetUInt32Value(UNIT_FIELD_FACTION_TEMPLATE, faction); }
        ...
        uint16 GetMaxSkillValueForLevel(Unit const* target = NULL) const { return (target ? GetCurrentLevelForTarget(target) : GetCurrentLevel()) * 5; }
        ...
        bool CanUseAttackType(uint8 attacktype) const
        {
            switch (attacktype)
            {
                ...
                case OFF_ATTACK:
					return !HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISARM_OFF_HAND);
                case RANGED_ATTACK:
        ...
        uint32 GetUnitMeleeSkill(Unit const* target = NULL) const { return (target ? GetCurrentLevelForTarget(target) : GetCurrentLevel()) * 5; }
        ...
        bool IsQuestGiver() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUEST_GIVER); }
        bool IsGossip() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP); }
        bool IsTaxi() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_FLIGHT_MASTER); }
        bool IsGuildMaster() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_PETITIONER); }
        bool IsBattleMaster() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_BATTLE_MASTER); }
        bool IsBanker() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_BANKER); }
        bool IsInnkeeper() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_INNKEEPER); }
        bool IsSpiritHealer() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPIRIT_HEALER); }
        bool IsSpiritGuide() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPIRIT_GUIDE); }
        bool IsTabardDesigner()const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_TABARD_DESIGNER); }
        bool IsAuctioner() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_AUCTIONEER); }
        bool IsArmorer() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_REPAIR); }
        bool IsServiceProvider() const
        {
            return HasFlag(UNIT_NPC_FLAGS,
                UNIT_NPC_FLAG_VENDOR | UNIT_NPC_FLAG_TRAINER | UNIT_NPC_FLAG_FLIGHT_MASTER |
                UNIT_NPC_FLAG_PETITIONER | UNIT_NPC_FLAG_BATTLE_MASTER | UNIT_NPC_FLAG_BANKER |
        ...
        bool IsSpiritService() const { return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPIRIT_HEALER | UNIT_NPC_FLAG_SPIRIT_GUIDE); }

        bool IsInFlight() const { return HasUnitState(UNIT_STATE_IN_FLIGHT); }

        bool IsInCombatActive() const { return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IN_COMBAT); }
		...
		*/
		...
		bool IsHovering() const { return m_movementInfo.HasMovementFlag(MOVEMENT_FLAG_HOVER); }
        ...
        {
            for (uint8 i = STAT_STRENGTH; i < MAX_STATS; ++i) SetFloatValue(UNIT_FIELD_POS_STAT_0+i, 0);
            for (uint8 i = STAT_STRENGTH; i < MAX_STATS; ++i) SetFloatValue(UNIT_FIELD_NEG_STAT_0+i, 0);
        }
        void ApplyStatBuffMod(Stats stat, float val, bool apply) { ApplyModSignedFloatValue((val > 0 ? UNIT_FIELD_POS_STAT_0+stat : UNIT_FIELD_NEG_STAT_0+stat), val, apply); }
        void ApplyStatPercentBuffMod(Stats stat, float val, bool apply)
        {
            ApplyPercentModFloatValue(UNIT_FIELD_POS_STAT_0+stat, val, apply);
            ApplyPercentModFloatValue(UNIT_FIELD_NEG_STAT_0+stat, val, apply);
        }
        ...
        float GetPosStat(Stats stat) const { return GetFloatValue(UNIT_FIELD_POS_STAT_0+stat); }
        float GetNegStat(Stats stat) const { return GetFloatValue(UNIT_FIELD_NEG_STAT_0+stat); }
