...
#endif
Unit::Unit(bool isWorldObject): WorldObject(isWorldObject), m_movedPlayer(NULL), m_lastSanctuaryTime(0),
m_TempSpeed(0.0f), IsAIEnabled(false), NeedChangeAI(false), LastCharmerGUID(0), m_ControlledByPlayer(false),
movespline(new Movement::MoveSpline()), i_AI(NULL), i_disabledAI(NULL), m_AutoRepeatFirstCast(false),
m_procDeep(0), m_removedAurasCount(0), i_motionMaster(this), m_ThreatManager(this), m_vehicle(NULL),
m_vehicleKit(NULL), m_unitTypeMask(UNIT_MASK_NONE), m_HostileRefManager(this), _lastDamagedTime(0)
{
#ifdef _MSC_VER
#pragma warning(default:4355)
#endif
    m_objectType |= TYPE_MASK_UNIT;
    m_objectTypeId = TYPE_ID_UNIT;
    ...
    m_auraModifiersGroup[UNIT_MOD_DAMAGE_OFF_HAND][TOTAL_PCT] = 0.5f;

    for (uint8 i = 0; i < MAX_ATTACK; ++i)
    {
        m_weaponDamage[i][MIN_DAMAGE] = BASE_MIN_DAMAGE;
        m_weaponDamage[i][MAX_DAMAGE] = BASE_MAX_DAMAGE;
    }
...
void Unit::Update(uint32 p_time)
{
    ...
    if (IsInCombatActive() && (GetTypeId() == TYPE_ID_PLAYER || (ToCreature()->IsPet() && IsControlledByPlayer())))
    {
...
void Unit::UpdateSplineMovement(uint32 t_diff)
{
    ...
    if (m_movesplineTimer.Passed() || arrived)
    {
        ...
        if (HasUnitMovementFlag(MOVEMENTFLAG_ON_TRANSPORT))
        {
...
{
    if (!victim || !victim->IsAlive() || victim->HasUnitState(UNIT_STATE_IN_FLIGHT) || (victim->GetTypeId() == TYPE_ID_UNIT && victim->ToCreature()->IsInEvadeMode()))
    {
		if (absorb)
			*absorb += damage;
		damage = 0;
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (victim->GetTypeId() == TYPE_ID_UNIT && victim->ToCreature()->IsAIEnabled)
    ...
    if (damagetype != NODAMAGE)
    {
		...
		{
			if (victim != this && victim->GetTypeId() == TYPE_ID_PLAYER)
			{
	...
	// Damage checker
	if (GetTypeId() == TYPE_ID_PLAYER && sWorld->getIntConfig(CONFIG_DAMAGE_CHECKER_MAX_DAMAGE) > 0
		...
		&& (uint8)ToPlayer()->GetSession()->GetSecurity() < sWorld->getIntConfig(CONFIG_DAMAGE_CHECKER_IMMUNE_MIN_GM_LEVEL))
	{
		...
		sLog->OutConsole("Damage Checker: Player name '%s', damage %u", ToPlayer()->GetName(), damage);
	}
    ...
    if (victim->GetTypeId() == TYPE_ID_PLAYER && victim->ToPlayer()->duel && damage >= (health-1))
    {
	...
	else if (victim->IsVehicle() && damage >= (health-1) && victim->GetCharmer() && victim->GetCharmer()->GetTypeId() ==  TYPE_ID_PLAYER)
	{
    ...
    if (GetTypeId() == TYPE_ID_PLAYER && this != victim)
    {
    ...
    else if (!victim->IsControlledByPlayer() || victim->IsVehicle())
    {
		...
		if (IsControlledByPlayer() || (ToTempSummon() && ToTempSummon()->GetSummoner() && ToTempSummon()->GetSummoner()->GetTypeId() == TYPE_ID_PLAYER))
    ...
    if (health <= damage)
    {
        ...
        if (victim->GetTypeId() == TYPE_ID_PLAYER && victim != this)
    ...
    else
    {
        ...
        {
            if (victim != this && victim->GetTypeId() == TYPE_ID_PLAYER && // does not support creature push_back
...
void Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    if (!spellInfo)
    {
        sLog->OutErrorConsole("CastSpell: unknown spell by caster: %s %u)", (GetTypeId() == TYPE_ID_PLAYER ? "player (GUID:" : "creature (Entry:"), (GetTypeId() == TYPE_ID_PLAYER ? GetGUIDLow() : GetEntry()));
        return;
    }
...
void Unit::CastSpell(Unit* victim, uint32 spellId, TriggerCastFlags triggerFlags /*= TRIGGER_NONE*/, Item* castItem /*= NULL*/, AuraEffect const* triggeredByAura /*= NULL*/, uint64 originalCaster /*= 0*/)
{
    ...
    if (!spellInfo)
    {
        sLog->OutErrorDatabase("CastSpell: unknown spell id %u by caster: %s %u)", spellId, (GetTypeId() == TYPE_ID_PLAYER ? "player (GUID:" : "creature (Entry:"), (GetTypeId() == TYPE_ID_PLAYER ? GetGUIDLow() : GetEntry()));
        return;
    }
...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss)
{
    ...
    if (!victim->IsAlive() || victim->HasUnitState(UNIT_STATE_IN_FLIGHT) || (victim->GetTypeId() == TYPE_ID_UNIT && victim->ToCreature()->IsInEvadeMode()))
        return;
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    if (damageInfo->target->IsImmunedToDamage(SpellSchoolMask(damageInfo->damageSchoolMask)))
    {
       ...
       damageInfo->TargetState = VICTIM_STATE_IS_IMMUNE;
    ...
    switch (damageInfo->hitOutCome)
    {
        case MELEE_HIT_EVADE:
			...
			damageInfo->TargetState     = VICTIM_STATE_EVADES;
        ...
        case MELEE_HIT_MISS:
			...
			damageInfo->TargetState     = VICTIM_STATE_INTACT;
			damageInfo->procEx         |= PROC_EX_MISS;
			damageInfo->damage          = 0;
			damageInfo->cleanDamage     = 0;
            break;
        case MELEE_HIT_NORMAL:
			damageInfo->TargetState     = VICTIM_STATE_HIT;
        ...
        case MELEE_HIT_PARRY:
            damageInfo->TargetState  = VICTIM_STATE_PARRY;
            damageInfo->procEx      |= PROC_EX_PARRY;
            damageInfo->cleanDamage += damageInfo->damage;
            damageInfo->damage = 0;
            break;
        case MELEE_HIT_DODGE:
            damageInfo->TargetState  = VICTIM_STATE_DODGE;
        ...
        case MELEE_HIT_BLOCK:
            ...
            {
                damageInfo->TargetState = VICTIM_STATE_BLOCKS;
        ...
        case MELEE_HIT_GLANCING:
        {
            ...
            damageInfo->TargetState  = VICTIM_STATE_HIT;
            damageInfo->procEx      |= PROC_EX_NORMAL_HIT;
            int32 leveldif = int32(victim->GetCurrentLevel()) - int32(GetCurrentLevel());
            if (leveldif > 3)
                leveldif = 3;
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->blocked_amount && damageInfo->TargetState != VICTIM_STATE_BLOCKS)
        victim->HandleEmoteCommand(EMOTE_ONESHOT_PARRY_SHIELD);

    if (damageInfo->TargetState == VICTIM_STATE_PARRY)
    {
        // Get attack timers
    ...
    if ((damageInfo->hitOutCome == MELEE_HIT_CRIT || damageInfo->hitOutCome == MELEE_HIT_CRUSHING || damageInfo->hitOutCome == MELEE_HIT_NORMAL || damageInfo->hitOutCome == MELEE_HIT_GLANCING) &&
        GetTypeId() != TYPE_ID_PLAYER && !ToCreature()->IsControlledByPlayer() && !victim->HasInArc(M_PI, this)
        && (victim->GetTypeId() == TYPE_ID_PLAYER || !victim->ToCreature()->isWorldBoss())&& !victim->IsVehicle())
    {
        ...
        if (victim->GetCurrentLevel() < 30)
            Probability = 0.65f * victim->GetCurrentLevel() + 0.5f;
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    if (GetTypeId() == TYPE_ID_PLAYER)
    {
        ...
        float maxArmorPen = 0;
        if (victim->GetCurrentLevel() < 60)
            maxArmorPen = float(400 + 85 * victim->GetCurrentLevel());
        else
            maxArmorPen = 400 + 85 * victim->GetCurrentLevel() + 4.5f * 85 * (victim->GetCurrentLevel() - 59);
    ...
    float levelModifier = GetCurrentLevel();
    if (levelModifier > 59)
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
	...
	if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0 && (!spellInfo || (spellInfo->AttributesEx4 & SPELL_ATTR4_IGNORE_RESISTANCES) == 0))
    {
        ...
        uint32 level = victim->GetCurrentLevel();
...
{
    if (victim->GetTypeId() == TYPE_ID_UNIT && victim->ToCreature()->IsInEvadeMode())
    ...
    if (victim->GetTypeId() == TYPE_ID_PLAYER && crit_chance > 0 && !victim->IsStandState())
    {
    ...
    if (victim->GetTypeId() == TYPE_ID_PLAYER && !victim->HasInArc(M_PI, this) && !victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION))
    {
    ...
    else
    {
        ...
        dodge_chance += GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_COMBAT_RESULT_CHANCE, VICTIM_STATE_DODGE) * 100;
    ...
    else
    {
        ...
        if (victim->GetTypeId() == TYPE_ID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY))
        {
        ...
        if (victim->GetTypeId() == TYPE_ID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_BLOCK))
        {
            tmp = block_chance;
    ...
    if (tmp > 0 && roll < (sum += tmp))
    {
        ...
        if (GetTypeId() == TYPE_ID_UNIT && (ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_CRIT))
    ...
    if (attType != RANGED_ATTACK &&
        (GetTypeId() == TYPE_ID_PLAYER || ToCreature()->IsPet()) &&
        victim->GetTypeId() != TYPE_ID_PLAYER && !victim->ToCreature()->IsPet() &&
        GetCurrentLevel() < victim->GetCurrentLevelForTarget(this))
    {
    ...
    if (GetCurrentLevelForTarget(victim) >= victim->GetCurrentLevelForTarget(this) + 4 &&
        // can be from by creature (if can) or from controlled player that considered as creature
        !IsControlledByPlayer() &&
        !(GetTypeId() == TYPE_ID_UNIT && ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_CRUSH))
    {
...
uint32 Unit::CalculateDamage(WeaponAttackType attType, bool normalized, bool addTotalPct)
{
    ...
    if (GetTypeId() == TYPE_ID_PLAYER && (normalized || !addTotalPct))
    ...
    else
    {
        switch (attType)
        {
            ...
            case BASE_ATTACK:
                ...
                break;
            case OFF_ATTACK:
                minDamage = GetFloatValue(UNIT_FIELD_MIN_DAMAGE); // UNIT_FIELD_MIN_OFF_HAND_DAMAGE
                maxDamage = GetFloatValue(UNIT_FIELD_MAX_DAMAGE); // UNIT_FIELD_MAX_OFF_HAND_DAMAGE
                break;
...
float Unit::CalculateLevelPenalty(SpellInfo const* spellProto) const
{
    ...
    float LvlFactor = (float(spellProto->SpellLevel) + 6.0f) / float(GetCurrentLevel());
    if (LvlFactor > 1.0f)
        LvlFactor = 1.0f;
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    if (victim)
        sLog->outDetail("%s %u stopped attacking %s %u", (GetTypeId() == TYPE_ID_PLAYER ? "Player" : "Creature"), GetGUIDLow(), (victim->GetTypeId() == TYPE_ID_PLAYER ? "player" : "creature"), victim->GetGUIDLow());
    else
        sLog->outDetail("%s %u stopped attacking", (GetTypeId() == TYPE_ID_PLAYER ? "Player" : "Creature"), GetGUIDLow());
}
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (spell->DmgClass == SPELL_DAMAGE_CLASS_RANGED && !spell->IsRangedWeaponSpell())
        attackerWeaponSkill = GetCurrentLevel() * 5;
    ...
    if (canDodge)
    {
        ...
        dodgeChance += GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_COMBAT_RESULT_CHANCE, VICTIM_STATE_DODGE) * 100;
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if ((!victim->IsAlive() && victim->GetTypeId() != TYPE_ID_PLAYER) || spell->AttributesEx3 & SPELL_ATTR3_IGNORE_HIT_RESULT)
        return SPELL_MISS_NONE;

    SpellSchoolMask schoolMask = spell->GetSchoolMask();
    // PvP - PvE spell misschances per leveldif > 2
    int32 lchance = victim->GetTypeId() == TYPE_ID_PLAYER ? 7 : 11;
    int32 thisLevel = GetCurrentLevelForTarget(victim);
    if (GetTypeId() == TYPE_ID_UNIT && ToCreature()->isTrigger())
        thisLevel = std::max<int32>(thisLevel, spell->SpellLevel);
    int32 leveldif = int32(victim->GetCurrentLevelForTarget(this)) - thisLevel;
    ...
    //if (spell->IsTargetingArea()) // Need Test
...
uint32 Unit::GetDefenseSkillValue(Unit const* target) const
{
    if (GetTypeId() == TYPE_ID_PLAYER)
    {
        ...
        uint32 value = (target && target->GetTypeId() == TYPE_ID_PLAYER)
...
float Unit::GetUnitParryChance() const
{
    ...
    else if (GetTypeId() == TYPE_ID_UNIT)
    {
...
float Unit::GetUnitBlockChance() const
{
    ...
    if (Player const* player = ToPlayer())
    {
        if (player->CanBlock())
        {
            Item* tmpitem = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
...
void Unit::_UpdateAutoRepeatSpell()
{
    ...
    if ((GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->isMoving()) || IsNonMeleeSpellCasted(false, false, true, m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id == 75))
    {
        // cancel wand shoot
...
AuraApplication * Unit::_CreateAuraApplication(Aura* aura, uint8 effMask)
{
    ...
    if (!IsAlive() && !aurSpellInfo->IsDeathPersistent() &&
        (GetTypeId() != TYPE_ID_PLAYER || !ToPlayer()->GetSession()->PlayerLoading()))
        return NULL;
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
	...
	//if (aurApp->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE && GetTypeId() == TYPE_ID_UNIT && ToCreature()->IsTotem()&& ToTotem()->GetSummonerGUID() == aura->GetCasterGUID())
	if (aurApp->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE && GetTypeId() == TYPE_ID_UNIT && ToCreature()->IsTotem())
    {
...
int32 Unit::GetTotalAuraModifierByMiscValue(AuraType auratype, int32 misc_value, SpellInfo const * spell) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == misc_value)
            if (!(spell && spell->SpellFamilyName == SPELL_FAMILY_PRIEST && spell->SpellIconID == 548 && (*i)->GetId() == 58943)) // Mind Flay & Da Voodoo Shuffle
                modifier += (*i)->GetAmount();
    }
    return modifier;
}
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if (GetTypeId() == TYPE_ID_PLAYER && gameObj->GetSpellId())
    {
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    switch (aura->GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_MANA_LEECH:
            ...
            break;
        default:
            sLog->OutErrorConsole("Unit::SendPeriodicAuraLog: unknown aura %u", uint32(aura->GetAuraType()));
            return;
    }
...
bool Unit::HandleHasteAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    Item* castItem = triggeredByAura->GetBase()->GetCastItemGUID() && GetTypeId() == TYPE_ID_PLAYER
    ...
    if (!triggerEntry)
    {
        sLog->OutErrorConsole("Unit::HandleHasteAuraProc: Spell %u has non-existing triggered spell %u", hasteSpell->Id, triggered_spell_id);
        return false;
    }

    if (cooldown && GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->HasSpellCooldown(triggered_spell_id))
        return false;
    ...
    if (cooldown && GetTypeId() == TYPE_ID_PLAYER)
...
bool Unit::HandleSpellCritChanceAuraProc(Unit* victim, uint32 /*damage*/, AuraEffect* triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    if (!triggerEntry)
    {
        sLog->OutErrorConsole("Unit::HandleHasteAuraProc: Spell %u has non-existing triggered spell %u", triggeredByAuraSpell->Id, triggered_spell_id);
        return false;
    }
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELL_FAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 28764:
                {
                    ...
                    // find Mage Armor
                    if (!GetAuraEffect(SPELL_AURA_MOD_MANA_REGEN_INTERRUPT, SPELL_FAMILY_MAGE, 0x10000000, 0, 0))
                        return false;
                ...
                case 39446:
                {
                    if (GetTypeId() != TYPE_ID_PLAYER || !IsAlive())
                        return false;
            ...
            if (dummySpell->SpellIconID == 2120)
            {
                ...
                switch (dummySpell->Id)
                {
                    ...
                    case 31572:
						...
						break;
                    default:
                        sLog->OutErrorConsole("Unit::HandleDummyAuraProc: non handled spell id: %u", dummySpell->Id);
                        return false;
                }
                break;
            }
            ...
            switch (dummySpell->Id)
            {
                ...
                case 12848:
                {
                    switch (dummySpell->Id)
                    {
                        ...
                        case 12848:
							...
							break;
                        default:
                            sLog->OutErrorConsole("Unit::HandleDummyAuraProc: non handled spell id: %u (IG)", dummySpell->Id);
                            return false;
                    }
                ...
                case 56372:
                {
                    ...
                    for (SpellCooldowns::const_iterator itr = cooldowns.begin(); itr != cooldowns.end(); ++itr)
                    {
                        ...
                        // Frost Nova
                        if (cdSpell && cdSpell->SpellFamilyName == SPELL_FAMILY_MAGE && cdSpell->SpellFamilyFlags[0] & 0x00000040)
            ...
            if (dummySpell->SpellIconID == 1697)
            {
                ...
                switch (dummySpell->Id)
                {
                    ...
                    case 42770:
						...
						break;
                    default:
                        sLog->OutErrorConsole("Unit::HandleDummyAuraProc: non handled spell id: %u (SW)", dummySpell->Id);
                    return false;
                }
        ...
        case SPELL_FAMILY_PRIEST:
        {
            ...
            if (dummySpell->SpellIconID == 2820)
            {
                ...
                if (basepoints0 > target->GetCurrentLevel() * 125)
                    basepoints0 = target->GetCurrentLevel() * 125;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 55677:
                {
					...
					if (!target || !target->IsFriendlyTo(this) || target->GetTypeId() != TYPE_ID_PLAYER)
                        return false;
        ...
        case SPELL_FAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                ...
                case 54815:
                {
                    ...
                    if (AuraEffect const* AurEff = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_DRUID, 0x00800000, 0x0, 0x0, GetGUID()))
                    {
                ...
                case 24932:
                {
                   if (triggerAmount <= 0)
                    ...
                    if (GetTypeId() == TYPE_ID_PLAYER && !ToPlayer()->HasSpellCooldown(34299))
            ...
            // Eclipse
            if (dummySpell->SpellIconID == 2856 && GetTypeId() == TYPE_ID_PLAYER)
            {
        ...
        case SPELL_FAMILY_ROGUE:
        {
            ...
            switch (dummySpell->SpellIconID)
            {
                ...
                case 2909: // Cut to the Chase
                {
                    ...
                    if (AuraEffect const* aur = GetAuraEffect(SPELL_AURA_MOD_MELEE_HASTE, SPELL_FAMILY_ROGUE, 0x40000, 0, 0))
                    {
            ...
            switch (dummySpell->SpellIconID)
            {
                ...
                case 2236: // Thrill of the Hunt
                {
                    ...
                    if (procSpell->SpellFamilyFlags[2] & 0x200)
                    {
                        if (!victim)
                            return false;
                        if (AuraEffect const* pEff = victim->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY, SPELL_FAMILY_HUNTER, 0x0, 0x80000000, 0x0, GetGUID()))
        ...
        case SPELL_FAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    switch (castItem->GetEnchantmentId(EnchantmentSlot(TEMP_ENCHANTMENT_SLOT)))
                    {
                        ...
                        default:
                        {
                            sLog->OutErrorConsole("Unit::HandleDummyAuraProc: non handled item enchantment (rank?) %u for spell id: %u (Windfury)",
                    ...
                    {
                        sLog->OutErrorConsole("Unit::HandleDummyAuraProc: non-existing spell id: %u (Windfury)", spellId);
                        return false;
                    }
                ...
                case 70817:
                {
                    ...
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_SHAMAN, 0x10000000, 0x0, 0x0, GetGUID()))
                    {
            ...
            {
                if (GetTypeId() != TYPE_ID_PLAYER  || !victim || !victim->IsAlive() || !castItem || !castItem->IsEquipped())
                    return false;
                ...
                if (castItem->GetSlot() == EQUIPMENT_SLOT_OFF_HAND && procFlag & PROC_FLAG_DONE_OFFHAND_ATTACK)
                {
                ...
                else if (castItem->GetSlot() == EQUIPMENT_SLOT_MAIN_HAND && procFlag & PROC_FLAG_DONE_MAINHAND_ATTACK)
                {
            ...
            if (dummySpell->SpellIconID == 2287)
            {
                ...
                // Water Shield
                if (AuraEffect const* aurEff = GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELL_FAMILY_SHAMAN, 0, 0x00000020, 0))
                {
            ...
            {
                if (!procSpell || GetTypeId() != TYPE_ID_PLAYER || !victim)
                    return false;

                // custom cooldown processing case
                if (cooldown && GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->HasSpellCooldown(dummySpell->Id))
                    return false;
                ...
                switch (procSpell->Id)
                {
                    ...
                    case 49271:
						...
						break;     // Rank  8
                    default:
                        sLog->OutErrorConsole("Unit::HandleDummyAuraProc: non handled spell id: %u (LO)", procSpell->Id);
                        return false;
                }
            ...
            {
                // Lightning Shield
                if (AuraEffect const* aurEff = GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELL_FAMILY_SHAMAN, 0x400, 0, 0))
                {
                    uint32 spell = sSpellMgr->GetSpellWithRank(26364, aurEff->GetSpellInfo()->GetRank());

                    // custom cooldown processing case
                    if (GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->HasSpellCooldown(spell))
        ...
        case SPELL_FAMILY_DEATH_KNIGHT:
        {
            ...
            // Sudden Doom
            if (dummySpell->SpellIconID == 1939 && GetTypeId() == TYPE_ID_PLAYER)
            {
                ...
                for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
                {
                    ...
                    if (spellProto->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT
        ...
        case SPELL_FAMILY_POTION:
        {
            // alchemist's stone
            if (dummySpell->Id == 17619)
            {
                if (procSpell->SpellFamilyName == SPELL_FAMILY_POTION)
                {
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                    {
    ...
    if (!triggerEntry)
    {
        sLog->OutErrorConsole("Unit::HandleDummyAuraProc: Spell %u has non-existing triggered spell %u", dummySpell->Id, triggered_spell_id);
        return false;
    }
    ...
    if (cooldown && GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->HasSpellCooldown(cooldown_spell_id))
        return false;
...
bool Unit::HandleObsModEnergyAuraProc(Unit* victim, uint32 /*damage*/, AuraEffect* triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    if (!triggerEntry)
    {
        sLog->OutErrorConsole("Unit::HandleObsModEnergyAuraProc: Spell %u has non-existing triggered spell %u", dummySpell->Id, triggered_spell_id);
        return false;
    }
...
bool Unit::HandleModDamagePctTakenAuraProc(Unit* victim, uint32 /*damage*/, AuraEffect* triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    if (!triggerEntry)
    {
        sLog->OutErrorConsole("Unit::HandleModDamagePctTakenAuraProc: Spell %u has non-existing triggered spell %u", dummySpell->Id, triggered_spell_id);
        return false;
    }
        ...
        case SPELL_FAMILY_DEATH_KNIGHT:
        {
            ...
            if (dummySpell->SpellIconID == 3041 || (dummySpell->SpellIconID == 22 && dummySpell->Id != 62459) || dummySpell->SpellIconID == 2622)
            {
                *handled = true;
                ...
                if (Player* player = ToPlayer())
                {
                    if (player->GetCurrentClass() != CLASS_DEATH_KNIGHT)
                        return false;
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
        ...
        switch (auraSpellInfo->SpellFamilyName)
        {
                ...
                if (auraSpellInfo->SpellIconID == 2127)     // Blazing Speed
                {
                    switch (auraSpellInfo->Id)
                    {
                        ...
                        case 31642:  // Rank 2
                            ...
                            break;
                        default:
                            sLog->OutErrorConsole("Unit::HandleProcTriggerSpell: Spell %u miss posibly Blazing Speed", auraSpellInfo->Id);
                            return false;
                    }
                }
                break;
                ...
                else if (auraSpellInfo->SpellIconID == 1875)
                {
                    switch (auraSpellInfo->Id)
                    {
                        ...
                        case 27816:
							...
							break;
                        default:
                            sLog->OutErrorConsole("Unit::HandleProcTriggerSpell: Spell %u not handled in BR", auraSpellInfo->Id);
                        return false;
                    }
                ...
                if (auraSpellInfo->SpellIconID == 3247)     // Piercing Shots
                {
                    switch (auraSpellInfo->Id)
                    {
                        ...
                        case 53238:  // Rank 3
                            ...
                            break;
                        default:
                            sLog->OutErrorConsole("Unit::HandleProcTriggerSpell: Spell %u miss posibly Piercing Shots", auraSpellInfo->Id);
                            return false;
                    }
            ...
            case SPELL_FAMILY_PALADIN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    {
                        if (!victim || !victim->IsAlive() || GetTypeId() != TYPE_ID_PLAYER)
                            return false;
						...
						//  Fix item procs of Reign of the Dead
						if (cooldown && GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->HasSpellCooldown(stack_spell_id))
							return false;
                    ...
                    default:
                        ...
                        if (auraSpellInfo->SpellIconID == 241)
                        {
                            ...
                            if (procSpell->SpellFamilyFlags[1] & 0x00010000)
                            {
                                switch (procSpell->Id)
                                {
                                    ...
                                    case 48821:
										...
										break;
                                    default:
                                        sLog->OutErrorConsole("Unit::HandleProcTriggerSpell: Spell %u not handled in HShock", procSpell->Id);
                                       return false;
                                }
                            }
                            ...
                            if (!originalSpell)
                            {
                                sLog->OutErrorConsole("Unit::HandleProcTriggerSpell: Spell %u unknown but selected as original in Illu", originalSpellId);
                                return false;
                            }
            ...
            case SPELL_FAMILY_DEATH_KNIGHT:
            {
                ...
                {
                    if (GetTypeId() != TYPE_ID_PLAYER || GetCurrentClass() != CLASS_DEATH_KNIGHT)
                        return false;
    ...
    if (triggerEntry == NULL)
    {
        ...
        // sLog->OutErrorConsole("Unit::HandleProcTriggerSpell: Spell %u has 0 in EffectTriggered[%d]. Unhandled custom case?", auraSpellInfo->Id, triggeredByAura->GetEffIndex());
        return false;
    }
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 12867:
        {
            ...
            if (procFlags & PROC_FLAG_DONE_OFFHAND_ATTACK)
                item = ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
            else
                item = ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAIN_HAND);
    ...
    // Blade Barrier
    if (auraSpellInfo->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT && auraSpellInfo->SpellIconID == 85 && procSpell)
    {
        Player* player = ToPlayer();
        if (!player || player->GetCurrentClass() != CLASS_DEATH_KNIGHT)
            return false;
    ...
    // Rime
    else if (auraSpellInfo->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT && auraSpellInfo->SpellIconID == 56)
    {
        if (GetTypeId() != TYPE_ID_PLAYER)
            return false;
    ...
    switch (trigger_spell_id)
    {
        ...
        case 52916:
        {
            ...
            if (cooldown && target->GetTypeId() == TYPE_ID_PLAYER && target->ToPlayer()->HasSpellCooldown(trigger_spell_id))
                return false;
        ...
        case 15250: // Rogue Setup
        {
            ...
            if (!victim || (GetTypeId() == TYPE_ID_PLAYER && victim != ToPlayer()->GetSelectedUnit()))
                return false;
        ...
        case 48090:
        {
            ...
            if (IsPet())
                if (Unit* owner = GetOwner())
                {
                    if (AuraEffect* aurEff = owner->GetDummyAuraEffect(SPELL_FAMILY_WARLOCK, 3220, 0))
                    {
        ...
        case 58679:
        {
            ...
            if (!procSpell || !(procSpell->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT && procSpell->SpellFamilyFlags[0] == 0x80002000))
                return false;
            break;
        }
        // Glyph of Death Grip
        ...
        case 71169:
        {
            ...
            if (GetTypeId() != TYPE_ID_PLAYER || !victim || victim->GetTypeId() != TYPE_ID_UNIT || victim->GetCreatureType() == CREATURE_TYPE_CRITTER)
                return false;
    ...
    if (cooldown && GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->HasSpellCooldown(trigger_spell_id))
        return false;
...
bool Unit::HandleOverrideClassScriptAuraProc(Unit* victim, uint32 /*damage*/, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 cooldown)
{
    ...
    if (!triggerEntry)
    {
        sLog->OutErrorConsole("Unit::HandleOverrideClassScriptAuraProc: Spell %u triggering for class script id %u", triggered_spell_id, scriptId);
        return false;
    }
...
void Unit::SetPowerType(Powers new_powertype)
{
    ...
    else if (Pet* pet = ToCreature()->ToPet())
    {
        ...
        {
            Unit* owner = GetOwner();
            if (owner && (owner->GetTypeId() == TYPE_ID_PLAYER) && owner->ToPlayer()->GetGroup())
...
FactionTemplateEntry const* Unit::getFactionTemplateEntry() const
{
    ...
    if (!entry)
    {
        ...
        {
            if (Player const* player = ToPlayer())
                sLog->OutErrorConsole("Player %s has invalid faction (faction template id) #%u", player->GetName(), GetFaction());
            else if (Creature const* creature = ToCreature())
                sLog->OutErrorConsole("Creature (template id: %u) has invalid faction (faction template id) #%u", creature->GetCreatureTemplate()->Entry, GetFaction());
            else
                sLog->OutErrorConsole("Unit (name=%s, type=%u) has invalid faction (faction template id) #%u", GetName(), uint32(GetTypeId()), GetFaction());
...
bool Unit::IsHostileTo(Unit const* unit) const
{
    ...
    if (unit->GetTypeId() == TYPE_ID_PLAYER && ((Player const*)unit)->IsGameMaster())
        return false;
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    if (GetTypeId() == TYPE_ID_PLAYER && IsMounted())
        return false;
    ...
    if (GetTypeId() == TYPE_ID_UNIT && !ToCreature()->IsPet())
    {
	...
	if (this->GetTypeId() == TYPE_ID_PLAYER)
	{
...
void Unit::RemoveAllAttackers()
{
    while (!m_attackers.empty())
    {
        ...
        if (!(*iter)->AttackStop())
        {
            sLog->OutErrorConsole("WORLD: Unit has an attacker that isn't attacking it!");
...
Player* Unit::GetCharmerOrOwnerPlayerOrPlayerItself() const
{
    ...
    return GetTypeId() == TYPE_ID_PLAYER ? (Player*)this : NULL;
}
...
Minion *Unit::GetFirstMinion() const
{
    if (uint64 pet_guid = GetMinionGUID())
    {
        ...
        sLog->OutErrorConsole("Unit::GetFirstMinion: Minion %u not exist.", GUID_LOPART(pet_guid));
...
Unit* Unit::GetCharm() const
{
    if (uint64 charm_guid = GetCharmGUID())
    {
        ...
        sLog->OutErrorConsole("Unit::GetCharm: Charmed creature %u not exist.", GUID_LOPART(charm_guid));
...
void Unit::SetMinion(Minion *minion, bool apply)
{
    ...
    if (apply)
    {
        ...
        if (GetTypeId() == TYPE_ID_PLAYER && minion->IsPet())
            for (uint8 i = 0; i < MAX_MOVE_TYPE; ++i)
        ...
        //if (minion->HasUnitTypeMask(UNIT_MASK_GUARDIAN))
        {
            if (RemoveUInt64Value(UNIT_FIELD_SUMMON, minion->GetGUID()))
            {
                ...
                for (ControlList::iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr)
                {
                    ...
                    ASSERT((*itr)->GetTypeId() == TYPE_ID_UNIT);
                    ...
                    if (AddUInt64Value(UNIT_FIELD_SUMMON, (*itr)->GetGUID()))
                    {
                        ...
                        if (GetTypeId() == TYPE_ID_PLAYER && !GetCharmGUID())
                        {
                            if ((*itr)->IsPet())
...
void Unit::GetAllMinionsByEntry(std::list<Creature*>& Minions, uint32 entry)
{
    for (Unit::ControlList::iterator itr = m_Controlled.begin(); itr != m_Controlled.end();)
    {
        Unit* unit = *itr;
        ++itr;
        if (unit->GetEntry() == entry && unit->GetTypeId() == TYPE_ID_UNIT
...
void Unit::SetCharm(Unit* charm, bool apply)
{
    ...
    else
    {
        ...
        if (charm->GetTypeId() == TYPE_ID_PLAYER || !charm->ToCreature()->HasUnitTypeMask(UNIT_MASK_MINION) || charm->GetOwnerGUID() != GetGUID())
		{
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth)
{
    ...
    if (GetTypeId() == TYPE_ID_UNIT && ToCreature()->IsTotem())
        unit = GetOwner();
...
void Unit::RemoveAllControlled()
{
    ...
    while (!m_Controlled.empty())
    {
        ...
        if (target->GetCharmerGUID() == GetGUID())
            ...
            sLog->OutErrorConsole("Unit %u is trying to release unit %u which is neither charmed nor owned by it", GetEntry(), target->GetEntry());
    }
    if (GetPetGUID())
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
    ...
    else if (GetTypeId() == TYPE_ID_UNIT && ToCreature()->IsPet())
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 4993:
            {
                ...
                for (AuraApplicationMap::const_iterator itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
                {
                    ...
                    if (m_spell->SpellFamilyName != SPELL_FAMILY_WARLOCK || !(m_spell->SpellFamilyFlags[1] & 0x0004071B || m_spell->SpellFamilyFlags[0] & 0x8044C402))
                        continue;
            ...
            {
                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_DRUID, 0x200002, 0, 0))
                    AddPctN(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            ...
            case 7293:
            {
                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_DEATH_KNIGHT, 0, 0x02000000, 0))
            ...
            case 7377:
            {
                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_PRIEST, 0x8000, 0, 0, GetGUID()))
                    AddPctN(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            // Marked for Death
            ...
            case 7602:
            {
                if (victim->GetAuraEffect(SPELL_AURA_MOD_STALKED, SPELL_FAMILY_HUNTER, 0x400, 0, 0))
                    AddPctN(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            // Dirty Deeds
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELL_FAMILY_MAGE:
            ...
            if (spellProto->SpellIconID == 186)
            {
                ...
                {
                    // Glyph of Ice Lance
                    if (owner->HasAura(56377) && victim->GetCurrentLevel() > owner->GetCurrentLevel())
                        DoneTotalMod *= 4.0f;
                    else
                        DoneTotalMod *= 3.0f;
                }
        ...
        case SPELL_FAMILY_PRIEST:
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x800000)
            {
                ...
                if (AuraEffect* aurEff = GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS, SPELL_FAMILY_PRIEST, 2848, 1))
                    // Increase Mind Flay damage if Shadow Word: Pain present on target
                    if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_PRIEST, 0x8000, 0, 0, GetGUID()))
                        AddPctN(DoneTotalMod, aurEff->GetAmount());
            }
            ...
            else if (spellProto->SpellFamilyFlags[0] & 0x80)
            {
                ...
                if (AuraEffect* aurEff = GetAuraEffect(55692, 0))
                    if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_PRIEST, 0x100000, 0, 0, GetGUID()))
                        AddPctN(DoneTotalMod, aurEff->GetAmount());
            }
            // Shadow Word: Death
        ...
        case SPELL_FAMILY_HUNTER:
            ...
            if (spellProto->SpellFamilyFlags[1] & 0x1)
                if (AuraEffect* aurEff = GetAuraEffect(56826, 0))  // Glyph of Steady Shot
                    if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_HUNTER, 0x00004000, 0, 0, GetGUID()))
                        AddPctN(DoneTotalMod, aurEff->GetAmount());
            break;
        case SPELL_FAMILY_DEATH_KNIGHT:
            // Improved Icy Touch
            if (spellProto->SpellFamilyFlags[0] & 0x2)
                if (AuraEffect* aurEff = GetDummyAuraEffect(SPELL_FAMILY_DEATH_KNIGHT, 2721, 0))
                    AddPctN(DoneTotalMod, aurEff->GetAmount());
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x2 || spellProto->SpellFamilyFlags[1] & 0x6)
                if (AuraEffect* aurEff = GetDummyAuraEffect(SPELL_FAMILY_DEATH_KNIGHT, 196, 0))
    ...
    switch (spellProto->DmgClass)
    {
		...
		case SPELL_DAMAGE_CLASS_MAGIC:
        {
            if (schoolMask & SPELL_SCHOOL_MASK_NORMAL)
                crit_chance = 0.0f;
            // For other schools
            else if (GetTypeId() == TYPE_ID_PLAYER)
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    ...
                    case SPELL_FAMILY_DRUID:
						...
						{
                            if (AuraEffect const* aurEff = GetDummyAuraEffect(SPELL_FAMILY_DRUID, 109, 0))
                                crit_chance += aurEff->GetAmount();
						}
                        ...
                        if (spellProto->SpellFamilyFlags[0] & 0x4 && spellProto->SpellIconID == 1485)
                        {
                            ...
                            if (AuraEffect const* aurEff = GetDummyAuraEffect(SPELL_FAMILY_DRUID, 1771, 0))
							{
                                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_DRUID, 0x00000002, 0, 0))
                                    crit_chance += aurEff->GetAmount();
							}
							break;
                        }
                    ...
                    case SPELL_FAMILY_PALADIN:
                        ...
                        {
                            if (victim->GetCreatureTypeMask() & CREATURE_TYPE_MASK_DEMON_OR_UNDEAD)
                                return true;
                            break;
                        }
						break;
                    case SPELL_FAMILY_SHAMAN:
                        ...
                        {
                            if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_SHAMAN, 0x10000000, 0, 0, GetGUID()))
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    case SPELL_FAMILY_DRUID:
						...
						{
                            if (AuraEffect const* rendAndTear = GetDummyAuraEffect(SPELL_FAMILY_DRUID, 2859, 1))
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId() == TYPE_ID_UNIT && IsTotem())
        if (Unit* owner = GetOwner())
            return owner->SpellHealingBonusDone(victim, spellProto, healamount, damagetype, stack);

    // No bonus healing for potion spells
    if (spellProto->SpellFamilyName == SPELL_FAMILY_POTION)
        return healamount;

	// Warlock Healthstones No bonus heal.
	if (spellProto->SpellFamilyName == SPELL_FAMILY_WARLOCK && (spellProto->SpellFamilyFlags[0] & 0x10000))
		return healamount;
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            {
                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELL_FAMILY_DRUID, 0x40, 0, 0))
                    AddPctN(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            case 8477: // Nourish Heal Boost
            {
                ...
                for (AuraApplicationMap::const_iterator itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
                {
                    ...
                    if (aura->GetCasterGUID() != GetGUID())
                        continue;
                    SpellInfo const* m_spell = aura->GetSpellInfo();
                    if (m_spell->SpellFamilyName != SPELL_FAMILY_DRUID ||
            ...
            {
                if (victim->GetAuraEffect(SPELL_AURA_DUMMY, SPELL_FAMILY_SHAMAN, 0, 0x00000400, 0, GetGUID()))
                    AddPctN(DoneTotalMod, (*i)->GetAmount());
				break;
            }
			default:
    ...
    if (DoneAdvertisedBenefit)
    {
        ...
        if (spellProto->SpellFamilyName == SPELL_FAMILY_SHAMAN && spellProto->SpellFamilyFlags[1] & 0x80000)
            factorMod *= 0.45f;
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
	...
	// Nourish cast
	if (spellProto->SpellFamilyName == SPELL_FAMILY_DRUID && spellProto->SpellFamilyFlags[1] & 0x2000000)
	{
		// Rejuvenation, Regrowth, Lifebloom, or Wild Growth
		if (GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELL_FAMILY_DRUID, 0x50, 0x4000010, 0))
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
    ...
    {
        SpellImmuneList const& list = m_spellImmune[IMMUNITY_AURA_TYPE];
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    if (spellProto)
    {
        ...
        if (spellProto->SpellFamilyName == SPELL_FAMILY_DRUID && spellProto->SpellFamilyFlags[0] & 0x00008800)
...
void Unit::RemoveMount()
{
    ...
    // RemoveMount as a vehicle
    if (GetTypeId() == TYPE_ID_PLAYER && GetVehicleKit())
    {
...
void Unit::SetInCombatWith(Unit* enemy)
{
    ...
    // check for duel
    if (eOwner->GetTypeId() == TYPE_ID_PLAYER && eOwner->ToPlayer()->duel)
    {
...
void Unit::CombatStart(Unit* target, bool initialAggro)
{
    if (initialAggro)
    {
        ...
        if (!target->IsInCombatActive() && target->GetTypeId() != TYPE_ID_PLAYER
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (IsInCombatActive() || HasUnitState(UNIT_STATE_EVADE))
        return;
    ...
    if (Creature* creature = ToCreature())
    {
        ...
        if (enemy)
        {
            if (IsAIEnabled)
            {
                creature->AI()->EnterToBattle(enemy);
                RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER); // unit has engaged in combat, remove immunity so players can fight back
            }
...
void Unit::ClearInCombat()
{
    ...
    if (Creature* creature = ToCreature())
    {
        if (creature->GetCreatureTemplate() && creature->GetCreatureTemplate()->unit_flags & UNIT_FLAG_NO_AGGRO_FOR_PLAYER)
            SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER); // set immunity state to the one from db on evade
...
bool Unit::IsTargetableForAttack(bool checkFakeDeath) const
{
    ...
    if (HasFlag(UNIT_FIELD_FLAGS,
        UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NO_AGGRO_FOR_PLAYER))
        return false;

    if (GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->IsGameMaster())
        return false;
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if (target->HasUnitState(UNIT_STATE_UNATTACKABLE) || (target->GetTypeId() == TYPE_ID_PLAYER && target->ToPlayer()->IsGameMaster()))
        return false;
    ...
    if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_TAXI_FLIGHT | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_UNK_16)
        || (!HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_CREATURE))
        || (!target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_CREATURE))
        || (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER))
        // check if this is a world trigger cast - GOs are using world triggers to cast their spells, so we need to ignore their immunity flag here, this is a temp workaround, needs removal when go cast is implemented properly
        || (GetEntry() != WORLD_TRIGGER && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER)))
        return false;
    ...
    // Not all neutral creatures can be attacked (even some unfriendly faction does not react aggresive to you, like Sporeggar)
    if (GetReactionTo(target) == REP_NEUTRAL && target->GetReactionTo(this) <= REP_NEUTRAL)
    {
        if (!(target->GetTypeId() == TYPE_ID_PLAYER && GetTypeId() == TYPE_ID_PLAYER) && !(target->GetTypeId() == TYPE_ID_UNIT && GetTypeId() == TYPE_ID_UNIT))
        {
            Player const* player = target->GetTypeId() == TYPE_ID_PLAYER ? target->ToPlayer() : ToPlayer();
            Unit const* creature = target->GetTypeId() == TYPE_ID_UNIT ? target : this;
	...
	// Fix totem and pet attack from neutral factions
	// Argent Crusade
	if (target->GetFaction() == 2070 || target->GetFaction() == 2071
		|| target->GetFaction() == 2072 || target->GetFaction() == 2073
		|| target->GetFaction() == 2131 || target->GetFaction() == 2134
		|| target->GetFaction() == 2138 || target->GetFaction() == 2230
		// Kirin Tor
		|| target->GetFaction() == 2006 || target->GetFaction() == 2007
		|| target->GetFaction() == 2008 || target->GetFaction() == 2009
		// Aldor
		|| target->GetFaction() == 1743 || target->GetFaction() == 1776
		|| target->GetFaction() == 1777 || target->GetFaction() == 1805
		|| target->GetFaction() == 1844 || target->GetFaction() == 1854
		|| target->GetFaction() == 1875
		// The Ashen Verdict
		|| target->GetFaction() == 2216 || target->GetFaction() == 2217
		|| target->GetFaction() == 2218 || target->GetFaction() == 2219
		// Consortium
		|| target->GetFaction() == 1730 || target->GetFaction() == 1731
		|| target->GetFaction() == 1788 || target->GetFaction() == 1836
		// Kaluak
		|| target->GetFaction() == 1949 || target->GetFaction() == 1950
		|| target->GetFaction() == 2047 || target->GetFaction() == 2148
		// Shattered Sun Offensive
		|| target->GetFaction() == 1956 || target->GetFaction() == 1957
		|| target->GetFaction() == 1960 || target->GetFaction() == 1967
		// Shatari Skyguard
		|| target->GetFaction() == 1856 || target->GetFaction() == 1870
		// Shatar
		|| target->GetFaction() == 1741 || target->GetFaction() == 1775
		// Lower City
		|| target->GetFaction() == 1818 || target->GetFaction() == 1851
		// The Scale of Sands
		|| target->GetFaction() == 1778
		// Scryers
		|| target->GetFaction() == 1744 || target->GetFaction() == 1746
		|| target->GetFaction() == 1838 || target->GetFaction() == 1845
		|| target->GetFaction() == 1855 || target->GetFaction() == 1876
		// Cenarion Expedition
		|| target->GetFaction() == 1659 || target->GetFaction() == 1660
		|| target->GetFaction() == 1661 || target->GetFaction() == 1710
		|| target->GetFaction() == 1728 || target->GetFaction() == 1987
		// Cenarion Circle
		|| target->GetFaction() == 635 || target->GetFaction() == 994
		|| target->GetFaction() == 996 || target->GetFaction() == 1254
		|| target->GetFaction() == 1608
		// Argent Dawn
		|| target->GetFaction() == 794 || target->GetFaction() == 814
		|| target->GetFaction() == 1624 || target->GetFaction() == 1625
		|| target->GetFaction() == 1766 || target->GetFaction() == 1767
		|| target->GetFaction() == 2086 || target->GetFaction() == 2087
		// Ashtongue Deathsworn
		|| target->GetFaction() == 1820 || target->GetFaction() == 1858
		|| target->GetFaction() == 1866
		// Darkmoon Faire
		|| target->GetFaction() == 1555 || target->GetFaction() == 1896
		// Honor Hold
		|| target->GetFaction() == 1666 || target->GetFaction() == 1667
		|| target->GetFaction() == 1671 || target->GetFaction() == 1737
		//Dark Portal Defenders and Violet Eye
		|| target->GetFaction() == 1755 || target->GetFaction() == 1696
		// Ratchet
		|| target->GetFaction() == 69 || target->GetFaction() == 637
		// Gadgetzan
		|| target->GetFaction() == 474 || target->GetFaction() == 475
		// Gelkins Clan
		|| target->GetFaction() ==  132
		// Shendralar
		|| target->GetFaction() == 1354 || target->GetFaction() == 1355
		// Wyrmrest Accord
		|| target->GetFaction() == 2010 || target->GetFaction() == 2011
		|| target->GetFaction() == 2012 || target->GetFaction() == 2013
		|| target->GetFaction() == 2041 || target->GetFaction() == 2057
		// Knights of the Ebon Blade
		|| target->GetFaction() == 2050 || target->GetFaction() == 2051
		|| target->GetFaction() == 2144 || target->GetFaction() == 2214
		|| target->GetFaction() == 2226
		// Ogrila
		|| target->GetFaction() == 1872 || target->GetFaction() == 1874)
		return false;
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    if (target->HasUnitState(UNIT_STATE_UNATTACKABLE)
        || (target->GetTypeId() == TYPE_ID_PLAYER && target->ToPlayer()->IsGameMaster()))
        return false;
    ...
    if (!bySpell || !(bySpell->AttributesEx6 & SPELL_ATTR6_ASSIST_IGNORE_IMMUNE_FLAG))
    {
        if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
        {
            if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER))
                return false;
        }
        else
        {
            if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_CREATURE))
                return false;
        }
    }
...
int32 Unit::ModifyHealth(int32 dVal)
{
	...
	if (dVal < 0 && GetTypeId() != TYPE_ID_PLAYER && !IsPet())
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
	...
	// Guild-Level-System (Bonus: Riding Speed)
	uint8 bonusSpeed = 0;
	if (GetTypeId() == TYPE_ID_PLAYER)
	{
		Player* player = ToPlayer();
		if (Guild* guild = player->GetGuild())
		{
			...
			{
				if (sWorld->getBoolConfig(CONFIG_GUILD_BONUS_MOUNT_SPEED_ENABLE))
				{
    ...
    switch (mtype)
    {
        ...
        case MOVE_FLIGHT:
        {
            if (GetTypeId() == TYPE_ID_UNIT && IsControlledByPlayer()) // not sure if good for pet
            {
            ...
            if (GetTypeId() == TYPE_ID_PLAYER && GetVehicle())
                GetVehicleBase()->UpdateSpeed(MOVE_FLIGHT, true);
            break;
        }
        default:
            sLog->OutErrorConsole("Unit::UpdateSpeed: Unsupported move type (%d)", mtype);
            return;
    }
    ...
    if (!forced)
    {
        switch (mtype)
        {
            ...
            case MOVE_PITCH_RATE:
                ...
                break;
            default:
                sLog->OutErrorConsole("Unit::SetSpeed: Unsupported move type (%d), data not sent to client.", mtype);
                return;
        }
...
{
    ASSERT(GetTypeId() == TYPE_ID_UNIT);

    if (!taunter || (taunter->GetTypeId() == TYPE_ID_PLAYER && taunter->ToPlayer()->IsGameMaster()))
        return;
...
Unit* Creature::SelectVictim()
{
    ...
    else if (!HasReactState(REACT_PASSIVE))
    {
        ...
        if (!target && IsSummon())
        {
            ...
            {
                if (owner->IsInCombatActive())
                    target = owner->getAttackerForHelper();
                if (!target)
                {
                    for (ControlList::const_iterator itr = owner->m_Controlled.begin(); itr != owner->m_Controlled.end(); ++itr)
                    {
                        if ((*itr)->IsInCombatActive())
                        {
    ...
    {
        if ((*itr) && !CanCreatureAttack(*itr) && (*itr)->GetTypeId() != TYPE_ID_PLAYER
...
int32 Unit::ModSpellDuration(SpellInfo const* spellProto, Unit const* target, int32 duration, bool positive, uint32 effectMask)
{
    ...
    else
    {
        ...
        if (target->GetTypeId() == TYPE_ID_PLAYER)
        {
            if (spellProto->SpellFamilyName == SPELL_FAMILY_POTION && (sSpellMgr->IsSpellMemberOfSpellGroup(spellProto->Id, SPELL_GROUP_ELIXIR_BATTLE) ||
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (!(spellProto->Attributes & (SPELL_ATTR0_ABILITY|SPELL_ATTR0_TRADESPELL)) && ((GetTypeId() == TYPE_ID_PLAYER && spellProto->SpellFamilyName) || GetTypeId() == TYPE_ID_UNIT))
	...
	// Instant Cast Glyphs, Scrolls, Profession Craft in active fun system
	if (sWorld->getBoolConfig(CONFIG_FUN_INSTANT_CAST_TIME_FOR_CRAFT))
	{
		if (spellProto->Effects[0].Effect == SPELL_EFFECT_ENCHANT_ITEM
			|| spellProto->Effects[0].Effect == SPELL_EFFECT_APPLY_GLYPH
			|| spellProto->Effects[0].Effect == SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC
			|| spellProto->Effects[0].Effect == SPELL_EFFECT_CREATE_ITEM)
			castTime = 1;
	}
...
float Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32 &duration, Unit* caster, DiminishingLevels Level, int32 limitduration)
{
    ...
    if (limitduration > 0 && duration > limitduration)
    {
        ...
        if ((target->GetTypeId() == TYPE_ID_PLAYER
            ...
            && source->GetTypeId() == TYPE_ID_PLAYER)
    ...
    {
        if (GetTypeId() == TYPE_ID_UNIT && (ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_TAUNT_DIMINISH))
        {
    ...
    // Some diminishings applies to mobs too (for example, Stun)
        ...
        && ((targetOwner ? (targetOwner->GetTypeId() == TYPE_ID_PLAYER) : (GetTypeId() == TYPE_ID_PLAYER))
        || (GetTypeId() == TYPE_ID_UNIT && ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_ALL_DIMINISH)))
...
bool Unit::HandleStatModifier(UnitMods unitMod, UnitModifierType modifierType, float amount, bool apply)
{
    ...
    {
        sLog->OutErrorConsole("ERROR in HandleStatModifier(): non-existing UnitMods or wrong UnitModifierType!");
        return false;
    }
    ...
    switch (unitMod)
    {
        ...
        case UNIT_MOD_ATTACK_POWER_RANGED:
			...
			break;
        case UNIT_MOD_DAMAGE_MAIN_HAND:
			UpdateDamagePhysical(BASE_ATTACK);
			break;
        case UNIT_MOD_DAMAGE_OFF_HAND:
...
float Unit::GetModifierValue(UnitMods unitMod, UnitModifierType modifierType) const
{
    if (unitMod >= UNIT_MOD_END || modifierType >= MODIFIER_TYPE_END)
    {
        sLog->OutErrorConsole("attempt to access non-existing modifier value from UnitMods!");
        return 0.0f;
    }
...
float Unit::GetTotalAuraModValue(UnitMods unitMod) const
{
    ...
    {
        sLog->OutErrorConsole("attempt to access non-existing UnitMods in GetTotalAuraModValue()!");
        return 0.0f;
    }
...
void Unit::SetLevel(uint8 lvl)
{
    ...
    // group update
    if (GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->GetGroup())
...
void Unit::SetHealth(uint32 val)
{
    if (getDeathState() == JUST_DIED)
        val = 0;
    else if (GetTypeId() == TYPE_ID_PLAYER && getDeathState() == DEAD)
        val = 1;
    else
    {
...
void Unit::SetPower(Powers power, uint32 val)
{
    ...
    SetStatInt32Value(UNIT_FIELD_POWER_1 + power, val);
...
void Unit::SetMaxPower(Powers power, uint32 val)
{
    ...
    SetStatInt32Value(UNIT_FIELD_MAX_POWER_1 + power, val);
...
uint32 Unit::GetCreatePowers(Powers power) const
{
    ...
    switch (power)
    {
        ...
        case POWER_RAGE:
			return 1000;
        case POWER_FOCUS:
			return (GetTypeId() == TYPE_ID_PLAYER || !((Creature const*)this)->IsPet() || ((Pet const*)this)->getPetType() != HUNTER_PET ? 0 : 100);
        case POWER_ENERGY:
			return 100;
        case POWER_HAPPINESS:
			return (GetTypeId() == TYPE_ID_PLAYER || !((Creature const*)this)->IsPet() || ((Pet const*)this)->getPetType() != HUNTER_PET ? 0 : 1050000);
        case POWER_RUNIC_POWER:
			return 1000;
        case POWER_RUNE:
			return 0;
  ...
  m_stayX(0.0f), m_stayY(0.0f), m_stayZ(0.0f)
{
    ...
    if (m_unit->GetTypeId() == TYPE_ID_UNIT)
    {
...
{
    if (m_unit->GetTypeId() == TYPE_ID_PLAYER)                // charmed players don't have spells
    {
        InitEmptyActionBar();
        return;
    }
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    if (GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->GetSession()->PlayerLoading())
        return;
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && target)
    {
        ...
        if (GetTypeId() == TYPE_ID_PLAYER)
        {
            ...
            {
                if (target->GetTypeId() != TYPE_ID_PLAYER && target->GetCreatureType() != CREATURE_TYPE_CRITTER)
        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT|PROC_EX_PARRY|PROC_EX_DODGE|PROC_EX_BLOCK))
        {
            ...
            if (isVictim)
            {
                ...
                if (procExtra & PROC_EX_DODGE)
                {
                    ...
                    if (GetCurrentClass() != CLASS_ROGUE) // skip Rogue Riposte
                    {
            ...
            else // For attacker
            {
                ...
                if (procExtra & PROC_EX_DODGE && GetTypeId() == TYPE_ID_PLAYER && GetCurrentClass() == CLASS_WARRIOR)
                {
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
		...
		if ((procExtra & PROC_EX_ABSORB) && isVictim && ((spellProto->SpellFamilyName == SPELL_FAMILY_PRIEST && spellProto->SpellFamilyFlags[2] & 0x00000400) || (spellProto->SpellFamilyName == SPELL_FAMILY_SHAMAN && (spellProto->SpellFamilyFlags[1] & 0x00000400 || spellProto->SpellFamilyFlags[1] & 0x00000020))))
			continue; // Prevent Priest Defensive Procs on Absorb (IF, PoM)
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        uint32 cooldown = 0;
        if (GetTypeId() == TYPE_ID_PLAYER && i->spellProcEvent && i->spellProcEvent->cooldown)
...
void Unit::ClearAllReactives()
{
    ...
    if (GetCurrentClass() == CLASS_HUNTER && HasAuraState(AURA_STATE_HUNTER_PARRY))
        ModifyAuraState(AURA_STATE_HUNTER_PARRY, false);

    if (GetCurrentClass() == CLASS_WARRIOR && GetTypeId() == TYPE_ID_PLAYER)
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
    ...
    // select random
    return Quantum::DataPackets::SelectRandomContainerElement(targets);
}
...
void Unit::ApplyAttackTimePercentMod(WeaponAttackType att, float val, bool apply)
{
    ...
    if (val > 0)
    {
        ...
        ApplyPercentModFloatValue(UNIT_FIELD_BASE_ATTACK_TIME+att, val, !apply);
    }
    else
    {
        ApplyPercentModFloatVar(m_modAttackSpeedPct[att], -val, apply);
...
uint32 Unit::GetCastingTimeForBonus(SpellInfo const* spellProto, DamageEffectType damagetype, uint32 CastingTime) const
{
    ...
    if (CastingTime == 0 && GetTypeId() == TYPE_ID_UNIT && !ToCreature()->IsPet())
        return 3500;
...
{
    if (!normalized || GetTypeId() != TYPE_ID_PLAYER)
...
Pet* Unit::CreateTamedPetFrom(Creature* creatureTarget, uint32 spell_id)
{
    ...
    uint8 level = creatureTarget->GetCurrentLevel() + 5 < GetCurrentLevel() ? (GetCurrentLevel() - 5) : creatureTarget->GetCurrentLevel();
...
Pet* Unit::CreateTamedPetFrom(uint32 creatureEntry, uint32 spell_id)
{
    ...
    if (!pet->CreateBaseAtCreatureInfo(creatureInfo, this) || !InitTamedPet(pet, GetCurrentLevel(), spell_id))
    {
        delete pet;
        return NULL;
    }
...
bool Unit::InitTamedPet(Pet* pet, uint8 level, uint32 spell_id)
{
    ...
    {
        sLog->OutErrorConsole("Pet::InitStatsForLevel() failed for creature (Entry: %u)!", pet->GetEntry());
        return false;
    }
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    if (EventProcFlag & PROC_FLAG_KILL && GetTypeId() == TYPE_ID_PLAYER)
    {
        bool allow = false;
    ...
    if (!isVictim && GetTypeId() == TYPE_ID_PLAYER)
    {
        Player* player = ToPlayer();
        if (spellProto->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            ...
            if (attType == BASE_ATTACK)
                item = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAIN_HAND);
            else if (attType == OFF_ATTACK)
                item = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
            else
        ...
        else if (spellProto->EquippedItemClass == ITEM_CLASS_ARMOR)
        {
            ...
            Item* item = player->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect* triggeredByAura)
{
    ...
    if (!(spellProto->SpellFamilyName == SPELL_FAMILY_PRIEST && spellProto->SpellFamilyFlags[1] & 0x20))
    {
...
bool Unit::HandleAuraRaidProcFromCharge(AuraEffect* triggeredByAura)
{
    ...
    switch (spellProto->Id)
    {
        ...
        case 43593:            // Cold Stare
            ...
            break;
        default:
            sLog->OutErrorConsole("Unit::HandleAuraRaidProcFromCharge, received unhandled spell: %u", spellProto->Id);
            return false;
    }
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
	...
	if (Player* killerPlayer = GetCharmerOrOwnerPlayerOrPlayerItself())
		killerPlayer->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS, 1, 0, victim);

    // if talent known but not triggered (check priest class for speedup check)
    bool SpiritOfRedemption = false;

    if (victim->GetTypeId() == TYPE_ID_PLAYER && victim->GetCurrentClass() == CLASS_PRIEST)
    {
        ...
        for (AuraEffectList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
        {
            if ((*itr)->GetSpellInfo()->SpellIconID == 1654)
            {
				...
				victim->CastSpell(victim, 27792, true); // Test
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        if (GetTypeId() == TYPE_ID_UNIT && IsAIEnabled)
    ...
    if (victim->GetTypeId() == TYPE_ID_PLAYER)
    {
        ...
        else if (GetTypeId() == TYPE_ID_PLAYER && victim != this)
...
void Unit::SetStunned(bool apply)
{
    ...
    else
    {
        ...
        Unit* owner = GetOwner();
        if (!owner || (owner->GetTypeId() == TYPE_ID_PLAYER && !owner->ToPlayer()->IsMounted()))
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    // RemoveMount players when charmed
    if (GetTypeId() == TYPE_ID_PLAYER)
        RemoveMount();

    ASSERT(type != CHARM_TYPE_POSSESS || charmer->GetTypeId() == TYPE_ID_PLAYER);
    ...
    if (GetTypeId() == TYPE_ID_PLAYER && ToPlayer()->GetTransport())
    {
    ...
    if (!IsVehicle() || (IsVehicle() && map && !map->IsBattleground()))
        SetCurrentFaction(charmer->GetFaction());
    ...
    if (charmer->GetTypeId() == TYPE_ID_PLAYER)
    {
        switch (type)
        {
            ...
            case CHARM_TYPE_CHARM:
                if (GetTypeId() == TYPE_ID_UNIT && charmer->GetCurrentClass() == CLASS_WARLOCK)
                {
                    ...
                    if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                    {
                        ...
                        SetUInt32Value(UNIT_FIELD_PET_NAME_TIME_STAMP, uint32(time(NULL))); // cast can't be helped
                    }
                }
...
void Unit::RemoveCharmedBy(Unit* charmer)
{
    ...
    ASSERT(type != CHARM_TYPE_POSSESS || charmer->GetTypeId() == TYPE_ID_PLAYER);
    ASSERT(type != CHARM_TYPE_VEHICLE || (GetTypeId() == TYPE_ID_UNIT && IsVehicle()));
                ...
                if (GetTypeId() == TYPE_ID_UNIT && charmer->GetCurrentClass() == CLASS_WARLOCK)
                {
                    ...
                    if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                    {
                        ...
                        if (GetCharmInfo())
                            ...
                            sLog->OutErrorConsole("Aura::HandleModCharm: target="UI64FMTD" with typeid=%d has a charm aura but no charm info!", GetGUID(), GetTypeId());
                    }
                }
                break;
            default:
    ...
    if (charmer->GetTypeId() == TYPE_ID_PLAYER && this != charmer->GetFirstControlled())
        charmer->ToPlayer()->SendRemoveControlBar();
    else if (GetTypeId() == TYPE_ID_PLAYER || (GetTypeId() == TYPE_ID_UNIT && !ToCreature()->IsGuardian()))
        DeleteCharmInfo();
}
...
{
    if (GetTypeId() == TYPE_ID_PLAYER)
        ToPlayer()->setFactionForRace(GetCurrentRace());
    else
    {
        if (HasUnitTypeMask(UNIT_MASK_MINION))
        {
            if (Unit* owner = GetOwner())
            {
                SetCurrentFaction(owner->GetFaction());
                return;
            }
        }
        ...
        if (CreatureTemplate const* cinfo = ToCreature()->GetCreatureTemplate())  // normal creature
        {
            ...
            SetCurrentFaction((faction && faction->friendlyMask & 0x004) ? cinfo->faction_H : cinfo->faction_A);
        }
    }
}
...
void Unit::RemoveVehicleKit()
{
    ...
    m_updateFlag &= ~UPDATEFLAG_VEHICLE;
    m_unitTypeMask &= ~UNIT_MASK_VEHICLE;
    RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELL_CLICK);
...
bool Unit::IsInPartyWith(Unit const* unit) const
{
    ...
    if (u1->GetTypeId() == TYPE_ID_PLAYER && u2->GetTypeId() == TYPE_ID_PLAYER)
...
void Unit::ApplyResilience(Unit const* victim, float* crit, int32* damage, bool isCrit, CombatRating type) const
{
    ...
    if (IsVehicle() || (victim->IsVehicle() && victim->GetTypeId() != TYPE_ID_PLAYER))
        return;

    Unit const* source = NULL;
    if (GetTypeId() == TYPE_ID_PLAYER)
        source = this;
    else if (GetTypeId() == TYPE_ID_UNIT && GetOwner() && GetOwner()->GetTypeId() == TYPE_ID_PLAYER)
        source = GetOwner();

    Unit const* target = NULL;
    if (victim->GetTypeId() == TYPE_ID_PLAYER)
        target = victim;
    else if (victim->GetTypeId() == TYPE_ID_UNIT && victim->GetOwner() && victim->GetOwner()->GetTypeId() == TYPE_ID_PLAYER)
...
void Unit::SetPhaseMask(uint32 newPhaseMask, bool update)
{
    ...
    if (IsInWorld())
    {
        ...
        if (GetTypeId() == TYPE_ID_UNIT || (!ToPlayer()->IsGameMaster() && !ToPlayer()->GetSession()->PlayerLogout()))
        {
...
uint32 Unit::GetModelForForm(ShapeshiftForm form) const
{
    if (GetTypeId() == TYPE_ID_PLAYER)
    {
        switch (form)
        {
            case FORM_CAT:
                ...
                // Based on Skin color
                else if (GetCurrentRace() == RACE_TAUREN)
                {
                    uint8 skinColor = GetByteValue(PLAYER_BYTES, 0);
                    // Male
                    if (GetCurrentGender() == GENDER_MALE)
                    {
                        switch (skinColor)
                        {
                            case 12: // White
                ...
                else if (Player::TeamForRace(GetCurrentRace()) == ALLIANCE)
                    return 892;
                else
                    return 8571;
            case FORM_DIREBEAR:
            case FORM_BEAR:
                ...
                else if (GetCurrentRace() == RACE_TAUREN)
                {
                    ...
                    return 2289;
            case FORM_FLIGHT:
                if (Player::TeamForRace(GetCurrentRace()) == ALLIANCE)
                    return 20857;
                return 20872;
    ...
    if (formEntry && formEntry->modelID_A)
    {
        ...
        else
        {
            ...
            if (!modelid && Player::TeamForRace(GetCurrentRace()) == HORDE)
                modelid = formEntry->modelID_A;
        }
    }
    ...
    for (SpellClickInfoContainer::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        if (itr->second.IsFitToRequirements(clicker, this))
        {
            ...
            if (seatId > -1)
            {
                ...
                if (!valid)
                {
                    sLog->OutErrorDatabase("Spell %u specified in npc_spellclick_spells is not a valid vehicle enter aura!", itr->second.spellId);
                    return false;
                }
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if (Player* player = ToPlayer())
    {
        if (vehicle->GetBase()->GetTypeId() == TYPE_ID_PLAYER && player->IsInCombatActive())
            return;
...
void Unit::BuildMovementPacket(ByteBuffer *data) const
{
    ...
    *data << GetOrientation();
    ...
    // 0x00000200
    if (GetUnitMovementFlags() & MOVEMENTFLAG_ON_TRANSPORT)
    {
        if (m_vehicle)
...
void Unit::RewardRage(uint32 damage, uint32 weaponSpeedHitFactor, bool attacker)
{
    ...
    float rageconversion = ((0.0091107836f * GetCurrentLevel() * GetCurrentLevel()) + 3.225598133f * GetCurrentLevel()) + 4.2652911f;

    // Unknown if correct, but lineary adjust rage conversion above level 70
    if (GetCurrentLevel() > 70)
        rageconversion += 13.27f * (GetCurrentLevel() - 70);
...
{
    sLog->OutErrorConsole("Unit::OutDebugInfo");
    sLog->OutConsole("GUID "UI64FMTD", entry %u, type %u, name %s", GetGUID(), GetEntry(), (uint32)GetTypeId(), GetName());
    sLog->OutConsole("OwnerGUID "UI64FMTD", MinionGUID "UI64FMTD", CharmerGUID "UI64FMTD", CharmedGUID "UI64FMTD, GetOwnerGUID(), GetMinionGUID(), GetCharmerGUID(), GetCharmGUID());
    sLog->OutConsole("In world %u, unit type mask %u", (uint32)(IsInWorld() ? 1 : 0), m_unitTypeMask);
    if (IsInWorld())
        sLog->OutConsole("Mapid %u", GetMapId());

    sLog->OutConsoleInLine("Summon Slot: ");
    for (uint32 i = 0; i < MAX_SUMMON_SLOT; ++i)
        sLog->OutConsoleInLine(UI64FMTD", ", m_SummonSlot[i]);
    sLog->OutConsole();

    sLog->OutConsoleInLine("Controlled List: ");
    for (ControlList::const_iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr)
        sLog->OutConsoleInLine(UI64FMTD", ", (*itr)->GetGUID());
    sLog->OutConsole();

    sLog->OutConsoleInLine("Aura List: ");
    for (AuraApplicationMap::const_iterator itr = m_appliedAuras.begin(); itr != m_appliedAuras.end(); ++itr)
        sLog->OutConsoleInLine("%u, ", itr->first);
    sLog->OutConsole();

    if (IsVehicle())
    {
        sLog->OutConsoleInLine("Passenger List: ");
        for (SeatMap::iterator itr = GetVehicleKit()->Seats.begin(); itr != GetVehicleKit()->Seats.end(); ++itr)
            if (Unit* passenger = ObjectAccessor::GetUnit(*GetVehicleBase(), itr->second.Passenger))
                sLog->OutConsoleInLine(UI64FMTD", ", passenger->GetGUID());
        sLog->OutConsole();
    }

    if (GetVehicle())
        sLog->OutConsole("On vehicle %u.", GetVehicleBase()->GetEntry());
}
...
{
    if (enable == HasUnitMovementFlag(MOVEMENT_FLAG_HOVER))
        return false;

    if (enable)
    {
        //! No need to check height on ascent
        AddUnitMovementFlag(MOVEMENT_FLAG_HOVER);
        if (float hh = GetFloatValue(UNIT_FIELD_HOVER_HEIGHT))
            UpdateHeight(GetPositionZ() + hh);
    }
    else
    {
        RemoveUnitMovementFlag(MOVEMENT_FLAG_HOVER);
        if (float hh = GetFloatValue(UNIT_FIELD_HOVER_HEIGHT))
        {
...
{
    if (GetTypeId() == TYPE_ID_PLAYER)
        ToPlayer()->SendMovementSetHover(HasUnitMovementFlag(MOVEMENT_FLAG_HOVER));
...
{
    if (GetTypeId() == TYPE_ID_PLAYER)
        ToPlayer()->SendMovementSetWaterWalking(HasUnitMovementFlag(MOVEMENTFLAG_WATER_WALKING));
