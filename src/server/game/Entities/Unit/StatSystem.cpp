...
{
    float val2 = 0.0f;
    float level = float(GetCurrentLevel());
    ...
    else
    {
        switch (GetCurrentClass())
        {
            ...
            case CLASS_DRUID:
            {
                        ...
                        if (aurEff->GetSpellInfo()->SpellIconID == 1563)
                        {
                            switch (aurEff->GetEffIndex())
                            {
                                ...
                                case 1: // Predatory Strikes (effect 1)
                                    if (Item* mainHand = m_items[EQUIPMENT_SLOT_MAIN_HAND])
                                    {
                ...
                {
                    case FORM_CAT:
                        val2 = GetCurrentLevel() * (mLevelMult + 2.0f) + GetStat(STAT_STRENGTH) * 2.0f + GetStat(STAT_AGILITY) - 20.0f + weapon_bonus + m_baseFeralAP;
                        break;
                    case FORM_BEAR:
                    case FORM_DIREBEAR:
                        val2 = GetCurrentLevel() * (mLevelMult + 3.0f) + GetStat(STAT_STRENGTH) * 2.0f - 20.0f + weapon_bonus + m_baseFeralAP;
                        break;
                    case FORM_MOONKIN:
                        val2 = GetCurrentLevel() * (mLevelMult + 1.5f) + GetStat(STAT_STRENGTH) * 2.0f - 20.0f + m_baseFeralAP;
                        break;
                    default:
    ...
    if (ranged)
    {
        if ((GetCurrentClassMask() & CLASSMASK_WAND_USERS) == 0)
        {
    ...
    else
    {
        ...
        if (GetCurrentClass() == CLASS_SHAMAN || GetCurrentClass() == CLASS_PALADIN)                      // mental quickness
            UpdateSpellDamageAndHealingBonus();
...
void Player::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float& min_damage, float& max_damage)
{
    ...
    float weapon_mindamage = GetWeaponDamageRange(attType, MIN_DAMAGE);
    float weapon_maxdamage = GetWeaponDamageRange(attType, MAX_DAMAGE);

    if (IsInFeralForm())                                    //check if player is druid and in cat or bear forms
    {
        uint8 lvl = GetCurrentLevel();
        if (lvl > 60)
            lvl = 60;
...
float Player::GetMissPercentageFromDefence() const
{
    ...
    uint32 pclass = GetCurrentClass()-1;
...
bool Guardian::UpdateStats(Stats stat)
{
    ...
    if (IsPetGhoul() && (stat == STAT_STAMINA || stat == STAT_STRENGTH))
    {
        ...
        aurEff = owner->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELL_FAMILY_DEATH_KNIGHT, 3010, 0);
        if (aurEff)
        {
    ...
    {
        if (owner->GetCurrentClass() == CLASS_WARLOCK && IsPet())
        {
    ...
    {
        if (owner->GetCurrentClass() == CLASS_WARLOCK || owner->GetCurrentClass() == CLASS_MAGE)
        {
...
void Guardian::UpdateMaxHealth()
{
	...
	{
		if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(56247))
		{
			if (GetEntry() == ENTRY_VOIDWALKER && owner->HasAura(spellInfo->Id) && owner->GetTypeId() == TYPE_ID_PLAYER)
				AddPctN(value, float(spellInfo->Effects[EFFECT_0].CalcValue()));
		}
	}
...
void Guardian::UpdateMaxPower(Powers power)
{
	...
	if (HasUnitTypeMask(UNIT_MASK_SUMMON || UNIT_MASK_MINION || UNIT_MASK_GUARDIAN || UNIT_MASK_PET || UNIT_MASK_HUNTER_PET))
	{
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    if (m_owner->GetTypeId() == TYPE_ID_PLAYER)
    {
        //force of nature
    ...
    UnitMods unitMod = UNIT_MOD_DAMAGE_MAIN_HAND;
    ...
    float weapon_mindamage = GetWeaponDamageRange(BASE_ATTACK, MIN_DAMAGE);
    float weapon_maxdamage = GetWeaponDamageRange(BASE_ATTACK, MAX_DAMAGE);
...
void Guardian::SetBonusDamage(int32 damage)
{
    ...
    if (GetOwner()->GetTypeId() == TYPE_ID_PLAYER)
