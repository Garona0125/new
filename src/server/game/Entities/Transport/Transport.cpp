...
void MapManager::LoadTransports()
{
    ...
    {
		sLog->OutConsole("=======================================================");
        sLog->OutConsole(">> Loaded 0 transports. DB table `transports` is empty!");
		sLog->OutConsole("=======================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!goinfo)
        {
            sLog->OutErrorDatabase("Transport ID:%u, Name: %s, will not be loaded, gameobject_template missing", entry, name.c_str());
            continue;
        }

        if (goinfo->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
        {
            sLog->OutErrorDatabase("Transport ID:%u, Name: %s, will not be loaded, gameobject_template type wrong", entry, name.c_str());
            continue;
        }

        // sLog->OutConsole("Loading transport %d between %s, %s", entry, name.c_str(), goinfo->name);
        ...
        {
            sLog->OutErrorDatabase("Transport (path id %u) path size = 0. Transport ignored, check DBC files or transport GO data0 field.", goinfo->moTransport.taxiPathId);
            delete t;
            continue;
        }
    ...
    if (result)                                              // wrong data found
    {
        do
        {
            ...
            uint32 entry = fields[1].GetUInt32();
            std::string name = fields[2].GetString();
            sLog->OutErrorDatabase("Transport %u '%s' have record (GUID: %u) in `gameobject`. Transports must not have any records in `gameobject` or its behavior will be unpredictable/bugged.", entry, name.c_str(), guid);
        }
        while (result->NextRow());
    }

	sLog->OutConsole("==================================");
    sLog->OutConsole(">> Loaded %u transports in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==================================");
    sLog->OutConsole();
}
...
Transport* MapManager::LoadTransportInMap(Map* instance, uint32 goEntry, uint32 period)
{
    ...
    if (!goInfo)
    {
        sLog->OutErrorDatabase("Transport ID:%u, will not be loaded, gameobject_template missing", goEntry);
        return NULL;
    }

    if (goInfo->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
    {
        sLog->OutErrorDatabase("Transport ID:%u, Name: %s, will not be loaded, gameobject_template type wrong", goEntry, goInfo->name.c_str());
        return NULL;
    }
    ...
    if (!t->GenerateWaypoints(goInfo->moTransport.taxiPathId, mapsUsed))
    {
        sLog->OutErrorDatabase("Transport (path id %u) path size = 0. Transport ignored, check DBC files or the gameobject's data0 field.", goInfo->moTransport.taxiPathId);
        delete t;
        return NULL;
    }
...
void MapManager::LoadTransportNPCs()
{
    ...
    {
		sLog->OutConsole("===================================================================");
        sLog->OutConsole(">> Loaded 0 transport NPCs. DB table `creature_transport` is empty!");
		sLog->OutConsole("===================================================================");
        sLog->OutConsole();
        return;
    }
	...
	sLog->OutConsole("======================================");
    sLog->OutConsole(">> Loaded %u transport npcs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("======================================");
    sLog->OutConsole();
}
...
bool Transport::Create(uint32 guidlow, uint32 entry, uint32 mapid, float x, float y, float z, float ang, uint32 animprogress, uint32 dynflags)
{
    ...
    if (!IsPositionValid())
    {
        sLog->OutErrorConsole("Transport (GUID: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
            guidlow, x, y);
        return false;
    }
    ...
    if (!goinfo)
    {
        sLog->OutErrorDatabase("Transport not created: entry in `gameobject_template` not found, guidlow: %u map: %u  (X: %f Y: %f Z: %f) ang: %f", guidlow, mapid, x, y, z, ang);
        return false;
    }
...
bool Transport::GenerateWaypoints(uint32 pathid, std::set<uint32> &mapids)
{
    ...
    //        sLog->OutConsole("%f, %f, %f, %f, %f, %f, %f", keyFrames[i].x, keyFrames[i].y, keyFrames[i].distUntilStop, keyFrames[i].distSinceStop, keyFrames[i].distFromPrev, keyFrames[i].tFrom, keyFrames[i].tTo);
    //    }
    ...
    for (size_t i = 0; i < keyFrames.size() - 1; ++i)
    {
        ...
        if (((d < keyFrames[i + 1].distFromPrev) && (tTo > 0)))
        {
            while ((d < keyFrames[i + 1].distFromPrev) && (tTo > 0))
            {
                ...
                if (d > 0)
                {
                    ...
                    //                    sLog->OutConsole("T: %d, D: %f, x: %f, y: %f, z: %f", t, d, newX, newY, newZ);
                    if (teleport)
        ...
        //        sLog->OutConsole("T: %d, x: %f, y: %f, z: %f, t:%d", t, pos.x, pos.y, pos.z, teleport);
...
void Transport::Update(uint32 p_diff)
{
    ...
    {
        if (!AIM_Initialize())
            sLog->OutErrorConsole("Could not initialize GameObjectAI for Transport");
    }
	else
        AI()->UpdateAI(p_diff);
...
uint32 Transport::AddNPCPassenger(uint32 tguid, uint32 entry, float x, float y, float z, float o, uint32 anim)
{
    ...
    creature->AddUnitMovementFlag(MOVEMENTFLAG_ON_TRANSPORT);
    creature->m_movementInfo.guid = GetGUID();
    creature->m_movementInfo.t_pos.Relocate(x, y, z, o);

    if (anim)
        creature->SetUInt32Value(UNIT_NPC_EMOTE_STATE, anim);
    ...
    {
        sLog->OutErrorConsole("Creature (guidlow %d, entry %d) not created. Suggested coordinates isn't valid (X: %f Y: %f)", creature->GetGUIDLow(), creature->GetEntry(), creature->GetPositionX(), creature->GetPositionY());
        delete creature;
        return 0;
    }
