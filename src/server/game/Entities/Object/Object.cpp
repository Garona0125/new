...
uint32 GuidHigh2TypeId(uint32 guid_hi)
{
    switch (guid_hi)
    {
        ...
        case HIGHGUID_PLAYER:
			return TYPE_ID_PLAYER;
        case HIGHGUID_GAMEOBJECT:
			return TYPE_ID_GAMEOBJECT;
        case HIGHGUID_DYNAMICOBJECT:
			return TYPE_ID_DYNAMIC_OBJECT;
        case HIGHGUID_CORPSE:
			return TYPE_ID_CORPSE;
...
{
    m_objectTypeId = TYPE_ID_OBJECT;
    m_objectType = TYPE_MASK_OBJECT;
...
void Object::_Create(uint32 guidlow, uint32 entry, HighGuid guidhigh)
{
    ...
    switch (m_objectType)
    {
        //case TYPE_ID_ITEM:       type = 3; break;
        //case TYPE_ID_CONTAINER:  type = 7; break;   //+4
        //case TYPE_ID_UNIT:       type = 9; break;   //+2
        //case TYPE_ID_PLAYER:     type = 25; break;  //+16
        //case TYPE_ID_GAMEOBJECT: type = 33; break;  //+8
        case TYPE_ID_DYNAMIC_OBJECT:
			type = 65;
			break;  //+32
        //case TYPE_ID_CORPSE:     type = 129; break;  //+64
        default:
			type = m_objectType;
			break;
    }
...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData* data, Player* target) const
{
    ...
    if (flags & UPDATEFLAG_STATIONARY_POSITION)
    {
        ...
        if (isType(TYPE_MASK_DYNAMIC_OBJECT) || isType(TYPE_MASK_CORPSE) || isType(TYPE_MASK_PLAYER))
        ...
        if (isType(TYPE_MASK_GAMEOBJECT))
        {
            switch (((GameObject*)this)->GetGoType())
            {
                case GAMEOBJECT_TYPE_TRAP:
                case GAMEOBJECT_TYPE_DUEL_ARBITER:
                case GAMEOBJECT_TYPE_FLAG_STAND:
                case GAMEOBJECT_TYPE_FLAG_DROP:
                    updatetype = UPDATETYPE_CREATE_OBJECT2;
                    break;
...
void Object::_BuildValuesUpdate(uint8 updatetype, ByteBuffer * data, UpdateMask* updateMask, Player* target) const
{
    ...
    {
        if (isType(TYPE_MASK_GAMEOBJECT) && !((GameObject*)this)->IsDynTransport())
        {
    ...
    {
        if (isType(TYPE_MASK_GAMEOBJECT) && !((GameObject*)this)->IsTransport())
        {
            if (((GameObject*)this)->ActivateToQuest(target) || target->IsGameMaster())
                IsActivateToQuest = true;
    ...
    *data << (uint8)updateMask->GetBlockCount();
    ...
    if (isType(TYPE_MASK_UNIT))                               // unit (creature/player) case
    {
        for (uint16 index = 0; index < m_valuesCount; ++index)
        {
            if (updateMask->GetBit(index))
            {
                ...
                else if (index >= UNIT_FIELD_BASE_ATTACK_TIME && index <= UNIT_FIELD_RANGED_ATTACK_TIME)
                {
                    // convert from float to uint32 and send
                    *data << uint32(m_floatValues[index] < 0 ? 0 : m_floatValues[index]);
                }
                // there are some float values which may be negative or can't get negative due to other checks
                else if ((index >= UNIT_FIELD_NEG_STAT_0   && index <= UNIT_FIELD_NEG_STAT_4) ||
                    (index >= UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE  && index <= (UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE + 6)) ||
                    (index >= UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE  && index <= (UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE + 6)) ||
                    (index >= UNIT_FIELD_POS_STAT_0   && index <= UNIT_FIELD_POS_STAT_4))
                {
                        ...
                        if (cinfo->flags_extra & CREATURE_FLAG_EXTRA_TRIGGER)
                        {
                            ...
                            else
                            {
                                if (cinfo->Modelid2)
                                    ...
                                    *data << MODEL_ID_INVISIBLE; // world invisible trigger's model
                            }
                        }
                        else
                            *data << m_uint32Values[index];
    ...
    else if (isType(TYPE_MASK_GAMEOBJECT))                    // gameobject case
    {
        for (uint16 index = 0; index < m_valuesCount; ++index)
        {
            if (updateMask->GetBit(index))
                    ...
                    if (IsActivateToQuest)
                    {
                        switch (ToGameObject()->GetGoType())
                        {
                            case GAMEOBJECT_TYPE_CHEST:
                                if (target->IsGameMaster())
                                    *data << uint16(GO_DYNAMIC_FLAG_LO_ACTIVATE);
                                else
                                    *data << uint16(GO_DYNAMIC_FLAG_LO_ACTIVATE | GO_DYNAMIC_FLAG_LO_SPARKLE);
                                *data << uint16(-1);
                                break;
                            case GAMEOBJECT_TYPE_GENERIC:
                                if (target->IsGameMaster())
                                    ...
                                    *data << uint16(GO_DYNAMIC_FLAG_LO_SPARKLE);
                                *data << uint16(-1);
                                break;
...
void Object::SetByteValue(uint16 index, uint8 offset, uint8 value)
{
    ...
    if (offset > 4)
    {
        sLog->OutErrorConsole("Object::SetByteValue: wrong offset %u", offset);
        return;
    }
...
void Object::SetUInt16Value(uint16 index, uint8 offset, uint16 value)
{
    ...
    if (offset > 2)
    {
        sLog->OutErrorConsole("Object::SetUInt16Value: wrong offset %u", offset);
        return;
    }
...
void Object::SetByteFlag(uint16 index, uint8 offset, uint8 newFlag)
{
    ...
    if (offset > 4)
    {
        sLog->OutErrorConsole("Object::SetByteFlag: wrong offset %u", offset);
        return;
    }
...
void Object::RemoveByteFlag(uint16 index, uint8 offset, uint8 oldFlag)
{
    ...
    if (offset > 4)
    {
        sLog->OutErrorConsole("Object::RemoveByteFlag: wrong offset %u", offset);
        return;
    }
...
{
    sLog->OutErrorConsole("Attempt %s non-existed value field: %u (count: %u) for object typeid: %u type mask: %u", (set ? "set value to" : "get value from"), index, m_valuesCount, GetTypeId(), m_objectType);
...
{
    sLog->OutConsole("MOVEMENT INFO");
    sLog->OutConsole("guid " UI64FMTD, guid);
    sLog->OutConsole("flags %u", flags);
    sLog->OutConsole("flags2 %u", flags2);
    sLog->OutConsole("time %u current time " UI64FMTD "", flags2, uint64(::time(NULL)));
    sLog->OutConsole("position: `%s`", pos.ToString().c_str());

    if (flags & MOVEMENTFLAG_ON_TRANSPORT)
    {
        sLog->OutConsole("TRANSPORT:");
        sLog->OutConsole("guid: " UI64FMTD, t_guid);
        sLog->OutConsole("position: `%s`", t_pos.ToString().c_str());
        sLog->OutConsole("seat: %i", t_seat);
        sLog->OutConsole("time: %u", t_time);
        if (flags2 & MOVEMENTFLAG2_INTERPOLATED_MOVEMENT)
            sLog->OutConsole("time2: %u", t_time2);
    }

    if ((flags & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING)) || (flags2 & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING))
        sLog->OutConsole("pitch: %f", pitch);

    sLog->OutConsole("fallTime: %u", fallTime);
    if (flags & MOVEMENTFLAG_FALLING)
        sLog->OutConsole("j_zspeed: %f j_sinAngle: %f j_cosAngle: %f j_xyspeed: %f", j_zspeed, j_sinAngle, j_cosAngle, j_xyspeed);

    if (flags & MOVEMENTFLAG_SPLINE_ELEVATION)
        sLog->OutConsole("splineElevation: %f", splineElevation);
}
...
void WorldObject::SetActive(bool on)
{
    ...
    if (on)
    {
        ...
        else if (GetTypeId() == TYPE_ID_DYNAMIC_OBJECT)
...
bool WorldObject::CanDetectStealthOf(WorldObject const* obj) const
{
    ...
    for (uint32 i = 0; i < TOTAL_STEALTH_TYPES; ++i)
    {
        ...
        detectionValue += int32(GetCurrentLevelForTarget(obj) - 1) * 5;

        // Apply modifiers
        detectionValue += m_stealthDetect.GetValue(StealthType(i));
		if (obj->isType(TYPE_MASK_GAMEOBJECT))
			if (Unit* owner = ((GameObject*)obj)->GetOwner())
				detectionValue -= int32(owner->GetCurrentLevelForTarget(this) - 1) * 5;
...
void WorldObject::SendPlaySound(uint32 Sound, bool OnlySelf)
{
    ...
    data << Sound;
    if (OnlySelf && GetTypeId() == TYPE_ID_PLAYER)
...
void WorldObject::MonsterTextEmote(const char* text, uint64 TargetGuid, bool IsBossEmote)
{
    ...
    SendMessageToSetInRange(&data, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXT_EMOTE), true);
}
...
void WorldObject::MonsterTextEmote(int32 textId, uint64 TargetGuid, bool IsBossEmote)
{
    ...
    Trinity::LocalizedPacketDo<Trinity::MonsterChatBuilder> say_do(say_build);
    Trinity::PlayerDistWorker<Trinity::LocalizedPacketDo<Trinity::MonsterChatBuilder> > say_worker(this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXT_EMOTE), say_do);
    TypeContainerVisitor<Trinity::PlayerDistWorker<Trinity::LocalizedPacketDo<Trinity::MonsterChatBuilder> >, WorldTypeMapContainer > message(say_worker);
    cell.Visit(p, message, *GetMap(), *this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXT_EMOTE));
}
...
void WorldObject::MonsterWhisper(int32 textId, uint64 receiver, bool IsBossWhisper)
{
    ...
    LocaleConstant loc_idx = player->GetSession()->GetSessionDbLocaleIndex();
    char const* text = sObjectMgr->GetQuantumSystemText(textId, loc_idx);
...
{
    ASSERT(map);
    ASSERT(!IsInWorld() || GetTypeId() == TYPE_ID_CORPSE);
...
void WorldObject::AddObjectToRemoveList()
{
    ...
    if (!map)
    {
        sLog->OutErrorConsole("Object (TypeId: %u Entry: %u GUID: %u) at attempt add to move list not have valid map (Id: %u).", GetTypeId(), GetEntry(), GetGUIDLow(), GetMapId());
        return;
    }
...
TempSummon* WorldObject::SummonCreature(uint32 entry, const Position &pos, TempSummonType spwtype, uint32 duration, uint32 /*vehId*/) const
{
    if (Map* map = FindMap())
    {
        if (TempSummon* summon = map->SummonCreature(entry, pos, NULL, duration, isType(TYPE_MASK_UNIT) ? (Unit*)this : NULL))
        {
...
Pet* Player::SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 duration)
{
    ...
    {
        sLog->OutErrorConsole("Pet (guidlow %d, entry %d) not summoned. Suggested coordinates isn't valid (X: %f Y: %f)", pet->GetGUIDLow(), pet->GetEntry(), pet->GetPositionX(), pet->GetPositionY());
        delete pet;
        return NULL;
    }
    ...
    {
        sLog->OutErrorConsole("no such creature entry %u", entry);
        delete pet;
        return NULL;
    }
    ...
    pet->InitStatsForLevel(GetCurrentLevel());
    ...
    switch (petType)
    {
        case SUMMON_PET:
            ...
            pet->SetUInt32Value(UNIT_FIELD_PET_NEXT_LEVEL_EXP, 1000);
            pet->SetFullHealth();
            pet->SetPower(POWER_MANA, pet->GetMaxPower(POWER_MANA));
            pet->SetUInt32Value(UNIT_FIELD_PET_NAME_TIME_STAMP, uint32(time(NULL))); // cast can't be helped in this case
            break;
        default:
            break;
    }
...
GameObject* WorldObject::SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime)
{
    ...
    if (!goinfo)
    {
        sLog->OutErrorDatabase("Gameobject template %u not found in database!", entry);
        return NULL;
    }
    Map* map = GetMap();
    ...
    if (GetTypeId() == TYPE_ID_PLAYER || GetTypeId() == TYPE_ID_UNIT) //not sure how to handle this
...
Creature* WorldObject::SummonTrigger(float x, float y, float z, float ang, uint32 duration, CreatureAI* (*GetAI)(Creature*))
{
    ...
    if (GetTypeId() == TYPE_ID_PLAYER || GetTypeId() == TYPE_ID_UNIT)
    {
        summon->SetCurrentFaction(((Unit*)this)->GetFaction());
        summon->SetLevel(((Unit*)this)->GetCurrentLevel());
    }
...
{
    ASSERT((GetTypeId() == TYPE_ID_GAMEOBJECT || GetTypeId() == TYPE_ID_UNIT) && "Only GOs and creatures can summon npc groups!");

    std::vector<TempSummonData> const* data = sObjectMgr->GetSummonGroup(GetEntry(), GetTypeId() == TYPE_ID_GAMEOBJECT ? SUMMONER_TYPE_GAMEOBJECT : SUMMONER_TYPE_CREATURE, group);
    if (!data)
        return;
...
Creature* WorldObject::FindCreatureWithDistance(uint32 entry, float range, bool alive) const
{
...
GameObject* WorldObject::FindGameobjectWithDistance(uint32 entry, float range) const
{
    GameObject* go = NULL;
...
Player* WorldObject::FindPlayerWithDistance(float range, bool alive) const
{
	Player* player = NULL;
...
void WorldObject::DestroyForNearbyPlayers()
{
    ...
    for (std::list<Player*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
    {
        ...
        if (isType(TYPE_MASK_UNIT) && ((Unit*)this)->GetCharmerGUID() == player->GetGUID()) // TODO: this is for puppet
            continue;
