...
#include "Config.h"
#include "../../../scripts/QuantumCoreScripts/SystemScripts/world_transmogrification_system.h"
    ...
    if (_group)
    {
        ...
        for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
            if (Player* member = itr->getSource())
                ...
                {
                    const uint8 lvl = member->GetCurrentLevel();
                    // 2.1. _count - number of alive group members within reward distance;
                    ++_count;
                    // 2.2. _sumLevel - sum of levels of alive group members within reward distance;
                    _sumLevel += lvl;
                    ...
                    if (_victim->GetCurrentLevel() > grayLevel && (!_maxNotGrayMember || _maxNotGrayMember->GetCurrentLevel() < lvl))
                        _maxNotGrayMember = member;
                }
        // 2.5. _isFullXP - flag identifying that for all group members victim is not gray,
        //      so 100% XP will be rewarded (50% otherwise).
        _isFullXP = _maxNotGrayMember && (_maxLevel == _maxNotGrayMember->GetCurrentLevel());
    }
    else
        _count = 1;
}
...
inline void KillRewarder::_RewardXP(Player* player, float rate)
{
    ...
    if (_group)
    {
        ...
        if (_maxNotGrayMember && player->IsAlive() &&
            _maxNotGrayMember->GetCurrentLevel() >= player->GetCurrentLevel())
            xp = _isFullXP ?
            ...
            for (GroupReference *itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
                if (Player* member = itr->getSource())
                    if (
                        ...
                        member->HasItemOrGemWithIdEquipped(sWorld->getIntConfig(CONFIG_XP_BOOST_ITEM_ID), 1))
                    {
                        boostItem = true;
                        break;
                    }
    ...
    {
        boostItem = player->HasItemOrGemWithIdEquipped(sWorld->getIntConfig(CONFIG_XP_BOOST_ITEM_ID), 1);
    }
    ...
    if (xp)
    {
        ...
        if (boostItem && player->GetCurrentLevel() <= sWorld->getIntConfig(CONFIG_XP_BOOST_MAX_LEVEL))
            if (_group)
...
void KillRewarder::_RewardPlayer(Player* player, bool isDungeon)
{
    ...
    if (!_isBattleGround)
    {
        ...
        if (_victim->GetTypeId() == TYPE_ID_PLAYER)
    ...
    if (!_isPvP || _isBattleGround)
    {
        const float rate = _group ?
            _groupRate * float(player->GetCurrentLevel()) / _sumLevel : // Group rate depends on summary level.
            ...
            for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
			{
                if (Player* member = itr->getSource())
				{
                    if (member->IsAtGroupRewardDistance(_victim))
					{
						...
						member->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_SPECIAL_PVP_KILL, 1, 0, _victim);
					}
				}
			}
        }
...
#pragma warning(default:4355)
    ...
    m_objectType |= TYPE_MASK_PLAYER;
    m_objectTypeId = TYPE_ID_PLAYER;
	...
	// PlayedTimeReward
	PlayedTimeRewardIntervalTime = sWorld->getIntConfig(CONFIG_PLAYED_TIME_REWARD_INTERVAL_TIME);
	PlayedTimeRewardMoney = sWorld->getIntConfig(CONFIG_PLAYED_TIME_REWARD_MONEY);
	PlayedTimeRewardHonorPoints = sWorld->getIntConfig(CONFIG_PLAYED_TIME_REWARD_HONOR);
	PlayedTimeRewardArenaPoints = sWorld->getIntConfig(CONFIG_PLAYED_TIME_REWARD_ARENA);
...
bool Player::Create(uint32 guidlow, CharacterCreateInfo* createInfo)
{
    ...
    if (!info)
    {
        sLog->OutErrorConsole("Player::Create: Possible hacking-attempt: Account %u tried creating a character named '%s' with an invalid race/class pair (%u/%u) - refusing to do so.",
    ...
    if (!cEntry)
    {
        sLog->OutErrorConsole("Player::Create: Possible hacking-attempt: Account %u tried creating a character named '%s' with an invalid character class (%u) - refusing to do so (wrong DBC-files?)",
    ...
    {
        sLog->OutErrorConsole("Player::Create: Possible hacking-attempt: Account %u tried creating a character named '%s' with an invalid gender (%hu) - refusing to do so",
    ...
    SetUInt32Value(PLAYER_GUILD_ID, 0);
    SetUInt32Value(PLAYER_GUILD_RANK, 0);
    SetUInt32Value(PLAYER_GUILD_TIME_STAMP, 0);
    ...
    // set starting level
    uint32 start_level = GetCurrentClass() != CLASS_DEATH_KNIGHT
            ...
            if (iProto->Class == ITEM_CLASS_CONSUMABLE && iProto->SubClass == ITEM_SUBCLASS_FOOD)
            {
                switch (iProto->Spells[0].SpellCategory)
                {
                    case SPELL_CATEGORY_FOOD:                                // food
                        count = GetCurrentClass() == CLASS_DEATH_KNIGHT ? 10 : 4;
                        break;
...
bool Player::StoreNewItemInBestSlots(uint32 titem_id, uint32 titem_amount)
{
    ...
    // item can't be added
    sLog->OutErrorConsole("STORAGE: Can't equip or store initial item %u for race %u class %u, error msg = %u", titem_id, GetCurrentRace(), GetCurrentClass(), msg);
    return false;
}
    ...
    if (m_MirrorTimerFlags & UNDERWATER_INWATER)
    {
        ...
        else                                                              // If activated - do tick
        {
            ...
            if (m_MirrorTimer[BREATH_TIMER] < 0)
            {
                ...
                uint32 damage = GetMaxHealth() / 5 + urand(0, GetCurrentLevel()-1);
...
void Player::Update(uint32 p_time)
{
    ...
    if (IsCharmed())
        if (Unit* charmer = GetCharmer())
            if (charmer->GetTypeId() == TYPE_ID_UNIT && charmer->IsAlive())
                UpdateCharmedAI();
	...
	// Played Time Reward
	if (PlayedTimeRewardIntervalTime > 0)
	{
		if (p_time >= PlayedTimeRewardIntervalTime)
		{
			PlayedTimeRewardIntervalTime = sWorld->getIntConfig(CONFIG_PLAYED_TIME_REWARD_INTERVAL_TIME);
			ModifyMoney(PlayedTimeRewardMoney);
			ModifyHonorPoints(PlayedTimeRewardHonorPoints);
			ModifyArenaPoints(PlayedTimeRewardArenaPoints);

			//sWorld->SendWorldText(LANG_PLAYED_TIME_REWARD_ANNOUNCE);
			sLog->OutConsole("REWARD MANAGER: All Players Awarded!");
		}
		else PlayedTimeRewardIntervalTime -= p_time;
	}
	...
	// Special function for PVP Rank and Proof of Demice Achievements
	sScriptMgr->OnPlayerUpdate(this, p_time);

	// Fix player bug in stuck stats from Gear Scalling Aura
	if (!ToPlayer()->IsAlive() || !HasAuraType(SPELL_AURA_CONTROL_VEHICLE))
	{
		ToPlayer()->RemoveAurasDueToSpell(65266); //Gear Scalling Aura
		ToPlayer()->RemoveAurasDueToSpell(65635); //Gear Scalling Aura
		ToPlayer()->RemoveAurasDueToSpell(65636); //Gear Scalling Aura
		ToPlayer()->RemoveAurasDueToSpell(66666); //Gear Scalling Aura
		ToPlayer()->RemoveAurasDueToSpell(66667); //Gear Scalling Aura
		ToPlayer()->RemoveAurasDueToSpell(66668); //Gear Scalling Aura
	}

	// Fix stuck Overkill + Master of Subtlety (part 1)
	if (HasAura(31665))
		ToPlayer()->RemoveAurasDueToSpell(58427);

	// Fix stuck Overkill + Master of Subtlety (part 2)
	if (HasAura(58427))
		ToPlayer()->RemoveAurasDueToSpell(31665);

	// Interrupt casting Bladestorm after disarm
	if (HasAura(46924) && HasAuraType(SPELL_AURA_MOD_DISARM))
		ToPlayer()->RemoveAurasDueToSpell(46924);
	...
	// Fix Stack Of Enrage + Wrecking Crew
	if (HasAura(14204) && HasAura(57522))
		ToPlayer()->RemoveAura(57522);

	// Fix Saronite Vapors (General Vezax)
	if (HasAura(63323) || HasAura(63322))
	{
		ToPlayer()->RemoveAura(62692);
		ToPlayer()->RemoveAura(64848);
	}

	// Remove Visual Aura of Celerity after fade Aura of Celerity
	if (!HasAura(62320))
		ToPlayer()->RemoveAura(62398);

	// Remove Flying Black Qiraji Battle Tank in Arena, Bg and Instance
	if (HasAura(31700) && ToPlayer()->InArena() || ToPlayer()->InBattleground() || ToPlayer()->GetMap()->IsDungeon() || ToPlayer()->GetMap()->IsRaid())
		ToPlayer()->RemoveAura(31700);

	// Visual Resurrection Sickness
	if (ToPlayer()->IsAlive() && HasAura(15007) && !HasAura(35841))
		AddAura(35841, this);

	// Remove Visual Resurrection Sickness
	if (ToPlayer()->IsAlive() && !HasAura(15007) && HasAura(35841))
		ToPlayer()->RemoveAura(35841);

	// Visual Aura For Priest Shadowform
	if (HasAura(15473) && !HasAura(51126))
		AddAura(51126, this);

	// Remove Visual Priest Shadowform
	if (!HasAura(15473) && HasAura(51126))
		ToPlayer()->RemoveAura(51126);

	// Mount Check Part 1
	if (HasAura(31700) && (HasAura(75614) || (HasAura(75617) || (HasAura(75618) || (HasAura(75619) || (HasAura(75620) || (HasAura(76153)
		|| (HasAura(458) || (HasAura(459) || (HasAura(468) || (HasAura(470) || (HasAura(471) || (HasAura(472) || (HasAura(578) || (HasAura(579)
		|| (HasAura(580) || (HasAura(581) || (HasAura(3363) || (HasAura(5784) || (HasAura(6648) || (HasAura(6653) || (HasAura(6654)
		|| (HasAura(6777) || (HasAura(6896) || (HasAura(6897) || (HasAura(6898) || (HasAura(6899) || (HasAura(8394) || (HasAura(8395)
		|| (HasAura(8396)  || (HasAura(8980) || (HasAura(10787) || (HasAura(10788) || (HasAura(10789) || (HasAura(10790) || (HasAura(10792)
		|| (HasAura(10793) || (HasAura(10795) || (HasAura(10796) || (HasAura(10798) || (HasAura(10799) || (HasAura(10800) || (HasAura(10801)
		|| (HasAura(10802) || (HasAura(10803) || (HasAura(10804) || (HasAura(10873) || (HasAura(10969) || (HasAura(13819) || (HasAura(15779)
		|| (HasAura(15780) || (HasAura(15781) || (HasAura(16055) || (HasAura(16056) || (HasAura(16058) || (HasAura(16059) || (HasAura(16060)
		|| (HasAura(16080) || (HasAura(16081) || (HasAura(16082) || (HasAura(16083) || (HasAura(16084) || (HasAura(17229) || (HasAura(17450)
		|| (HasAura(17453) || (HasAura(17454) || (HasAura(17455) || (HasAura(17456) || (HasAura(17458) || (HasAura(17459) || (HasAura(17460)
		|| (HasAura(17461) || (HasAura(17462) || (HasAura(17463) || (HasAura(17464) || (HasAura(17465) || (HasAura(17481) || (HasAura(18363)
		|| (HasAura(18989) || (HasAura(18990) || (HasAura(18991) || (HasAura(18992) || (HasAura(22717) || (HasAura(22718) || (HasAura(22719)
		|| (HasAura(22720) || (HasAura(22721) || (HasAura(22722) || (HasAura(22723) || (HasAura(22724) || (HasAura(23161) || (HasAura(23214)
		|| (HasAura(23219) || (HasAura(23220) || (HasAura(23221) || (HasAura(23222) || (HasAura(23223) || (HasAura(23225) || (HasAura(23227)
		|| (HasAura(23228) || (HasAura(23229) || (HasAura(23238) || (HasAura(23239) || (HasAura(23240) || (HasAura(23241) || (HasAura(23242)
		|| (HasAura(23243) || (HasAura(23246) || (HasAura(23247) || (HasAura(23248) || (HasAura(23249) || (HasAura(23250) || (HasAura(23251)
		)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
	{
		ToPlayer()->RemoveMount();
		ToPlayer()->RemoveAurasByType(SPELL_AURA_MOUNTED);
		ToPlayer()->RemoveAura(30424);
	}

	// Mount Check Part 2
	if (HasAura(31700) && (HasAura(23252)
		|| (HasAura(23338) || (HasAura(23509) || (HasAura(23510) || (HasAura(24242) || (HasAura(24252) || (HasAura(24576)
		|| (HasAura(25675) || (HasAura(25858) || (HasAura(25859) || (HasAura(25863) || (HasAura(25953) || (HasAura(26054) || (HasAura(26055)
		|| (HasAura(26056) || (HasAura(26655) || (HasAura(26656) || (HasAura(28828) || (HasAura(29059) || (HasAura(30174) || (HasAura(30829)
		|| (HasAura(30837) || (HasAura(31973) || (HasAura(32235) || (HasAura(32239) || (HasAura(32240) || (HasAura(32242) || (HasAura(32243)
		|| (HasAura(32244) || (HasAura(32245) || (HasAura(32246) || (HasAura(32289) || (HasAura(32290) || (HasAura(32292) || (HasAura(32295)
		|| (HasAura(32296) || (HasAura(32297) || (HasAura(32345) || (HasAura(32420) || (HasAura(33630) || (HasAura(33631) || (HasAura(33660)
		|| (HasAura(34068) || (HasAura(34406) || (HasAura(34407) || (HasAura(34767) || (HasAura(34769) || (HasAura(34790) || (HasAura(34795)
		|| (HasAura(34896) || (HasAura(34897) || (HasAura(34898) || (HasAura(34899) || (HasAura(35018) || (HasAura(35020) || (HasAura(35022)
		|| (HasAura(35025) || (HasAura(35027) || (HasAura(35028) || (HasAura(35710) || (HasAura(35711) || (HasAura(35712) || (HasAura(35713)
		|| (HasAura(35714) || (HasAura(36702) || (HasAura(37015) || (HasAura(39315) || (HasAura(39316) || (HasAura(39317) || (HasAura(39318)
		|| (HasAura(39319) || (HasAura(39450) || (HasAura(39798) || (HasAura(39800) || (HasAura(39801) || (HasAura(39802) || (HasAura(39803)
		|| (HasAura(39910) || (HasAura(40192) || (HasAura(40212) || (HasAura(41252) || (HasAura(41513) || (HasAura(41514) || (HasAura(41515)
		|| (HasAura(41516) || (HasAura(41517) || (HasAura(41518) || (HasAura(42667) || (HasAura(42668) || (HasAura(42680) || (HasAura(42683)
		|| (HasAura(42692) || (HasAura(42776) || (HasAura(42777) || (HasAura(43688) || (HasAura(43899) || (HasAura(43900) || (HasAura(43927)
		|| (HasAura(44151) || (HasAura(44153) || (HasAura(44317) || (HasAura(44655) || (HasAura(44744) || (HasAura(44824) || (HasAura(44825)
		|| (HasAura(44827) || (HasAura(46197) || (HasAura(46199) || (HasAura(46628) || (HasAura(46980) || (HasAura(47037) || (HasAura(47977)
		|| (HasAura(48023) || (HasAura(48024) || (HasAura(48025) || (HasAura(48027) || (HasAura(48778) || (HasAura(48954) || (HasAura(49193)
		|| (HasAura(49322) || (HasAura(49378) || (HasAura(49379) || (HasAura(49908) || (HasAura(50869) || (HasAura(50870) || (HasAura(51412)
		|| (HasAura(51617) || (HasAura(51621) || (HasAura(51960) || (HasAura(54726) || (HasAura(54727) || (HasAura(54729) || (HasAura(54753)
		|| (HasAura(55164) || (HasAura(55293) || (HasAura(55531) || (HasAura(58615) || (HasAura(58819) || (HasAura(58983) || (HasAura(58997)
		|| (HasAura(58999) || (HasAura(59567) || (HasAura(59568) || (HasAura(59569) || (HasAura(59570) || (HasAura(59571) || (HasAura(59572)
		|| (HasAura(59573) || (HasAura(59650) || (HasAura(59785) || (HasAura(59788) || (HasAura(59791) || (HasAura(59793) || (HasAura(59797)
		|| (HasAura(59799) || (HasAura(59802) || (HasAura(59804) || (HasAura(59961) || (HasAura(59976) || (HasAura(59996) || (HasAura(60002)
		|| (HasAura(60021) || (HasAura(60024) || (HasAura(60025) || (HasAura(60114) || (HasAura(60116) || (HasAura(60118) || (HasAura(60119)
		|| (HasAura(60120) || (HasAura(60136) || (HasAura(60140) || (HasAura(60424) || (HasAura(61229) || (HasAura(61230) || (HasAura(61289)
		|| (HasAura(61294) || (HasAura(61309) || (HasAura(61425) || (HasAura(61442) || (HasAura(61444) || (HasAura(61446) || (HasAura(61447)
		|| (HasAura(61451) || (HasAura(61465) || (HasAura(61467) || (HasAura(61469) || (HasAura(61470) || (HasAura(61996) || (HasAura(61997)
		))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
		))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
	{
		ToPlayer()->RemoveMount();
	...
	// Mount Check Part 4
	if (HasAura(31700) && (HasAura(62048)
		|| (HasAura(63232) || (HasAura(63635) || (HasAura(63636) || (HasAura(63637) || (HasAura(63638) || (HasAura(63639)
		|| (HasAura(63640) || (HasAura(63641) || (HasAura(63642) || (HasAura(63643) || (HasAura(63796) || (HasAura(63844) || (HasAura(63956)
		|| (HasAura(63963) || (HasAura(64656) || (HasAura(64657) || (HasAura(64658) || (HasAura(64659) || (HasAura(64681) || (HasAura(64731)
		|| (HasAura(64761) || (HasAura(64927) || (HasAura(64977) || (HasAura(65439) || (HasAura(65637) || (HasAura(65638) || (HasAura(65639)
		|| (HasAura(65640) || (HasAura(65641) || (HasAura(65642) || (HasAura(65643) || (HasAura(65644) || (HasAura(65645) || (HasAura(65646)
		|| (HasAura(65917) || (HasAura(66087) || (HasAura(66088) || (HasAura(66090) || (HasAura(66091) || (HasAura(66122) || (HasAura(66123)
		|| (HasAura(66124) || (HasAura(66846) || (HasAura(66847) || (HasAura(66906) || (HasAura(66907) || (HasAura(67336) || (HasAura(67466)
		|| (HasAura(68056) || (HasAura(68057) || (HasAura(68187) || (HasAura(68188) || (HasAura(68768) || (HasAura(68769) || (HasAura(69395)
		|| (HasAura(71342) || (HasAura(71343) || (HasAura(71344) || (HasAura(71345) || (HasAura(71346) || (HasAura(71347) || (HasAura(71810)
		|| (HasAura(72281) || (HasAura(72282) || (HasAura(72283) || (HasAura(72284) || (HasAura(72286) || (HasAura(72807) || (HasAura(72808)
		|| (HasAura(73313) || (HasAura(74854) || (HasAura(74855) || (HasAura(74856) || (HasAura(74918) || (HasAura(75387) || (HasAura(75596)
		|| (HasAura(75614) || (HasAura(75617) || (HasAura(75618) || (HasAura(75619) || (HasAura(75620) || (HasAura(75957) || (HasAura(75972)
		|| (HasAura(75973) || (HasAura(76153) || (HasAura(76154)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
	{
		ToPlayer()->RemoveMount();
		ToPlayer()->RemoveAurasByType(SPELL_AURA_MOUNTED);
		ToPlayer()->RemoveAura(30424);
    ...
    if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING))
    {
        if (roll_chance_i(3) && GetTimeInnEnter() > 0) // freeze update
        {
            ...
            {
                float bubble = 0.125f*sWorld->getRate(RATE_REST_INGAME); // speed collect rest bonus (section/in hour)
	...
	if (GetCurrentClass() == CLASS_DEATH_KNIGHT)
	{
		// Update rune timers
		for (uint8 i = 0; i < MAX_RUNES; ++i)
		{
...
void Player::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        if (!cur)
        {
            sLog->OutErrorConsole("setDeathState: attempt to kill a dead player %s(%d)", GetName(), GetGUIDLow());
            return;
        }
...
bool Player::BuildEnumData(PreparedQueryResult result, WorldPacket* data)
{
    ...
    if (!info)
    {
        sLog->OutErrorConsole("Player %u has incorrect race/class pair. Don't build enum.", guid);
        return false;
    }
    else if (!IsValidGender(gender))
    {
        sLog->OutErrorConsole("Player (%u) has incorrect gender (%hu), don't build enum.", guid, gender);
        return false;
    }
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    {
        sLog->OutErrorConsole("TeleportTo: invalid map (%d) or invalid coordinates (X: %f, Y: %f, Z: %f, O: %f) given when teleporting player (GUID: %u, name: %s, map: %d, X: %f, Y: %f, Z: %f, O: %f).",
    ...
    {
        sLog->OutErrorConsole("Player (GUID: %u, name: %s) tried to enter a forbidden map %u", GetGUIDLow(), GetName(), mapid);
    ...
    if (m_transport)
    {
        if (options & TELE_TO_NOT_LEAVE_TRANSPORT)
            AddUnitMovementFlag(MOVEMENTFLAG_ON_TRANSPORT);
        else
        {
            m_transport->RemovePassenger(this);
            m_transport = NULL;
    ...
    if (GetMapId() == mapid)
    {
        ...
        if (!GetSession()->PlayerLogout())
        {
            Position oldPos;
            GetPosition(&oldPos);
			if (HasUnitMovementFlag(MOVEMENT_FLAG_HOVER))
    ...
    {
        if (GetCurrentClass() == CLASS_DEATH_KNIGHT && GetMapId() == 609 && !IsGameMaster() && !HasSpell(50977))
            return false;
...
void Player::RegenerateAll()
{
    ...
    if (m_regenTimerCount >= 2000)
    {
        ...
        if (!IsInCombatActive() || IsPolymorphed() || m_baseHealthRegen || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT) || HasAuraType(SPELL_AURA_MOD_HEALTH_REGEN_IN_COMBAT))
            RegenerateHealth();
...
void Player::Regenerate(Powers power)
{
    ...
    if (HasAuraTypeWithValue(SPELL_AURA_PREVENT_REGENERATE_POWER, power))
    {
        ...
        SetStatInt32Value(UNIT_FIELD_POWER_1 + power, curValue);
        ...
        SendMessageToSet(&data, GetTypeId() == TYPE_ID_PLAYER ? true : false);
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (GetCurrentLevel() < 15)
                ManaIncreaseRate = sWorld->getRate(RATE_POWER_MANA) * (2.066f - (GetCurrentLevel() * 0.066f));
        ...
        {
            if (!IsInCombatActive() && !HasAuraType(SPELL_AURA_INTERRUPT_REGEN))
            {
    ...
    if (power != POWER_MANA)
    {
        ...
        if (power != POWER_RUNIC_POWER || IsInCombatActive())
    ...
    if (m_regenTimerCount >= 2000)
        ...
        UpdateUInt32Value(UNIT_FIELD_POWER_1 + power, curValue);
}
...
void Player::RegenerateHealth()
{
    ...
    if (GetCurrentLevel() < 15)
        HealthIncreaseRate = sWorld->getRate(RATE_HEALTH) * (2.066f - (GetCurrentLevel() * 0.066f));
    ...
    else if (!IsInCombatActive() || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT))
    {
...
bool Player::CanInteractWithQuestGiver(Object* questGiver)
{
    ...
    {
        case TYPE_ID_UNIT:
            return GetNPCIfCanInteractWith(questGiver->GetGUID(), UNIT_NPC_FLAG_QUEST_GIVER) != NULL;
        case TYPE_ID_GAMEOBJECT:
            return GetGameObjectIfCanInteractWith(questGiver->GetGUID(), GAMEOBJECT_TYPE_QUEST_GIVER) != NULL;
        case TYPE_ID_PLAYER:
    ...
    if (GameObject* go = GetMap()->GetGameObject(guid))
    {
        if (go->GetGoType() == type)
        {
            ...
            switch (type)
            {
                ...
                case GAMEOBJECT_TYPE_MAIL_BOX:
                    maxdist = 10.0f;
                    break;
                case GAMEOBJECT_TYPE_FISHING_HOLE:
...
void Player::SetGameMaster(bool on)
{
    if (on)
    {
		...
		CastSpell(this, SPELL_GM_ON);
        SetCurrentFaction(2141);
        SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_GM);
        SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_ALLOW_CHEAT_SPELLS);

        if (Pet* pet = GetPet())
        {
            pet->SetCurrentFaction(2141);
    ...
    else
    {
        ...
        m_ExtraFlags &= ~ PLAYER_EXTRA_GM_ON;
		CastSpell(this, SPELL_GM_OFF);
        setFactionForRace(GetCurrentRace());
        RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_GM);
        RemoveFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_ALLOW_CHEAT_SPELLS);

        if (Pet* pet = GetPet())
        {
            pet->SetCurrentFaction(GetFaction());
...
void Player::GiveXP(uint32 xp, Unit* victim, float group_rate)
{
    ...
    if (victim && victim->GetTypeId() == TYPE_ID_UNIT && !victim->ToCreature()->hasLootRecipient())
        return;

    uint8 level = GetCurrentLevel();
...
{
    uint8 oldLevel = GetCurrentLevel();
    if (level == oldLevel)
        return;
    ...
    sObjectMgr->GetPlayerLevelInfo(GetCurrentRace(), GetCurrentClass(), level, &info);

    PlayerClassLevelInfo classInfo;
    sObjectMgr->GetPlayerClassLevelInfo(GetCurrentClass(), level, &classInfo);
    ...
    if (MailLevelReward const* mailReward = sObjectMgr->GetMailLevelReward(level, GetCurrentRaceMask()))
    {
...
void Player::InitStatsForLevel(bool reapplyMods)
{
    ...
    PlayerClassLevelInfo classInfo;
    sObjectMgr->GetPlayerClassLevelInfo(GetCurrentClass(), GetCurrentLevel(), &classInfo);

    PlayerLevelInfo info;
    sObjectMgr->GetPlayerLevelInfo(GetCurrentRace(), GetCurrentClass(), GetCurrentLevel(), &info);

    SetUInt32Value(PLAYER_FIELD_MAX_LEVEL, sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL));
    SetUInt32Value(PLAYER_NEXT_LEVEL_XP, sObjectMgr->GetXPForLevel(GetCurrentLevel()));
    ...
    SetFloatValue(UNIT_FIELD_BASE_ATTACK_TIME, 2000.0f);
    SetFloatValue(UNIT_FIELD_BASE_ATTACK_TIME + 1, 2000.0f); // offhand attack time
    SetFloatValue(UNIT_FIELD_RANGED_ATTACK_TIME, 2000.0f);
    ...
    RemoveFlag(UNIT_FIELD_FLAGS,
        ...
        UNIT_FLAG_NO_AGGRO_FOR_PLAYER | UNIT_FLAG_NO_AGGRO_FOR_CREATURE  | UNIT_FLAG_LOOTING |
...
bool Player::AddTalent(uint32 spellId, uint8 spec, bool learning)
{
    ...
    if (!spellInfo)
    {
        ...
        {
            sLog->OutErrorConsole("Player::addSpell: Non-existed in SpellStore spell #%u request, deleting for all characters in `character_spell`.", spellId);
            ...
            sLog->OutErrorConsole("Player::addSpell: Non-existed in SpellStore spell #%u request.", spellId);
    ...
    if (!SpellMgr::IsSpellValid(spellInfo, this, false))
    {
        ...
        if (!IsInWorld() && !learning)                       // spell load case
        {
            sLog->OutErrorConsole("Player::addTalent: Broken spell #%u learning not allowed, deleting for all characters in `character_talent`.", spellId);
            ...
            sLog->OutErrorConsole("Player::addTalent: Broken spell #%u learning not allowed.", spellId);
...
bool Player::addSpell(uint32 spellId, bool active, bool learning, bool dependent, bool disabled, bool loading /*= false*/)
{
    ...
    if (!SpellMgr::IsSpellValid(spellInfo, this, false))
    {
        ...
        if (!IsInWorld() && !learning)                       // spell load case
        {
            sLog->OutErrorConsole("Player::addSpell: Broken spell #%u learning not allowed, deleting for all characters in `character_spell`.", spellId);
            ...
            sLog->OutErrorConsole("Player::addSpell: Broken spell #%u learning not allowed.", spellId);
...
void Player::removeSpell(uint32 spell_id, bool disabled, bool learn_low_rank)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_OFF_HAND_CHECK_AT_SPELL_UNLEARN))
		AutoUnequipOffhandIfNeed();
...
void Player::_LoadSpellCooldowns(PreparedQueryResult result)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                sLog->OutErrorConsole("Player %u has unknown spell %u in `character_spell_cooldown`, skipping.", GetGUIDLow(), spell_id);
                continue;
            }
...
bool Player::resetTalents(bool no_cost)
{
    ...
    for (uint32 talentId = 0; talentId < sTalentStore.GetNumRows(); ++talentId)
    {
        ...
        if ((GetCurrentClassMask() & talentTabInfo->ClassMask) == 0)
            continue;
...
void Player::SetFreeTalentPoints(uint32 points)
{
    ...
    SetUInt32Value(PLAYER_CHARACTER_POINTS_1, points);
}
...
TrainerSpellState Player::GetTrainerSpellState(TrainerSpell const* trainer_spell) const
{
    ...
    if (GetCurrentLevel() < trainer_spell->reqLevel)
        return TRAINER_SPELL_RED;
...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    uint32 charDelete_method = sWorld->getIntConfig(CONFIG_CHAR_DELETE_METHOD);
    uint32 charDelete_minLvl = sWorld->getIntConfig(CONFIG_CHAR_DELETE_MIN_LEVEL);
    ...
    switch (charDelete_method)
    {
        ...
        default:
            sLog->OutErrorConsole("Player::DeleteFromDB: Unsupported delete method: %u.", charDelete_method);
    }
...
{
    uint32 keepDays = sWorld->getIntConfig(CONFIG_CHAR_DELETE_KEEP_DAYS);
    if (!keepDays)
        return;
...
{
    sLog->OutConsole("Player::DeleteOldChars: Deleting all characters which have been deleted %u days before...", keepDays);
	...
	if (result)
    {
		sLog->OutConsole("Player::DeleteOldChars: Found " UI64FMTD " character(s) to delete", result->GetRowCount());
         do
         {
			 Field* fields = result->Fetch();
...
void Player::SetMovement(PlayerMovementType pType)
{
    ...
    switch (pType)
    {
        ...
        case MOVE_LAND_WALK:
			...
			break;
        default:
            sLog->OutErrorConsole("Player::SetMovement: Unsupported move type (%d), data not sent to client.", pType);
            return;
    }
    data.append(GetPackGUID());
    data << uint32(0);
...
void Player::BuildPlayerRepop()
{
    ...
    if (GetCurrentRace() == RACE_NIGHT_ELF)
    ...
    if (GetCorpse())
    {
        sLog->OutErrorConsole("BuildPlayerRepop: player %s(%d) already has a corpse", GetName(), GetGUIDLow());
        return;
    }
    ...
    if (!corpse)
    {
        sLog->OutErrorConsole("Error creating corpse for Player %s [%u]", GetName(), GetGUIDLow());
        return;
    }
	...
	// Guild-Level-System (Bonus: Fast Spirit)
	if (!GetMap()->IsBattlegroundOrArena())
	{
		if (Guild* guild = GetGuild())
		{
			if (sWorld->getBoolConfig(CONFIG_GUILD_BONUS_FAST_SPIRIT_ENABLE))
			{
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
    ...
    if (int32(GetCurrentLevel()) >= startLevel)
    {
        // set resurrection sickness
        CastSpell(this, 15007, true);

        // not full duration
        if (int32(GetCurrentLevel()) < startLevel+9)
        {
            int32 delta = (int32(GetCurrentLevel()) - startLevel + 1)*MINUTE;
...
void Player::CreateCorpse()
{
    ...
    _cfb1 = ((0x00) | (race << 8) | (GetCurrentGender() << 16) | (skin << 24));
...
uint32 Player::DurabilityRepair(uint16 pos, bool cost, float discountMod, bool guildBank)
{
    ...
    if (cost)
    {
        ...
        if (LostDurability>0)
        {
            ...
            if (!dcost)
            {
                sLog->OutErrorConsole("RepairDurability: Wrong item lvl %u", ditemProto->ItemLevel);
                return TotalCost;
            }
            ...
            if (!dQualitymodEntry)
            {
                sLog->OutErrorConsole("RepairDurability: Wrong dQualityModEntry %u", dQualitymodEntryId);
                return TotalCost;
            }
			...
			// Guild-Level-System (Bonus: Cheap low cost repair)
			if (Guild* guild = GetGuild())
			{
				if (sWorld->getBoolConfig(CONFIG_GUILD_BONUS_FOR_REPAIR_ENABLE))
				{
...
void Player::RepopAtGraveyard()
{
    ...
    if (Battleground* bg = GetBattleground())
        ...
        ClosestGrave = sObjectMgr->GetClosestGraveYard(GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId(), GetTeam(), GetCurrentClass());
            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
void Player::HandleBaseModValue(BaseModGroup modGroup, BaseModType modType, float amount, bool apply)
{
    ...
    {
        sLog->OutErrorConsole("ERROR in HandleBaseModValue(): non existed BaseModGroup of wrong BaseModType!");
        return;
    }
...
float Player::GetBaseModValue(BaseModGroup modGroup, BaseModType modType) const
{
    if (modGroup >= BASEMOD_END || modType > MOD_END)
    {
        sLog->OutErrorConsole("trial to access non existed BaseModGroup or wrong BaseModType!");
        return 0.0f;
    }
...
float Player::GetTotalBaseModValue(BaseModGroup modGroup) const
{
    ...
    {
        sLog->OutErrorConsole("wrong BaseModGroup in GetTotalBaseModValue()!");
        return 0.0f;
    }
...
float Player::GetRatingMultiplier(CombatRating cr) const
{
    ...
    GtOCTClassCombatRatingScalarEntry const* classRating = sGtOCTClassCombatRatingScalarStore.LookupEntry((GetCurrentClass()-1)*GT_MAX_RATING+cr+1);
...
void Player::UpdateWeaponSkill(WeaponAttackType attType)
{
    ...
    if (victim && victim->GetTypeId() == TYPE_ID_UNIT && (victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_SKILLGAIN))
        return;
...
{
    uint8 plevel = GetCurrentLevel();                              // if defense than victim == attacker
    uint8 greylevel = Trinity::XP::GetGrayLevel(plevel);
    uint8 moblevel = victim->GetCurrentLevelForTarget(this);
    ...
    if (!defence)
        if (GetCurrentClass() == CLASS_WARRIOR || GetCurrentClass() == CLASS_ROGUE)
...
void Player::SetSkill(uint16 id, uint16 step, uint16 newVal, uint16 maxVal)
{
    ...
    else if (newVal)                                        //add
    {
            ...
            if (!GetUInt32Value(PLAYER_SKILL_INDEX(i)))
        {
            ...
            if (!pSkill)
            {
                sLog->OutErrorConsole("Skill not found in SkillLineStore: skill #%u", id);
                return;
            }
...
bool Player::IsActionButtonDataValid(uint8 button, uint32 action, uint8 type)
{
    ...
    {
        sLog->OutErrorConsole("Action %u not added into button %u for player %s: button must be < %u", action, button, GetName(), MAX_ACTION_BUTTONS);
        return false;
    }

    if (action >= MAX_ACTION_BUTTON_ACTION_VALUE)
    {
        sLog->OutErrorConsole("Action %u not added into button %u for player %s: action must be < %u", action, button, GetName(), MAX_ACTION_BUTTON_ACTION_VALUE);
        return false;
    }
    ...
    switch (type)
    {
        case ACTION_BUTTON_SPELL:
            ...
            {
                sLog->OutErrorConsole("Spell action %u not added into button %u for player %s: spell not exist", action, button, GetName());
                return false;
            }
        ...
        case ACTION_BUTTON_ITEM:
            ...
            {
                sLog->OutErrorConsole("Item action %u not added into button %u for player %s: item not exist", action, button, GetName());
                return false;
            }
            break;
        default:
...
{
	if (GetMapId() == 571 && GetCurrentLevel() < 68 && sWorld->getBoolConfig(CONFIG_NORTHREND_WITH_68_LEVEL))
	{
		switch(GetTeam())
		{
		case ALLIANCE:
			TeleportTo(0, -8830.62f, 622.80f, 93.85f, 3.9f);
            break;
        case HORDE:
            TeleportTo(1, 1629.35f, -4373.39f, 31.26f, 3.5f);
            break;
		}
		return true;
	}
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    {
        sLog->OutErrorConsole("Wrong area flag %u in map data for (X: %f Y: %f) point to field PLAYER_EXPLORED_ZONES_1 + %u ( %u must be < %u ).", areaFlag, GetPositionX(), GetPositionY(), offset, offset, PLAYER_EXPLORED_ZONES_SIZE);
        return;
    }
    ...
    if (!(currFields & val))
    {
		...
		if (!areaEntry)
        {
			sLog->OutErrorConsole("Player %u discovered unknown area (x: %f y: %f z: %f map: %u", GetGUIDLow(), GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId());
			return;
        }
		...
		if (areaEntry->area_level > 0)
        {
            ...
            if (GetCurrentLevel() >= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                SendExplorationExperience(area, 0);
            }
            else
            {
                int32 diff = int32(GetCurrentLevel()) - areaEntry->area_level;
				uint32 XP = 0;
                if (diff < -5)
                {
					XP = uint32(sObjectMgr->GetBaseXP(GetCurrentLevel()+5)*sWorld->getRate(RATE_XP_EXPLORE));
                }
                else if (diff > 5)
                {
				...
				if (GetSession()->IsPremiumAccount())
...
uint32 Player::TeamForRace(uint8 race)
{
    if (ChrRacesEntry const* rEntry = sChrRacesStore.LookupEntry(race))
    {
        ...
        sLog->OutErrorConsole("Race (%u) has wrong teamid (%u) in DBC: wrong DBC files?", uint32(race), rEntry->TeamID);
    }
    else
        sLog->OutErrorConsole("Race (%u) not found in DBC: wrong DBC files?", uint32(race));
...
void Player::setFactionForRace(uint8 race)
{
    ...
    SetCurrentFaction(rEntry ? rEntry->FactionID : 0);
}
...
int32 Player::CalculateReputationGain(uint32 creatureOrQuestLevel, int32 rep, int32 faction, bool for_quest, bool noQuestBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= Trinity::XP::GetGrayLevel(GetCurrentLevel()))
        percent *= rate;
...
void Player::RewardReputation(Unit* victim, float rate)
{
    ...
    if (GetChampioningFaction())
    {
        ...
        if (map && map->IsDungeon())
        {
            ...
            if (instance)
            {
                ...
                {
					// Fix For Normal Dungeon Mode Rep Rewards
                    if (map->IsDungeon() && pAccessRequirement->levelMin == 68
						|| pAccessRequirement->levelMin == 69
						|| pAccessRequirement->levelMin == 70
						|| pAccessRequirement->levelMin == 71
						|| pAccessRequirement->levelMin == 72
						|| pAccessRequirement->levelMin == 73
						|| pAccessRequirement->levelMin == 74
						|| pAccessRequirement->levelMin == 75
						|| pAccessRequirement->levelMin == 76
						|| pAccessRequirement->levelMin == 77
						|| pAccessRequirement->levelMin == 78
						|| pAccessRequirement->levelMin == 79
						|| pAccessRequirement->levelMin == 80)
    ...
    {
        int32 donerep1 = CalculateReputationGain(victim->GetCurrentLevel(), Rep->RepValue1, ChampioningFaction ? ChampioningFaction : Rep->RepFaction1, false);
    ...
    {
        int32 donerep2 = CalculateReputationGain(victim->GetCurrentLevel(), Rep->RepValue2, ChampioningFaction ? ChampioningFaction : Rep->RepFaction2, false);
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (InArena())
    {
        if (!uVictim || uVictim == this || uVictim->GetTypeId() != TYPE_ID_PLAYER)
            return false;
    ...
    if (honor_f <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPE_ID_PLAYER)
        {
            ...
            uint8 k_level = GetCurrentLevel();
            uint8 k_grey = Trinity::XP::GetGrayLevel(k_level);
            uint8 v_level = victim->GetCurrentLevel();
			...
			uint32 vtitle = victim->GetUInt32Value(PLAYER_FIELD_KNOWN_TITLES);
			//uint32 victim_title = 0;
			uint32 ktitle = GetUInt32Value(PLAYER_FIELD_KNOWN_TITLES);
			...
			UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HK_CLASS, victim->GetCurrentClass());
			UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HK_RACE, victim->GetCurrentRace());
			UpdateKnownTitles();
	...
	if (Guild* guild = GetGuild())
	{
		if (sWorld->getBoolConfig(CONFIG_GUILD_BONUS_HONOR_ENABLE))
		{
    ...
    switch (type)
    {
        ...
        case DUEL_WON:
            ...
            if (duel->opponent)
            {
                ...
                if (GetCurrentClass() == CLASS_DEATH_KNIGHT && duel->opponent->GetQuestStatus(12733) == QUEST_STATUS_INCOMPLETE)
...
void Player::_ApplyItemBonuses(ItemTemplate const* proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
    ...
    uint32 ssd_level = GetCurrentLevel();
    ...
    if (slot == EQUIPMENT_SLOT_RANGED && (proto->InventoryType == INVENTORY_TYPE_RANGED || proto->InventoryType == INVENTORY_TYPE_THROWN || proto->InventoryType == INVENTORY_TYPE_RANGED_RIGHT))
    {
        attType = RANGED_ATTACK;
    }
    else if (slot == EQUIPMENT_SLOT_OFF_HAND)
    {
        attType = OFF_ATTACK;
    }
	...
	if (GetCurrentClass() == CLASS_DRUID)
    {
		int32 dpsMod = 0;
		int32 feral_bonus = 0;
		if (ssv)
...
void Player::_ApplyWeaponDamage(uint8 slot, ItemTemplate const* proto, ScalingStatValuesEntry const* ssv, bool apply)
{
    ...
    if (minDamage > 0)
    {
        damage = apply ? minDamage : BASE_MIN_DAMAGE;
        SetBaseWeaponDamage(attType, MIN_DAMAGE, damage);
    }

    if (maxDamage  > 0)
    {
        damage = apply ? maxDamage : BASE_MAX_DAMAGE;
        SetBaseWeaponDamage(attType, MAX_DAMAGE, damage);
    }
    ...
    if (proto->Delay && !IsInFeralForm())
    {
        ...
        else if (slot == EQUIPMENT_SLOT_MAIN_HAND)
...
void Player::_ApplyWeaponDependentAuraDamageMod(Item* item, WeaponAttackType attackType, AuraEffect const* aura, bool apply)
{
    ...
    if ((aura->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) == 0 && (GetCurrentClassMask() & CLASSMASK_WAND_USERS) == 0)
        return;
    ...
    {
        case BASE_ATTACK:
			unitMod = UNIT_MOD_DAMAGE_MAIN_HAND;
			break;
        case OFF_ATTACK:
			unitMod = UNIT_MOD_DAMAGE_OFF_HAND;
			break;
        case RANGED_ATTACK:
...
void Player::CastItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item* item, ItemTemplate const* proto)
{
    ...
    //if (damageInfo->procVictim & PROC_FLAG_TAKEN_ANY_DAMAGE)
    {
        for (uint8 i = 0; i < MAX_ITEM_SPELLS; ++i)
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorConsole("WORLD: unknown Item spellid %i", spellData.SpellId);
                continue;
            }
    ...
    for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorConsole("Player::CastItemCombatSpell(GUID: %u, name: %s, enchant: %i): unknown spell %i is casted, ignoring...",
...
void Player::CastItemUseSpell(Item* item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex)
{
    ...
    if (proto->Spells[0].SpellId == 483 || proto->Spells[0].SpellId == 55884)
    {
        ...
        if (!spellInfo)
        {
            sLog->OutErrorConsole("Player::CastItemUseSpell: Item (Entry: %u) in have wrong spell id %u, ignoring ", proto->ItemId, learn_spell_id);
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if (!spellInfo)
        {
            sLog->OutErrorConsole("Player::CastItemUseSpell: Item (Entry: %u) in have wrong spell id %u, ignoring", proto->ItemId, spellData.SpellId);
            continue;
        }
    ...
    for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorConsole("Player::CastItemUseSpell Enchant %i, cast unknown spell %i", pEnchant->ID, pEnchant->spellid[s]);
                continue;
            }
...
void Player::RemovedInsignia(Player* looterPlr)
{
    ...
    bones->loot.gold = GetCurrentLevel();
    ...
    else
    {
        ...
        if (loot_type == LOOT_PICKPOCKETING)
        {
            if (!creature->lootForPickPocketed)
            {
                ...
                const uint32 a = urand(0, creature->GetCurrentLevel()/2);
                const uint32 b = urand(0, GetCurrentLevel()/2);
...
void Player::ResetPetTalents()
{
    ...
    if (!charmInfo)
    {
        sLog->OutErrorConsole("Object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
    pet->resetTalents();
...
{
    uint8 playerClass = GetCurrentClass();
    ...
    {
        case INVENTORY_TYPE_HEAD:
            slots[0] = EQUIPMENT_SLOT_HEAD;
            break;
        case INVENTORY_TYPE_NECK:
            slots[0] = EQUIPMENT_SLOT_NECK;
            break;
        case INVENTORY_TYPE_SHOULDERS:
            slots[0] = EQUIPMENT_SLOT_SHOULDERS;
            break;
        case INVENTORY_TYPE_BODY:
            slots[0] = EQUIPMENT_SLOT_BODY;
            break;
        case INVENTORY_TYPE_CHEST:
            slots[0] = EQUIPMENT_SLOT_CHEST;
            break;
        case INVENTORY_TYPE_ROBE:
            slots[0] = EQUIPMENT_SLOT_CHEST;
            break;
        case INVENTORY_TYPE_WAIST:
            slots[0] = EQUIPMENT_SLOT_WAIST;
            break;
        case INVENTORY_TYPE_LEGS:
            slots[0] = EQUIPMENT_SLOT_LEGS;
            break;
        case INVENTORY_TYPE_FEET:
            slots[0] = EQUIPMENT_SLOT_FEET;
            break;
        case INVENTORY_TYPE_WRISTS:
            slots[0] = EQUIPMENT_SLOT_WRISTS;
            break;
        case INVENTORY_TYPE_HANDS:
            slots[0] = EQUIPMENT_SLOT_HANDS;
            break;
        case INVENTORY_TYPE_FINGER:
            slots[0] = EQUIPMENT_SLOT_FINGER_1;
            slots[1] = EQUIPMENT_SLOT_FINGER_2;
            break;
        case INVENTORY_TYPE_TRINKET:
            slots[0] = EQUIPMENT_SLOT_TRINKET_1;
            slots[1] = EQUIPMENT_SLOT_TRINKET_2;
            break;
        case INVENTORY_TYPE_CLOAK:
            slots[0] = EQUIPMENT_SLOT_BACK;
            break;
        case INVENTORY_TYPE_WEAPON:
        {
            slots[0] = EQUIPMENT_SLOT_MAIN_HAND;

            // suggest offhand slot only if know dual wielding
            // (this will be replace mainhand weapon at auto equip instead unwonted "you don't known dual wielding" ...
            if (CanDualWield())
                slots[1] = EQUIPMENT_SLOT_OFF_HAND;
            break;
        }
        case INVENTORY_TYPE_SHIELD:
            slots[0] = EQUIPMENT_SLOT_OFF_HAND;
            break;
        case INVENTORY_TYPE_RANGED:
            slots[0] = EQUIPMENT_SLOT_RANGED;
            break;
        case INVENTORY_TYPE_2H_WEAPON:
            slots[0] = EQUIPMENT_SLOT_MAIN_HAND;
            if (Item* mhWeapon = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAIN_HAND))
            {
            ...
            if (GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND))
            {
            ...
            if (CanDualWield() && CanTitanGrip() && proto->SubClass != ITEM_SUBCLASS_WEAPON_POLEARM && proto->SubClass != ITEM_SUBCLASS_WEAPON_STAFF)
                slots[1] = EQUIPMENT_SLOT_OFF_HAND;
            break;
        case INVENTORY_TYPE_TABARD:
            slots[0] = EQUIPMENT_SLOT_TABARD;
            break;
        case INVENTORY_TYPE_WEAPON_MAIN_HAND:
            slots[0] = EQUIPMENT_SLOT_MAIN_HAND;
            break;
        case INVENTORY_TYPE_WEAPON_OFF_HAND:
            slots[0] = EQUIPMENT_SLOT_OFF_HAND;
            break;
        case INVENTORY_TYPE_HOLDABLE:
            slots[0] = EQUIPMENT_SLOT_OFF_HAND;
            break;
        case INVENTORY_TYPE_THROWN:
            slots[0] = EQUIPMENT_SLOT_RANGED;
            break;
        case INVENTORY_TYPE_RANGED_RIGHT:
            slots[0] = EQUIPMENT_SLOT_RANGED;
            break;
        case INVENTORY_TYPE_BAG:
            slots[0] = INVENTORY_SLOT_BAG_START + 0;
            slots[1] = INVENTORY_SLOT_BAG_START + 1;
            slots[2] = INVENTORY_SLOT_BAG_START + 2;
            slots[3] = INVENTORY_SLOT_BAG_START + 3;
            break;
        case INVENTORY_TYPE_RELIC:
        {
            switch (proto->SubClass)
            {
    ...
    else
    {
        ...
        for (uint8 i = 0; i < 4; ++i)
            if (slots[i] != NULL_SLOT && !GetItemByPos(INVENTORY_SLOT_BAG_0, slots[i]))
                ...
                if (slots[i] != EQUIPMENT_SLOT_OFF_HAND || !IsTwoHandUsed())
                    return slots[i];
...
Item* Player::GetShield(bool useable) const
{
    Item* item = NULL;
    if (useable)
        item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
    else
        item = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
    ...
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            if (not_loading)
            {
                ...
                if (IsInCombatActive() && (pProto->Class == ITEM_CLASS_WEAPON || pProto->InventoryType == INVENTORY_TYPE_RELIC) && m_weaponChangeTimer != 0)
            ...
            if (ssd && ssd->MaxLevel < DEFAULT_MAX_LEVEL && ssd->MaxLevel < GetCurrentLevel())
                return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;
            ...
            {
                case EQUIPMENT_SLOT_MAIN_HAND:
                    ignore = EQUIPMENT_SLOT_OFF_HAND;
                    break;
                case EQUIPMENT_SLOT_OFF_HAND:
                    ignore = EQUIPMENT_SLOT_MAIN_HAND;
                    break;
                case EQUIPMENT_SLOT_FINGER_1:
                    ignore = EQUIPMENT_SLOT_FINGER_2;
                    break;
                case EQUIPMENT_SLOT_FINGER_2:
                    ignore = EQUIPMENT_SLOT_FINGER_1;
                    break;
                case EQUIPMENT_SLOT_TRINKET_1:
                    ignore = EQUIPMENT_SLOT_TRINKET_2;
                    break;
                case EQUIPMENT_SLOT_TRINKET_2:
                    ignore = EQUIPMENT_SLOT_TRINKET_1;
                    break;
            }
            ...
            if (eslot == EQUIPMENT_SLOT_OFF_HAND)
            {
                // Do not allow polearm to be equipped in the offhand (rare case for the only 1h polearm 41750)
				if (type == INVENTORY_TYPE_WEAPON && pProto->SubClass == ITEM_SUBCLASS_WEAPON_POLEARM)
					return EQUIP_ERR_ITEM_DOESNT_GO_TO_SLOT;

				else if (type == INVENTORY_TYPE_WEAPON || type == INVENTORY_TYPE_WEAPON_OFF_HAND)
                {
                    if (!CanDualWield())
                        return EQUIP_ERR_CANT_DUAL_WIELD;
                }
                else if (type == INVENTORY_TYPE_2H_WEAPON)
                {
            ...
            if (type == INVENTORY_TYPE_2H_WEAPON)
            {
                if (eslot == EQUIPMENT_SLOT_OFF_HAND)
                {
                    if (!CanTitanGrip())
                ...
                else if (eslot != EQUIPMENT_SLOT_MAIN_HAND)
                    return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;

                if (!CanTitanGrip())
                {
                    // offhand item must can be stored in inventory for offhand item and it also must be unequipped
                    Item* offItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
                    ItemPosCountVec off_dest;
                    if (offItem && (!not_loading ||
                        CanUnequipItem(uint16(INVENTORY_SLOT_BAG_0) << 8 | EQUIPMENT_SLOT_OFF_HAND, false) != EQUIP_ERR_OK ||
...
InventoryResult Player::CanBankItem(uint8 bag, uint8 slot, ItemPosCountVec &dest, Item* pItem, bool swap, bool not_loading) const
{
    ...
    {
        sLog->OutErrorConsole("Possible hacking attempt: Player %s [guid: %u] tried to move token [guid: %u, entry: %u] out of the currency bag!",
...
InventoryResult Player::CanUseItem(ItemTemplate const* proto) const
{
    ...
    if (proto)
    {
        ...
        if ((proto->AllowableClass & GetCurrentClassMask()) == 0 || (proto->AllowableRace & GetCurrentRaceMask()) == 0)
            return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;
        ...
        if (GetCurrentLevel() < proto->RequiredLevel)
...
InventoryResult Player::CanRollForItemInLFG(ItemTemplate const* proto, WorldObject const* lootedObject) const
{
   ...
   // Used by group, function NeedBeforeGreed, to know if a prototype can be used by a player
    ...
    uint8 _class = GetCurrentClass();

    if (proto->Class == ITEM_CLASS_WEAPON && GetSkillValue(item_weapon_skills[proto->SubClass]) == 0)
        return EQUIP_ERR_NO_REQUIRED_PROFICIENCY;

    if (proto->Class == ITEM_CLASS_ARMOR && proto->SubClass > ITEM_SUBCLASS_ARMOR_MISC && proto->SubClass < ITEM_SUBCLASS_ARMOR_BUCKLER && proto->InventoryType != INVENTORY_TYPE_CLOAK)
    {
        if (_class == CLASS_WARRIOR || _class == CLASS_PALADIN || _class == CLASS_DEATH_KNIGHT)
        {
            if (GetCurrentLevel() < 40)
            {
...
InventoryResult Player::CanUseAmmo(uint32 item) const
{
    ...
    if (pProto)
    {
        if (pProto->InventoryType!= INVENTORY_TYPE_AMMO)
...
Item* Player::EquipItem(uint16 pos, Item* pItem, bool update)
{
    ...
    if (!pItem2)
    {
        ...
        if (IsAlive())
        {
            ...
            if (pProto && IsInCombatActive() && (pProto->Class == ITEM_CLASS_WEAPON || pProto->InventoryType == INVENTORY_TYPE_RELIC) && m_weaponChangeTimer == 0)
            {
                uint32 cooldownSpell = GetCurrentClass() == CLASS_ROGUE ? 6123 : 6119;
                SpellInfo const* spellProto = sSpellMgr->GetSpellInfo(cooldownSpell);

                if (!spellProto)
                    sLog->OutErrorConsole("Weapon switch cooldown spell %u couldn't be found in Spell.dbc", cooldownSpell);
                else
                {
...
void Player::SetVisibleItemSlot(uint8 slot, Item* pItem)
{
    if (pItem)
    {
		if (pItem->GetFakeEntry())
			SetUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRY_ID + (slot * 2), pItem->GetFakeEntry());
		else
			SetUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRY_ID + (slot * 2), pItem->GetEntry());
        SetUInt16Value(PLAYER_VISIBLE_ITEM_1_ENCHANTMENT + (slot * 2), 0, pItem->GetEnchantmentId(PERM_ENCHANTMENT_SLOT));
        SetUInt16Value(PLAYER_VISIBLE_ITEM_1_ENCHANTMENT + (slot * 2), 1, pItem->GetEnchantmentId(TEMP_ENCHANTMENT_SLOT));
    }
    else
    {
        SetUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRY_ID + (slot * 2), 0);
...
void Player::ApplyEnchantment(Item* item, EnchantmentSlot slot, bool apply, bool apply_dur, bool ignore_condition)
{
    ...
    if (pEnchant->requiredLevel > GetCurrentLevel())
        return;
        ...
        for (int s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            switch (enchant_display_type)
            {
                ...
                case ITEM_ENCHANTMENT_TYPE_DAMAGE:
                    if (item->GetSlot() == EQUIPMENT_SLOT_MAIN_HAND)
                        HandleStatModifier(UNIT_MOD_DAMAGE_MAIN_HAND, TOTAL_VALUE, float(enchant_amount), apply);
                    else if (item->GetSlot() == EQUIPMENT_SLOT_OFF_HAND)
                        HandleStatModifier(UNIT_MOD_DAMAGE_OFF_HAND, TOTAL_VALUE, float(enchant_amount), apply);
...
//                            break;
                ...
                {
                    if (GetCurrentClass() == CLASS_SHAMAN)
                    {
                        float addValue = 0.0f;
                        if (item->GetSlot() == EQUIPMENT_SLOT_MAIN_HAND)
                        {
                            addValue = float(enchant_amount * item->GetTemplate()->Delay / 1000.0f);
                            HandleStatModifier(UNIT_MOD_DAMAGE_MAIN_HAND, TOTAL_VALUE, addValue, apply);
                        }
                        else if (item->GetSlot() == EQUIPMENT_SLOT_OFF_HAND)
                        {
                            addValue = float(enchant_amount * item->GetTemplate()->Delay / 1000.0f);
                            HandleStatModifier(UNIT_MOD_DAMAGE_OFF_HAND, TOTAL_VALUE, addValue, apply);
                        }
                    }
                    break;
                }
                ...
                case ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET:
                    ...
                    break;
                default:
                    sLog->OutErrorConsole("Unknown item enchantment (id = %d) display type: %d", enchant_id, enchant_display_type);
                    break;
            }                                               /*switch (enchant_display_type)*/
        }                                                   /*for*/
    }
...
void Player::PrepareGossipMenu(WorldObject* source, uint32 menuId /*= 0*/, bool showQuests /*= false*/)
{
    ...
    if (source->GetTypeId() == TYPE_ID_UNIT)
    {
		npcflags = source->GetUInt32Value(UNIT_NPC_FLAGS);
		if (npcflags & UNIT_NPC_FLAG_QUEST_GIVER && showQuests)
			PrepareQuestMenu(source->GetGUID());
	}

    if (source->GetTypeId() == TYPE_ID_GAMEOBJECT)
		if (source->ToGameObject()->GetGoType() == GAMEOBJECT_TYPE_QUEST_GIVER)
			PrepareQuestMenu(source->GetGUID());
    ...
    for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
    {
        ...
        if (Creature* creature = source->ToCreature())
        {
            ...
            switch (itr->second.OptionType)
            {
                case GOSSIP_OPTION_ARMORER:
                    ...
                    break;
                case GOSSIP_OPTION_SPIRIT_HEALER:
                    if (!IsDead())
                        canTalk = false;
                    break;
                case GOSSIP_OPTION_VENDOR:
                {
                    ...
                    {
                        sLog->OutErrorDatabase("Creature %u (Entry: %u) have UNIT_NPC_FLAG_VENDOR but have empty trading item list.", creature->GetGUIDLow(), creature->GetEntry());
                        canTalk = false;
                    }
                    break;
                }
                ...
                case GOSSIP_OPTION_LEARNDUALSPEC:
                    if (!(GetSpecsCount() == 1 && creature->IsCanTrainingAndResetTalentsOf(this) && !(GetCurrentLevel() < sWorld->getIntConfig(CONFIG_MIN_DUALSPEC_LEVEL))))
                        canTalk = false;
                    break;
                ...
                case GOSSIP_OPTION_STABLEPET:
                    if (GetCurrentClass() != CLASS_HUNTER)
                        canTalk = false;
                    break;
                ...
                case GOSSIP_OPTION_SPIRIT_GUIDE:
                ...
                case GOSSIP_OPTION_OUTDOORPVP:
                    ...
                    break;
                default:
                    sLog->OutErrorDatabase("Creature entry %u have unknown gossip option %u for menu %u", creature->GetEntry(), itr->second.OptionType, itr->second.MenuId);
                    canTalk = false;
                    break;
            }
        }
        else if (GameObject* go = source->ToGameObject())
        {
            ...
            {
                case GOSSIP_OPTION_GOSSIP:
                    if (go->GetGoType() != GAMEOBJECT_TYPE_QUEST_GIVER && go->GetGoType() != GAMEOBJECT_TYPE_GOOBER)
                        canTalk = false;
                    break;
                default:
                    canTalk = false;
...
void Player::SendPreparedGossip(WorldObject* source)
{
    ...
    else if (source->GetTypeId() == TYPE_ID_GAMEOBJECT)
    {
...
void Player::OnGossipSelect(WorldObject* source, uint32 gossipListId, uint32 menuId)
{
    ...
    if (source->GetTypeId() == TYPE_ID_GAMEOBJECT)
    {
        ...
        {
            sLog->OutErrorConsole("Player guid %u request invalid gossip option for GameObject entry %u", GetGUIDLow(), source->GetEntry());
            return;
        }
    }
    ...
    switch (gossipOptionId)
    {
        ...
        case GOSSIP_OPTION_TRAINER:
            ...
            break;
        case GOSSIP_OPTION_LEARNDUALSPEC:
            if (GetSpecsCount() == 1 && GetCurrentLevel() >= sWorld->getIntConfig(CONFIG_MIN_DUALSPEC_LEVEL))
            {
        ...
        case GOSSIP_OPTION_BATTLEFIELD:
        {
            ...
            {
                sLog->OutErrorConsole("a user (guid %u) requested battlegroundlist from a npc who is no battlemaster", GetGUIDLow());
                return;
            }
...
void Player::SendPreparedQuest(uint64 guid)
{
    ...
    if (questMenu.GetMenuItemCount() == 1)
    {
        ...
        if (quest)
        {
            ...
            {
                Object* object = ObjectAccessor::GetObjectByTypeMask(*this, guid, TYPE_MASK_UNIT | TYPE_MASK_GAMEOBJECT | TYPE_MASK_ITEM);
...
bool Player::CanSeeStartQuest(Quest const* quest)
{
    ...
    {
        return GetCurrentLevel() + sWorld->getIntConfig(CONFIG_QUEST_HIGH_LEVEL_HIDE_DIFF) >= quest->GetMinLevel();
    }
...
bool Player::CanCompleteQuest(uint32 quest_id)
{
    if (quest_id)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_SPECIAL_FLAGS_DELIVER))
            {
            ...
            if (qInfo->HasFlag(QUEST_SPECIAL_FLAGS_KILL_OR_CAST | QUEST_SPECIAL_FLAGS_SPEAKTO))
            {
            ...
            if (qInfo->HasFlag(QUEST_SPECIAL_FLAGS_PLAYER_KILL))
                if (qInfo->GetPlayersSlain() != 0 && q_status.PlayerCount < qInfo->GetPlayersSlain())
                    return false;

            if (qInfo->HasFlag(QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT) && !q_status.Explored)
                return false;

            if (qInfo->HasFlag(QUEST_SPECIAL_FLAGS_TIMED) && q_status.Timer == 0)
                return false;
...
bool Player::CanCompleteRepeatableQuest(Quest const* quest)
{
    ...
    if (quest->HasFlag(QUEST_SPECIAL_FLAGS_DELIVER))
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
...
void Player::AddQuest(Quest const* quest, Object* questGiver)
{
    ...
    if (quest->HasFlag(QUEST_SPECIAL_FLAGS_KILL_OR_CAST | QUEST_SPECIAL_FLAGS_SPEAKTO))
    {
        for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
            questStatusData.CreatureOrGOCount[i] = 0;
    }

    if (quest->HasFlag(QUEST_SPECIAL_FLAGS_PLAYER_KILL))
    ...
    if (quest->HasFlag(QUEST_SPECIAL_FLAGS_TIMED))
    {
        ...
        // shared timed quest
        if (questGiver && questGiver->GetTypeId() == TYPE_ID_PLAYER)
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
	...
	if (Guild* guild = GetGuild())
	{
		if (sWorld->getBoolConfig(CONFIG_GUILD_BONUS_XP_ENABLE))
		{
    ...
    // honor reward
	if (uint32 honor = quest->CalculateHonorGain(GetCurrentLevel()))
...
{
    if (GetCurrentLevel() < qInfo->GetMinLevel())
    {
        if (msg)
            SendCanTakeQuestResponse(INVALIDREASON_QUEST_FAILED_LOW_LEVEL);
        return false;
    }
    else if (qInfo->GetMaxLevel() > 0 && GetCurrentLevel() > qInfo->GetMaxLevel())
    {
        if (msg)
...
bool Player::SatisfyQuestClass(Quest const* qInfo, bool msg) const
{
    ...
    if ((reqClass & GetCurrentClassMask()) == 0)
    {
        if (msg)
            SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
...
bool Player::SatisfyQuestRace(Quest const* qInfo, bool msg)
{
    ...
    if (reqraces == 0)
        return true;
    if ((reqraces & GetCurrentRaceMask()) == 0)
    {
        if (msg)
...
{
    if (!m_timedquests.empty() && qInfo->HasFlag(QUEST_SPECIAL_FLAGS_TIMED))
    {
        if (msg)
...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        if (!qInfo || !qInfo->HasFlag(QUEST_SPECIAL_FLAGS_DELIVER))
            continue;
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_SPECIAL_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
...
void Player::_LoadArenaTeamInfo(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!arenaTeam)
            {
                sLog->OutErrorConsole("Player::_LoadArenaTeamInfo: couldn't load arenateam %u", arenaTeamId);
                continue;
            }
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    if (!result)
    {
        sLog->OutErrorConsole("Player (GUID: %u) not found in table `characters`, can't load. ", guid);
        return false;
    }
    ...
    {
        sLog->OutErrorConsole("Player (GUID: %u) loading from wrong account (is: %u, should be: %u)", guid, GetSession()->GetAccountId(), dbAccountId);
        return false;
    }

    if (holder->GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_BANNED))
    {
        sLog->OutErrorConsole("Player (GUID: %u) is banned, can't load.", guid);
        return false;
    }
    ...
    {
        sLog->OutErrorConsole("Player (GUID: %u) has wrong gender (%hu), can't be loaded.", guid, Gender);
        return false;
    }
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    {
        sLog->OutErrorConsole("Player (guidlow %d) have invalid coordinates (MapId: %u X: %f Y: %f Z: %f O: %f). Teleport to default race/class locations.", guid, mapId, GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
        RelocateToHomebind();
    }
    ...
    else if (mapEntry && mapEntry->IsBattlegroundOrArena())
    {
        ...
        else
        {
            ...
            {
                sLog->OutErrorConsole("Player (guidlow %d) was in BG in database, but BG was not found, and entry point was invalid! Teleport to default race/class locations.", guid);
                RelocateToHomebind();
            }
            else
                Relocate(&_loc);
    ...
    else if (transGUID)
    {
        ...
        {
            sLog->OutErrorConsole("Player (guidlow %d) have invalid transport coordinates (X: %f Y: %f Z: %f O: %f). Teleport to bind location.",
        ...
        else
        {
            ...
            if (!m_transport)
            {
                sLog->OutErrorConsole("Player (guidlow %d) have problems with transport guid (%u). Teleport to bind location.",
                    guid, transGUID);
    ...
    else if (!taxi_nodes.empty())
    {
        ...
        else if (!m_taxi.LoadTaxiDestinationsFromString(taxi_nodes, GetTeam()))
        {
            ...
            {
                sLog->OutErrorConsole("Character %u have wrong data in taxi destination list, teleport to homebind.", GetGUIDLow());
                RelocateToHomebind();
            }
            else                                                // have start node, to it
            {
                sLog->OutErrorConsole("Character %u have too short taxi destination list, teleport to original node.", GetGUIDLow());
    ...
    if (!map)
    {
        ...
        if (at)
        {
            sLog->OutErrorConsole("Player (guidlow %d) is teleported to gobacktrigger (Map: %u X: %f Y: %f Z: %f O: %f).", guid, mapId, GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
            Relocate(at->target_X, at->target_Y, at->target_Z, GetOrientation());
            mapId = at->target_mapId;
        }
        else
        {
            sLog->OutErrorConsole("Player (guidlow %d) is teleported to home (Map: %u X: %f Y: %f Z: %f O: %f).", guid, mapId, GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
            RelocateToHomebind();
        }
        ...
        if (!map)
        {
            PlayerInfo const* info = sObjectMgr->GetPlayerInfo(GetCurrentRace(), GetCurrentClass());
            mapId = info->mapId;
            Relocate(info->positionX, info->positionY, info->positionZ, 0.0f);
            sLog->OutErrorConsole("Player (guidlow %d) have invalid coordinates (X: %f Y: %f Z: %f O: %f). Teleport to default race/class locations.", guid, GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
            map = sMapMgr->CreateMap(mapId, this);
            if (!map)
            {
                sLog->OutErrorConsole("Player (guidlow %d) has invalid default map coordinates (X: %f Y: %f Z: %f O: %f). or instance couldn't be created", guid, GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation());
                return false;
            }
        }
    }
    ...
    if (instanceId && !sInstanceSaveMgr->GetInstanceSave(instanceId) && !map->IsBattlegroundOrArena())
    {
        ...
        {
            sLog->OutErrorConsole("Player %s(GUID: %u) logged in to a reset instance (map: %u) and there is no area-trigger leading to this map. Thus he can't be ported back to the entrance. This _might_ be an exploit attempt.", GetName(), GetGUIDLow(), mapId);
            RelocateToHomebind();
        }
    }
    ...
    {
        sLog->OutErrorConsole("Player can have not more %u stable slots, but have in DB %u", MAX_PET_STABLES, uint32(m_stableSlots));
	...
	{
		sLog->OutErrorConsole("entities.player", "Player (GUID: %u) tried to login while forced to rename, can't load.'", GetGUIDLow());
		return false;
	}
    ...
    {
        m_activeSpec = 0;
        sLog->OutErrorConsole("Player %s(GUID: %u) has SpecCount = %u and ActiveSpec = %u.", GetName(), GetGUIDLow(), m_specsCount, m_activeSpec);
    }
...
void Player::_LoadActions(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                sLog->OutErrorConsole("  ...at loading, and will deleted in DB also");
    ...
    */
    ...
    if (result)
    {
        do
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorConsole("Unknown aura (spellid %u), ignore.", spellid);
                continue;
            }
            ...
            if (GlyphPropertiesEntry const* gp = sGlyphPropertiesStore.LookupEntry(glyph))
            {
                if (GlyphSlotEntry const* gs = sGlyphSlotStore.LookupEntry(GetGlyphSlot(i)))
                {
                    if (gp->TypeFlags == gs->TypeFlags)
                    {
                        ...
                        sLog->OutErrorConsole("Player %s has glyph with typeflags %u in slot with typeflags %u, removing.", m_name.c_str(), gp->TypeFlags, gs->TypeFlags);
                }
                else
                    sLog->OutErrorConsole("Player %s has not existing glyph slot entry %u on index %u", m_name.c_str(), GetGlyphSlot(i), i);
            }
            else
                sLog->OutErrorConsole("Player %s has not existing glyph entry %u on index %u", m_name.c_str(), glyph, i);
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (Item* item = _LoadItem(trans, zoneId, timeDiff, fields))
            {
                ...
                else
                {
                    ...
                    {
                        sLog->OutErrorConsole("Player::_LoadInventory: player (GUID: %u, name: '%s') has item (GUID: %u, entry: %u) which doesnt have a valid bag (Bag GUID: %u, slot: %u). Possible cheat?",
                ...
                {
                    sLog->OutErrorConsole("Player::_LoadInventory: player (GUID: %u, name: '%s') has item (GUID: %u, entry: %u) which can't be loaded into inventory (Bag GUID: %u, slot: %u) by reason %u. Item will be sent by mail.",
        ...
        {
            std::string subject = GetSession()->GetQuantumSystemText(LANG_NOT_EQUIPPED_ITEM);
...
Item* Player::_LoadItem(SQLTransaction& trans, uint32 zoneId, uint32 timeDiff, Field* fields)
{
    ...
    if (ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemEntry))
    {
        ...
        {
            sLog->OutErrorConsole("Player::_LoadInventory: player (GUID: %u, name: '%s') has broken item (GUID: %u, entry: %u) in inventory. Deleting item.",
    ...
    {
        sLog->OutErrorConsole("Player::_LoadInventory: player (GUID: %u, name: '%s') has unknown item (entry: %u) in inventory. Deleting item.",
...
void Player::_LoadMailedItems(Mail* mail)
{
    ...
    do
    {
        ...
        if (!proto)
        {
            sLog->OutErrorConsole("Player %u has unknown item_template (ProtoType) in mailed items(GUID: %u template: %u) in mail (%u), deleted.", GetGUIDLow(), itemGuid, itemTemplate, mail->messageID);
        ...
        {
            sLog->OutErrorConsole("Player::_LoadMailedItems - Item in mail (%u) doesn't exist !!!! - item guid: %u, deleted from mail", mail->messageID, itemGuid);
...
void Player::_LoadMail()
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                sLog->OutErrorConsole("Player::_LoadMail - Mail (%u) have not existed MailTemplateId (%u), remove at load", m->messageID, m->mailTemplateId);
                m->mailTemplateId = 0;
            }
    ...
    if (result)
    {
        do
        {
            ...
            if (quest)
            {
                ...
                {
                    questStatusData.Status = QUEST_STATUS_INCOMPLETE;
                    sLog->OutErrorConsole("Player %s (GUID: %u) has invalid quest %d status (%u), replaced by QUEST_STATUS_INCOMPLETE(3).",
                ...
                if (quest->HasFlag(QUEST_SPECIAL_FLAGS_TIMED) && !GetQuestRewardStatus(quest_id))
                {
                    AddTimedQuest(quest_id);
...
void Player::_LoadDailyQuestStatus(PreparedQueryResult result)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                sLog->OutErrorConsole("Player (GUID: %u) have more 25 daily quest records in `charcter_queststatus_daily`", GetGUIDLow());
                break;
            }
...
void Player::_LoadGroup(PreparedQueryResult result)
{
    ...
    if (result)
    {
        if (Group* group = sGroupMgr->GetGroupByDbStoreId((*result)[0].GetUInt32()))
        {
            ...
            if (GetCurrentLevel() >= LEVELREQUIREMENT_HEROIC)
            {
...
void Player::_LoadBoundInstances(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                sLog->OutErrorConsole("_LoadBoundInstances: player %s(%d) has bind to not existed or not dungeon map %d", GetName(), GetGUIDLow(), mapId);
                deleteInstance = true;
            }
            else if (difficulty >= MAX_DIFFICULTY)
            {
                sLog->OutErrorConsole("_LoadBoundInstances: player %s(%d) has bind to not existed difficulty %d instance for map %u", GetName(), GetGUIDLow(), difficulty, mapId);
                deleteInstance = true;
            }
            else
            {
                ...
                {
                    sLog->OutErrorConsole("_LoadBoundInstances: player %s(%d) is in group %d but has a non-permanent character bind to map %d, %d, %d", GetName(), GetGUIDLow(), GUID_LOPART(group->GetGUID()), mapId, instanceId, difficulty);
                    deleteInstance = true;
                }
            }
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!IsGameMaster() && ar)
    {
        ...
        {
            if (ar->levelMin && GetCurrentLevel() < ar->levelMin)
                LevelMin = ar->levelMin;
            if (ar->levelMax && GetCurrentLevel() > ar->levelMax)
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetQuantumSystemText(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetQuantumSystemText(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetQuantumSystemText(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
...
bool Player::_LoadHomeBind(PreparedQueryResult result)
{
    ...
    if (!info)
    {
        sLog->OutErrorConsole("Player (Name %s) has incorrect race/class pair. Can't be loaded.", GetName());
        return false;
    }
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
        ...
        stmt->setUInt8(index++, GetCurrentRace());
        stmt->setUInt8(index++, GetCurrentClass());
        stmt->setUInt8(index++, GetCurrentGender());
        stmt->setUInt8(index++, GetCurrentLevel());
        ...
        for (uint32 i = 0; i < EQUIPMENT_SLOT_END * 2; ++i)
            ss << GetUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRY_ID + i) << ' ';
...
void Player::_SaveInventory(SQLTransaction& trans)
{
    ...
    for (std::set<uint32>::iterator itr = m_refundableItems.begin(); itr!= m_refundableItems.end(); itr = i_next)
    {
        ...
        {
            sLog->OutErrorConsole("Can't find item guid %u but is in refundable storage for player %u ! Removing.", *itr, GetGUIDLow());
    ...
    for (size_t i = 0; i < m_itemUpdateQueue.size(); ++i)
    {
        ...
        if (item->GetState() != ITEM_REMOVED)
        {
            ...
            if (test == NULL)
            {
                ...
                sLog->OutErrorConsole("Player(GUID: %u Name: %s)::_SaveInventory - the bag(%u) and slot(%u) values for the item with guid %u (state %d) are incorrect, the player doesn't have an item at that position!", lowGuid, GetName(), item->GetBagSlot(), item->GetSlot(), item->GetGUIDLow(), (int32)item->GetState());
            ...
            {
                sLog->OutErrorConsole("Player(GUID: %u Name: %s)::_SaveInventory - the bag(%u) and slot(%u) values for the item with guid %u are incorrect, the item with guid %u is there instead!", lowGuid, GetName(), item->GetBagSlot(), item->GetSlot(), item->GetGUIDLow(), test->GetGUIDLow());
...
void Player::_SaveStats(SQLTransaction& trans)
{
    ...
    if (!sWorld->getIntConfig(CONFIG_MIN_LEVEL_STAT_SAVE) || GetCurrentLevel() < sWorld->getIntConfig(CONFIG_MIN_LEVEL_STAT_SAVE))
        return;
...
void Player::outDebugValues() const
{
    ...
    sLog->outDebug(LOG_FILTER_UNITS, "MIN_OFF_HAND_DAMAGE is: \t%f\tMAX_OFFHAND_DAMAGE is: \t%f", GetFloatValue(UNIT_FIELD_MIN_OFF_HAND_DAMAGE), GetFloatValue(UNIT_FIELD_MAX_OFF_HAND_DAMAGE));
...
{
    if (!m_contestedPvPTimer || IsInCombatActive())
        return;
...
Pet* Player::GetPet() const
{
    if (uint64 pet_guid = GetPetGUID())
    {
        ...
        //sLog->OutErrorConsole("Player::GetPet: Pet %u not exist.", GUID_LOPART(pet_guid));
...
void Player::StopCastingCharm()
{
    ...
    if (charm->GetTypeId() == TYPE_ID_UNIT)
    {
...
void Player::TextEmote(const std::string& text)
{
    ...
    SendMessageToSetInRange(&data, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXT_EMOTE), true, !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHAT));
}
...
void Player::PossessSpellInitialize()
{
    ...
    if (!charmInfo)
    {
        sLog->OutErrorConsole("Player::PossessSpellInitialize(): charm ("UI64FMTD") has no charminfo!", charm->GetGUID());
        return;
    }
...
void Player::CharmSpellInitialize()
{
    ...
    if (!charmInfo)
    {
        sLog->OutErrorConsole("Player::CharmSpellInitialize(): the player's charm ("UI64FMTD") has no charminfo!", charm->GetGUID());
        return;
    }

    uint8 addlist = 0;
    if (charm->GetTypeId() != TYPE_ID_PLAYER)
    {
        //CreatureInfo const* cinfo = charm->ToCreature()->GetCreatureTemplate();
        //if (cinfo && cinfo->type == CREATURE_TYPE_DEMON && GetCurrentClass() == CLASS_WARLOCK)
        {
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
    ...
    if (GetSession()->isLogingOut() || IsInCombatActive() || HasUnitState(UNIT_STATE_STUNNED) || HasUnitState(UNIT_STATE_ROOT))
    {
    ...
    // can use this taxi.
    uint32 mount_display_id = sObjectMgr->GetTaxiMountDisplayId(sourcenode, GetTeam(), npc == NULL || (sourcenode == 315 && GetCurrentClass() == CLASS_DEATH_KNIGHT));
    ...
    if (money < totalcost)
    {
        GetSession()->SendActivateTaxiReply(ERR_TAXI_NOTE_NOUGH_MONEY);
        m_taxi.ClearTaxiDestinations();
        return false;
    }
...
void Player::InitDataForForm(bool reapplyMods)
{
    ...
    switch (form)
    {
        ...
        {
            ChrClassesEntry const* cEntry = sChrClassesStore.LookupEntry(GetCurrentClass());
...
void Player::InitDisplayIds()
{
    ...
    if (!info)
    {
        sLog->OutErrorConsole("Player %u has incorrect race/class pair. Can't init display ids.", GetGUIDLow());
        return;
    }

    uint8 gender = GetCurrentGender();
    switch (gender)
    {
        case GENDER_FEMALE:
        ...
        case GENDER_MALE:
            ...
            break;
        default:
            sLog->OutErrorConsole("Invalid gender %u for player", gender);
            return;
    }
}
...
bool Player::BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if (crItem->ExtendedCost)
    {
        ...
        if (!iece)
        {
            sLog->OutErrorConsole("Item %u have wrong ExtendedCost field value %u", pProto->ItemId, crItem->ExtendedCost);
            return false;
        }
    ...
    if (crItem->IsGoldRequired(pProto) && pProto->BuyPrice > 0) //Assume price cannot be negative (do not know why it is int32)
    {
        ...
        {
            sLog->OutErrorConsole("Player %s tried to buy %u item id %u, causing overflow", GetName(), (uint32)count, pProto->ItemId);
...
void Player::UpdatePotionCooldown(Spell* spell)
{
    ...
    if (!m_lastPotionId || IsInCombatActive())
        return;
...
void Player::SetBattlegroundEntryPoint()
{
    ...
    else
    {
        ...
        if (GetMap()->IsDungeon())
        {
            if (const WorldSafeLocsEntry* entry = sObjectMgr->GetClosestGraveYard(GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId(), GetTeam(), GetCurrentClass()))
                m_bgData.joinPos = WorldLocation(entry->map_id, entry->x, entry->y, entry->z, 0.0f);
            else
                sLog->OutErrorConsole("SetBattlegroundEntryPoint: Dungeon map %u has no linked graveyard, setting home location as entry point.", GetMapId());
        }
...
void Player::LeaveBattleground(bool teleportToEntryPoint)
{
    if (Battleground* bg = GetBattleground())
    {
		...
		if (bg->isBattleground() && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_TRACK_DESERTERS) && (bg->GetStatus() == STATUS_IN_PROGRESS || bg->GetStatus() == STATUS_WAIT_JOIN))
		{
...
WorldLocation Player::GetStartPosition() const
{
    ...
    if (GetCurrentClass() == CLASS_DEATH_KNIGHT && HasSpell(50977))
        mapId = 0;
...
void Player::UpdateVisibilityOf(WorldObject* target)
{
    ...
    else
    {
        if (canSeeOrDetect(target, false, true))
        {
            //if (target->isType(TYPE_MASK_UNIT) && ((Unit*)target)->m_Vehicle)
            ...
            if (target->isType(TYPE_MASK_UNIT))
...
void Player::UpdateVisibilityOf(T* target, UpdateData& data, std::set<Unit*>& visibleNow)
{
    ...
    else //if (visibleNow.size() < 30 || target->GetTypeId() == TYPE_ID_UNIT && target->ToCreature()->IsVehicle())
    {
        if (canSeeOrDetect(target, false, true))
        {
            //if (target->isType(TYPE_MASK_UNIT) && ((Unit*)target)->m_Vehicle)
...
void Player::SendInitialPacketsAfterAddToMap()
{
	...
	if (!IsGameMaster())
		CastSpell(this, 7265, true);                        // ABOUT TO SPAWN
...
void Player::resetSpells(bool myClassOnly)
{
    ...
    if (myClassOnly)
    {
        ChrClassesEntry const* clsEntry = sChrClassesStore.LookupEntry(GetCurrentClass());
        if (!clsEntry)
            return;
...
void Player::learnDefaultSpells()
{
    ...
    {
        uint32 tspell = *itr;
        sLog->outDebug(LOG_FILTER_PLAYER_LOADING, "PLAYER (Class: %u Race: %u): Adding initial spell, id = %u", uint32(GetCurrentClass()), uint32(GetCurrentRace()), tspell);
...
{
    uint32 raceMask  = GetCurrentRaceMask();
    uint32 classMask = GetCurrentClassMask();
    for (uint32 j=0; j<sSkillLineAbilityStore.GetNumRows(); ++j)
    {
...
void Player::UpdateForQuestWorldObjects()
{
    ...
    for (ClientGUIDs::iterator itr=m_clientGUIDs.begin(); itr != m_clientGUIDs.end(); ++itr)
    {
        ...
        else if (IS_CRE_OR_VEH_GUID(*itr))
        {
            ...
            if (!obj->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELL_CLICK))
                continue;
...
void Player::AutoUnequipOffhandIfNeed(bool force /*= false*/)
{
    ...
    if (!offItem)
     ...
     if (!CanDualWield() && (offItem->GetTemplate()->InventoryType == INVENTORY_TYPE_WEAPON_OFF_HAND || offItem->GetTemplate()->InventoryType == INVENTORY_TYPE_WEAPON))
          force = true;

    // need unequip offhand for 2h-weapon without TitanGrip (in any from hands)
    if (!force && (CanTitanGrip() || (offItem->GetTemplate()->InventoryType != INVENTORY_TYPE_2H_WEAPON && !IsTwoHandUsed())))
        return;
    ...
    {
        RemoveItem(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND, true);
        StoreItem(off_dest, offItem, true);
    }
    else
    {
        MoveItemFromInventory(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND, true);
        SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
bool Player::HasItemFitToSpellRequirements(SpellInfo const* spellInfo, Item const* ignoreItem)
{
    ...
    switch (spellInfo->EquippedItemClass)
    {
        ...
        {
            for (uint8 i= EQUIPMENT_SLOT_MAIN_HAND; i < EQUIPMENT_SLOT_TABARD; ++i)
			{
        ...
        case ITEM_CLASS_ARMOR:
        {
            ...
            for (uint8 i= EQUIPMENT_SLOT_START; i< EQUIPMENT_SLOT_MAIN_HAND; ++i)
                if (Item* item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, i))
                    if (item != ignoreItem && item->IsFitToSpellRequirements(spellInfo))
                        return true;

            // shields can be equipped to offhand slot
            if (Item* item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND))
                if (item != ignoreItem && item->IsFitToSpellRequirements(spellInfo))
                    return true;
        ...
        default:
            sLog->OutErrorConsole("HasItemFitToSpellRequirements: Not handled spell requirement for item class %u", spellInfo->EquippedItemClass);
            break;
    }
...
void Player::RemoveItemDependentAurasAndCasts(Item* pItem)
{
    for (AuraMap::iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end();)
    {
		...
		if (aura->GetId() == 46924) // Canceling Bladestorm
		{
			++itr;
			continue;
		} // canceling bladestorm
...
uint32 Player::GetResurrectionSpellId()
{
    ...
    for (AuraEffectList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
    {
        ...
        if (prio < 2 && (*itr)->GetSpellInfo()->SpellVisual[0] == 99 && (*itr)->GetSpellInfo()->SpellIconID == 92)
        {
            ...
            {
                case 20707:
					spell_id = 3026; // rank 1
					break;
                case 20762:
					spell_id = 20758; // rank 2
					break;
                case 20763:
					spell_id = 20759; // rank 3
					break;
                case 20764:
					spell_id = 20760; // rank 4
					break;
                case 20765:
					spell_id = 20761; // rank 5
					break;
                case 27239:
					spell_id = 27240; // rank 6
					break;
                case 47883:
					spell_id = 47882; // rank 7
					break;
                default:
                    sLog->OutErrorConsole("Unhandled spell %u: S.Resurrection", (*itr)->GetId());
                    continue;
            }
...
{
    uint8 v_level = victim->GetCurrentLevel();
    uint8 k_grey  = Trinity::XP::GetGrayLevel(GetCurrentLevel());

    // Victim level less gray level
    if (v_level <= k_grey)
        return false;

    if (victim->GetTypeId() == TYPE_ID_UNIT)
    {
        if (victim->ToCreature()->IsTotem() || victim->ToCreature()->IsPet() || victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_XP_AT_KILL)
			return false;
    }
    return true;
}
...
bool Player::GetsRecruitAFriendBonus(bool forXP)
{
    ...
    if (GetCurrentLevel() <= sWorld->getIntConfig(CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL) || !forXP)
    {
        if (Group* group = this->GetGroup())
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                if (forXP)
                {
                    ...
                    if (player->GetCurrentLevel() > sWorld->getIntConfig(CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL))
                        continue;

                    // level difference must be small enough to get RaF bonus, UNLESS we are lower level
                    if (player->GetCurrentLevel() < GetCurrentLevel())
                        if (uint8(GetCurrentLevel() - player->GetCurrentLevel()) > sWorld->getIntConfig(CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL_DIFFERENCE))
                            continue;
                }
...
void Player::RewardPlayerAndGroupAtEvent(uint32 creature_id, WorldObject* pRewardSource)
{
    if (!pRewardSource)
        return;
    uint64 creature_guid = (pRewardSource->GetTypeId() == TYPE_ID_UNIT) ? pRewardSource->GetGUID() : uint64(0);
...
PartyResult Player::CanUninviteFromGroup() const
{
    ...
    if (grp->isLFGGroup())
    {
		...
		for (GroupReference const* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
			if (itr->getSource() && itr->getSource()->IsInCombatActive())
...
void Player::SetViewpoint(WorldObject* target, bool apply)
{
    if (apply)
    {
        ...
        if (target->isType(TYPE_MASK_UNIT) && !GetVehicle())
...
WorldObject* Player::GetViewpoint() const
{
    if (uint64 guid = GetUInt64Value(PLAYER_FARSIGHT))
		...
		return (WorldObject*)ObjectAccessor::GetObjectByTypeMask(*this, guid, TYPE_MASK_SEER);
	return NULL;
}
...
{
	return (!HasStealthAura() && !HasInvisibilityAura() && IsAlive()); // not stealthed, not invisible, live player.
}
...
{
    uint32 base_talent = GetCurrentLevel() < 10 ? 0 : GetCurrentLevel()-9;

    if (GetCurrentClass() != CLASS_DEATH_KNIGHT || GetMapId() != 609)
        return uint32(base_talent * sWorld->getRate(RATE_TALENT));

    uint32 talentPointsForLevel = GetCurrentLevel() < 56 ? 0 : GetCurrentLevel() - 55;
...
void Player::_LoadSkills(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!pSkill)
            {
                sLog->OutErrorConsole("Character %u has skill %u that does not exist.", GetGUIDLow(), skill);
                continue;
            }
            ...
            if (value == 0)
            {
                sLog->OutErrorConsole("Character %u has skill %u with value 0. Will be deleted.", GetGUIDLow(), skill);
            ...
            {
                sLog->OutErrorConsole("Character %u has more than %u skills.", GetGUIDLow(), PLAYER_MAX_SKILLS);
                break;
            }
        }
        while (result->NextRow());
    ...
    if (GetCurrentClass() == CLASS_DEATH_KNIGHT)
    {
        uint8 base_level = std::min(GetCurrentLevel(), uint8(sWorld->getIntConfig(CONFIG_START_HEROIC_PLAYER_LEVEL)));
        if (base_level < 1)
            base_level = 1;
...
void Player::LearnTalent(uint32 talentId, uint32 talentRank)
{
    ...
    if (spellid == 0)
    {
        sLog->OutErrorConsole("Talent.dbc have for talent: %u Rank: %u spell id = 0", talentId, talentRank);
        return;
    }
...
{
    if (!c->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELL_CLICK))
        return false;
    ...
    if (m_specsCount)
    {
        ...
        for (uint32 specIdx = 0; specIdx < m_specsCount; ++specIdx)
        {
            ...
            *data << uint8(talentIdCount);                  // [PH], talentIdCount
            ...
            uint32 const* talentTabIds = GetTalentTabPages(GetCurrentClass());
...
void Player::SetEquipmentSet(uint32 index, EquipmentSet eqset)
{
    if (eqset.Guid != 0)
    {
        ...
        {
            sLog->OutErrorConsole("Player %s tried to save equipment set "UI64FMTD" (index %u), but that equipment set not found!", GetName(), eqset.Guid, index);
            return;
        }
    }
...
float Player::GetAverageItemLevel()
{
    ...
    for (int i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        if (i == EQUIPMENT_SLOT_TABARD || i == EQUIPMENT_SLOT_RANGED || i == EQUIPMENT_SLOT_OFF_HAND || i == EQUIPMENT_SLOT_BODY)
            continue;
...
void Player::CreateWowarmoryFeed(uint32 type, uint32 data, uint32 item_guid, uint32 item_quality)
{
    ...
    */
    if (GetGUIDLow() == 0)
    {
        sLog->OutErrorConsole("[Wowarmory]: player is not initialized, unable to create log entry!");
        return;
    }
    if (type <= 0 || type > 3)
    {
        sLog->OutErrorConsole("[Wowarmory]: unknown feed type: %d, ignore.", type);
        return;
    }
    if (data == 0)
    {
        sLog->OutErrorConsole("[Wowarmory]: empty data (GUID: %u), ignore.", GetGUIDLow());
        return;
    }
...
uint32 Player::SuitableForTransmogrification(Item* oldItem, Item* newItem)
{
    ...
    if (newClass == ITEM_CLASS_WEAPON && newSubClass != ITEM_SUBCLASS_WEAPON_FISHING_POLE && oldSubClass != ITEM_SUBCLASS_WEAPON_FISHING_POLE)
    {
        if (newSubClass == oldSubClass || ((newSubClass == ITEM_SUBCLASS_WEAPON_BOW || newSubClass == ITEM_SUBCLASS_WEAPON_GUN || newSubClass == ITEM_SUBCLASS_WEAPON_CROSSBOW) && (oldSubClass == ITEM_SUBCLASS_WEAPON_BOW || oldSubClass == ITEM_SUBCLASS_WEAPON_GUN || oldSubClass == ITEM_SUBCLASS_WEAPON_CROSSBOW)))
            if (newInventorytype == oldInventorytype || (newInventorytype == INVENTORY_TYPE_WEAPON && (oldInventorytype == INVENTORY_TYPE_WEAPON_MAIN_HAND || oldInventorytype == INVENTORY_TYPE_WEAPON_OFF_HAND)))
                return ERR_FAKE_OK;
            else
        ...
        if (newSubClass == oldSubClass)
            if (newInventorytype == oldInventorytype || (newInventorytype == INVENTORY_TYPE_CHEST && oldInventorytype == INVENTORY_TYPE_ROBE) || (newInventorytype == INVENTORY_TYPE_ROBE && oldInventorytype == INVENTORY_TYPE_CHEST))
                return ERR_FAKE_OK;
            else
                return ERR_FAKE_BAD_INVENTORYTYPE;
        else
