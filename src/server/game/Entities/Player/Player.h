...
struct PvPInfo
{
    PvPInfo() : inHostileArea(false), IsInNoPvPArea(false), inFFAPvPArea(false), endTimer(0) {}
...
enum EquipmentSlots                                         // 19 slots
{
    ...
    EQUIPMENT_SLOT_FINGER_1     = 10,
    EQUIPMENT_SLOT_FINGER_2     = 11,
    EQUIPMENT_SLOT_TRINKET_1    = 12,
    EQUIPMENT_SLOT_TRINKET_2    = 13,
    EQUIPMENT_SLOT_BACK         = 14,
    EQUIPMENT_SLOT_MAIN_HAND    = 15,
    EQUIPMENT_SLOT_OFF_HAND     = 16,
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        PlayerTaxi m_taxi;
        void InitTaxiNodesForLevel() { m_taxi.InitTaxiNodesForLevel(GetCurrentRace(), GetCurrentClass(), GetCurrentLevel()); }
		...
		// PlayedTimeReward
		uint32 PlayedTimeRewardIntervalTime;
		uint32 PlayedTimeRewardMoney;
		uint32 PlayedTimeRewardHonorPoints;
		uint32 PlayedTimeRewardArenaPoints;
        ...
        {
            Item* mainItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAIN_HAND);
            return mainItem && mainItem->GetTemplate()->InventoryType == INVENTORY_TYPE_2H_WEAPON && !CanTitanGrip();
        }
        ...
        int32 GetQuestLevel(Quest const* quest) const { return quest && (quest->GetQuestLevel() > 0) ? quest->GetQuestLevel() : GetCurrentLevel(); }
        ...
        uint32 GetFreeTalentPoints() const { return GetUInt32Value(PLAYER_CHARACTER_POINTS_1); }
        ...
        uint32 GetFreePrimaryProfessionPoints() const { return GetUInt32Value(PLAYER_CHARACTER_POINTS_2); }
        void SetFreePrimaryProfessions(uint16 profs) { SetUInt32Value(PLAYER_CHARACTER_POINTS_2, profs); }
        ...
        void SetInGuild(uint32 GuildId) { SetUInt32Value(PLAYER_GUILD_ID, GuildId); }
        void SetRank(uint8 rankId) { SetUInt32Value(PLAYER_GUILD_RANK, rankId); }
        uint8 GetRank() const { return uint8(GetUInt32Value(PLAYER_GUILD_RANK)); }
        void SetGuildIdInvited(uint32 GuildId) { m_GuildIdInvited = GuildId; }
        uint32 GetGuildId() const { return GetUInt32Value(PLAYER_GUILD_ID);  }
		Guild* GetGuild();
