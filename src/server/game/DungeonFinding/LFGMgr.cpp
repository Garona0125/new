...
void LFGMgr::LoadRewards()
{
    ...
    {
		sLog->OutConsole("=========================================================================");
        sLog->OutConsole(">> Loaded 0 lfg dungeon rewards. DB table `lfg_dungeon_rewards` is empty!");
		sLog->OutConsole("=========================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("Dungeon %u specified in table `lfg_dungeon_rewards` does not exist!", dungeonId);
            continue;
        }

        if (!maxLevel || maxLevel > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
            sLog->OutErrorDatabase("Level %u specified for dungeon %u in table `lfg_dungeon_rewards` can never be reached!", maxLevel, dungeonId);
            maxLevel = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
        }

        if (firstQuestId && !sObjectMgr->GetQuestTemplate(firstQuestId))
        {
            sLog->OutErrorDatabase("First quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", firstQuestId, dungeonId);
            firstQuestId = 0;
        }

        if (otherQuestId && !sObjectMgr->GetQuestTemplate(otherQuestId))
        {
            sLog->OutErrorDatabase("Other quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", otherQuestId, dungeonId);
            otherQuestId = 0;
        }
	...
	sLog->OutConsole("===========================================");
    sLog->OutConsole(">> Loaded %u lfg dungeon rewards in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===========================================");
    sLog->OutConsole();
}
...
void LFGMgr::LoadEntrancePositions()
{
    ...
    {
		sLog->OutConsole("======================================================================");
        sLog->OutConsole(">> Loaded 0 lfg entrance positions. DB table `lfg_entrances` is empty!");
		sLog->OutConsole("======================================================================");
		sLog->OutConsole();
        return;
    }
	...
	sLog->OutConsole("==============================================");
    sLog->OutConsole(">> Loaded %u lfg entrance positions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==============================================");
	sLog->OutConsole();
}
...
void LFGMgr::Update(uint32 diff)
{
    ...
    if (m_QueueTimer > LFG_QUEUEUPDATE_INTERVAL)
    {
        ...
        for (LfgQueueInfoMap::const_iterator itQueue = m_QueueInfoMap.begin(); itQueue != m_QueueInfoMap.end(); ++itQueue)
        {
            ...
            if (!queue)
            {
                sLog->OutErrorConsole("LFGMgr::Update: [" UI64FMTD "] queued with null queue info!", itQueue->first);
                continue;
            }
...
LfgProposal* LFGMgr::FindNewGroups(LfgGuidList& check, LfgGuidList& all)
{
    ...
    if (check.empty() || check.size() > MAX_GROUP_SIZE || !CheckCompatibility(check, pProposal))
        return NULL;
...
bool LFGMgr::CheckCompatibility(LfgGuidList check, LfgProposal*& pProposal)
{
    ...
    for (LfgGuidList::const_iterator it = check.begin(); it != check.end() && numLfgGroups < 2 && numPlayers <= MAX_GROUP_SIZE; ++it)
    {
        ...
        {
            sLog->OutErrorConsole("LFGMgr::CheckCompatibility: [" UI64FMTD "] is not queued but listed as queued!", (*it));
            RemoveFromQueue(guid);
            return false;
        }
    ...
    if (check.size() == 1 && numPlayers != MAX_GROUP_SIZE)   // Single group with less than MAX_GROUP_SIZE - Compatibles
        return true;
    ...
    // ----- Group is compatible, if we have MAX_GROUP_SIZE members then match is found
    ...
    pProposal = new LfgProposal(Quantum::DataPackets::SelectRandomContainerElement(compatibleDungeons));
    ...
    else
    {
        ...
        for (LfgPlayerList::const_iterator it = players.begin(); it != players.end(); ++it)
        {
            ...
            if (player->groupLowGuid != lowgroupguid)
                sLog->OutErrorConsole("LFGMgr::UpdateProposal: [" UI64FMTD "] group mismatch: actual (%u) - queued (%u)", (*it)->GetGUID(), lowgroupguid, player->groupLowGuid);
            ...
            if (itQueue == m_QueueInfoMap.end())
            {
                sLog->OutErrorConsole("LFGMgr::UpdateProposal: Queue info for guid [" UI64FMTD "] not found!", guid);
    ...
    else
    {
        ...
        else if (player->GetMapId() != uint32(dungeon->map))  // Do not teleport players in dungeon to the entrance
        {
            ...
            if (!mapid)
            {
				...
				{
					sLog->outDebug(LOG_FILTER_LFG, "LfgMgr::TeleportPlayer: Failed to teleport [" UI64FMTD "]: No areatrigger found for map: %u difficulty: %u", player->GetGUID(), dungeon->map, dungeon->difficulty);
					error = LFG_TELEPORTERROR_INVALID_LOCATION;
				}
            ...
            if (error == LFG_TELEPORTERROR_OK)
            {
                ...
                {
                    error = LFG_TELEPORTERROR_INVALID_LOCATION;
                    sLog->OutErrorConsole("LfgMgr::TeleportPlayer: Failed to teleport [" UI64FMTD "] to map %u: ", player->GetGUID(), mapid);
                }
            }
        }
    }
...
void LFGMgr::RewardDungeonDoneFor(const uint32 dungeonId, Player* player)
{
    ...
    LfgReward const* reward = GetRandomDungeonReward(rDungeonId, player->GetCurrentLevel());
    if (!reward)
        return;
