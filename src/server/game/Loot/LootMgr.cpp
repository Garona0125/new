...
uint32 LootStore::LoadLootTable()
{
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("Table '%s' entry %d item %d: maxcount value (%u) to large. must be less %u - skipped", GetName(), entry, item, maxcount, std::numeric_limits<uint8>::max());
...
void LootStore::ReportUnusedIds(LootIdSet const& lootIdSet) const
{
    ...
    for (LootIdSet::const_iterator itr = lootIdSet.begin(); itr != lootIdSet.end(); ++itr)
        sLog->OutErrorDatabase("Table '%s' entry %d isn't %s and not referenced from loot, and then useless.", GetName(), *itr, GetEntryName());
}

void LootStore::ReportNotExistedId(uint32 id) const
{
    sLog->OutErrorDatabase("Table '%s' entry %d (%s) does not exist but used as loot id in DB.", GetName(), id, GetEntryName());
}
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    {
        sLog->OutErrorDatabase("Table '%s' entry %d item %d: group (%u) must be less %u - skipped", store.GetName(), entry, itemid, group, 1 << 7);
        return false;
    }

    if (mincountOrRef == 0)
    {
        sLog->OutErrorDatabase("Table '%s' entry %d item %d: wrong mincountOrRef (%d) - skipped", store.GetName(), entry, itemid, mincountOrRef);
        return false;
    }
    ...
    if (mincountOrRef > 0)                                  // item (quest or non-quest) entry, maybe grouped
    {
        ...
        if (!proto)
        {
            sLog->OutErrorDatabase("Table '%s' entry %d item %d: item entry not listed in `item_template` - skipped", store.GetName(), entry, itemid);
            return false;
        }

        if (chance == 0 && group == 0)                      // Zero chance is allowed for grouped entries only
        {
            sLog->OutErrorDatabase("Table '%s' entry %d item %d: equal-chanced grouped entry, but group not defined - skipped", store.GetName(), entry, itemid);
            return false;
        }

        if (chance != 0 && chance < 0.000001f)             // loot with low chance
        {
            sLog->OutErrorDatabase("Table '%s' entry %d item %d: low chance (%f) - skipped",
        ...
        {
            sLog->OutErrorDatabase("Table '%s' entry %d item %d: max count (%u) less that min count (%i) - skipped", store.GetName(), entry, itemid, int32(maxcount), mincountOrRef);
            return false;
        }
    }
    ...
    {
        if (needs_quest)
            sLog->OutErrorDatabase("Table '%s' entry %d item %d: quest chance will be treated as non-quest chance", store.GetName(), entry, itemid);
        else if (chance == 0)                              // no chance for the reference
        {
            sLog->OutErrorDatabase("Table '%s' entry %d item %d: zero chance is specified for a reference, skipped", store.GetName(), entry, itemid);
            return false;
        }
    }
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/)
{
    ...
    {
        if (!noEmptyError)
            sLog->OutErrorDatabase("Table '%s' loot id #%u used but it doesn't have records.", store.GetName(), lootId);
        return false;
    }
...
void LootTemplate::LootGroup::Verify(LootStore const& lootstore, uint32 id, uint8 group_id) const
{
    ...
    {
        sLog->OutErrorDatabase("Table '%s' entry %u group %d has total chance > 100%% (%f)", lootstore.GetName(), id, group_id, chance);
    }

    if (chance >= 100.0f && !EqualChanced.empty())
    {
        sLog->OutErrorDatabase("Table '%s' entry %u group %d has items with chance=0%% but group total chance >= 100%% (%f)", lootstore.GetName(), id, group_id, chance);
    }
}
...
bool LootTemplate::addConditionItem(Condition* cond)
{
    ...
    {
        sLog->OutErrorConsole("LootTemplate::addConditionItem: condition is null");
        return false;
    }
    if (!Entries.empty())
    {
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Creature Loot Templates...");
	...
	{
		sLog->OutConsole("================================================");
        sLog->OutConsole(">> Loaded %u creature loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("================================================");
	}
    else
	{
		sLog->OutConsole("================================================================================");
        sLog->OutConsole(">> Loaded 0 creature loot templates. DB table `creature_loot_template` is empty!");
		sLog->OutConsole("================================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Disenchanting Loot Templates...");
	...
	{
		sLog->OutConsole("====================================================");
        sLog->OutConsole(">> Loaded %u disenchanting loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("====================================================");
	}
    else
	{
		sLog->OutConsole("=======================================================================================");
        sLog->OutConsole(">> Loaded 0 disenchanting loot templates. DB table `disenchant_loot_template` is empty!");
		sLog->OutConsole("=======================================================================================");
	}
    sLog->OutConsole();
}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Fishing Loot Templates...");
	...
	{
		sLog->OutConsole("==============================================");
        sLog->OutConsole(">> Loaded %u fishing loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("==============================================");
	}
    else
	{
		sLog->OutConsole("==============================================================================");
        sLog->OutConsole(">> Loaded 0 fishing loot templates. DB table `fishing_loot_template` is empty!");
		sLog->OutConsole("==============================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Gameobject Loot Templates...");
	...
	{
		sLog->OutConsole("=================================================");
        sLog->OutConsole(">> Loaded %u gameobject loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("=================================================");
	}
    else
	{
		sLog->OutConsole("====================================================================================");
        sLog->OutConsole(">> Loaded 0 gameobject loot templates. DB table `gameobject_loot_template` is empty!");
		sLog->OutConsole("====================================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Item Loot Templates...");
	...
	{
		sLog->OutConsole("===========================================");
		sLog->OutConsole(">> Loaded %u item loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("===========================================");
	}
    else
	{
		sLog->OutConsole("========================================================================");
		sLog->OutConsole(">> Loaded 0 item loot templates. DB table `item_loot_template` is empty!");
		sLog->OutConsole("========================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Milling Loot Templates...");
	...
	{
		sLog->OutConsole("==============================================");
        sLog->OutConsole(">> Loaded %u milling loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("==============================================");
	}
    else
	{
		sLog->OutConsole("==============================================================================");
        sLog->OutConsole(">> Loaded 0 milling loot templates. DB table `milling_loot_template` is empty!");
		sLog->OutConsole("==============================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Pickpocketing Loot Templates...");
	...
	{
		sLog->OutConsole("====================================================");
        sLog->OutConsole(">> Loaded %u pickpocketing loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("====================================================");
	}
    else
	{
		sLog->OutConsole("==========================================================================================");
        sLog->OutConsole(">> Loaded 0 pickpocketing loot templates. DB table `pickpocketing_loot_template` is empty!");
		sLog->OutConsole("==========================================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Prospecting Loot Templates...");
	...
	{
		sLog->OutConsole("==================================================");
        sLog->OutConsole(">> Loaded %u prospecting loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("==================================================");
	}
    else
	{
		sLog->OutConsole("======================================================================================");
        sLog->OutConsole(">> Loaded 0 prospecting loot templates. DB table `prospecting_loot_template` is empty!");
		sLog->OutConsole("======================================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Mail Loot Templates...");
	...
	{
		sLog->OutConsole("===========================================");
        sLog->OutConsole(">> Loaded %u mail loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("===========================================");
	}
    else
	{
		sLog->OutConsole("=======================================================================");
        sLog->OutConsole(">> Loaded 0 mail loot templates. DB table `mail_loot_template` is empty");
		sLog->OutConsole("=======================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Skinning Loot Templates...");
	...
	{
		sLog->OutConsole("===============================================");
        sLog->OutConsole(">> Loaded %u skinning loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("===============================================");
	}
    else
	{
		sLog->OutConsole("================================================================================");
        sLog->OutConsole(">> Loaded 0 skinning loot templates. DB table `skinning_loot_template` is empty!");
		sLog->OutConsole("================================================================================");
	}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Spell Loot Templates...");
	...
	{
		sLog->OutConsole("============================================");
        sLog->OutConsole(">> Loaded %u spell loot templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("============================================");
	}
    else
	{
		sLog->OutConsole("==========================================================================");
        sLog->OutConsole(">> Loaded 0 spell loot templates. DB table `spell_loot_template` is empty!");
		sLog->OutConsole("==========================================================================");
	}
    sLog->OutConsole();
}
...
{
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Reference Loot Templates...");
	...
	sLog->OutConsole("===========================================");
    sLog->OutConsole(">> Loaded refence loot templates in %u ms", GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===========================================");
    sLog->OutConsole();
}