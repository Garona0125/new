...
enum QuestFlags
{
	...
	QUEST_SPECIAL_FLAGS_MONTHLY              = 0x01000000,  // Set by 16 in SpecialFlags in DB if the quest is reset at the begining of the month

    QUEST_SPECIAL_FLAGS_DB_ALLOWED = 0xFFFFF | QUEST_SPECIAL_FLAGS_REPEATABLE | QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT | QUEST_SPECIAL_FLAGS_AUTO_ACCEPT | QUEST_SPECIAL_FLAGS_DF_QUEST | QUEST_SPECIAL_FLAGS_MONTHLY,

    // Trinity flags for internal use only
    QUEST_SPECIAL_FLAGS_DELIVER              = 0x04000000,   // Internal flag computed only
    QUEST_SPECIAL_FLAGS_SPEAKTO              = 0x08000000,   // Internal flag computed only
    QUEST_SPECIAL_FLAGS_KILL_OR_CAST         = 0x10000000,   // Internal flag computed only
    QUEST_SPECIAL_FLAGS_TIMED                = 0x20000000,   // Internal flag computed only
    QUEST_SPECIAL_FLAGS_PLAYER_KILL          = 0x40000000,   // Internal flag computed only
};
...
class Quest
{
    ...
    public:
        ...
        bool IsRepeatable() const { return Flags & QUEST_SPECIAL_FLAGS_REPEATABLE; }
        ...
        bool IsDFQuest() const { return Flags & QUEST_SPECIAL_FLAGS_DF_QUEST; }
