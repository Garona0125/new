...
Quest::Quest(Field* questRecord)
{
    ...
    if (Flags & QUEST_SPECIAL_FLAGS_AUTO_ACCEPT)
...
uint32 Quest::XPValue(Player* player) const
{
    if (player)
    {
        int32 quest_level = (Level == -1 ? player->GetCurrentLevel() : Level);
        const QuestXPEntry* xpentry = sQuestXPStore.LookupEntry(quest_level);
        if (!xpentry)
            return 0;

        int32 diffFactor = 2 * (quest_level - player->GetCurrentLevel()) + 20;
        if (diffFactor < 1)
            diffFactor = 1;
