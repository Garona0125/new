...
{
    public:
        CreatureTextBuilder(WorldObject* obj, ChatMsg msgtype, uint8 textGroup, uint32 id, uint32 language, uint64 targetGUID): _source(obj), _msgType(msgtype), _textGroup(textGroup), _textId(id), _language(language), _targetGUID(targetGUID){}
...
void CreatureTextMgr::LoadCreatureTexts()
{
    ...
    {
		sLog->OutConsole("=============================================================");
        sLog->OutConsole(">> Loaded 0 ceature texts. DB table `creature_text` is empty!");
		sLog->OutConsole("=============================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (temp.sound)
        {
            if (!sSoundEntriesStore.LookupEntry(temp.sound)){
                sLog->OutErrorDatabase("CreatureTextMgr:  Entry %u, Group %u in table `creature_text` has Sound %u but sound does not exist.", temp.entry, temp.group, temp.sound);
                temp.sound = 0;
            }
        }
        if (!GetLanguageDescByID(temp.lang))
        {
            sLog->OutErrorDatabase("CreatureTextMgr:  Entry %u, Group %u in table `creature_text` using Language %u but Language does not exist.", temp.entry, temp.group, uint32(temp.lang));
            temp.lang = LANG_UNIVERSAL;
        }
        if (temp.type >= MAX_CHAT_MSG_TYPE)
        {
            sLog->OutErrorDatabase("CreatureTextMgr:  Entry %u, Group %u in table `creature_text` has Type %u but this Chat Type does not exist.", temp.entry, temp.group, uint32(temp.type));
        ...
        if (temp.emote)
        {
            ...
            {
                sLog->OutErrorDatabase("CreatureTextMgr:  Entry %u, Group %u in table `creature_text` has Emote %u but emote does not exist.", temp.entry, temp.group, uint32(temp.emote));
	...
	sLog->OutConsole("==========================================================");
    sLog->OutConsole(">> Loaded %u creature texts for %u creatures in %u ms", textCount, creatureCount, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==========================================================");
    sLog->OutConsole();
}
...
void CreatureTextMgr::LoadCreatureTextLocales()
{
	...
	sLog->OutConsole("==================================================");
    sLog->OutConsole(">> Loaded %u creature localized texts in %u ms", textCount, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==================================================");
    sLog->OutConsole();
}
...
uint32 CreatureTextMgr::SendChat(Creature* source, uint8 textGroup, uint64 whisperGuid /*= 0*/, ChatMsg msgType /*= CHAT_MSG_ADDON*/, Language language /*= LANG_ADDON*/, TextRange range /*= TEXT_RANGE_NORMAL*/, uint32 sound /*= 0*/, Team team /*= TEAM_OTHER*/, bool gmOnly /*= false*/, Player* srcPlr /*= NULL*/)
{
    ...
    {
        sLog->OutErrorDatabase("CreatureTextMgr: Could not find Text for Creature(%s) Entry %u in 'creature_text' table. Ignoring.", source->GetName(), source->GetEntry());
        return 0;
    }
    ...
    {
        sLog->OutErrorDatabase("CreatureTextMgr: Could not find TextGroup %u for Creature(%s) GuidLow %u Entry %u. Ignoring.",uint32(textGroup), source->GetName(), source->GetGUIDLow(), source->GetEntry());
        return 0;
    }
...
float CreatureTextMgr::GetRangeForChatType(ChatMsg msgType) const
{
    ...
    switch (msgType)
    {
        ...
        case CHAT_MSG_RAID_BOSS_EMOTE:
            dist = sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXT_EMOTE);
            break;
        default:
            break;
    }
...
void CreatureTextMgr::SetRepeatId(Creature* source, uint8 textGroup, uint8 id)
{
    ...
    if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
        ...
        sLog->OutErrorDatabase("CreatureTextMgr: TextGroup %u for Creature(%s) GuidLow %u Entry %u, id %u already added", uint32(textGroup), source->GetName(), source->GetGUIDLow(), source->GetEntry(), uint32(id));
}
