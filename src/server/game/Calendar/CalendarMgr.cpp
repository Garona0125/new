...
void CalendarMgr::LoadFromDB()
{
    ...
    //                                                       0   1        2      3            4     5        6          7      8
    if (QueryResult result = CharacterDatabase.Query("SELECT id, creator, title, description, type, dungeon, eventtime, flags, time2 FROM character_calendar_events"))
	{
		do
        {
	...
	sLog->OutConsole("=======================================");
	sLog->OutConsole(">> Loaded %u calendar events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=======================================");
	sLog->OutConsole();
    count = 0;

    //                                                       0   1      2        3       4       5           6     7
    if (QueryResult result = CharacterDatabase.Query("SELECT id, event, invitee, sender, status, statustime, rank, text FROM character_calendar_invites"))
	{
        do
        {
            Field* fields = result->Fetch();
	...
	sLog->OutConsole("========================================");
    sLog->OutConsole(">> Loaded %u calendar Invites in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("========================================");
	sLog->OutConsole();
...
CalendarEvent* CalendarMgr::GetEvent(uint64 eventId) const
{
    ...
    sLog->OutErrorConsole("CalendarMgr::GetEvent: [" UI64FMTD "] not found!", eventId);
    return NULL;
}
...
CalendarInvite* CalendarMgr::GetInvite(uint64 inviteId) const
{
    ...
    sLog->OutErrorConsole("CalendarMgr::GetInvite: [" UI64FMTD "] not found!", inviteId);
    return NULL;
}
...
void CalendarMgr::SendCalendarEventInvite(CalendarInvite const& invite)
{
    ...
    uint8 level = player ? player->GetCurrentLevel() : Player::GetLevelFromDB(invitee);
...
void CalendarMgr::SendCalendarEvent(uint64 guid, CalendarEvent const& calendarEvent, CalendarSendEventType sendType)
{
    ...
    for (CalendarInviteStore::const_iterator itr = eventInviteeList.begin(); itr != eventInviteeList.end(); ++itr)
    {
        ...
        uint8 inviteeLevel = invitee ? invitee->GetCurrentLevel() : Player::GetLevelFromDB(inviteeGuid);
