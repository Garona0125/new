...
bool ReputationMgr::IsAtWar(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        sLog->OutErrorConsole("ReputationMgr::IsAtWar: Can't get AtWar flag of %s for unknown faction (faction id) #%u.", _player->GetName(), faction_id);
        return 0;
    }
...
int32 ReputationMgr::GetReputation(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        sLog->OutErrorConsole("ReputationMgr::GetReputation: Can't get reputation of %s for unknown faction (faction id) #%u.", _player->GetName(), faction_id);
        return 0;
    }
...
int32 ReputationMgr::GetBaseReputation(FactionEntry const* factionEntry) const
{
    ...
    uint32 raceMask = _player->GetCurrentRaceMask();
    uint32 classMask = _player->GetCurrentClassMask();
    for (int i=0; i < 4; i++)
    {
...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const* factionEntry, int32 standing, bool incremental)
{
    ...
    if (itr != _factions.end())
    {
		...
		{
			if (sWorld->getBoolConfig(CONFIG_GUILD_BONUS_REPUTATION_ENABLE))
			{
...
void ReputationMgr::SetVisible(FactionTemplateEntry const*factionTemplateEntry)
{
    ...
    if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(factionTemplateEntry->faction))
        ...
        if (!(factionEntry->BaseRepRaceMask[1] & _player->GetCurrentRaceMask() && factionEntry->BaseRepValue[1] == Reputation_Bottom))
