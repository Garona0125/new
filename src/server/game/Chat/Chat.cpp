...
{
    const char *format = GetQuantumSystemText(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetQuantumSystemText(int32 entry) const
{
    return m_session->GetQuantumSystemText(entry);
}
...
{
    SendSysMessage(GetQuantumSystemText(entry));
}
...
bool ChatHandler::SetDataForCommandInTable(ChatCommand* table, const char* text, uint32 security, std::string const& help, std::string const& fullcommand)
{
    ...
    for (uint32 i = 0; table[i].Name != NULL; i++)
    {
        ...
        {
            sLog->OutErrorDatabase("Table `command` have unexpected subcommand '%s' in command '%s', skip.", text, fullcommand.c_str());
            return false;
        }
    ...
    if (!cmd.empty())
    {
        if (table == getCommandTable())
            sLog->OutErrorDatabase("Table `command` have not existed command '%s', skip.", cmd.c_str());
        else
            sLog->OutErrorDatabase("Table `command` have not existed subcommand '%s' in command '%s', skip.", cmd.c_str(), fullcommand.c_str());
    }
...
const char *CliHandler::GetQuantumSystemText(int32 entry) const
{
...
{
    return GetQuantumSystemText(LANG_CONSOLE_COMMAND);
}
