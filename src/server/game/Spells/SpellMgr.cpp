...
DiminishingGroup GetDiminishingReturnsGroupForSpell(SpellInfo const* spellproto, bool triggered)
{
    ...
    switch (spellproto->SpellFamilyName)
    {
        case SPELL_FAMILY_GENERIC:
        {
			...
			// Consume Ruins of Ahn'Qiraj
			else if (spellproto->Id == 25371)
				return DIMINISHING_CONTROLLED_STUN;
			// Purge Ruins of Ahn'Qiraj
			else if (spellproto->Id == 25756)
				return DIMINISHING_CONTROLLED_STUN;
			// Nullify Temple of Ahn'Qiraj
			else if (spellproto->Id == 26552)
				return DIMINISHING_CONTROLLED_STUN;
			// ToC Icehowl Massive Crash
			else if (spellproto->SpellVisual[0] == 14150)
				return DIMINISHING_CONTROLLED_STUN;
			// Warlock Summoning Disorientation
		...
		case SPELL_FAMILY_SHAMAN: // Earthbind Totem
		{
                    ...
                    if (!spellInfo->IsLootCrafting())
                    {
                        if (msg)
                        {
                            if (player)
                                ...
                                sLog->OutErrorDatabase("Craft spell %u not have create item entry.", spellInfo->Id);
                        }
                        return false;
                    }
                ...
                else if (!sObjectMgr->GetItemTemplate(spellInfo->Effects[i].ItemType))
                {
                    if (msg)
                    {
                        if (player)
                            ...
                            sLog->OutErrorDatabase("Craft spell %u create not-exist in DB item (Entry: %u) and then...", spellInfo->Id, spellInfo->Effects[i].ItemType);
                    }
                    return false;
                }
                ...
                if (!IsSpellValid(spellInfo2, player, msg))
                {
                    if (msg)
                    {
                        if (player)
                            ...
                            sLog->OutErrorDatabase("Spell %u learn to invalid spell %u, and then...", spellInfo->Id, spellInfo->Effects[i].TriggerSpell);
                    }
                    return false;
                }
                break;
            ...
            if (spellInfo->Reagent[j] > 0 && !sObjectMgr->GetItemTemplate(spellInfo->Reagent[j]))
            {
                if (msg)
                {
                    if (player)
                        ...
                        sLog->OutErrorDatabase("Craft spell %u have not-exist reagent in DB item (Entry: %u) and then...", spellInfo->Id, spellInfo->Reagent[j]);
                }
                return false;
            }
        }
...
uint32 SpellMgr::GetSpellIdForDifficulty(uint32 spellId, Unit const* caster) const
{
    ...
    {
        sLog->OutErrorConsole("SpellMgr::GetSpellIdForDifficulty: Incorrect Difficulty for spell %u.", spellId);
    ...
    {
        sLog->OutErrorDatabase("SpellMgr::GetSpellIdForDifficulty: spell %u mode %u spell is 0. Check spelldifficulty_dbc!", spellId, mode);
        return spellId;
    }
...
bool SpellArea::IsFitToRequirements(Player const* player, uint32 newZone, uint32 newArea) const
{
    if (gender != GENDER_NONE)                   // not in expected gender
        if (!player || gender != player->GetCurrentGender())
            return false;

    if (raceMask)                                // not in expected race
        if (!player || !(raceMask & player->GetCurrentRaceMask()))
            return false;
	...
	switch (spellId)
    {
		...
		case 58730: // No fly Zone - Wintergrasp
        {
            ...
            if (sWorld->getBoolConfig(CONFIG_OUTDOOR_PVP_WINTERGRASP_ENABLED))
            {
				...
				AreaTableEntry const* area = GetAreaEntryByAreaID(player->GetAreaId()); // test
...
void SpellMgr::LoadSpellRanks()
{
    ...
    {
		sLog->OutConsole("==============================");
        sLog->OutConsole(">> Loaded 0 spell rank records");
		sLog->OutConsole("==============================");
        sLog->OutConsole();
        sLog->OutConsole("DB table `spell_ranks` table is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (!first)
        {
            sLog->OutErrorDatabase("Spell rank identifier(first_spell_id) %u listed in `spell_ranks` does not exist!", lastSpell);
            continue;
        }
        // check if chain is long enough
        if (rankChain.size() < 2)
        {
            sLog->OutErrorDatabase("There is only 1 spell rank for identifier(first_spell_id) %u in `spell_ranks`, entry is not needed!", lastSpell);
            continue;
        }
        int32 curRank = 0;
        bool valid = true;
        ...
        for (std::list<std::pair<int32, int32> >::iterator itr = rankChain.begin() ; itr!= rankChain.end(); ++itr)
        {
            ...
            if (!spell)
            {
                sLog->OutErrorDatabase("Spell %u (rank %u) listed in `spell_ranks` for chain %u does not exist!", itr->first, itr->second, lastSpell);
                valid = false;
                break;
            }
            ++curRank;
            ...
            {
                sLog->OutErrorDatabase("Spell %u (rank %u) listed in `spell_ranks` for chain %u does not have proper rank value(should be %u)!", itr->first, itr->second, lastSpell, curRank);
                valid = false;
                break;
            }
        }
	...
	sLog->OutConsole("==========================================");
    sLog->OutConsole(">> Loaded %u spell rank records in %u ms", rows, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==========================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellRequired()
{
    ...
    {
		sLog->OutConsole("==================================");
        sLog->OutConsole(">> Loaded 0 spell required records");
		sLog->OutConsole("==================================");
        sLog->OutConsole();
        sLog->OutConsole("DB table `spell_required` table is empty!");
        return;
    }
    uint32 rows = 0;
    ...
    do
    {
        ...
        if (!spell)
        {
            sLog->OutErrorDatabase("spell_id %u in `spell_required` table is not found in dbcs, skipped", spell_id);
            continue;
        }
        SpellInfo const* req_spell = GetSpellInfo(spell_req);
        if (!req_spell)
        {
            sLog->OutErrorDatabase("req_spell %u in `spell_required` table is not found in dbcs, skipped", spell_req);
            continue;
        }
        if (GetFirstSpellInChain(spell_id) == GetFirstSpellInChain(spell_req))
        {
            sLog->OutErrorDatabase("req_spell %u and spell_id %u in `spell_required` table are ranks of the same spell, entry not needed, skipped", spell_req, spell_id);
            continue;
        }
        if (IsSpellRequiringSpell(spell_id, spell_req))
        {
            sLog->OutErrorDatabase("duplicated entry of req_spell %u and spell_id %u in `spell_required`, skipped", spell_req, spell_id);
            continue;
        }
	...
	sLog->OutConsole("==============================================");
    sLog->OutConsole(">> Loaded %u spell required records in %u ms", rows, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==============================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellLearnSkills()
{
	...
	sLog->OutConsole("===================================================");
    sLog->OutConsole(">> Loaded %u Spell Learn Skills from DBC in %u ms", dbc_count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellLearnSpells()
{
    ...
    {
		sLog->OutConsole("==============================");
        sLog->OutConsole(">> Loaded 0 spell learn spells");
		sLog->OutConsole("==============================");
        sLog->OutConsole();
        sLog->OutConsole("DB table `spell_learn_spell` table is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_learn_spell` does not exist", spell_id);
            continue;
        }

        if (!GetSpellInfo(node.spell))
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_learn_spell` learning not existed spell %u", spell_id, node.spell);
            continue;
        }

        if (GetTalentSpellCost(node.spell))
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_learn_spell` attempt learning talent spell %u, skipped", spell_id, node.spell);
            continue;
        }
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (entry->Effects[i].Effect == SPELL_EFFECT_LEARN_SPELL)
            {
                ...
                for (SpellLearnSpellMap::const_iterator itr = db_node_bounds.first; itr != db_node_bounds.second; ++itr)
                {
                    ...
                    {
                        sLog->OutErrorDatabase("Spell %u auto-learn spell %u in spell.dbc then the record in `spell_learn_spell` is redundant, please fix DB.",
	...
	sLog->OutConsole("============================================================");
    sLog->OutConsole(">> Loaded %u spell learn spells + %u found in DBC in %u ms", count, dbc_count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("============================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellTargetPositions()
{
    ...
    {
		sLog->OutConsole("================================================================================");
        sLog->OutConsole(">> Loaded 0 spell target coordinates. DB table `spell_target_position` is empty.");
		sLog->OutConsole("================================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!mapEntry)
        {
            sLog->OutErrorDatabase("Spell (ID:%u) target map (ID: %u) does not exist in `Map.dbc`.", Spell_ID, st.target_mapId);
            continue;
        }

        if (st.target_X==0 && st.target_Y==0 && st.target_Z==0)
        {
            sLog->OutErrorDatabase("Spell (ID:%u) target coordinates not provided.", Spell_ID);
            continue;
        }
        ...
        if (!spellInfo)
        {
            sLog->OutErrorDatabase("Spell (ID:%u) listed in `spell_target_position` does not exist.", Spell_ID);
            continue;
        }
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (spellInfo->Effects[i].TargetA.GetTarget() == TARGET_DEST_DB || spellInfo->Effects[i].TargetB.GetTarget() == TARGET_DEST_DB)
            {
                ...
                if (spellInfo->Effects[i].Effect == SPELL_EFFECT_BIND && spellInfo->Effects[i].MiscValue)
                {
                    ...
                    {
                        sLog->OutErrorDatabase("Spell (Id: %u) listed in `spell_target_position` expected point to zone %u bit point to zone %u.", Spell_ID, spellInfo->Effects[i].MiscValue, area_id);
                        break;
                    }
                }
        ...
        if (!found)
        {
            sLog->OutErrorDatabase("Spell (Id: %u) listed in `spell_target_position` does not have target TARGET_DEST_DB (17).", Spell_ID);
            continue;
        }
	...
	sLog->OutConsole("==================================================");
    sLog->OutConsole(">> Loaded %u spell teleport coordinates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellGroups()
{
    ...
    if (!result)
    {
		sLog->OutConsole();
		sLog->OutConsole("===============================================");
        sLog->OutConsole(">> Loaded %u spell group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("===============================================");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("SpellGroup id %u listed in `spell_group` is in core range, but is not defined in core!", group_id);
            continue;
        }
    ...
    for (SpellGroupSpellMap::iterator itr = mSpellGroupSpell.begin(); itr!= mSpellGroupSpell.end() ;)
    {
        if (itr->second < 0)
        {
            ...
            {
                sLog->OutErrorDatabase("SpellGroup id %u listed in `spell_group` does not exist", abs(itr->second));
        ...
        else
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_group` does not exist", itr->second);
                mSpellGroupSpell.erase(itr++);
            }
            else if (spellInfo->GetRank() > 1)
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_group` is not first rank of spell", itr->second);
                mSpellGroupSpell.erase(itr++);
            }
            else
                ++itr;
...
void SpellMgr::LoadSpellGroupStackRules()
{
    ...
    {
		sLog->OutConsole("===================================");
        sLog->OutConsole(">> Loaded 0 spell group stack rules");
		sLog->OutConsole("===================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("SpellGroupStackRule %u listed in `spell_group_stack_rules` does not exist", stack_rule);
            continue;
        }
        ...
        {
            sLog->OutErrorDatabase("SpellGroup id %u listed in `spell_group_stack_rules` does not exist", group_id);
            continue;
        }
	...
	sLog->OutConsole("===============================================");
    sLog->OutConsole(">> Loaded %u spell group stack rules in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===============================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellProcEvents()
{
    ...
    {
		sLog->OutConsole("===================================================");
        sLog->OutConsole(">> Loaded %u spell proc event conditions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("===================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!spell)
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_proc_event` does not exist", entry);
            continue;
        }
        ...
        if (spell->ProcFlags == 0)
        {
            if (spe.procFlags == 0)
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_proc_event` probally not triggered spell", entry);
                continue;
            }
            customProc++;
        }
	...
	{
		sLog->OutConsole("=======================================================================");
        sLog->OutConsole(">> Loaded %u extra and %u custom spell proc event conditions in %u ms",  count, customProc, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("=======================================================================");
	}
    else
	{
		sLog->OutConsole("=========================================================");
        sLog->OutConsole(">> Loaded %u extra spell proc event conditions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("=========================================================");
	}
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellProcs()
{
    ...
    {
		sLog->OutConsole("======================================================");
        sLog->OutConsole(">> Loaded %u spell proc conditions and data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("======================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!spellEntry)
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_proc` does not exist", spellId);
            continue;
        }
        ...
        if (allRanks)
        {
            ...
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_proc` is not first rank of spell.", fields[0].GetInt32());
                continue;
            }
        }
        ...
        while (true)
        {
            ...
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_proc` has duplicate entry in the table", spellId);
                break;
            }
            ...
            if (procEntry.schoolMask & ~SPELL_SCHOOL_MASK_ALL)
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has wrong `schoolMask` set: %u", spellId, procEntry.schoolMask);
            if (procEntry.spellFamilyName && (procEntry.spellFamilyName < 3 || procEntry.spellFamilyName > 17 || procEntry.spellFamilyName == 14 || procEntry.spellFamilyName == 16))
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has wrong `spellFamilyName` set: %u", spellId, procEntry.spellFamilyName);
            if (procEntry.chance < 0)
            {
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has negative value in `chance` field", spellId);
                procEntry.chance = 0;
            }
            if (procEntry.ratePerMinute < 0)
            {
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has negative value in `ratePerMinute` field", spellId);
                procEntry.ratePerMinute = 0;
            }
            if (cooldown < 0)
            {
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has negative value in `cooldown` field", spellId);
                procEntry.cooldown = 0;
            }
            if (procEntry.chance == 0 && procEntry.ratePerMinute == 0)
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u doesn't have `chance` and `ratePerMinute` values defined, proc will not be triggered", spellId);
            if (procEntry.charges > 99)
            {
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has too big value in `charges` field", spellId);
                procEntry.charges = 99;
            }
            if (!procEntry.typeMask)
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u doesn't have `typeMask` value defined, proc will not be triggered", spellId);
            if (procEntry.spellTypeMask & ~PROC_SPELL_PHASE_MASK_ALL)
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has wrong `spellTypeMask` set: %u", spellId, procEntry.spellTypeMask);
            if (procEntry.spellTypeMask && !(procEntry.typeMask & (SPELL_PROC_FLAG_MASK | PERIODIC_PROC_FLAG_MASK)))
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has `spellTypeMask` value defined, but it won't be used for defined `typeMask` value", spellId);
            if (!procEntry.spellPhaseMask && procEntry.typeMask & REQ_SPELL_PHASE_PROC_FLAG_MASK)
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u doesn't have `spellPhaseMask` value defined, but it's required for defined `typeMask` value, proc will not be triggered", spellId);
            if (procEntry.spellPhaseMask & ~PROC_SPELL_PHASE_MASK_ALL)
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has wrong `spellPhaseMask` set: %u", spellId, procEntry.spellPhaseMask);
            if (procEntry.spellPhaseMask && !(procEntry.typeMask & REQ_SPELL_PHASE_PROC_FLAG_MASK))
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has `spellPhaseMask` value defined, but it won't be used for defined `typeMask` value", spellId);
            if (procEntry.hitMask & ~PROC_HIT_MASK_ALL)
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has wrong `hitMask` set: %u", spellId, procEntry.hitMask);
            if (procEntry.hitMask && !(procEntry.typeMask & TAKEN_HIT_PROC_FLAG_MASK || (procEntry.typeMask & DONE_HIT_PROC_FLAG_MASK && (!procEntry.spellPhaseMask || procEntry.spellPhaseMask & (PROC_SPELL_PHASE_HIT | PROC_SPELL_PHASE_FINISH)))))
                sLog->OutErrorDatabase("`spell_proc` table entry for spellId %u has `hitMask` value defined, but it won't be used for defined `typeMask` and `spellPhaseMask` values", spellId);
...
void SpellMgr::LoadSpellBonusess()
{
    ...
    {
		sLog->OutConsole("========================================");
        sLog->OutConsole(">> Loaded %u spell bonus data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("========================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!spell)
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_bonus_data` does not exist", entry);
            continue;
        }
	...
	sLog->OutConsole("==============================================");
    sLog->OutConsole(">> Loaded %u extra spell bonus data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==============================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellThreats()
{
    ...
    {
		sLog->OutConsole("===================================");
        sLog->OutConsole(">> Loaded 0 aggro generating spells");
		sLog->OutConsole("===================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_threat` does not exist", entry);
            continue;
        }
	...
	sLog->OutConsole("============================================");
    sLog->OutConsole(">> Loaded %u Spell Threat Entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("============================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSkillLineAbilityMap()
{
	...
	sLog->OutConsole("========================================================");
    sLog->OutConsole(">> Loaded %u Skill Line Ability Multi Map Data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("========================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    {
		sLog->OutConsole("=================================================================");
        sLog->OutConsole(">> Loaded 0 spell pet auras. DB table `spell_pet_auras` is empty.");
		sLog->OutConsole("=================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        else
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_pet_auras` does not exist", spell);
                continue;
            }
            ...
            {
                sLog->OutErrorConsole("Spell %u listed in `spell_pet_auras` does not have dummy aura or dummy effect", spell);
                continue;
            }
            ...
            if (!spellInfo2)
            {
                sLog->OutErrorDatabase("Aura %u listed in `spell_pet_auras` does not exist", aura);
                continue;
            }
	...
	sLog->OutConsole("=======================================");
    sLog->OutConsole(">> Loaded %u spell pet auras in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=======================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadEnchantCustomAttr()
{
	...
	sLog->OutConsole("=================================================");
    sLog->OutConsole(">> Loaded %u custom enchant attributes in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellEnchantProcData()
{
    ...
    {
		sLog->OutConsole("===========================================================");
        sLog->OutConsole(">> Loaded %u spell enchant proc event conditions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("===========================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!ench)
        {
            sLog->OutErrorDatabase("Enchancment %u listed in `spell_enchant_proc_data` does not exist", enchantId);
            continue;
        }
	...
	sLog->OutConsole("=====================================================");
    sLog->OutConsole(">> Loaded %u enchant proc data definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=====================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellLinked()
{
    ...
    {
		sLog->OutConsole("==================================================================");
        sLog->OutConsole(">> Loaded 0 linked spells. DB table `spell_linked_spell` is empty.");
		sLog->OutConsole("==================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!spellInfo)
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_linked_spell` does not exist", abs(trigger));
            continue;
        }
        spellInfo = GetSpellInfo(abs(effect));
        if (!spellInfo)
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_linked_spell` does not exist", abs(effect));
            continue;
        }
	...
	sLog->OutConsole("=====================================");
    sLog->OutConsole(">> Loaded %u linked spells in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=====================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadPetLevelupSpellMap()
{
	...
	sLog->OutConsole("======================================================================");
    sLog->OutConsole(">> Loaded %u pet levelup and default spells for %u families in %u ms", count, family_count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("======================================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadPetDefaultSpells()
{
	...
	sLog->OutConsole("==================================================================");
    sLog->OutConsole(">> Loaded addition spells for %u pet spell data entries in %u ms", countData, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==================================================================");
    sLog->OutConsole();

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Summonable Creature Templates...");
    oldMSTime = getMSTime();
	...
	sLog->OutConsole("=====================================================");
    sLog->OutConsole(">> Loaded %u summonable creature templates in %u ms", countCreature, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=====================================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellAreas()
{
    ...
    {
		sLog->OutConsole("====================================================================");
        sLog->OutConsole(">> Loaded 0 spell area requirements. DB table `spell_area` is empty.");
		sLog->OutConsole("====================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_area` does not exist", spell);
            continue;
        }

        {
            ...
            if (!ok)
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_area` already listed with similar requirements.", spell);
                continue;
            }
        }
        ...
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_area` have wrong area (%u) requirement", spell, spellArea.areaId);
            continue;
        }

        if (spellArea.questStart && !sObjectMgr->GetQuestTemplate(spellArea.questStart))
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_area` have wrong start quest (%u) requirement", spell, spellArea.questStart);
            continue;
        }
        ...
        if (spellArea.questEnd)
        {
            ...
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_area` have wrong end quest (%u) requirement", spell, spellArea.questEnd);
                continue;
            }
        }
        ...
        if (spellArea.auraSpell)
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_area` have wrong aura spell (%u) requirement", spell, abs(spellArea.auraSpell));
                continue;
            }

            if (uint32(abs(spellArea.auraSpell)) == spellArea.spellId)
            {
                sLog->OutErrorDatabase("Spell %u listed in `spell_area` have aura spell (%u) requirement for itself", spell, abs(spellArea.auraSpell));
                continue;
            }
            ...
            if (spellArea.autocast && spellArea.auraSpell > 0)
            {
                ...
                if (chain)
                {
                    sLog->OutErrorDatabase("Spell %u listed in `spell_area` have aura spell (%u) requirement that itself autocast from aura", spell, spellArea.auraSpell);
                    continue;
                }
        ...
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_area` have wrong race mask (%u) requirement", spell, spellArea.raceMask);
            continue;
        }

        if (spellArea.gender != GENDER_NONE && spellArea.gender != GENDER_FEMALE && spellArea.gender != GENDER_MALE)
        {
            sLog->OutErrorDatabase("Spell %u listed in `spell_area` have wrong gender (%u) requirement", spell, spellArea.gender);
            continue;
        }
	...
	sLog->OutConsole("===============================================");
    sLog->OutConsole(">> Loaded %u spell area requirements in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===============================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellInfoStore()
{
	...
	sLog->OutConsole("============================================");
    sLog->OutConsole(">> Loaded spell custom attributes in %u ms", GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("============================================");
    sLog->OutConsole();
}
...
void SpellMgr::LoadSpellCustomAttributes()
{
    uint32 oldMSTime = getMSTime();
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
            ...
            case 72625: // Ooze Eruption
                // ONLY SPELLS WITH SPELL_FAMILY_GENERIC and EFFECT_SCHOOL_DAMAGE
			...
			case 67345: // Whirl
			case 68284: // Charge (Argent Tournament)
...
void SpellMgr::LoadDbcDataCorrections()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
            ...
            case 31344: // Howl of Azgalor
                spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_150_YARDS;
                break;
            ...
            case 57697: // Lavastrike Hack - Sartharion
                ...
                spellInfo->EffectImplicitTargetB[0] = TARGET_DEST_TARGET_ENEMY;
                break;
            case 31225: // Shimmering Vessel (Rank 1)
            case 8593: // Symbol of Life (Rank 1)
            ...
            case 47202: // Everlasting Affliction
            case 47203: // Everlasting Affliction
            case 47204: // Everlasting Affliction
            case 47205: // Everlasting Affliction
            ...
            case 16835: // Natural shapeshifter
                spellInfo->DurationIndex = 21;
                break;
            case 51726: // Ebon Plague
			case 51734: // Ebon Plague
            ...
            case 69757: // Sons of Hodir Commendation Badge
                spellInfo->EffectBasePoints[0] = 519;
                break;
            ...
            case 54800: // Sigil of the Frozen Conscience - change class mask to custom extended flags of Icy Touch
				...
				// SpellFamilyFlags[0] & 0x00000040 in SPELL_FAMILY_DEATH_KNIGHT is currently unused (3.3.5a)
			...
			case 64026: // Halls of Winter Teleport
			...
			case 65027: // Descent into Madness Teleport
			case 65061: // Teleport to Mimiron Tram
            ...
            case 74297: // Harvest Souls
                spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_50000_YARDS;
                spellInfo->EffectRadiusIndex[1] = EFFECT_RADIUS_50000_YARDS;
                spellInfo->EffectRadiusIndex[2] = EFFECT_RADIUS_50000_YARDS;
                break;
            ...
            case 73579: // Summon Spirit Bomb
                spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_25_YARDS;
                break;
			...
			case 63311: // Glyph of Shadowflame
				...
				break;
			case 8122: // Psychic Scream (Rank 1)
			case 8124: // Psychic Scream (Rank 2)
			case 10888: // Psychic Scream (Rank 3)
			case 10890: // Psychic Scream (Rank 4)
			...
			case 23885: // Bloodthirst
				...
				break;
			case 24259: // Felhunter Spell Lock
				spellInfo->speed = 80;
				break;
			...
			case 48950: // Redemption
				spellInfo->SpellFamilyName = SPELL_FAMILY_PALADIN;
				break;
			...
			case 67329: // Burning Breath (Koralon-25)
				spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_100_YARDS;
				break;
			case 32205: // Place Mag'har Battle Standard
			case 38729: // Rod of Purification
			case 38736: // Rod of Purification
			case 45808: // Har'koa's Chains
			case 51577: // Har'koa's Chains
			case 51579: // Har'koa's Chains
			case 45323: // Returning Vrykul Artifact
			...
			case 69922: // Temper Queldelar
			case 69956: // Return Tempered Queldelar
			case 70663: // Shadow Nova
			case 76221: // Barrier Channel
				spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
				break;
			case 51073: // Shadow Barrage
				spellInfo->EffectBasePoints[0] = 0;
				break;
			case 1044: // Hand of Freedom
				spellInfo->AttributesEx5 |= SPELL_ATTR5_USABLE_WHILE_CONFUSED | SPELL_ATTR5_USABLE_WHILE_FEARED;
				break;
			case 45257: // Using Steam Tonk Controller
			case 45440: // Steam Tonk Controller
			case 45634: // Neural Needle
			case 57553: // Escape Voltarus
			case 60256: // Collect Sample
				spellInfo->AttributesEx4 &= ~SPELL_ATTR4_CAN_CAST_WHILE_CASTING; // Crashes client on pressing ESC
				break;
			case 57387: // Argent Cannon Assault
			case 57542: // Argent Cannon Assault
			case 57414: // Reckoning Bomb
			case 57415: // The Reckoning
				spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
				spellInfo->AttributesEx3 |= SPELL_ATTR3_NO_INITIAL_AGGRO;
				break;
			case 33783: // Empowering Shadows
			case 39364: // Empowering Shadows
			case 44406: // Energy Infusion
				spellInfo->AttributesEx3 |= SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
			...
			case 11389: // Detect Undead (from Elixir of Detect Undead)
				spellInfo->manaPerSecond = 0;
				break;
			case 59008: // Throat Rip
				spellInfo->EffectImplicitTargetA[1] = TARGET_UNIT_TARGET_ENEMY;
				break;
			case 36717: // Energy Discharge
			case 38829: // Energy Discharge
				spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_SRC_AREA_ENEMY;
				break;
			case 1852: // GM Silenced
			case 35956: // Strange Aura need test (proc after explosing mana bomb quest final code 11406 11407)
			case 43827: // The wielder of Frostmourne
			case 54262: // Blue Radiation
			case 59630: // Black Magic
			case 62320: // Aura of Celerity
			case 63388: // Shirt of Uber Aura
			case 72145: // Green Blight Residue (Rotface)
			case 72968: // Precious's Ribbon
				spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
				break;
			case 45602: // Ride Carpet
			case 51798: // Brewfest - Relay Race - Intro - Quest Complete HACK: This spell break quest complete for alliance and on retail not used
				spellInfo->EffectBasePoints[EFFECT_0] = 0;
				break;
			case 65142: // Ebon Plague
				spellInfo->AttributesEx3 &= ~SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
				break;
			case 56580: // Glutinous Poison
			case 59108: // Glutinous Poison
			...
			case 70602: // Corruption
				spellInfo->AttributesEx3 |= SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
				break;
			case 34933: // Arcane Explosion (Mana Tombs)
			case 42380: // Conflagration (Headless Horseman)
			case 42869: // Conflagration (Headless Horseman)
			case 46365: // Fire Festival Fury
			case 51422: // Wintergrasp Tower Cannon
			case 54355: // Detonation
			...
			case 32958: // Crystal Channel
				...
				break;
			case 36852: // Arcatraz Channel Beam
			case 36854: // Arcatraz Channel Beam
			case 36856: // Arcatraz Channel Beam
			case 36857: // Arcatraz Channel Beam
			case 36858: // Arcatraz Channel Beam
				spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
				spellInfo->RangeIndex = 6; // 100 yards
				break;
			// Runeforging no required spellfocus (For fun system)
			...
			case 70164: // Rune of the Nerubian Carapace
				...
				break;
			case 53821: // Teach: Death Gate
			case 63026: // Force Cast
			case 63137: // Force Cast
				spellInfo->EffectImplicitTargetA[0] = TARGET_DEST_DB;
				break;
			...
			case 11919: // Poison Proc
				spellInfo->EffectTriggerSpell[EFFECT_0] = 11918;
				break;
			case 6867: // Moss Covered Hands Proc
				spellInfo->EffectTriggerSpell[EFFECT_0] = 6866;
				break;
			case 6871: // Moss Covered Feet Proc
				spellInfo->EffectTriggerSpell[EFFECT_0] = 6870;
				break;
			case 43223: // Greater Blessing of Kings
				spellInfo->EffectRadiusIndex[0] = EFFECT_RADIUS_200_YARDS;
				break;
			case 57470: // Renewed Hope (Rank 1)
			case 57472: // Renewed Hope (Rank 2)
				// Should also affect Flash Heal
				spellInfo->EffectSpellClassMask[0][0] |= 0x800;
				break;
			case 60458: // Toss Your Luck!
                spellInfo->Effect[EFFECT_0] = SPELL_EFFECT_DUMMY;
                break;
			case 71929: // Frost Nova
				spellInfo->EffectRadiusIndex[1] = EFFECT_RADIUS_10_YARDS;
				break;
			default:
				break;
	...
	sLog->OutConsole("===============================================");
    sLog->OutConsole(">> Loaded spell dbc data corrections in %u ms", GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===============================================");
    sLog->OutConsole();
}