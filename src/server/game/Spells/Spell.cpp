...
{
    if (!m_targetMask)
        sLog->OutConsole("No targets");

    sLog->OutConsole("target mask: %u", m_targetMask);
    if (m_targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK | TARGET_FLAG_GAMEOBJECT_MASK))
        sLog->OutConsole("Object target: " UI64FMTD, m_objectTargetGUID);
    if (m_targetMask & TARGET_FLAG_ITEM)
        sLog->OutConsole("Item target: " UI64FMTD, m_itemTargetGUID);
    if (m_targetMask & TARGET_FLAG_TRADE_ITEM)
        sLog->OutConsole("Trade item target: " UI64FMTD, m_itemTargetGUID);
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
        sLog->OutConsole("Source location: transport guid:" UI64FMTD " trans offset: %s position: %s", m_srcTransGUID, m_srcTransOffset.ToString().c_str(), m_srcPos.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
        sLog->OutConsole("Destination location: transport guid:" UI64FMTD " trans offset: %s position: %s", m_dstTransGUID, m_dstTransOffset.ToString().c_str(), m_dstPos.ToString().c_str());
    if (m_targetMask & TARGET_FLAG_STRING)
        sLog->OutConsole("String: %s", m_strTarget.c_str());
    sLog->OutConsole("speed: %f", m_speed);
    sLog->OutConsole("elevation: %f", m_elevation);
}
...
, m_spellValue(new SpellValue(m_spellInfo))
{
    ...
    if (m_attackType == RANGED_ATTACK)
        // wand case
        if ((m_caster->GetCurrentClassMask() & CLASSMASK_WAND_USERS) != 0 && m_caster->GetTypeId() == TYPE_ID_PLAYER)
...
Spell::~Spell()
{
    ...
    if (m_referencedFromCurrentSpell && m_selfContainer && *m_selfContainer == this)
    {
        ...
        sLog->OutErrorConsole("SPELL: deleting spell for spell ID %u. However, spell still referenced.", m_spellInfo->Id);
        *m_selfContainer = NULL;
    }
...
void Spell::InitExplicitTargets(SpellCastTargets const& targets)
{
    ...
    else
    {
        ...
        if (neededTargets & (TARGET_FLAG_UNIT_MASK))
        {
            ...
            else if ((m_caster->GetTypeId() == TYPE_ID_UNIT) && neededTargets & (TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT))
...
void Spell::SelectEffectTypeImplicitTargets(uint8 effIndex)
{
    ...
    switch (m_spellInfo->Effects[effIndex].Effect)
    {
        ...
        case SPELL_EFFECT_SUMMON_PLAYER:
            if (m_caster->GetTypeId() == TYPE_ID_PLAYER && m_caster->ToPlayer()->GetSelection())
            {
...
void Spell::prepareDataForTriggerSystem(AuraEffect const* /*triggeredByAura*/)
{
    ...
    if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_HUNTER &&
    ...
    if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_WARLOCK && m_spellInfo->SpellFamilyFlags[0] & 0x00000040)
    {
    ...
    if (m_originalCaster && m_caster != m_originalCaster && m_caster->GetTypeId() == TYPE_ID_UNIT && m_caster->ToCreature()->IsTotem() && m_caster->IsControlledByPlayer())
    ...
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        if (targetGUID == ihit->targetGUID)             // Found in list
        {
            ...
            if (m_auraScaleMask && ihit->effectMask == m_auraScaleMask && m_caster != target)
            {
                ...
                if (uint32(target->GetCurrentLevel() + 10) >= auraSpell->SpellLevel)
                    ihit->scaleAura = true;
            }
            return;
        }
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
	...
	//Player without PvP-Flag deal no damage to PvP-Units with Aoe Spells
	if ((m_spellInfo->IsTargetingArea() || m_spellInfo->Id == 52212) && caster->GetTypeId() == TYPE_ID_PLAYER && unit->IsPvP() && !caster->IsPvP())
		return;
            ...
            m_targets.GetUnitTargetGUID() == target->targetGUID)
    {
        ...
        if (m_caster->GetTypeId() == TYPE_ID_PLAYER && (missInfo == SPELL_MISS_MISS ||
    ...
    if (canEffectTrigger && !procAttacker && !procVictim)
    {
        ...
        if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_HUNTER && m_spellInfo->SpellFamilyFlags[1] & 0x10000000)
    ...
    else if (m_damage > 0)
    {
        ...
        if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
        {
            ...
            if (caster->GetTypeId() == TYPE_ID_PLAYER && (m_spellInfo->Attributes & SPELL_ATTR0_STOP_ATTACK_TARGET) == 0 &&
    ...
    else
    {
        ...
        if (missInfo == SPELL_MISS_RESIST && m_spellInfo->AttributesCu & SPELL_ATTR0_CU_PICKPOCKET && unitTarget->GetTypeId() == TYPE_ID_UNIT)
        {
    ...
    {
        //AI functions
        if (spellHitTarget->GetTypeId() == TYPE_ID_UNIT)
        {
        ...
        if (m_originalCaster)
        {
            if (m_originalCaster->GetTypeId() == TYPE_ID_UNIT && m_originalCaster->ToCreature()->IsAIEnabled)
                m_originalCaster->ToCreature()->AI()->SpellHitTarget(spellHitTarget, m_spellInfo);
        }
        else
        {
            if (m_caster->GetTypeId() == TYPE_ID_UNIT && m_caster->ToCreature()->IsAIEnabled)
        ...
        if (unit->IsPvP() && m_caster->GetTypeId() == TYPE_ID_PLAYER)
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (unit->GetTypeId() == TYPE_ID_PLAYER)
    {
        ...
        unit->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET_2, m_spellInfo->Id);
    }
    ...
    if (m_caster != unit)
    {
        ...
        else if (m_caster->IsFriendlyTo(unit))
        {
            ...
            if (m_spellInfo->Speed > 0.0f && unit->GetTypeId() == TYPE_ID_PLAYER && !m_spellInfo->IsPositive())
                return SPELL_MISS_EVADE;
            ...
            if (unit->IsInCombatActive() && !(m_spellInfo->AttributesEx3 & SPELL_ATTR3_NO_INITIAL_AGGRO))
            {
    ...
    if (aura_effmask)
    {
        ...
        if (scaleAura)
        {
            aurSpellInfo = m_spellInfo->GetAuraRankForLevel(unitTarget->GetCurrentLevel());
            ASSERT(aurSpellInfo);
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                else
                {
					...
					if (m_spellInfo->Id == 6358 && unit->GetTypeId() == TYPE_ID_PLAYER && m_originalCaster->GetOwner())
					{
						float mod = 1.0f;
...
struct ChainHealingOrder : public std::binary_function<const Unit*, const Unit*, bool>
{
    ...
    {
        if (Target->GetTypeId() == TYPE_ID_PLAYER && MainTarget->GetTypeId() == TYPE_ID_PLAYER && Target->ToPlayer()->IsInSameRaidWith(MainTarget->ToPlayer()))
        {
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    switch (type)
    {
        ...
        case PUSH_CHAIN:
        {
            ...
            if (!target)
            {
                sLog->OutErrorConsole("SPELL: cannot find unit target for spell ID %u\n", m_spellInfo->Id);
                return;
            }
            pos = target;
            break;
            ...
            for (ConditionList::const_iterator i_spellST = conditions.begin(); i_spellST != conditions.end(); ++i_spellST)
            {
                ...
                switch ((*i_spellST)->ConditionValue1)
                {
                    ...
                    case SPELL_TARGET_TYPE_GAMEOBJECT:
                        ...
                        {
                            if (GameObject* go = m_caster->FindGameobjectWithDistance((*i_spellST)->ConditionValue2, range))
                            {
                    ...
                    default:
						if (Creature* cre = m_caster->FindCreatureWithDistance((*i_spellST)->ConditionValue2, range, (*i_spellST)->ConditionValue1 != SPELL_TARGET_TYPE_DEAD))
                        {
...
uint32 Spell::SelectEffectTargets(uint32 i, SpellImplicitTargetInfo const& cur)
{
    ...
    switch (cur.GetType())
    {
        case TARGET_TYPE_UNIT_CASTER:
        {
            switch (cur.GetTarget())
            {
                ...
                case TARGET_UNIT_PASSENGER_7:
                    if (m_caster->GetTypeId() == TYPE_ID_UNIT && m_caster->ToCreature()->IsVehicle())
        ...
        case TARGET_TYPE_UNIT_TARGET:
        {
            ...
            if (!target)
            {
                sLog->OutErrorConsole("SPELL: no unit target for spell ID %u", m_spellInfo->Id);
                break;
            }
        ...
        case TARGET_TYPE_UNIT_NEARBY:
        {
            ...
            if (!target)
                return 0;
            else if (target->GetTypeId() == TYPE_ID_GAMEOBJECT)
        ...
        case TARGET_TYPE_DEST_DEST: //5+8+1
        {
            if (!m_targets.HasDst())
            {
                sLog->OutErrorConsole("SPELL: no destination for spell ID %u", m_spellInfo->Id);
                break;
            }
        ...
        case TARGET_TYPE_DEST_SPECIAL:
        {
            switch (cur.GetTarget())
            {
                case TARGET_DEST_DB:
                    ...
                    {
                        sLog->OutErrorDatabase("SPELL: unknown target coordinates for spell ID %u", m_spellInfo->Id);
                        Unit* target = NULL;
            ...
            switch (cur.GetTarget())
            {
                case TARGET_UNIT_CHANNEL_TARGET:
                    ...
                    if (Unit* target = Unit::GetUnit(*m_caster, m_originalCaster->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->m_targets.GetUnitTargetGUID()))
                        ...
                        sLog->OutErrorConsole("SPELL: cannot find channel spell target for spell ID %u", m_spellInfo->Id);
                    break;
                case TARGET_DEST_CHANNEL_TARGET:
                    if (m_originalCaster->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->m_targets.HasDst())
                        ...
                        sLog->OutErrorConsole("SPELL: cannot find channel spell destination for spell ID %u", m_spellInfo->Id);
                    break;
        ...
        default:
        {
            switch (cur.GetTarget())
            {
                ...
                case TARGET_GAMEOBJECT_ITEM_TARGET:
                    ...
                    break;
                default:
                    sLog->OutErrorConsole("SPELL (caster[type: %u; guidlow: %u], spell: %u): unhandled spell target (%u)",
    ...
    if (pushType == PUSH_CHAIN) // Chain
    {
        ...
        if (!target)
        {
            sLog->OutErrorConsole("SPELL: no chain unit target for spell ID %u", m_spellInfo->Id);
            return 0;
        }
    ...
    else if (pushType)
    {
        ...
        switch (targetType)
        {
                            ...
                            if (WorldObject* result = FindCorpseUsing<Trinity::RaiseDeadObjectCheck>())
                            {
                                switch (result->GetTypeId())
                                {
									...
									case TYPE_ID_PLAYER:
										...
										// no break;
									case TYPE_ID_CORPSE: // wont work until corpses are allowed in target lists, but at least will send dest in packet
                        ...
                        case 51328:
                            ...
                            if (!(m_targets.GetUnitTarget() && ((m_targets.GetUnitTarget()->GetEntry() == 26125 && m_targets.GetUnitTarget()->GetOwnerGUID() == m_caster->GetGUID())
                                || (m_targets.GetUnitTarget()->getDeathState() == CORPSE
                                    && m_targets.GetUnitTarget()->GetTypeId() == TYPE_ID_UNIT
                                    && !(m_targets.GetUnitTarget()->GetCreatureTypeMask() & CREATURE_TYPE_MASK_MECHANICAL_OR_ELEMENTAL)
            ...
            default:
                switch (cur.GetTarget())
                {
                    ...
                    case TARGET_UNIT_TARGET_AREA_RAID_CLASS:
                    {
                        Player* targetPlayer = m_targets.GetUnitTarget() && m_targets.GetUnitTarget()->GetTypeId() == TYPE_ID_PLAYER ? (Player*)m_targets.GetUnitTarget() : NULL;
                        ...
                        if (group)
                        {
                            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
                            {
                                ...
                                if (target && targetPlayer->IsWithinDistInMap(target, radius) && targetPlayer->GetCurrentClass() == target->GetCurrentClass() && !m_caster->IsHostileTo(target))
                                    AddUnitTarget(target, effectMask);
                            }
                        }
        ...
        if (!unitList.empty())
        {
                ...
                case SPELL_FAMILY_GENERIC:
                    switch (m_spellInfo->Id)
                    {
                        ...
                        case 57669: // Replenishment
                            ...
                            if (m_caster->GetTypeId() == TYPE_ID_PLAYER && m_caster->ToPlayer()->InArena())
                            {
                                unitList.clear();
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Quantum::DataPackets::RandomResizeList(unitList, maxTargets);
            }
        ...
        if (!gobjectList.empty())
        {
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Quantum::DataPackets::RandomResizeList(gobjectList, maxTargets);
            }
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if ((m_spellInfo->IsChanneled() || m_casttime) && m_caster->GetTypeId() == TYPE_ID_PLAYER && m_caster->isMoving() && m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT)
    {
    ...
    else
    {
		...
		if ((m_casttime || m_spellInfo->IsChanneled()) && !(_triggeredCastFlags & TRIGGERED_IGNORE_SET_FACING))
			if (m_caster->GetGUID() != m_targets.GetObjectTargetGUID() && m_caster->GetTypeId() == TYPE_ID_UNIT)
...
void Spell::cast(bool skipCheck)
{
    ...
    if (m_caster->GetTypeId() != TYPE_ID_PLAYER && m_targets.GetUnitTarget() && m_targets.GetUnitTarget() != m_caster)
	...
	if (((m_spellInfo->Speed > 0.0f || GetCCDelay(m_spellInfo) > 0) && !m_spellInfo->IsChanneled()) || m_spellInfo->AttributesEx4 & SPELL_ATTR4_UNK4) // Crowd Control
    {
...
void Spell::update(uint32 difftime)
{
    ...
    if ((m_caster->GetTypeId() == TYPE_ID_PLAYER && m_timer != 0) &&
...
void Spell::finish(bool ok)
{
    ...
    if (m_spellInfo->IsChanneled() && m_caster->GetTypeId() == TYPE_ID_PLAYER)
    {
...
void Spell::SendSpellStart()
{
    ...
    if ((m_caster->GetTypeId() == TYPE_ID_PLAYER ||
        (m_caster->GetTypeId() == TYPE_ID_UNIT && m_caster->ToCreature()->IsPet()))
...
void Spell::SendSpellGo()
{
    ...
    if ((m_caster->GetTypeId() == TYPE_ID_PLAYER)
        && (m_caster->GetCurrentClass() == CLASS_DEATH_KNIGHT)
...
void Spell::WriteAmmoToPacket(WorldPacket* data)
{
    ...
    if (m_caster->GetTypeId() == TYPE_ID_PLAYER)
    {
        ...
        if (pItem)
        {
            ...
            if (ammoInventoryType == INVENTORY_TYPE_THROWN)
            ...
            else
            {
                ...
                else if (m_caster->HasAura(46699))      // Requires No Ammo
                {
                    ...
                    ammoInventoryType = INVENTORY_TYPE_AMMO;
                }
            }
        }
    }
...
void Spell::SendResurrectRequest(Player* target)
{
    ...
    char const* resurrectorName = m_caster->GetTypeId() == TYPE_ID_PLAYER ? "" : m_caster->GetNameForLocaleIdx(target->GetSession()->GetSessionDbLocaleIndex());
    ...
    data << uint8(m_caster->GetTypeId() == TYPE_ID_PLAYER ? 0 : 1); // "you'll be afflicted with resurrection sickness"
...
{
    if (!m_CastItem || m_caster->GetTypeId() != TYPE_ID_PLAYER)
        return;
    ...
    if (!proto)
    {
        ...
        sLog->OutErrorConsole("Cast item has no item prototype highId=%d, lowId=%d", m_CastItem->GetGUIDHigh(), m_CastItem->GetGUIDLow());
        return;
    }
...
void Spell::TakePower()
{
    ...
    {
        sLog->OutErrorConsole("Spell::TakePower: Unknown power type '%d'", powerType);
        return;
    }
...
void Spell::TakeAmmo()
{
    if (m_attackType == RANGED_ATTACK && m_caster->GetTypeId() == TYPE_ID_PLAYER)
    {
        Item* pItem = m_caster->ToPlayer()->GetWeaponForAttack(RANGED_ATTACK);
		...
		if ((pItem->GetTemplate()->InventoryType == INVENTORY_TYPE_THROWN || pItem->GetTemplate()->InventoryType == INVENTORY_TYPE_RANGED || pItem->GetTemplate()->InventoryType == INVENTORY_TYPE_RANGED_RIGHT) && roll_chance_f(sWorld->getRate(RATE_DURABILITY_LOSS_DAMAGE)))
        {
...
{
    if (m_caster->GetTypeId() != TYPE_ID_PLAYER || m_caster->GetCurrentClass() != CLASS_DEATH_KNIGHT)
        return;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId() == TYPE_ID_PLAYER && !(m_spellInfo->Attributes & SPELL_ATTR0_PASSIVE))
    {
        ...
        if (m_caster->ToPlayer()->HasSpellCooldown(m_spellInfo->Id))
        {
            if (m_triggeredByAuraSpell)
				...
				// !Hack, returning not ready bugs shadowform client side
				return m_spellInfo->Id == 15473 ? SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW : SPELL_FAILED_NOT_READY;
        }
    }
    ...
    if (!IsTriggered() && m_caster->GetTypeId() == TYPE_ID_PLAYER)
        if (Battleground* bg = m_caster->ToPlayer()->GetBattleground())
            if (bg->GetStatus() == STATUS_WAIT_LEAVE)
                return SPELL_FAILED_DONT_REPORT;

    if (m_caster->GetTypeId() == TYPE_ID_PLAYER && VMAP::VMapFactory::createOrGetVMapManager()->isLineOfSightCalcEnabled())
    {
    ...
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURASTATE))
    {
        ...
        if (reqCombat && m_caster->IsInCombatActive() && !m_spellInfo->CanBeUsedInCombat())
            return SPELL_FAILED_AFFECTING_COMBAT;
    }

    // cancel autorepeat spells if cast start when moving
    // (not wand currently autorepeat cast delayed to moving stop anyway in spell update code)
    if (m_caster->GetTypeId() == TYPE_ID_PLAYER && m_caster->ToPlayer()->isMoving())
    {
    ...
    if (vehicle && !(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE))
    {
        ...
        if (!(m_spellInfo->AttributesEx6 & SPELL_ATTR6_CASTABLE_WHILE_ON_VEHICLE) && !(m_spellInfo->Attributes & SPELL_ATTR0_CASTABLE_WHILE_MOUNTED)
			&& (vehicleSeat->m_flags & checkMask) != checkMask && m_caster->GetTypeId() == TYPE_ID_PLAYER)
            return SPELL_FAILED_DONT_REPORT;
    }
    ...
    if ((m_spellInfo->AttributesEx3 & SPELL_ATTR3_BATTLEGROUND) &&  m_caster->GetTypeId() == TYPE_ID_PLAYER)
    ...
    // zone check
    if (m_caster->GetTypeId() == TYPE_ID_UNIT || !m_caster->ToPlayer()->IsGameMaster())
    {
        uint32 zone, area;
        m_caster->GetZoneAndAreaId(zone, area);

        SpellCastResult locRes= m_spellInfo->CheckLocation(m_caster->GetMapId(), zone, area,
            m_caster->GetTypeId() == TYPE_ID_PLAYER ? m_caster->ToPlayer() : NULL);
        if (locRes != SPELL_CAST_OK)
            return locRes;
    }

    // not let players cast spells at mount (and let do it to creatures)
    if (m_caster->IsMounted() && m_caster->GetTypeId() == TYPE_ID_PLAYER && !(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE) &&
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            case SPELL_EFFECT_DUMMY:
            {
				...
				else if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT && m_spellInfo->SpellFamilyFlags[0] == 0x2000) // Death Coil (DeathKnight)
				{
					Unit* target = m_targets.GetUnitTarget();
				...
				{
					if (!m_caster->FindCreatureWithDistance(28653, 5))
				...
				else if (m_spellInfo->Id == 51690)          // Killing Spree
				{
					...
					if (target)
					{
						if (m_caster->GetUnitMovementFlags() & MOVEMENTFLAG_ON_TRANSPORT)
            ...
            case SPELL_EFFECT_LEARN_SPELL:
            {
                ...
                if (m_spellInfo->SpellLevel > pet->GetCurrentLevel())
            ...
            case SPELL_EFFECT_FEED_PET:
            {
                ...
                if (m_caster->IsInCombatActive() || pet->IsInCombatActive())
                    return SPELL_FAILED_AFFECTING_COMBAT;
            ...
            {
                if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_WARRIOR)
                {
            ...
            {
                if (m_caster->GetTypeId() != TYPE_ID_PLAYER || !m_targets.GetUnitTarget() || m_targets.GetUnitTarget()->GetTypeId() != TYPE_ID_UNIT)
                    return SPELL_FAILED_BAD_TARGETS;
                ...
                int32 TargetLevel = m_targets.GetUnitTarget()->GetCurrentLevel();
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
                ...
                if (m_caster->GetTypeId() != TYPE_ID_PLAYER  // only players can open locks, gather etc.
            ...
            case SPELL_EFFECT_CREATE_TAMED_PET:
            {
                ...
                {
                    if (m_targets.GetUnitTarget()->GetTypeId() != TYPE_ID_PLAYER)
                        return SPELL_FAILED_BAD_TARGETS;
            ...
            case SPELL_EFFECT_SUMMON_PET:
            {
                ...
                {
                    if (m_caster->GetTypeId() == TYPE_ID_PLAYER && m_caster->GetCurrentClass() == CLASS_WARLOCK)
                    {
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_AOE_CHARM:
            {
                ...
                if (Unit* target = m_targets.GetUnitTarget())
                {
                    if (target->GetTypeId() == TYPE_ID_UNIT && target->ToCreature()->IsVehicle())
					...
					if (target->GetOwner() && target->GetOwner()->GetTypeId() == TYPE_ID_PLAYER)
						return SPELL_FAILED_TARGET_IS_PLAYER_CONTROLLED;

                    int32 damage = CalculateDamage(i, target);
                    if (damage && int32(target->GetCurrentLevel()) > damage)
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                if (m_caster->GetTypeId() == TYPE_ID_PLAYER && !allowMount && !m_spellInfo->AreaGroupId)
            ...
            case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED:
            {
                ...
                if (m_originalCaster && m_originalCaster->GetTypeId() == TYPE_ID_PLAYER && m_originalCaster->IsAlive())
                {
            ...
            case SPELL_AURA_PERIODIC_MANA_LEECH:
            {
                ...
                if (m_caster->GetTypeId() != TYPE_ID_PLAYER || m_CastItem)
                    break;
...
SpellCastResult Spell::CheckPower()
{
    ...
    {
        sLog->OutErrorConsole("Spell::CheckPower: Unknown power type '%d'", m_spellInfo->PowerType);
...
SpellCastResult Spell::CheckItems()
{
    ...
    else
    {
		if (!(_triggeredCastFlags & TRIGGERED_IGNORE_EQUIPPED_ITEM_REQUIREMENT))
			if (m_caster->GetTypeId() == TYPE_ID_PLAYER && !m_caster->ToPlayer()->HasItemFitToSpellRequirements(m_spellInfo))
				return SPELL_FAILED_EQUIPPED_ITEM_CLASS;
    }
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CREATE_ITEM_2:
            {
                if (!IsTriggered() && m_spellInfo->Effects[i].ItemType)
                {
                    ...
                    if (msg != EQUIP_ERR_OK)
                    {
                        ...
                        {
                            if (!(m_spellInfo->SpellFamilyName == SPELL_FAMILY_MAGE && (m_spellInfo->SpellFamilyFlags[0] & 0x40000000)))
            ...
            {
                if (m_caster->GetTypeId() != TYPE_ID_PLAYER) return SPELL_FAILED_TARGET_NOT_PLAYER;
                if (m_attackType != RANGED_ATTACK)
                    break;
...
{
    if (!m_caster || m_caster->GetTypeId() != TYPE_ID_PLAYER || getState() != SPELL_STATE_CASTING)
        return;
...
void Spell::UpdatePointers()
{
    ...
    if (m_castItemGUID && m_caster->GetTypeId() == TYPE_ID_PLAYER)
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
    switch (m_spellInfo->Effects[eff].ApplyAuraName)
    {
        ...
        case SPELL_AURA_MOD_POSSESS_PET:
        case SPELL_AURA_AOE_CHARM:
            if (target->GetTypeId() == TYPE_ID_UNIT && target->IsVehicle())
                return false;
            if (target->IsMounted())
                return false;
            if (target->GetCharmerGUID())
            ...
            if (int32 damage = CalculateDamage(eff, target))
                if ((int32)target->GetCurrentLevel() > damage)
                    return false;
            break;
        default:
            break;
...
{
    return m_spellInfo->SpellVisual[0] || m_spellInfo->SpellVisual[1] || m_spellInfo->IsChanneled() || m_spellInfo->Speed > 0.0f || (!m_triggeredByAuraSpell && !IsTriggered());
}
...
SpellEvent::~SpellEvent()
{
    ...
    {
        sLog->OutErrorConsole("~SpellEvent: %s %u tried to delete non-deletable spell %u. Was not deleted, causes memory leak.",
            (m_Spell->GetCaster()->GetTypeId() == TYPE_ID_PLAYER ? "Player" : "Creature"), m_Spell->GetCaster()->GetGUIDLow(), m_Spell->m_spellInfo->Id);
        ASSERT(false);
    }
}
...
void Spell::HandleLaunchPhase()
{
    ...
    for (std::list<TargetInfo>::iterator ihit= m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        ...
        if (IsTriggered() && m_spellInfo->SpellFamilyName == SPELL_FAMILY_HUNTER && m_spellInfo->IsTargetingArea())
            usesAmmo = false;
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo& targetInfo, float* multiplier)
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (targetInfo.effectMask & (1<<i))
        {
            ...
            if (m_damage > 0)
            {
				// Fixed AoE damage reduction auras
        ...
        switch (lockInfo->Type[j])
        {
            ...
            case LOCK_KEY_SKILL:
            {
                ...
                if (skillId != SKILL_NONE)
                {
                    ...
                    skillValue = m_CastItem || m_caster->GetTypeId()!= TYPE_ID_PLAYER ?
...
void Spell::SelectTrajTargets()
{
	...
	if (a > -0.0001f)
		a = 0;
    DEBUG_TRAJ(sLog->OutErrorConsole("Spell::SelectTrajTargets: a %f b %f", a, b);)
    ...
    for (; itr != unitList.end(); ++itr)
    {
        ...
        DEBUG_TRAJ(sLog->OutErrorConsole("Spell::SelectTrajTargets: check %u, dist between %f %f, height between %f %f.", (*itr)->GetEntry(), objDist2d - size, objDist2d + size, dz - size, dz + size);)

        float dist = objDist2d - size;
        float height = dist * (a * dist + b);
        DEBUG_TRAJ(sLog->OutErrorConsole("Spell::SelectTrajTargets: dist %f, height %f.", dist, height);)
...
#define CHECK_DIST {\
    ...
    if (m_targets.GetSrc()->GetExactDist2d(m_targets.GetDst()) > bestDist)
    {
        ...
        if (itr != unitList.end())
        {
            ...
            sizeSq *= sizeSq;
            DEBUG_TRAJ(sLog->OutErrorConsole("Initial %f %f %f %f %f", x, y, z, distSq, sizeSq);)
            if (distSq > sizeSq)
            {
...
bool Spell::HasGlobalCooldown() const
{
    ...
    else if (m_caster->GetTypeId() == TYPE_ID_PLAYER)
