    ...
    class AbsorbAuraOrderPred
    {
        public:
            ...
            bool operator() (AuraEffect* aurEffA, AuraEffect* aurEffB) const
            {
                ...
                // Wards
                if ((spellProtoA->SpellFamilyName == SPELL_FAMILY_MAGE) ||
                    (spellProtoA->SpellFamilyName == SPELL_FAMILY_WARLOCK))
                    if (spellProtoA->GetCategory() == 56)
                        return true;
                if ((spellProtoB->SpellFamilyName == SPELL_FAMILY_MAGE) ||
                    (spellProtoB->SpellFamilyName == SPELL_FAMILY_WARLOCK))
                ...
                // Sacrifice
                if ((spellProtoA->SpellFamilyName == SPELL_FAMILY_WARLOCK) &&
                    (spellProtoA->SpellIconID == 693))
                    return true;
                if ((spellProtoB->SpellFamilyName == SPELL_FAMILY_WARLOCK) &&
