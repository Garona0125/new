    ...
    switch (GetAuraType())
    {
            ...
            switch (GetSpellInfo()->SpellFamilyName)
            {
                ...
                case SPELL_FAMILY_PRIEST:
                    ...
                    if (GetSpellInfo()->SpellFamilyFlags[0] & 0x1 && GetSpellInfo()->SpellFamilyFlags[2] & 0x400)
                    {
                        ...
                        // Borrowed Time
                        if (AuraEffect const* pAurEff = caster->GetDummyAuraEffect(SPELL_FAMILY_PRIEST, 2899, 1))
                        ...
                        // Twin Disciplines
                        if (AuraEffect const* pAurEff = caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELL_FAMILY_PRIEST, 0x400000, 0, 0, caster->GetGUID()))
        ...
        case SPELL_AURA_MANA_SHIELD:
            ...
            if (!caster)
                break;
            // Mana Shield
            if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_MAGE && GetSpellInfo()->SpellFamilyFlags[0] & 0x8000 && m_spellInfo->SpellFamilyFlags[2] & 0x8)
            {
        ...
        case SPELL_AURA_DUMMY:
            if (!caster)
                break;
            // Earth Shield
            if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[1] & 0x400)
            {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE:
            if (!caster)
                break;
            // Rupture
            if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_ROGUE && m_spellInfo->SpellFamilyFlags[0] & 0x100000)
            {
                m_canBeRecalculated = false;
            ...
            // Rip
            else if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_DRUID && m_spellInfo->SpellFamilyFlags[0] & 0x00800000 && GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
            {
                m_canBeRecalculated = false;
                // 0.01*$AP*cp
                if (caster->GetTypeId() != TYPE_ID_PLAYER)
            ...
            // Rend
            else if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_WARRIOR && GetSpellInfo()->SpellFamilyFlags[0] & 0x20)
            {
                m_canBeRecalculated = false;
                // $0.2 * (($MWB + $mwb) / 2 + $AP / 14 * $MWS) bonus per tick
                float ap = caster->GetTotalAttackPowerValue(BASE_ATTACK);
                int32 mws = caster->GetAttackTime(BASE_ATTACK);
                float mwb_min = caster->GetWeaponDamageRange(BASE_ATTACK, MIN_DAMAGE);
                float mwb_max = caster->GetWeaponDamageRange(BASE_ATTACK, MAX_DAMAGE);
        ...
        case SPELL_AURA_PERIODIC_HEAL:
            if (!caster)
                break;
            // Lightwell Renew
            if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_PRIEST && m_spellInfo->SpellFamilyFlags[2] & 0x4000)
            {
                if (caster->GetTypeId() == TYPE_ID_PLAYER)
        ...
        case SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN:
            if (!caster)
                break;
            // Icebound Fortitude
            if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT && m_spellInfo->SpellFamilyFlags[0] & 0x00100000)
            {
                if (caster->GetTypeId() == TYPE_ID_PLAYER)
                {
            ...
            // Hand of Salvation
            else if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_PALADIN && GetSpellInfo()->SpellFamilyFlags[0] & 0x00000100)
            {
                //Glyph of Salvation
        ...
        case SPELL_AURA_MOD_THREAT:
        {
            ...
            switch (GetId())
            {
                // Arcane Shroud
                case 26400:
                    level_diff = GetBase()->GetUnitOwner()->GetCurrentLevel() - 60;
                    multiplier = 2;
                    break;
        ...
        case SPELL_AURA_MOD_INCREASE_SPEED:
            ...
            if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_DRUID && GetSpellInfo()->SpellFamilyFlags[2] & 0x00000008)
...
{
    if (!m_spellmod || target->GetTypeId() != TYPE_ID_PLAYER)
        return;
                    ...
                    switch (GetId())
                    {
                        ...
                        case 61830:
                            ...
                            if (AuraEffect* aurEff = GetBase()->GetEffect(0))
                            {
                                ...
                                {
                                    m_isPeriodic = false;
                                    sLog->OutErrorConsole("Aura %d structure has been changed - first aura is no longer SPELL_AURA_MOD_POWER_REGEN", GetId());
                                }
                                else
                                {
...
bool AuraEffect::IsPeriodicTickCrit(Unit* target, Unit const* caster) const
{
    ...
    if (target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_ROGUE, 0x100000, 0x0, 0x0, caster->GetGUID()))
    {
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (target->GetTypeId() == TYPE_ID_PLAYER)
        {
            ...
            // Heart of the Wild
            if (HotWSpellId)
            {   // hacky, but the only way as spell family is not SPELL_FAMILY_DRUID
            ...
            switch (GetMiscValue())
            {
                case FORM_CAT:
                    // Savage Roar
                    if (target->GetAuraEffect(SPELL_AURA_DUMMY, SPELL_FAMILY_DRUID, 0, 0x10000000, 0))
                        target->CastSpell(target, 62071, true);
                    // Nurturing Instinct
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_MOD_SPELL_HEALING_OF_STAT_PERCENT, SPELL_FAMILY_DRUID, 2254, 0))
                    {
                        uint32 spellId3 = 0;
                        switch (aurEff->GetId())
                        {
                    ...
                    if (AuraEffect const* aurEff = target->GetDummyAuraEffect(SPELL_FAMILY_GENERIC, 2851, 0))
                    {
                ...
                case FORM_BEAR:
                    ...
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELL_FAMILY_DRUID, 961, 0))
                    {
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (form)
    {
        ...
        case FORM_SPIRITOFREDEMPTION:                       // 0x20
            break;
        default:
            sLog->OutErrorConsole("Auras: Unknown Shapeshift Type: %u", GetMiscValue());
    }
        ...
        if (PowerType != POWER_MANA)
        {
            ...
            switch (form)
            {
                ...
                case FORM_DIREBEAR:
                {
                    ...
                    if (AuraEffect const* dummy = target->GetDummyAuraEffect(SPELL_FAMILY_DRUID, 238, 0))
    ...
    else
    {
        ...
        if (!target->HasAuraType(SPELL_AURA_MOD_SHAPESHIFT))
        {
            ...
            if (target->GetCurrentClass() == CLASS_DRUID)
            {
                ...
                if (target->GetTypeId() == TYPE_ID_PLAYER)
                {
                    ...
                    for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
                    {
                        if (itr->second->state == PLAYERSPELL_REMOVED || itr->second->disabled)
							continue;
                        SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(itr->first);
                        if (spellInfo && spellInfo->SpellFamilyName == SPELL_FAMILY_WARRIOR && spellInfo->SpellIconID == 139)
    ...
    {
        // Dash
        if (AuraEffect* aurEff = target->GetAuraEffect(SPELL_AURA_MOD_INCREASE_SPEED, SPELL_FAMILY_DRUID, 0, 0, 0x8))
            aurEff->RecalculateAmount();
        ...
        {
            if (Item* pItem = target->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAIN_HAND))
                target->ToPlayer()->_ApplyWeaponDamage(EQUIPMENT_SLOT_MAIN_HAND, pItem->GetTemplate(), NULL, apply);
        }
    }
...
void AuraEffect::HandleAuraTransform(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (!target->GetModelForForm(target->GetShapeshiftForm()) || !GetSpellInfo()->IsPositive())
        {
                ...
                switch (GetId())
                {
                    ...
                    case 16739:
                    {
                        ...
                        switch (target->GetCurrentRace())
                        {
                            // Blood Elf
                            case RACE_BLOOD_ELF:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 17829 : 17830);
                                break;
                            // Orc
                            case RACE_ORC:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10139 : 10140);
                                break;
                            // Troll
                            case RACE_TROLL:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10135 : 10134);
                                break;
                            // Tauren
                            case RACE_TAUREN:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10136 : 10147);
                                break;
                            // Undead
                            case RACE_UNDEAD_PLAYER:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10146 : 10145);
                                break;
                            // Draenei
                            case RACE_DRAENEI:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 17827 : 17828);
                                break;
                            // Dwarf
                            case RACE_DWARF:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10141 : 10142);
                                break;
                            // Gnome
                            case RACE_GNOME:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10148 : 10149);
                                break;
                            // Human
                            case RACE_HUMAN:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10137 : 10138);
                                break;
                            // Night Elf
                            case RACE_NIGHT_ELF:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 10143 : 10144);
                                break;
                            default:
                                break;
                        }
                    ...
                    case 51926:
                    {
                        ...
                        switch (target->GetCurrentRace())
                        {
                            // Blood Elf
                            case RACE_BLOOD_ELF:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25032 : 25043);
                                break;
                            // Orc
                            case RACE_ORC:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25039 : 25050);
                                break;
                            // Troll
                            case RACE_TROLL:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25041 : 25052);
                                break;
                            // Tauren
                            case RACE_TAUREN:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25040 : 25051);
                                break;
                            // Undead
                            case RACE_UNDEAD_PLAYER:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25042 : 25053);
                                break;
                            // Draenei
                            case RACE_DRAENEI:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25033 : 25044);
                                break;
                            // Dwarf
                            case RACE_DWARF:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25034 : 25045);
                                break;
                            // Gnome
                            case RACE_GNOME:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25035 : 25046);
                                break;
                            // Human
                            case RACE_HUMAN:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25037 : 25048);
                                break;
                            // Night Elf
                            case RACE_NIGHT_ELF:
                                target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 25038 : 25049);
                                break;
                            default:
                                break;
                        }
                    ...
                    case 65386:
                    case 65495:
                        target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 29203 : 29204);
                        break;
						// Darkspear Pride
                    case 75532:
                        target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 31737 : 31738);
                        break;
						// Gnomeregan Pride
					case 75531:
						target->SetDisplayId(target->GetCurrentGender() == GENDER_MALE ? 31654 : 31655);
						break;
                    default:
                        break;
                }
            ...
            else
            {
                ...
                if (!ci)
                {
                    ...
                    sLog->OutErrorConsole("Auras: unknown creature id = %d (only need its modelid) From Spell Aura Transform in Spell ID = %d", GetMiscValue(), GetId());
                }
                else
                {
                    uint32 model_id = 0;
                    ...
                    // Polymorph (sheep)
                    if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_MAGE && GetSpellInfo()->SpellIconID == 82 && GetSpellInfo()->SpellVisual[0] == 12978)
					{
        ...
        // polymorph case
        if ((mode & AURA_EFFECT_HANDLE_REAL) && target->GetTypeId() == TYPE_ID_PLAYER && target->IsPolymorphed())
        {
            ...
            // RemoveMount polymorphed target (after patch 2.4.2)
            if (target->IsMounted())
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        target->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FEIGN_DEATH);
...
void AuraEffect::HandleAuraModDisarm(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    case SPELL_AURA_MOD_DISARM_OFFHAND:
        ...
        flag = UNIT_FLAG2_DISARM_OFF_HAND;
    ...
    if (target->GetTypeId() == TYPE_ID_PLAYER && !target->IsInFeralForm())
    {
    ...
    if (target->GetTypeId() == TYPE_ID_UNIT && target->ToCreature()->GetCurrentEquipmentId())
...
void AuraEffect::HandleAuraMounted(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (!ci)
        {
            sLog->OutErrorDatabase("AuraMounted: `creature_template`='%u' not found in database (only need its modelid)", GetMiscValue());
            return;
        }
...
void AuraEffect::HandleAuraWaterWalk(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
        target->AddUnitMovementFlag(MOVEMENTFLAG_WATER_WALKING);
    else
        target->RemoveUnitMovementFlag(MOVEMENTFLAG_WATER_WALKING);
...
void AuraEffect::HandleAuraFeatherFall(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (!apply && target->GetTypeId() == TYPE_ID_PLAYER)
...
void AuraEffect::HandleAuraModTotalThreat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (!target->IsAlive() || target->GetTypeId() != TYPE_ID_PLAYER)
        return;
...
void AuraEffect::HandleModPossessPet(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    Unit* target = aurApp->GetTarget();
    if (target->GetTypeId() != TYPE_ID_UNIT || !target->ToCreature()->IsPet())
        return;
...
void AuraEffect::HandleModStateImmunityMask(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    for (std::list <AuraType>::iterator iter = aura_immunity_list.begin(); iter != aura_immunity_list.end(); ++iter)
        target->ApplySpellImmune(GetId(), IMMUNITY_AURA_TYPE, *iter, apply);
...
void AuraEffect::HandleAuraModStateImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    target->ApplySpellImmune(GetId(), IMMUNITY_AURA_TYPE, GetMiscValue(), apply);
...
void AuraEffect::HandleAuraModResistance(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++)
    {
        if (GetMiscValue() & int32(1<<x))
        {
            ...
            if (target->GetTypeId() == TYPE_ID_PLAYER || target->ToCreature()->IsPet())
...
void AuraEffect::HandleModTargetResistance(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPE_ID_PLAYER && (GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL))
        target->ApplyModInt32Value(PLAYER_FIELD_MOD_TARGET_PHYSICAL_RESISTANCE, GetAmount(), apply);

    // show as spell penetration only full spell penetration bonuses (all resistances except armor and holy
    if (target->GetTypeId() == TYPE_ID_PLAYER && (GetMiscValue() & SPELL_SCHOOL_MASK_SPELL) == SPELL_SCHOOL_MASK_SPELL)
...
void AuraEffect::HandleAuraModStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        sLog->OutErrorConsole("WARNING: Spell %u effect %u has an unsupported misc value (%i) for SPELL_AURA_MOD_STAT ", GetId(), GetEffIndex(), GetMiscValue());
        return;
    }
...
void AuraEffect::HandleModPercentStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        sLog->OutErrorConsole("WARNING: Misc Value for SPELL_AURA_MOD_PERCENT_STAT not valid");
        return;
    }
...
void AuraEffect::HandleAuraModResistenceOfStatPercent(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    if (GetMiscValue() != SPELL_SCHOOL_MASK_NORMAL)
    {
        ...
        sLog->OutErrorConsole("Aura SPELL_AURA_MOD_RESISTANCE_OF_STAT_PERCENT(182) does not work for non-armor type resistances!");
        return;
    }
...
void AuraEffect::HandleAuraModRangedAttackPower(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if ((target->GetCurrentClassMask() & CLASSMASK_WAND_USERS) != 0)
        return;
...
void AuraEffect::HandleAuraModRangedAttackPowerOfStatPercent(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    // Recalculate bonus
    if (target->GetTypeId() == TYPE_ID_PLAYER && !(target->GetCurrentClassMask() & CLASSMASK_WAND_USERS))
...
void AuraEffect::HandleModDamageDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if ((GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) != 0)
    {
        ...
        if (GetSpellInfo()->EquippedItemClass == -1 || target->GetTypeId() != TYPE_ID_PLAYER)
        {
            target->HandleStatModifier(UNIT_MOD_DAMAGE_MAIN_HAND, TOTAL_VALUE, float(GetAmount()), apply);
            target->HandleStatModifier(UNIT_MOD_DAMAGE_OFF_HAND, TOTAL_VALUE, float(GetAmount()), apply);
...
void AuraEffect::HandleModDamagePercentDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (((GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) && (GetSpellInfo()->EquippedItemClass == -1 || target->GetTypeId() != TYPE_ID_PLAYER)) || GetId() == 31869) // Sanctified Retribution
    {
        target->HandleStatModifier(UNIT_MOD_DAMAGE_MAIN_HAND, TOTAL_PCT, float (GetAmount()), apply);
        target->HandleStatModifier(UNIT_MOD_DAMAGE_OFF_HAND, TOTAL_PCT, float (GetAmount()), apply);
...
void AuraEffect::HandleAuraDummy(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (mode & (AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY))
    {
        ...
        {
            // Overpower
            if (caster && m_spellInfo->SpellFamilyName == SPELL_FAMILY_WARRIOR &&
                m_spellInfo->SpellFamilyFlags[0] & 0x4)
            {
                ...
                if (target->IsNonMeleeSpellCasted(false, false, true, false, true))
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELL_FAMILY_WARRIOR, 2775, 0))
                        switch (aurEff->GetId())
                        {
            ...
            switch (GetId())
            {
                ...
                case 37096:                                     // Blood Elf Illusion
                {
                    if (caster)
                    {
                        switch (caster->GetCurrentGender())
                        {
                            case GENDER_FEMALE:
            ...
            switch (m_spellInfo->SpellFamilyName)
            {
                case SPELL_FAMILY_GENERIC:
                    switch (GetId())
                    {
                        ...
                        case 43681: // Inactive
                        {
                            if (target->GetTypeId() != TYPE_ID_PLAYER || aurApp->GetRemoveMode() != AURA_REMOVE_BY_EXPIRE)
                                return;
            ...
            switch (GetId())
            {
                ...
                case 57822: // Wyrmrest Champion
                {
                    ...
                    if (apply)
                    {
                        switch (m_spellInfo->Id)
                        {
                            case 57819: // Argent Crusade
								FactionID = 1106;
								break;
                            case 57820: // Knights of the Ebon Blade
								FactionID = 1098;
								break;
                            case 57821: // Kirin Tor
								FactionID = 1090;
								break;
                            case 57822: // The Wyrmrest Accord
								FactionID = 1091;
								break;
                        }
                    }
        ...
        case SPELL_FAMILY_DEATH_KNIGHT:
        {
			...
			if (GetId() == 46619) // Raise Ally
			{
				...
				if (apply)
				{
					...
					player->SetFloatValue(UNIT_FIELD_BOUNDING_RADIUS, float(1.0f));
					player->SetHealth(1);
...
void AuraEffect::HandleChannelDeathItem(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetSpellInfo()->Effects[m_effIndex].ItemType == 6265)
    {
        ...
        if (!plCaster->IsHonorOrXPTarget(target) ||
            (target->GetTypeId() == TYPE_ID_UNIT && !target->ToCreature()->isTappedBy(plCaster)))
            return;

        // If this is Drain Soul, check for Glyph of Drain Soul
        if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_WARLOCK && (GetSpellInfo()->SpellFamilyFlags[0] & 0x00004000))
        {
...
void AuraEffect::HandleAuraModFaction(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        target->SetCurrentFaction(GetMiscValue());
        if (target->GetTypeId() == TYPE_ID_PLAYER)
...
void AuraEffect::HandleAuraOpenStable(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() != TYPE_ID_PLAYER || !target->IsInWorld())
        return;
...
void AuraEffect::HandlePreventResurrection(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (aurApp->GetTarget()->GetTypeId() != TYPE_ID_PLAYER)
        return;
    ...
    switch (GetSpellInfo()->SpellFamilyName)
    {
        ...
        case SPELL_FAMILY_ROGUE:
        {
            switch (GetSpellInfo()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    Unit* spellTarget = Quantum::DataPackets::SelectRandomContainerElement(targets);
        ...
        case SPELL_FAMILY_DEATH_KNIGHT:
            ...
            if (GetSpellInfo()->SpellIconID == 3041 || GetSpellInfo()->SpellIconID == 22 || GetSpellInfo()->SpellIconID == 2622)
            {
                if (target->GetTypeId() != TYPE_ID_PLAYER)
                    return;
                if (target->ToPlayer()->GetCurrentClass() != CLASS_DEATH_KNIGHT)
                    return;
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
        ...
        switch (auraSpellInfo->SpellFamilyName)
        {
            case SPELL_FAMILY_GENERIC:
            {
                switch (auraId)
                {
                    ...
                    case 29528:
                        if (target->GetTypeId() != TYPE_ID_UNIT) // prevent error reports in case ignored player target
                            return;
                        break;
                    ...
                    case 38554:
                    {
                        if (!caster || target->GetTypeId() != TYPE_ID_UNIT)
                            return;
            ...
            case SPELL_FAMILY_SHAMAN:
            {
                switch (auraId)
                {
                    ...
                    case 28820:
                    {
                        ...
                        if (!target->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELL_FAMILY_SHAMAN, 0x400))
    ...
    else
    {
        ...
        switch (auraId)
        {
			...
			// Soar
			case 50325:
				if (caster && caster->IsInCombatActive())
					return;
				break;
            // Beacon of Light
            case 53563:
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        ...
        if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_WARLOCK && (GetSpellInfo()->SpellFamilyFlags[0] & 0x400) && GetSpellInfo()->SpellIconID == 544)
        {
        ...
        if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_WARLOCK && (GetSpellInfo()->SpellFamilyFlags[0] & 0x00004000))
        {
            if (caster->GetTypeId() == TYPE_ID_PLAYER && caster->ToPlayer()->IsHonorOrXPTarget(target))
            {
                if (roll_chance_i(20))
                {
        ...
        if (GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_GENERIC)
        {
            switch (GetId())
            {
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    else
    {
        ...
        if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_DRUID && m_spellInfo->SpellIconID == 2864)
        {
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    // Drain Mana
    if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_WARLOCK
...
void AuraEffect::HandlePeriodicEnergizeAuraTick(Unit* target, Unit* caster) const
{
	...
	if (target->GetTypeId() == TYPE_ID_PLAYER && target->GetPowerType() != powerType && !(m_spellInfo->AttributesEx7 & SPELL_ATTR7_CAN_RESTORE_SECONDARY_POWER))
		return;
...
void AuraEffect::HandleRaidProcFromChargeWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    if (!(GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_PRIEST && GetSpellInfo()->SpellFamilyFlags[1] & 0x20))
    {
