...
Aura* Aura::Create(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, Item* castItem, uint64 casterGUID)
{
    ...
    if (owner->isType(TYPE_MASK_UNIT))
...
m_casterLevel(caster ? caster->GetCurrentLevel() : m_spellInfo->SpellLevel), m_procCharges(0), m_stackAmount(1),
...
{
    return (m_owner->GetTypeId() == TYPE_ID_DYNAMIC_OBJECT) ? DYNOBJ_AURA_TYPE : UNIT_AURA_TYPE;
}
...
void Aura::_UnapplyForTarget(Unit* target, Unit* caster, AuraApplication * auraApp)
{
    ...
    {
        sLog->OutErrorConsole("Aura::_UnapplyForTarget, target:%u, caster:%u, spell:%u was not found in owners application map!",
...
void Aura::Update(uint32 diff, Unit* caster)
{
    if (m_duration > 0)
    {
        ...
        if (m_timeCla)
        {
            ...
            {
                if (int32 manaPerSecond = m_spellInfo->ManaPerSecond + m_spellInfo->ManaPerSecondPerLevel * caster->GetCurrentLevel())
                {
                    m_timeCla += 1000 - diff;
...
bool Aura::CanBeSaved() const
{
	...
	if (HasEffectType(SPELL_AURA_PROC_TRIGGER_SPELL) && GetSpellInfo()->ProcChance == 40 &&
    ...
    if (apply)
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELL_FAMILY_MAGE:
                ...
                switch (GetId())
                {
                    ...
                    case 12043: // Presence of Mind
                        // Arcane Potency
                        if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELL_FAMILY_MAGE, 2120, 0))
                        {
                            uint32 spellId = 0;
                            ...
                            switch (aurEff->GetId())
                            {
                                ...
                                case 31572:
									...
									break;
                                default:
                                    sLog->OutErrorConsole("Aura::HandleAuraSpecificMods: Unknown rank of Arcane Potency (%d) found", aurEff->GetId());
                            }
                            if (spellId)
			...
			case SPELL_FAMILY_HUNTER:
				...
				{
					if (AuraEffect * auraEff = target->GetAuraEffect(SPELL_AURA_DUMMY, SPELL_FAMILY_HUNTER, 3560, 1))
					{
						uint32 spellId;
				...
				if (GetId() == 68361)
				{
					...
					{
						if (AuraEffect* auraEff = owner->GetDummyAuraEffect(SPELL_FAMILY_HUNTER, 2234, 1))
            ...
            case SPELL_FAMILY_PRIEST:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0))
                {
                    ...
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELL_FAMILY_PRIEST, 3790, 1))
                    {
                    ...
                    if (aurEff)
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            case 49032:
								...
								break;
                            default:
                                sLog->OutErrorConsole("Aura::HandleAuraSpecificMods: Unknown rank of Crypt Fever/Ebon Plague (%d) found", aurEff->GetId());
                        }
    ...
    else
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELL_FAMILY_MAGE:
                ...
                {
                    // Shattered Barrier
                    if (AuraEffect * dummy = caster->GetDummyAuraEffect(SPELL_FAMILY_MAGE, 2945, 0))
            ...
            case SPELL_FAMILY_WARRIOR:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x2)
                {
                    if (removeMode != AURA_REMOVE_BY_DEFAULT)
                    {
                        ...
                        if (caster->GetDummyAuraEffect(SPELL_FAMILY_WARRIOR, 1935, 1))
                        {
                            ...
                            {
                                if ((*itr)!= target)
                                    (*itr)->RemoveAurasWithFamily(SPELL_FAMILY_WARRIOR, 0, 0x2, 0, GetCasterGUID());
                            }
                        }
                    }
                }
            ...
            case SPELL_FAMILY_WARLOCK:
                ...
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELL_FAMILY_WARLOCK, 98, 0))
                    {
                        uint32 spellId = 0;
                        switch (aurEff->GetId())
                        {
                            ...
                            case 53754:
								...
								break;
                            default:
                                sLog->OutErrorConsole("Aura::HandleAuraSpecificMods: Unknown rank of Improved Fear (%d) found", aurEff->GetId());
                        }
                        if (spellId)
            ...
            case SPELL_FAMILY_PRIEST:
                ...
                {
                    // Shadow Affinity
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELL_FAMILY_PRIEST, 178, 1))
                    {
            ...
            case SPELL_FAMILY_ROGUE:
                ...
                if (GetId() == 1784)
                    target->RemoveAurasWithFamily(SPELL_FAMILY_ROGUE, 0x0000800, 0, 0, target->GetGUID());
                break;
...
bool Aura::IsProcTriggeredOnEvent(AuraApplication* aurApp, ProcEventInfo& eventInfo) const
{
    ...
    Unit* target = aurApp->GetTarget();
    if (IsPassive() && target->GetTypeId() == TYPE_ID_PLAYER)
    {
        if (GetSpellInfo()->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            ...
            if (eventInfo.GetDamageInfo())
            {
                ...
                if (attType == BASE_ATTACK)
                    item = target->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAIN_HAND);
                else if (attType == OFF_ATTACK)
                    item = target->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
                else
        ...
        else if (GetSpellInfo()->EquippedItemClass == ITEM_CLASS_ARMOR)
        {
            ...
            Item* item = target->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFF_HAND);
