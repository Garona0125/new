...
void Spell::EffectResurrectNew(SpellEffIndex effIndex)
{
    ...
    if (unitTarget->GetTypeId() != TYPE_ID_PLAYER)
        return;
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELL_FAMILY_GENERIC:
            {
                ...
                switch (m_spellInfo->Id)                     // better way to check unknown
                {
					...
					case 51963:
                    {
						...
						damage = (m_caster->GetCurrentLevel() - 60) * 4 + 60;
						break;
					}
            ...
            case SPELL_FAMILY_WARRIOR:
            {
                ...
                {
                    uint8 level = m_caster->GetCurrentLevel();
            ...
            case SPELL_FAMILY_WARLOCK:
            {
                ...
                if ((m_spellInfo->SpellFamilyFlags[1] & 0x000040) && m_spellInfo->SpellIconID == 2128)
                {
                    ...
                    {
                        if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_WARLOCK, 0x4, 0, 0))
                            damage += damage/4;
                    }
                }
                ...
                else if (m_spellInfo->TargetAuraState == AURA_STATE_CONFLAGRATE)
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator i = mPeriodic.begin(); i != mPeriodic.end(); ++i)
                    {
                        ...
                        if ((*i)->GetSpellInfo()->SpellFamilyName != SPELL_FAMILY_WARLOCK ||
                ...
                {
                    if (m_caster->GetTypeId() == TYPE_ID_UNIT && m_caster->ToCreature()->IsPet())
                    {
            ...
            case SPELL_FAMILY_PRIEST:
            {
				...
				if (m_caster->GetShapeshiftForm() == FORM_SHADOW && (m_spellInfo->SpellFamilyFlags[0] & 0x00002000))
                {
                    ...
                    {
                        if ((*i)->GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_PRIEST &&
            ...
            {
                // Ferocious Bite
                if (m_caster->GetTypeId() == TYPE_ID_PLAYER && (m_spellInfo->SpellFamilyFlags[0] & 0x000800000) && m_spellInfo->SpellVisual[0] == 6587)
                {
                ...
                else if (m_spellInfo->SpellFamilyFlags[0] & 0x00000001)
                {
                    ...
                    if (AuraEffect const* aurEff = m_caster->GetDummyAuraEffect(SPELL_FAMILY_DRUID, 1771, 0))
                        if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_DRUID, 0x00200000, 0, 0))
            ...
            case SPELL_FAMILY_ROGUE:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x00000008)
                {
                    if (Player* player = m_caster->ToPlayer())
                    {
                        ...
                        if (uint32 combo = player->GetComboPoints())
                        {
                            ...
                            if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_ROGUE, 0x00010000, 0, 0, m_caster->GetGUID()))
                            {
                                ...
                                {
                                    if ((*iter)->GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_ROGUE && (*iter)->GetSpellInfo()->SpellIconID == 1960)
                                    {
                ...
                {
                    if (m_caster->GetTypeId() == TYPE_ID_PLAYER)
                    {
            ...
            case SPELL_FAMILY_PALADIN:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1]&0x00040000)
                {
                    ...
                    float average = (m_caster->GetFloatValue(UNIT_FIELD_MIN_DAMAGE) + m_caster->GetFloatValue(UNIT_FIELD_MAX_DAMAGE)) / float(m_caster->GetAttackTime(BASE_ATTACK) / 1000.0f);
				...
				{
					if (Aura* aura = m_caster->GetAura(71880))
					{
						if (roll_chance_i(25))
				...
				{
					if (Aura* aura = m_caster->GetAura(71892))
					{
						if (roll_chance_i(35))
            ...
            case SPELL_FAMILY_DEATH_KNIGHT:
            {
                ...
                {
                    if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_DEATH_KNIGHT, 0, 0, 0x00000002, m_caster->GetGUID()))
                    {
                        damage += m_damage / 2;
            ...
            case SPELL_FAMILY_MAGE:
            {
                ...
                if (m_spellInfo->Id == 71757)
                    if (unitTarget->GetTypeId() != TYPE_ID_UNIT || !(unitTarget->IsImmunedToSpellEffect(sSpellMgr->GetSpellInfo(44572), 0)))
						return;
                break;
            }
        }
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELL_FAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
			    case 42436: //Drink!
				{
					if (m_caster->GetTypeId() != TYPE_ID_PLAYER || !m_caster)
						return;
					Player* player = m_caster->ToPlayer();
					Unit* pTarget = player->FindCreatureWithDistance(24108, 12.0f, true);
                ...
                {
                    if (!unitTarget || m_caster->GetTypeId() != TYPE_ID_PLAYER || unitTarget->GetTypeId() != TYPE_ID_UNIT)
                        return;
        ...
        case SPELL_FAMILY_PALADIN:
			switch (m_spellInfo->Id)
			{
			    case 31789:                                 // Righteous Defense (step 1)
                {
                    ...
                    {
                        Unit* attacker = Quantum::DataPackets::SelectRandomContainerElement(attackers);
        ...
        case SPELL_FAMILY_DEATH_KNIGHT:
			switch (m_spellInfo->Id)
            {
                ...
                case 48289:
                    ...
                    break;
				case 61999:
					if (!unitTarget || m_caster->GetTypeId() != TYPE_ID_PLAYER || unitTarget->IsAlive())
					{
    ...
    if (spell_id)
    {
        ...
        if (!spellInfo)
        {
            sLog->OutErrorConsole("EffectDummy of spell %u: triggering unknown spell id %i\n", m_spellInfo->Id, spell_id);
            return;
        }
    ...
    else if (unitTarget && unitTarget->GetTypeId() == TYPE_ID_UNIT)
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() == TYPE_ID_PLAYER && m_spellInfo->CategoryRecoveryTime && spellInfo->CategoryRecoveryTime && m_spellInfo->GetCategory() == spellInfo->GetCategory())
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
	switch (m_spellInfo->Id)
    {
		...
		{
			if (Creature* TargetTeleport = m_caster->FindCreatureWithDistance(22515, 60.0f, true))
			{
				float x, y, z, o;
		...
		{
			if (Creature* TargetTeleport = m_caster->FindCreatureWithDistance(23472, 60.0f, true))
			{
				float x, y, z, o;
				TargetTeleport->GetPosition(x, y, z, o);
    ...
    if (!spellInfo)
    {
        sLog->OutErrorConsole("Spell::EffectForceCast of spell %u: triggering unknown spell id %i", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        sLog->OutErrorConsole("EffectTriggerRitualOfSummoning of spell %u: triggering unknown spell id %i", m_spellInfo->Id, triggered_spell_id);
        return;
    }
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 60321:  // Scroll of Recal III
            ...
            if (unitTarget->GetCurrentLevel() > uiMaxSafeLevel)
            {
    ...
    if (!m_targets.HasDst())
    {
        sLog->OutErrorConsole("Spell::EffectTeleportUnits - does not have destination for spell ID %u\n", m_spellInfo->Id);
        return;
    }
    ...
    else if (unitTarget->GetTypeId() == TYPE_ID_PLAYER)
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex)
{
    ...
    if (!unitTarget || unitTarget->GetTypeId() != TYPE_ID_PLAYER)
        return;
...
void Spell::EffectHeal(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            {
                if ((*i)->GetSpellInfo()->SpellFamilyName == SPELL_FAMILY_DRUID
            ...
            if (!targetAura)
            {
                sLog->OutErrorConsole("Target(GUID:" UI64FMTD ") has aurastate AURA_STATE_SWIFTMEND but no matching aura.", unitTarget->GetGUID());
                return;
            }
        ...
        // Nourish
        else if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_DRUID && m_spellInfo->SpellFamilyFlags[1] & 0x2000000)
        {
        ...
        else if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT && m_spellInfo->SpellFamilyFlags[0] & 0x00080000)
			addhealth = caster->SpellHealingBonusDone(unitTarget, m_spellInfo, int32(caster->CountPctFromMaxHealth(damage)), HEAL);
		else if (m_spellInfo->Id == 33778) // Lifebloom Final Tik Heal
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
	...
	if (unitTarget->GetTypeId() == TYPE_ID_PLAYER && unitTarget->GetPowerType() != power && !(m_spellInfo->AttributesEx7 & SPELL_ATTR7_CAN_RESTORE_SECONDARY_POWER))
		return;
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 9512:                                          // Restore Energy
            level_diff = m_caster->GetCurrentLevel() - 40;
            level_multiplier = 2;
            break;
        case 24571:                                         // Blood Fury
            level_diff = m_caster->GetCurrentLevel() - 60;
            level_multiplier = 10;
            break;
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        for (std::set<uint32>::iterator itr = avalibleElixirs.begin(); itr != avalibleElixirs.end() ;)
        {
            ...
            if (spellInfo->SpellLevel < m_spellInfo->SpellLevel || spellInfo->SpellLevel > unitTarget->GetCurrentLevel())
        ...
        if (!avalibleElixirs.empty())
        {
            ...
            m_caster->CastSpell(unitTarget, Quantum::DataPackets::SelectRandomContainerElement(avalibleElixirs), true, m_CastItem);
        }
    }
}
...
void Spell::SendLoot(uint64 guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        {
            sLog->OutErrorConsole("Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn time without being in GM mode!",
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_BUTTON:
                ...
                return;
            case GAMEOBJECT_TYPE_QUEST_GIVER:
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if (gameObjTarget)
    {
        ...
        else if (goInfo->type == GAMEOBJECT_TYPE_FLAG_STAND)
        {
            //CanUseBattlegroundObject() already called in CheckCast()
            // in battleground check
            if (Battleground* bg = player->GetBattleground())
...
void Spell::EffectSummonChangeItem(SpellEffIndex effIndex)
{
    ...
    else if (player->IsEquipmentPos(pos))
    {
        ...
        {
            if (msg == EQUIP_ERR_CANT_DO_RIGHT_NOW) dest = EQUIPMENT_SLOT_MAIN_HAND;
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    if (!properties)
    {
        sLog->OutErrorConsole("EffectSummonType: Unhandled summon type %u", m_spellInfo->Effects[effIndex].MiscValueB);
        return;
    }
    ...
    switch (properties->Category)
    {
        ...
        case SUMMON_CATEGORY_UNK:
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_MINIPET:
                {
                    ...
                    summon->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_AGGRO_FOR_PLAYER | UNIT_FLAG_NO_AGGRO_FOR_CREATURE);
                ...
                default:
                {
                    ...
                    for (uint32 count = 0; count < numSummons; ++count)
                    {
                        ...
                        if (properties->Category == SUMMON_CATEGORY_ALLY)
                        {
							...
							summon->SetCurrentFaction(m_originalCaster->GetFaction());
        ...
        case SUMMON_CATEGORY_VEHICLE:
            ...
            summon->SetCurrentFaction(faction);
            break;
    }
...
void Spell::EffectDispel(SpellEffIndex effIndex)
{
    ...
    // On success dispel
    // Devour Magic
    if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_WARLOCK && m_spellInfo->GetCategory() == SPELLCATEGORY_DEVOUR_MAGIC)
    {
...
void Spell::EffectDualWield(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget->GetTypeId() == TYPE_ID_UNIT)
...
void Spell::EffectDistract(SpellEffIndex /*effIndex*/)
{
    ...
    if (!unitTarget || unitTarget->IsInCombatActive())
        return;
...
void Spell::EffectPickPocket(SpellEffIndex /*effIndex*/)
{
    ...
    if (!unitTarget || unitTarget->GetTypeId() != TYPE_ID_UNIT || m_caster->IsFriendlyTo(unitTarget))
        return;

    // victim have to be alive and humanoid or undead
    if (unitTarget->IsAlive() && (unitTarget->GetCreatureTypeMask() & CREATURE_TYPE_MASK_HUMANOID_OR_UNDEAD) != 0)
...
void Spell::EffectUntrainTalents(SpellEffIndex /*effIndex*/)
{
    ...
    if (!unitTarget || m_caster->GetTypeId() == TYPE_ID_PLAYER)
        return;
...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Trinity::Honor::hk_honor_at_level(unitTarget->GetCurrentLevel(), float(damage));
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex)
{
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            sLog->OutErrorConsole("Spell::EffectEnchantItemPrismatic: attempt apply enchant spell %u with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u) but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not suppoted yet.",
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    if (!itemTarget)
        return;
    if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[0] & 0x400000)
    {
        uint32 spell_id = 0;
        ...
        switch (damage)
        {
            ...
            case 11:
				...
				break;
            default:
                sLog->OutErrorConsole("Spell::EffectEnchantItemTmp: Damage %u not handled in S'RW", damage);
                return;
        }
        ...
        if (!spellInfo)
        {
            sLog->OutErrorConsole("Spell::EffectEnchantItemTmp: unknown spell id %i", spell_id);
            return;
    ...
    if (!enchant_id)
    {
        sLog->OutErrorConsole("Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have 0 as enchanting id", m_spellInfo->Id, effIndex);
        return;
    }
    ...
    if (!pEnchant)
    {
        sLog->OutErrorConsole("Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have not existed enchanting id %u ", m_spellInfo->Id, effIndex, enchant_id);
        return;
    }
    ...
    else if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_ROGUE)
        duration = 3600;                                    // 1 hour
    // shaman family enchantments
    else if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_SHAMAN)
        duration = 1800;                                    // 30 mins
...
void Spell::EffectTameCreature(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_caster->GetCurrentClass() != CLASS_HUNTER)
        return;
    ...
    creatureTarget->DespawnAfterAction();

    uint8 level = (creatureTarget->GetCurrentLevel() < (m_caster->GetCurrentLevel() - 5)) ? (m_caster->GetCurrentLevel() - 5) : creatureTarget->GetCurrentLevel();
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
    ...
    if (OldSummon)
    {
        if (petentry == 0 || OldSummon->GetEntry() == petentry)
        {
            ...
            if (owner->GetTypeId() == TYPE_ID_PLAYER && OldSummon->isControlled())
    ...
    if (m_caster->GetTypeId() == TYPE_ID_UNIT)
    {
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELL_FAMILY_WARRIOR:
        {
			...
			{
				if (unitTarget->IsImmunedToSpellEffect(m_spellInfo,EFFECT_1) || unitTarget->GetTypeId() == TYPE_ID_PLAYER)
				{
					m_damage = 0;
					return;
				}
        ...
        case SPELL_FAMILY_DRUID:
        {
            ...
            {
                if (AuraEffect const* rendAndTear = m_caster->GetDummyAuraEffect(SPELL_FAMILY_DRUID, 2859, 0))
        ...
        case SPELL_FAMILY_DEATH_KNIGHT:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags[1] & 0x20000)
            {
                ...
                // Annihilation
                if (AuraEffect const* aurEff = m_caster->GetDummyAuraEffect(SPELL_FAMILY_DEATH_KNIGHT, 2710, EFFECT_0))
...
void Spell::EffectThreat(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_spellInfo->Id == 57994)
        damage = -int32(pow(float(m_caster->GetCurrentLevel()), 2.0f) / 3);
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
    ...
    if (m_originalCaster && m_originalCaster->GetTypeId() != TYPE_ID_PLAYER && pGameObj->GetGoType() != GAMEOBJECT_TYPE_CHEST)
        pGameObj->SetOwnerGUID(m_originalCasterGUID);

    // Wild object not have owner and check clickable by players
    map->AddToMap(pGameObj);

    if (pGameObj->GetGoType() == GAMEOBJECT_TYPE_FLAG_DROP && m_caster->GetTypeId() == TYPE_ID_PLAYER)
    {
        Player* player = m_caster->ToPlayer();
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 58622:
                {
					if (OutdoorPvPWG* pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr->GetOutdoorPvPToZoneId(4197))
						if (unitTarget->GetCurrentLevel() > 74)
						{
                ...
                case 23301:
                {
                    ...
                    Player* player = m_caster->ToPlayer();
                    if (player && unitTarget->IsDead())
                    {
                        player->KilledMonsterCredit(30220, 0);
                        unitTarget->ToCreature()->DespawnAfterAction(); //workaround for infinite credits
                    }
                ...
                case 63975:
                {
                    ...
                    if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELL_FAMILY_ROGUE, 0x00100000, 0, 0, m_caster->GetGUID()))
                    {
                ...
                case 69961:
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator i = mPeriodic.begin(); i != mPeriodic.end(); ++i)
                    {
                        ...
                        if (spellInfo->SpellFamilyName == SPELL_FAMILY_DEATH_KNIGHT && spellInfo->SpellFamilyFlags[2] & 0x2 &&
							aurEff->GetCasterGUID() == m_caster->GetGUID())
						{
                            ...
                            countMax += 9000;
                            // talent Epidemic
                            if (AuraEffect const* epidemic = m_caster->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELL_FAMILY_DEATH_KNIGHT, 234, EFFECT_0))
                ...
                case 8856:
                {
                    if (!itemTarget && m_caster->GetTypeId() != TYPE_ID_PLAYER)
                        return;
                ...
                case 24717:
                {
                    ...
                    if (unitTarget->GetCurrentGender() == GENDER_MALE)
                ...
                case 43375: // Mixing Blood - Quest 11306
                {
                    ...
                    switch (rand()% 4)
                    {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            spellid = 43376; // Fail without Effect
                            break;
                        //case 3:
                ...
                case 45151:
                {
                    ...
                    float radius = m_spellInfo->Effects[effIndex].CalcRadius();
                    if (unitTarget && unitTarget->GetTypeId() == TYPE_ID_PLAYER && unitTarget->GetDistance(m_caster) >= radius && !unitTarget->HasAura(46394) && unitTarget != m_caster)
                ...
                case 47770:
                {
                    ...
                    if (m_caster->GetCurrentGender() > 0)
                        gender = "her";
                ...
                case 51519:
                {
                    if (!unitTarget || unitTarget->GetTypeId() != TYPE_ID_UNIT)
                        return;
                ...
                case 60243: // Blood Parrot Despawn
                    if (unitTarget->GetTypeId() == TYPE_ID_UNIT && unitTarget->ToCreature()->IsSummon())
                ...
                {
                    if (!unitTarget || unitTarget->GetTypeId() != TYPE_ID_UNIT || m_caster->GetTypeId() != TYPE_ID_PLAYER)
                        return;

                    unitTarget->ToCreature()->DespawnAfterAction();
                ...
                {
                    if (!unitTarget || unitTarget->GetTypeId() != TYPE_ID_PLAYER || effIndex != 0)
                        return;
                ...
                {
                    if (m_caster->GetTypeId() != TYPE_ID_UNIT || !m_caster->ToCreature()->IsSummon())
                        return;
                    ...
                    switch (m_caster->GetEntry())
                    {
                        ...
                        case 31883:
							...
							break;
                        default:
                            sLog->OutErrorConsole("Unknown Lightwell spell caster %u", m_caster->GetEntry());
                            return;
                    }
            ...
            if (m_spellInfo->GetCategory() == SPELLCATEGORY_JUDGEMENT)
            {
				...
				switch (m_spellInfo->Id)
                {
                    ...
                    case 53408:
						...
						break;
                    default:
                        sLog->OutErrorConsole("Unsupported Judgement (seal trigger) spell (Id: %u) in Spell::EffectScriptEffect", m_spellInfo->Id);
                        return;
                }
...
void Spell::EffectSanctuary(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_caster->GetTypeId() == TYPE_ID_PLAYER && m_spellInfo->SpellFamilyName == SPELL_FAMILY_ROGUE && (m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_ROGUE_VANISH))
    {
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    if (!unitTarget || m_caster->GetTypeId() != TYPE_ID_PLAYER || unitTarget->GetTypeId() != TYPE_ID_PLAYER)
        return;
    ...
    pGameObj->SetUInt32Value(GAMEOBJECT_LEVEL, m_caster->GetCurrentLevel()+1);
    int32 duration = m_spellInfo->GetDuration();
    pGameObj->SetRespawnTime(duration > 0 ? duration/IN_MILLISECONDS : 0);
    pGameObj->SetSpellId(m_spellInfo->Id);
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_caster || m_caster->GetTypeId() != TYPE_ID_PLAYER)
        return;

    if (!sWorld->getBoolConfig(CONFIG_CAST_UNSTUCK))
        return;

	if (m_caster->GetCurrentClass() == CLASS_DEATH_KNIGHT && m_caster->GetMapId() == 609)
	{
    ...
    if (target && target->GetCurrentClass() == CLASS_DEATH_KNIGHT)
        return;
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() != TYPE_ID_PLAYER || m_glyphIndex >= MAX_GLYPH_SLOT_INDEX)
        return;
    ...
    if (minLevel && m_caster->GetCurrentLevel() < minLevel)
    {
...
void Spell::EffectEnchantHeldItem(SpellEffIndex effIndex)
{
    ...
    Item* item = item_owner->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAIN_HAND);
...
void Spell::EffectSummonObject(SpellEffIndex effIndex)
{
    ...
    //pGameObj->SetUInt32Value(GAMEOBJECT_LEVEL, m_caster->GetCurrentLevel());
    int32 duration = m_spellInfo->GetDuration();
    pGameObj->SetRespawnTime(duration > 0 ? duration/IN_MILLISECONDS : 0);
    pGameObj->SetSpellId(m_spellInfo->Id);
    m_caster->AddGameObject(pGameObj);
...
void Spell::EffectSkinning(SpellEffIndex /*effIndex*/)
{
    ...
    int32 targetLevel = creature->GetCurrentLevel();
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/)
{
    ...
    if (effectHandleMode == SPELL_EFFECT_HANDLE_HIT_TARGET)
    {
        ...
        if (!m_spellInfo->IsPositive() && m_caster->GetTypeId() == TYPE_ID_PLAYER)
...
void Spell::EffectKnockBack(SpellEffIndex effIndex)
{
    ...
    // Typhoon
    if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_DRUID && m_spellInfo->SpellFamilyFlags[1] & 0x01000000)
    {
        // Glyph of Typhoon
    ...
    // Thunderstorm
    if (m_spellInfo->SpellFamilyName == SPELL_FAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[1] & 0x00002000)
    {
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    if (!goinfo)
    {
        sLog->OutErrorDatabase("Gameobject (Entry: %u) not exist and not created at spell (ID: %u) cast", name_id, m_spellInfo->Id);
        return;
    }
    ...
    if (goinfo->type == GAMEOBJECT_TYPE_FISHING_NODE || goinfo->type == GAMEOBJECT_TYPE_FISHING_HOLE)
    {
        LiquidData liqData;
    ...
    switch (goinfo->type)
    {
        case GAMEOBJECT_TYPE_FISHING_NODE:
        {
    ...
    if (uint32 linkedEntry = pGameObj->GetGOInfo()->GetLinkedGameObjectEntry())
    {
        ...
        {
            linkedGO->SetRespawnTime(duration > 0 ? duration/IN_MILLISECONDS : 0);
            //linkedGO->SetUInt32Value(GAMEOBJECT_LEVEL, m_caster->GetCurrentLevel());
            linkedGO->SetSpellId(m_spellInfo->Id);
...
void Spell::EffectSkinPlayerCorpse(SpellEffIndex /*effIndex*/)
{
    ...
    if ((m_caster->GetTypeId() != TYPE_ID_PLAYER) || (unitTarget->GetTypeId() != TYPE_ID_PLAYER) || (unitTarget->IsAlive()))
        return;
...
void Spell::EffectCreateTamedPet(SpellEffIndex effIndex)
{
    ...
    if (!unitTarget || unitTarget->GetTypeId() != TYPE_ID_PLAYER || unitTarget->GetPetGUID() || unitTarget->GetCurrentClass() != CLASS_HUNTER)
        return;
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    uint8 level = caster->GetCurrentLevel();

    // level of pet summoned using engineering item based at engineering skill level
    if (m_CastItem && caster->GetTypeId() == TYPE_ID_PLAYER)
	{
        if (ItemTemplate const* proto = m_CastItem->GetTemplate())
		{
            if (proto->RequiredSkill == SKILL_ENGINEERING)
			{
                if (uint16 skill202 = caster->ToPlayer()->GetSkillValue(SKILL_ENGINEERING))
                    level = std::max(uint8(skill202/5), caster->GetCurrentLevel());
			}
		}
	}
    ...
    switch (m_spellInfo->Id)
    {
		case 57879: // Snake Trap
			if (AuraEffect *aurEff = m_originalCaster->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELL_FAMILY_HUNTER, 69, 2))
				numGuardians += aurEff->GetAmount();
			break;
    }
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        if (properties && properties->Category == SUMMON_CATEGORY_ALLY)
            summon->SetCurrentFaction(caster->GetFaction());

        if (summon->HasUnitTypeMask(UNIT_MASK_MINION) && m_targets.HasDst())
            ((Minion*)summon)->SetFollowAngle(m_caster->GetAngle(summon));

        if (summon->GetEntry() == ENTRY_RUNE_WEAPON)
        {
            if (uint32 weapon = m_caster->GetUInt32Value(PLAYER_VISIBLE_ITEM_16_ENTRY_ID))
            {
		...
		if (summon->GetEntry() == ENTRY_TREANT) // Force of Nature
		{
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex)
{
    ...
    {
        sLog->OutErrorConsole("EffectPlayMusic: Sound (Id: %u) not exist in spell %u.", soundid, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectPlayerNotification(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->Id)
    {
        case 58730: // Restricted Flight Area
		{
			if (sWorld->getBoolConfig(CONFIG_OUTDOOR_PVP_WINTERGRASP_ENABLED))
			{
				...
				{
					unitTarget->ToPlayer()->GetSession()->SendNotification(LANG_ZONE_NO_FLY_ZONE);
					unitTarget->PlayDirectSound(9417); // Fel Reaver sound
					unitTarget->MonsterTextEmote(LANG_NO_FLY_ZONE_WINTERGRASP, unitTarget->GetGUID(), true);
				}
    ...
    if (!sSoundEntriesStore.LookupEntry(soundid))
    {
        sLog->OutErrorConsole("EffectPlayerNotification: Sound (Id: %u) not exist in spell %u.", soundid, m_spellInfo->Id);
        return;
    }
...
void Spell::EffectBind(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Effects[effIndex].TargetA.GetTarget() == TARGET_DEST_DB || m_spellInfo->Effects[effIndex].TargetB.GetTarget() == TARGET_DEST_DB)
    {
        ...
        if (!st)
        {
            sLog->OutErrorDatabase("Spell::EffectBind - unknown teleport coordinates for spell ID %u", m_spellInfo->Id);
            return;
        }
...
void Spell::EffectSummonRaFFriend(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() != TYPE_ID_PLAYER || !unitTarget || unitTarget->GetTypeId() != TYPE_ID_PLAYER)
        return;
