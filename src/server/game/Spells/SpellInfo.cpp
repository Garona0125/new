...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* /*target*/) const
{
    ...
    if (caster)
    {
        int32 level = int32(caster->GetCurrentLevel());
        if (level > int32(_spellInfo->MaxLevel) && _spellInfo->MaxLevel > 0)
            level = int32(_spellInfo->MaxLevel);
        else if (level < int32(_spellInfo->BaseLevel))
            level = int32(_spellInfo->BaseLevel);
		//level -= int32(_spellInfo->SpellLevel);
		// Fix Mage Talent (Magic Absorption)
    ...
    if (caster)
    {
        ...
        if (!basePointsPerLevel && (_spellInfo->Attributes & SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION && _spellInfo->SpellLevel) &&
            ...
            value *= 0.25f * exp(caster->GetCurrentLevel() * (70 - _spellInfo->SpellLevel) / 1000.0f);
            //value = int32(value * (int32)GetCurrentLevel() / (int32)(_spellInfo->spellLevel ? _spellInfo->spellLevel : 1));
    }
...
float SpellEffectInfo::CalcRadius(Unit* caster, Spell* spell) const
{
    ...
    if (caster)
    {
        radius += RadiusEntry->RadiusPerLevel * caster->GetCurrentLevel();
...
{
    return (SpellFamilyName == SPELL_FAMILY_HUNTER && !(SpellFamilyFlags[1] & 0x10000000)) // for 53352, cannot find better way
...
SpellCastResult SpellInfo::CheckShapeshift(uint32 form) const
{
    ...
    if (form > 0)
    {
        ...
        if (!shapeInfo)
        {
            sLog->OutErrorConsole("GetErrorAtShapeshiftedCast: unknown shapeshift %u", form);
            return SPELL_CAST_OK;
        }
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, Unit const* target, bool implicit) const
{
    ...
    if (AttributesEx & SPELL_ATTR1_CANT_TARGET_IN_COMBAT && target->IsInCombatActive())
    ...
    if (target != caster && (caster->IsControlledByPlayer() || !IsPositive()) && target->GetTypeId() == TYPE_ID_PLAYER)
    {
	...
	if (target->HasUnitState(UNIT_STATE_IN_FLIGHT) && !(AttributesCu & SPELL_ATTR0_CU_ALLOW_INFLIGHT_TARGET))
        return SPELL_FAILED_BAD_TARGETS;
    ...
    if (caster != target)
    {
        if (caster->GetTypeId() == TYPE_ID_PLAYER)
        {
            ...
            if (AttributesCu & SPELL_ATTR0_CU_PICKPOCKET)
            {
                 if (target->GetTypeId() == TYPE_ID_PLAYER)
                     return SPELL_FAILED_BAD_TARGETS;
                 else if ((target->GetCreatureTypeMask() & CREATURE_TYPE_MASK_HUMANOID_OR_UNDEAD) == 0)
...
bool SpellInfo::CheckTargetCreatureType(Unit const* target) const
{
    ...
    if (SpellFamilyName == SPELL_FAMILY_WARLOCK && GetCategory() == 1179)
    {
...
AuraStateType SpellInfo::GetAuraState() const
{
    ...
    if (SpellFamilyName == SPELL_FAMILY_WARLOCK &&
        // Immolate
	...
	if (SpellFamilyName == SPELL_FAMILY_DRUID && SpellFamilyFlags[0] & 0x400)
    ...
    // Victorious
    if (SpellFamilyName == SPELL_FAMILY_WARRIOR &&  SpellFamilyFlags[1] & 0x00040000)
        return AURA_STATE_WARRIOR_VICTORY_RUSH;

    // Swiftmend state on Regrowth & Rejuvenation
    if (SpellFamilyName == SPELL_FAMILY_DRUID && SpellFamilyFlags[0] & 0x50)
        return AURA_STATE_SWIFTMEND;

    // Deadly poison aura state
    if (SpellFamilyName == SPELL_FAMILY_ROGUE && SpellFamilyFlags[0] & 0x10000)
...
uint32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask) const
{
    ...
    if (AttributesEx & SPELL_ATTR1_DRAIN_ALL_POWER)
    {
        ...
        sLog->OutErrorConsole("SpellInfo::CalcPowerCost: Unknown power type '%d' in spell %d", PowerType, Id);
        return 0;
    }
    ...
    if (ManaCostPercentage)
    {
        switch (PowerType)
        {
            ...
            case POWER_RUNIC_POWER:
                ...
                break;
            default:
                sLog->OutErrorConsole("CalculateManaCost: Unknown power type '%d' in spell %d", PowerType, Id);
                return 0;
        }
    }
    ...
    if (Attributes & SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION)
        powerCost = int32(powerCost / (1.117f * SpellLevel / caster->GetCurrentLevel() -0.1327f));
...
bool SpellInfo::_IsPositiveEffect(uint8 effIndex, bool deep) const
{
    ...
    switch (SpellFamilyName)
    {
        case SPELL_FAMILY_GENERIC:
            switch (Id)
            {
			    case 26102: // Sand Blast
				case 29214: // Wrath of the Plaguebringer
                case 34700: // Allergic Reaction
				case 54836: // Wrath of the Plaguebringer
				case 52207: // Devour Gumanoid (need test)
                case 61987: // Avenging Wrath Marker
                case 61988: // Divine Shield exclude aura
                case 64616: // Deafening Siren
				case 73076: // Throw Shield (Frostsworn General)
				case 69222: // Throw Shield (Frostsworn General)
                    return false;
                case 30877: // Tag Murloc
                case 61716: // Rabbit Costume
                case 61734: // Noblegarden Bunny
                case 62344: // Fists of Stone
				case 61819: // Manabonked!
    ...
    switch (Effects[effIndex].Effect)
    {
        ...
        case SPELL_EFFECT_ENERGIZE_PCT:
            return true;
		case SPELL_EFFECT_APPLY_AREA_AURA_ENEMY: // Generic fix for auras
			return false;
