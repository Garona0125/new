...
{
	PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_GET_ACCOUNT_SECURITY_GM_LEVEL);
	stmt->setUInt32(0, accountId);
	PreparedQueryResult result = LoginDatabase.Query(stmt);
...
{
	PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_GET_ACCOUNT_SECURITY_GM_LEVEL_BY_REALM_ID);
	stmt->setUInt32(0, accountId);
...
{
    return gmlevel >= SEC_MODERATOR && gmlevel <= SEC_TECHNICAL_MASTER;
}

bool IsEventMasterAccount(uint32 gmlevel)
{
	return gmlevel >= SEC_EVENT_MASTER && gmlevel <= SEC_TECHNICAL_MASTER;
}

bool IsGMAccount(uint32 gmlevel)
{
	return gmlevel >= SEC_GAME_MASTER && gmlevel <= SEC_TECHNICAL_MASTER;
}

bool IsAdminAccount(uint32 gmlevel)
{
    return gmlevel >= SEC_ADMINISTRATOR && gmlevel <= SEC_TECHNICAL_MASTER;
}

bool IsMainAdministratorAccount(uint32 gmlevel)
{
    return gmlevel == SEC_MAIN_ADMINISTRATOR && gmlevel <= SEC_TECHNICAL_MASTER;
}

bool IsDeveloperAccount(uint32 gmlevel)
{
    return gmlevel == SEC_DEVELOPER && gmlevel <= SEC_TECHNICAL_MASTER;
}

bool IsTechSupportAccount(uint32 gmlevel)
{
    return gmlevel == SEC_TECHNICAL_MASTER;
}
