...
void LoadDisables()
{
    ...
    {
		sLog->OutConsole("===================================================");
        sLog->OutConsole(">> Loaded 0 disables. DB table `disables` is empty!");
		sLog->OutConsole("===================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("Invalid type %u specified in `disables` table, skipped.", type);
            continue;
        }
        ...
        switch (type)
        {
            case DISABLE_TYPE_SPELL:
                ...
                {
                    sLog->OutErrorDatabase("Spell entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }

                if (!flags || flags > MAX_SPELL_DISABLE_TYPE)
                {
                    sLog->OutErrorDatabase("Disable flags for spell %u are invalid, skipped.", entry);
                    continue;
                }
            ...
            case DISABLE_TYPE_MAP:
            {
                ...
                if (!mapEntry)
                {
                    sLog->OutErrorDatabase("Map entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                ...
                switch (mapEntry->map_type)
                {
                    ...
                    case MAP_ARENA:
                        sLog->OutErrorDatabase("Battleground map %u specified to be disabled in map case, skipped.", entry);
                        continue;
                }
                if (isFlagInvalid)
                {
                    sLog->OutErrorDatabase("Disable flags for map %u are invalid, skipped.", entry);
                    continue;
                }
                break;
            }
            case DISABLE_TYPE_BATTLEGROUND:
                ...
                {
                    sLog->OutErrorDatabase("Battleground entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->OutErrorDatabase("Disable flags specified for battleground %u, useless data.", entry);
                break;
            case DISABLE_TYPE_OUTDOORPVP:
                if (entry > MAX_OUTDOORPVP_TYPES)
                {
                    sLog->OutErrorDatabase("OutdoorPvPTypes value %u from `disables` is invalid, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->OutErrorDatabase("Disable flags specified for outdoor PvP %u, useless data.", entry);
                break;
            case DISABLE_TYPE_ACHIEVEMENT_CRITERIA:
                if (!sAchievementCriteriaStore.LookupEntry(entry))
                {
                    sLog->OutErrorDatabase("Achievement Criteria entry %u from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    sLog->OutErrorDatabase("Disable flags specified for Achievement Criteria %u, useless data.", entry);
                break;
                ...
                switch (mapEntry->map_type)
                {
                    case MAP_COMMON:
                        if (flags & VMAP_DISABLE_AREAFLAG)
                            sLog->OutConsole("Areaflag disabled for world map %u.", entry);
                        if (flags & VMAP_DISABLE_LIQUIDSTATUS)
                            sLog->OutConsole("Liquid status disabled for world map %u.", entry);
                        break;
                    case MAP_INSTANCE:
                    case MAP_RAID:
                        if (flags & VMAP_DISABLE_HEIGHT)
                            sLog->OutConsole("Height disabled for instance map %u.", entry);
                        if (flags & VMAP_DISABLE_LOS)
                            sLog->OutConsole("LoS disabled for instance map %u.", entry);
                        break;
                    case MAP_BATTLEGROUND:
                        if (flags & VMAP_DISABLE_HEIGHT)
                            sLog->OutConsole("Height disabled for battleground map %u.", entry);
                        if (flags & VMAP_DISABLE_LOS)
                            sLog->OutConsole("LoS disabled for battleground map %u.", entry);
                        break;
                    case MAP_ARENA:
                        if (flags & VMAP_DISABLE_HEIGHT)
                            sLog->OutConsole("Height disabled for arena map %u.", entry);
                        if (flags & VMAP_DISABLE_LOS)
                            sLog->OutConsole("LoS disabled for arena map %u.", entry);
                        break;
                    default:
                        break;
                }
	...
	sLog->OutConsole("================================");
    sLog->OutConsole(">> Loaded %u disables in %u ms", total_count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("================================");
    sLog->OutConsole();
}
...
void CheckQuestDisables()
{
    ...
    if (!count)
    {
        sLog->OutConsole(">> Checked 0 quest disables.");
        sLog->OutConsole();
        return;
    }
    ...
    for (DisableTypeMap::iterator itr = m_DisableMap[DISABLE_TYPE_QUEST].begin(); itr != m_DisableMap[DISABLE_TYPE_QUEST].end();)
    {
        ...
        {
            sLog->OutErrorDatabase("Quest entry %u from `disables` doesn't exist, skipped.", entry);
            m_DisableMap[DISABLE_TYPE_QUEST].erase(itr++);
            continue;
        }
        if (itr->second.flags)
            sLog->OutErrorDatabase("Disable flags specified for quest %u, useless data.", entry);
        ++itr;
    }

    sLog->OutConsole(">> Checked %u quest disables in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->OutConsole();
}
...
bool IsDisabledFor(DisableType type, uint32 entry, Unit const* unit, uint8 flags)
{
    ...
    switch (type)
    {
        case DISABLE_TYPE_SPELL:
        {
            ...
            if (unit)
            {
                if ((spellFlags & SPELL_DISABLE_PLAYER && unit->GetTypeId() == TYPE_ID_PLAYER) ||
                    (unit->GetTypeId() == TYPE_ID_UNIT && ((unit->ToCreature()->IsPet() && spellFlags & SPELL_DISABLE_PET) || spellFlags & SPELL_DISABLE_CREATURE)))
                {
