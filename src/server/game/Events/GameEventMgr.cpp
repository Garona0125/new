...
{
    {
        ...
        if (!result)
        {
            mGameEvent.clear();
			sLog->OutConsole("========================================================");
            sLog->OutConsole(">> Loaded 0 game events. DB table `game_event` is empty!");
			sLog->OutConsole("========================================================");
            sLog->OutConsole();
            return;
        }
        ...
        do
        {
            ...
            if (event_id == 0)
            {
                sLog->OutErrorDatabase("`game_event` game event entry 0 is reserved and can't be used.");
                continue;
            }
            ...
            {
                sLog->OutErrorDatabase("`game_event` game event id (%i) isn't a world event and has length = 0, thus it can't be used.", event_id);
                continue;
            }
            ...
            if (pGameEvent.holiday_id != HOLIDAY_NONE)
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event` game event id (%i) have not existed holiday id %u.", event_id, pGameEvent.holiday_id);
		...
		sLog->OutConsole("===================================");
        sLog->OutConsole(">> Loaded %u game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("===================================");
        sLog->OutConsole();
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Saves Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("=================================================================================");
            sLog->OutConsole(">> Loaded 0 game event saves in game events. DB table `game_event_save` is empty!");
			sLog->OutConsole("=================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event_save` game event entry (%i) is out of range compared to max event entry in `game_event`", event_id);
                    continue;
                }
                ...
                {
                    sLog->OutErrorDatabase("game_event_save includes event save for non-worldevent id %u", event_id);
                    continue;
                }
			...
			sLog->OutConsole("=======================================================");
            sLog->OutConsole(">> Loaded %u game event saves in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=======================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Prerequisite Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("=================================================================================================");
            sLog->OutConsole(">> Loaded 0 game event prerequisites in game events. DB table `game_event_prerequisite` is empty!");
			sLog->OutConsole("=================================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    sLog->OutErrorDatabase("`game_event_prerequisite` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
                ...
                if (mGameEvent[event_id].state != GAMEEVENT_NORMAL && mGameEvent[event_id].state != GAMEEVENT_INTERNAL)
                {
                    ...
                    {
                        sLog->OutErrorDatabase("`game_event_prerequisite` game event prerequisite id (%i) is out of range compared to max event id in `game_event`", prerequisite_event);
                        continue;
                    }
                ...
                {
                    sLog->OutErrorDatabase("game_event_prerequisiste includes event entry for non-worldevent id %u", event_id);
                    continue;
                }
			...
			sLog->OutConsole("===============================================================");
            sLog->OutConsole(">> Loaded %u game event prerequisites in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("===============================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Creature Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("==============================================================================");
            sLog->OutConsole(">> Loaded 0 creatures in game events. DB table `game_event_creature` is empty!");
			sLog->OutConsole("==============================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
				...
				if (!data)
				{
					sLog->OutErrorDatabase("`game_event_creature` contains creature (GUID: %u) not found in `creature` table.", guid);
					continue;
				}

                if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
                {
                    sLog->OutErrorDatabase("`game_event_creature` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
			...
			sLog->OutConsole("================================================");
            sLog->OutConsole(">> Loaded %u creatures in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event GO Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("==================================================================================");
            sLog->OutConsole(">> Loaded 0 gameobjects in game events. DB table `game_event_gameobject` is empty!");
			sLog->OutConsole("==================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
				...
				if (!data)
				{
					sLog->OutErrorDatabase("`game_event_gameobject` contains gameobject (GUID: %u) not found in `gameobject` table.", guid);
					continue;
				}

                if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
                {
                    sLog->OutErrorDatabase("`game_event_gameobject` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
			...
			sLog->OutConsole("==================================================");
            sLog->OutConsole(">> Loaded %u gameobjects in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("==================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Model/Equipment Change Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("===============================================================================================");
            sLog->OutConsole(">> Loaded 0 model/equipment changes in game events. DB table `game_event_model_equip` is empty!");
			sLog->OutConsole("===============================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event_model_equip` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
                ...
                if (newModelEquipSet.equipment_id > 0)
                {
                    ...
                    {
                        sLog->OutErrorDatabase("Table `game_event_model_equip` have creature (Guid: %u) with equipment_id %u not found in table `creature_equip_template`, set to no equipment.", guid, newModelEquipSet.equipment_id);
                        continue;
                    }
                }
			...
			sLog->OutConsole("==============================================================");
            sLog->OutConsole(">> Loaded %u model/equipment changes in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("==============================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Quest Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("===========================================================================================");
            sLog->OutConsole(">> Loaded 0 quests additions in game events. DB table `game_event_creature_quest` is empty!");
			sLog->OutConsole("===========================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event_creature_quest` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
			...
			sLog->OutConsole("=======================================================");
            sLog->OutConsole(">> Loaded %u quests additions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=======================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event GO Quest Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("================================================================================================");
            sLog->OutConsole(">> Loaded 0 go quests additions in game events. DB table `game_event_gameobject_quest` is empty!");
			sLog->OutConsole("================================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event_gameobject_quest` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
    ...
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Quest Condition Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("==================================================================================================");
            sLog->OutConsole(">> Loaded 0 quest event conditions in game events. DB table `game_event_quest_condition` is empty!");
			sLog->OutConsole("==================================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    sLog->OutErrorDatabase("`game_event_quest_condition` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
			...
			sLog->OutConsole("=============================================================");
            sLog->OutConsole(">> Loaded %u quest event conditions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=============================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Condition Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("================================================================================");
            sLog->OutConsole(">> Loaded 0 conditions in game events. DB table `game_event_condition` is empty!");
			sLog->OutConsole("================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    sLog->OutErrorDatabase("`game_event_condition` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
			...
			sLog->OutConsole("=================================================");
            sLog->OutConsole(">> Loaded %u conditions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Condition Save Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("==========================================================================================");
            sLog->OutConsole(">> Loaded 0 condition saves in game events. DB table `game_event_condition_save` is empty!");
			sLog->OutConsole("==========================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    sLog->OutErrorDatabase("`game_event_condition_save` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
                ...
                {
                    sLog->OutErrorDatabase("game_event_condition_save contains not present condition evt id %u cond id %u", event_id, condition);
                    continue;
                }
			...
			sLog->OutConsole("======================================================");
            sLog->OutConsole(">> Loaded %u condition saves in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("======================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event NPC Flag Data...");
    {
        uint32 oldMSTime = getMSTime();

        //                                                0       1        2
        QueryResult result = WorldDatabase.Query("SELECT guid, event_id, npc_flag FROM game_event_npc_flags");

        if (!result)
        {
			sLog->OutConsole("===============================================================================");
            sLog->OutConsole(">> Loaded 0 npc flags in game events. DB table `game_event_npc_flags` is empty!");
			sLog->OutConsole("===============================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                uint32 guid = fields[0].GetUInt32();
                uint16 event_id = fields[1].GetUInt8();
                uint32 npc_flag = fields[2].GetUInt32();

                if (event_id >= mGameEvent.size())
                {
                    sLog->OutErrorDatabase("`game_event_npc_flags` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }

                mGameEventNPCFlags[event_id].push_back(GuidNPCFlagPair(guid, npc_flag));
			...
			sLog->OutConsole("================================================");
            sLog->OutConsole(">> Loaded %u npc flags in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Seasonal Quest Relations...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("============================================================================================================");
            sLog->OutConsole(">> Loaded 0 seasonal quests additions in game events. DB table `game_event_seasonal_questrelation` is empty!");
			sLog->OutConsole("============================================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event_seasonal_questrelation` quest id (%u) does not exist in `quest_template`", questId);
                    continue;
                }

                if (eventEntry >= mGameEvent.size())
                {
                    sLog->OutErrorDatabase("`game_event_seasonal_questrelation` event id (%u) is out of range compared to max event in `game_event`", eventEntry);
                    continue;
                }
    ...
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Vendor Additions Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("=======================================================================================");
            sLog->OutConsole(">> Loaded 0 vendor additions in game events. DB table `game_event_npc_vendor` is empty!");
			sLog->OutConsole("=======================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event_npc_vendor` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
			...
			sLog->OutConsole("=======================================================");
            sLog->OutConsole(">> Loaded %u vendor additions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=======================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Battleground Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("======================================================================================================");
            sLog->OutConsole(">> Loaded 0 battleground holidays in game events. DB table `game_event_battleground_holiday` is empty!");
			sLog->OutConsole("======================================================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (event_id >= mGameEvent.size())
                {
                    sLog->OutErrorDatabase("`game_event_battleground_holiday` game event id (%u) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }
			...
			sLog->OutConsole("============================================================");
            sLog->OutConsole(">> Loaded %u battleground holidays in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("============================================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Game Event Pool Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("=======================================================================");
            sLog->OutConsole(">> Loaded 0 pools for game events. DB table `game_event_pool` is empty!");
			sLog->OutConsole("=======================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`game_event_pool` game event id (%i) is out of range compared to max event id in `game_event`", event_id);
                    continue;
                }

                if (!sPoolMgr->CheckPool(entry))
                {
                    sLog->OutErrorDatabase("Pool Id (%u) has all creatures or gameobjects with explicit chance sum <>100 and no equal chance defined. The pool system cannot pick one to spawn.", entry);
                    continue;
                }
			...
			sLog->OutConsole("=============================================");
            sLog->OutConsole(">> Loaded %u pools for game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=============================================");
            sLog->OutConsole();
        }
    }
...
uint32 GameEventMgr::GetNPCFlag(Creature* creature)
{
    uint32 mask = 0;
...
void GameEventMgr::StartArenaSeason()
{
    ...
    if (!result)
    {
        sLog->OutConsole("ArenaSeason (%u) must be an existant Arena Season", season);
        return;
    }
    ...
    {
        sLog->OutConsole("EventEntry %u for ArenaSeason (%u) does not exists", eventId, season);
        return;
    }

    StartEvent(eventId, true);
    sLog->OutConsole("Arena Season %u started...", season);
    sLog->OutConsole();
}
...
uint32 GameEventMgr::Update()                               // return the next event delay in ms
{
    ...
    for (uint16 itr = 1; itr < mGameEvent.size(); ++itr)
    {
        ...
        // so first queue it
        //sLog->OutErrorDatabase("Checking event %u", itr);
...
void GameEventMgr::UpdateEventNPCFlags(uint16 event_id)
{
    ...
    for (NPCFlagList::iterator itr = mGameEventNPCFlags[event_id].begin(); itr != mGameEventNPCFlags[event_id].end(); ++itr)
    {
        ...
        {
            Creature* creature = HashMapHolder<Creature>::Find(MAKE_NEW_GUID(itr->first, data->id, HIGHGUID_UNIT));
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        sLog->OutConsole("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventCreatureGuids element %i (size: " SIZEFMTD ")",
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
    {
        sLog->OutConsole("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventGameobjectGuids element %i (size: " SIZEFMTD ")",
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventPoolIds.size()))
    {
        sLog->OutConsole("GameEventMgr::GameEventSpawn attempt access to out of range mGameEventPoolIds element %u (size: " SIZEFMTD ")",
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        sLog->OutConsole("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventCreatureGuids element %i (size: " SIZEFMTD ")",
            internal_event_id, mGameEventCreatureGuids.size());
        return;
    }
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
    {
        sLog->OutConsole("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventGameobjectGuids element %i (size: " SIZEFMTD ")",
            internal_event_id, mGameEventGameobjectGuids.size());
        return;
    }
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventPoolIds.size()))
    {
        sLog->OutConsole("GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventPoolIds element %u (size: " SIZEFMTD ")", internal_event_id, mGameEventPoolIds.size());
        return;
    }
