...
void MapManager::checkAndCorrectGridStatesArray()
{
    ...
    for (int i=0; i<MAX_GRID_STATE; i++)
    {
        ...
        {
            sLog->OutErrorConsole("MapManager::checkGridStates(), GridState: si_GridStates is currupt !!!");
            ok = false;
...
bool MapManager::CanPlayerEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    if (entry->IsRaid())
    {
        ...
        if ((!group || !group->isRaidGroup()) && !sWorld->getBoolConfig(CONFIG_INSTANCE_IGNORE_RAID))
        {
            ...
            player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetQuantumSystemText(LANG_INSTANCE_RAID_GROUP_ONLY), mapName);
	...
	if (sWorld->getBoolConfig(CONFIG_OUTDOOR_PVP_WINTERGRASP_ENABLED) && mapid==624)
	{
		if (!player->HasAura(58045))
		{
			player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetQuantumSystemText(LANG_INSTANCE_CLOSED), mapName);
			return false;
		}
	}
...
uint32 MapManager::GenerateInstanceId()
{
    ...
    {
        sLog->OutErrorConsole("Instance ID overflow!! Can't continue, shutting down server. ");
