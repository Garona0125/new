...
bool Map::ExistMap(uint32 mapid, int gx, int gy)
{
    ...
    if (!pf)
        sLog->OutErrorConsole("Map file '%s': does not exist!", tmp);
    else
    {
        ...
        if (fread(&header, sizeof(header), 1, pf) == 1)
        {
            if (header.mapMagic != MapMagic.asUInt || header.versionMagic != MapVersionMagic.asUInt)
                sLog->OutErrorConsole("Map file '%s' is from an incompatible clientversion. Please recreate using the mapextractor.", tmp);
            else
                ret = true;
        }
        fclose(pf);
    ...
    if (VMAP::IVMapManager* vmgr = VMAP::VMapFactory::createOrGetVMapManager())
    {
        if (vmgr->isMapLoadingEnabled())
        {
            ...
            if (!exists)
            {
                ...
                sLog->OutErrorConsole("VMap file '%s' is missing or points to wrong version of vmap file. Redo vmaps with latest version of vmap_assembler.exe.", (sWorld->GetDataPath()+"vmaps/"+name).c_str());
                return false;
            }
        }
    }
...
void Map::LoadMap(int gx, int gy, bool reload)
{
    ...
    {
        sLog->OutErrorConsole("Error loading map file: \n %s\n", tmp);
    }
    delete [] tmp;
...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    if (!p.IsCoordValid())
    {
        sLog->OutErrorConsole("Map::SwitchGridContainers: Object " UI64FMTD " has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
        return;
    }
...
{
    //TRINITY_GUARD(ACE_Thread_Mutex, Lock); test
...
bool Map::AddPlayerToMap(Player* player)
{
    ...
    {
        sLog->OutErrorConsole("Map::Add: Player (GUID: %u) has invalid coordinates X:%f Y:%f grid cell [%u:%u]", player->GetGUIDLow(), player->GetPositionX(), player->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
        return false;
    }
...
bool Map::AddToMap(T *obj)
{
    ...
    if (!cellCoord.IsCoordValid())
    {
        sLog->OutErrorConsole("Map::Add: Object " UI64FMTD " has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID(), obj->GetPositionX(), obj->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
...
void Map::PlayerRelocation(Player* player, float x, float y, float z, float orientation)
{
	...
	if (player->HasUnitMovementFlag(MOVEMENT_FLAG_HOVER))
...
void Map::CreatureRelocation(Creature* creature, float x, float y, float z, float ang, bool respawnRelocationOnFail)
{
	...
	//! Client automatically projects correct position based on Z coord sent in monster move
	//! and UNIT_FIELD_HOVERHEIGHT sent in object updates
	if (creature->HasUnitMovementFlag(MOVEMENT_FLAG_HOVER))
    ...
    if (HavePlayers())
    {
        for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
        {
            ...
            if (!player->IsBeingTeleportedFar())
            {
                ...
                sLog->OutErrorConsole("Map::UnloadAll: player %s is still in map %u during unload, this should not happen!", player->GetName(), GetId());
...
bool GridMap::loadData(char *filename)
{
    ...
    if (header.mapMagic == MapMagic.asUInt && header.versionMagic == MapVersionMagic.asUInt)
    {
        ...
        {
            sLog->OutErrorConsole("Error loading map area data\n");
            fclose(in);
            return false;
        }
        // loadup height data
        ...
        {
            sLog->OutErrorConsole("Error loading map height data\n");
            fclose(in);
            return false;
        }
        // loadup liquid data
        ...
        {
            sLog->OutErrorConsole("Error loading map liquids data\n");
            fclose(in);
            return false;
        }
        fclose(in);
    ...
    sLog->OutErrorConsole("Map file '%s' is from an incompatible clientversion. Please recreate using the mapextractor.", filename);
    fclose(in);
    return false;
}
...
inline void Map::setNGrid(NGridType *grid, uint32 x, uint32 y)
{
    ...
    {
        sLog->OutErrorConsole("map::setNGrid() Invalid grid coordinates found: %d, %d!", x, y);
        ASSERT(false);
    }
    i_grids[x][y] = grid;
}
...
void Map::RemoveAllObjectsInRemoveList()
{
    while (!i_objectsToSwitch.empty())
    {
        ...
        if (obj->GetTypeId() == TYPE_ID_UNIT && !obj->IsPermanentWorldObject())
    ...
    while (!i_objectsToRemove.empty())
    {
        ...
        switch (obj->GetTypeId())
        {
            case TYPE_ID_CORPSE:
            {
                ...
                if (!corpse)
                    sLog->OutErrorConsole("Tried to delete corpse/bones %u that is not in map.", obj->GetGUIDLow());
                else
        ...
        case TYPE_ID_UNIT:
            ...
            break;
        default:
            sLog->OutErrorConsole("Non-grid object (TypeId: %u) is in grid object remove list, ignored.", obj->GetTypeId());
            break;
        }
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->IsPet() && c->GetDBTableGUIDLow())
    {
        ...
        else
        {
            ...
            sLog->OutErrorConsole("Active creature (GUID: %u Entry: %u) added to grid[%u, %u] but spawn grid[%u, %u] was not loaded.",
...
void Map::RemoveFromActive(Creature* c)
{
    ...
    if (!c->IsPet() && c->GetDBTableGUIDLow())
    {
        ...
        {
            GridCoord p2 = Trinity::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
            sLog->OutErrorConsole("Active creature (GUID: %u Entry: %u) removed from grid[%u, %u] but spawn grid[%u, %u] was not loaded.",
                c->GetGUIDLow(), c->GetEntry(), p.x_coord, p.y_coord, p2.x_coord, p2.y_coord);
        }
    }
}
...
bool InstanceMap::CanEnter(Player* player)
{
    ...
    {
        sLog->OutErrorConsole("InstanceMap::CanEnter - player %s(%u) already in map %d, %d, %d!", player->GetName(), player->GetGUIDLow(), GetId(), GetInstanceId(), GetSpawnMode());
        ASSERT(false);
        return false;
    }
            ...
            else
            {
                if (group)
                {
                    ...
                    if (playerBind && playerBind->save != mapSave)
                    {
                        ...
                        if (groupBind)
                            sLog->OutErrorConsole("InstanceMap::Add: the group is bound to the instance %d, %d, %d, %d, %d, %d", groupBind->save->GetMapId(), groupBind->save->GetInstanceId(), groupBind->save->GetDifficulty(), groupBind->save->GetPlayerCount(), groupBind->save->GetGroupCount(), groupBind->save->CanReset());
                        //ASSERT(false);
                        return false;
                    }
                    ...
                    else
                    {
                        ...
                        if (groupBind->save != mapSave)
                        {
                            ...
                            if (mapSave)
                                sLog->OutErrorConsole("MapSave players: %d, group count: %d", mapSave->GetPlayerCount(), mapSave->GetGroupCount());
                            else
                                sLog->OutErrorConsole("MapSave NULL");
                            if (groupBind->save)
                                sLog->OutErrorConsole("GroupBind save players: %d, group count: %d", groupBind->save->GetPlayerCount(), groupBind->save->GetGroupCount());
                            else
                                sLog->OutErrorConsole("GroupBind save NULL");
                            return false;
                        }
...
void InstanceMap::PermBindAllPlayers(Player* source)
{
    ...
    if (!save)
    {
        sLog->OutErrorConsole("Cannot bind player (GUID: %u, Name: %s), because no instance save is available for instance map (Name: %s, Entry: %u, InstanceId: %u)!", source->GetGUIDLow(), source->GetName(), source->GetMap()->GetMapName(), source->GetMapId(), GetInstanceId());
        return;
    }
...
void InstanceMap::SetResetSchedule(bool on)
{
    ...
    if (IsDungeon() && !HavePlayers() && !IsRaidOrHeroicDungeon())
    {
        if (InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(GetInstanceId()))
			...
			sLog->OutConsole("InstanceMap::SetResetSchedule: cannot turn schedule %s, there is no save information for instance (map [id: %u, name: %s], instance id: %u, difficulty: %u)",
...
bool BattlegroundMap::CanEnter(Player* player)
{
    if (player->GetMapRef().getTarget() == this)
    {
        sLog->OutErrorConsole("BGMap::CanEnter - player %u is already in map!", player->GetGUIDLow());
        ASSERT(false);
        return false;
    }
