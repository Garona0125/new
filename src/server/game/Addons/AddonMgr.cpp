...
void LoadFromDB()
{
    ...
    if (result)
    {
		...
		sLog->OutConsole("====================================");
		sLog->OutConsole(">> Loaded %u known addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("====================================");
		sLog->OutConsole();
    }
    else
	{
		sLog->OutConsole("=====================================================");
		sLog->OutConsole(">> Loaded 0 known addons. DB table `addons` is empty!");
		sLog->OutConsole("=====================================================");
		sLog->OutConsole();
	}
    ...
    if (result)
    {
		...
		sLog->OutConsole("=====================================");
        sLog->OutConsole(">> Loaded %u banned addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("=====================================");
		sLog->OutConsole();
    }
}
