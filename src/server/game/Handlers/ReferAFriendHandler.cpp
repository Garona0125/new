...
void WorldSession::HandleGrantLevel(WorldPacket& recvData)
{
    ...
    else if (target->GetCurrentLevel() >= _player->GetCurrentLevel())
        error = ERR_REFER_A_FRIEND_TARGET_TOO_HIGH;
    else if (target->GetCurrentLevel() >= sWorld->getIntConfig(CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL))
...
void WorldSession::HandleAcceptGrantLevel(WorldPacket& recvData)
{
    ...
    _player->GiveLevel(_player->GetCurrentLevel() + 1);
}