...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (proto->InventoryType != INVENTORY_TYPE_NON_EQUIP && !pItem->IsEquipped())
    {
        pUser->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, pItem, NULL);
        return;
    }
...
void WorldSession::HandleOpenItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (!(proto->Flags & ITEM_PROTO_FLAG_OPENABLE) && !item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED))
    {
        ...
        sLog->OutErrorConsole("Possible hacking attempt: Player %s [guid: %u] tried to open item [guid: %u, entry: %u] which is not openable!",
    ...
    if (lockId)
    {
        ...
        if (!lockInfo)
        {
            ...
            sLog->OutErrorConsole("WORLD::OpenItem: item [guid = %u] has an unknown lockId: %u!", item->GetGUIDLow(), lockId);
            return;
        }
    ...
    if (item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED))// wrapped?
    {
        ...
        {
            sLog->OutErrorConsole("Wrapped item %u don't have record in character_gifts table and will deleted", item->GetGUIDLow());
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    if (mover != _player && mover->GetTypeId() == TYPE_ID_PLAYER)
    {
    ...
    if (!spellInfo)
    {
        sLog->OutErrorConsole("WORLD: unknown spell id %u", spellId);
        recvPacket.rfinish(); // prevent spam at ignore packet
        return;
    }

    if (mover->GetTypeId() == TYPE_ID_PLAYER)
    {
    ...
    {
        // not have spell in spellbook or spell passive and not casted by client
        if ((mover->GetTypeId() == TYPE_ID_UNIT && !mover->ToCreature()->HasSpell(spellId)) || spellInfo->IsPassive())
        {
            Unit* charmer = mover->GetCharmer();
            if (mover->IsVehicle() && charmer && charmer->GetTypeId() == TYPE_ID_PLAYER)
            {
    ...
    {
        SpellInfo const* actualSpellInfo = spellInfo->GetAuraRankForLevel(targets.GetUnitTarget()->GetCurrentLevel());
...
void WorldSession::HandleCancelAuraOpcode(WorldPacket& recvPacket)
{
	...
	if (sWorld->getBoolConfig(CONFIG_ALLOW_TRACK_BOTH_RESOURCES) && spellInfo->HasAura(SPELL_AURA_TRACK_RESOURCES))
	{
		...
		{
			// Build list of spell IDs to cancel. Trying to cancel the aura while iterating over AuraEffectList caused "incompatible iterator" errors on second pass
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket& recvPacket)
{
    ...
    if (!spellInfo)
    {
        sLog->OutErrorConsole("WORLD: unknown PET spell id %u", spellId);
        return;
    }
    ...
    if (!pet)
    {
        sLog->OutErrorConsole("HandlePetCancelAura: Attempt to cancel an aura for non-existant pet %u by player '%s'", uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
        return;
    }

    if (pet != GetPlayer()->GetGuardianPet() && pet != GetPlayer()->GetCharm())
    {
        sLog->OutErrorConsole("HandlePetCancelAura: Pet %u is not a pet of player '%s'", uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    data << uint8(creator->GetCurrentRace());
    data << uint8(creator->GetCurrentGender());
    data << uint8(creator->GetCurrentClass());

    if (creator->GetTypeId() == TYPE_ID_PLAYER)
    {
...
void WorldSession::HandleUpdateProjectilePosition(WorldPacket& recvPacket)
{
	...
	Position pos = *spell->m_targets.GetDst();
	pos.Relocate(x, y, z);
