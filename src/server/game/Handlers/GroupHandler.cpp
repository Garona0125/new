...
void WorldSession::HandleGroupAcceptOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("HandleGroupAcceptOpcode: player %s(%d) tried to accept an invite to his own group", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleGroupUninviteGuidOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("WorldSession::HandleGroupUninviteGuidOpcode: leader %s(%d) tried to uninvite himself from the group.", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleGroupUninviteOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("WorldSession::HandleGroupUninviteOpcode: leader %s(%d) tried to uninvite himself from the group.", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
        return;
    }
...
void WorldSession::HandlePartyAssignmentOpcode(WorldPacket& recvData)
{
    ...
    switch (assignment)
    {
        case GROUP_ASSIGN_MAIN_ASSIST:
            group->RemoveUniqueGroupMemberFlag(MEMBER_FLAG_MAIN_ASSIST);
            group->SetGroupMemberFlag(guid, apply, MEMBER_FLAG_MAIN_ASSIST);
            break;
        case GROUP_ASSIGN_MAIN_TANK:
            group->RemoveUniqueGroupMemberFlag(MEMBER_FLAG_MAIN_TANK);           // Remove main assist flag from current if any.
            group->SetGroupMemberFlag(guid, apply, MEMBER_FLAG_MAIN_TANK);
        default:
            break;
    }
...
void WorldSession::BuildPartyMemberStatsChangedPacket(Player* player, WorldPacket* data)
{
    ...
    *data << (uint32) mask;
    ...
    if (mask & GROUP_UPDATE_FLAG_LEVEL)
        *data << (uint16) player->GetCurrentLevel();
...
void WorldSession::HandleRequestPartyMemberStatsOpcode(WorldPacket &recvData)
{
    ...
    data << (uint16) player->GetCurrentLevel();             // GROUP_UPDATE_FLAG_LEVEL
...
void WorldSession::HandleOptOutOfLootOpcode(WorldPacket& recvData)
{
    ...
    {
        if (passOnLoot != 0)
            sLog->OutErrorConsole("CMSG_OPT_OUT_OF_LOOT value<>0 for not-loaded character!");
        return;
    }
