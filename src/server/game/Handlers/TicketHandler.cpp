...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket& recvData)
{
    ...
    if (GetPlayer()->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ))
    {
        SendNotification(GetQuantumSystemText(LANG_TICKET_REQ), sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ));
        return;
    }
    ...
    if (!ticket || ticket->IsClosed())
    {
        ...
        if (count && decompressedSize && decompressedSize < 0xFFFF)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_GMTICKET_CREATE possibly corrupt. Uncompression failed.");
                recvData.rfinish();
                return;
            }
