...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    if (!classEntry)
    {
        ...
        SendPacket(&data);
        sLog->OutErrorConsole("Class (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", class_, GetAccountId());
        return;
    }
    ...
    if (!raceEntry)
    {
        data << (uint8)CHAR_CREATE_FAILED;
        SendPacket(&data);
        sLog->OutErrorConsole("Race (%u) not found in DBC while creating new char for account (ID: %u): wrong DBC files or cheater?", race_, GetAccountId());
        return;
    }
    ...
    if (raceEntry->expansion > Expansion())
    {
        ...
        sLog->OutErrorConsole("Expansion %u account:[%d] tried to Create character with expansion %u race (%u)", Expansion(), GetAccountId(), raceEntry->expansion, race_);
        SendPacket(&data);
        return;
    }
    ...
    if (classEntry->expansion > Expansion())
    {
        ...
        sLog->OutErrorConsole("Expansion %u account:[%d] tried to Create character with expansion %u class (%u)", Expansion(), GetAccountId(), classEntry->expansion, class_);
        SendPacket(&data);
        return;
    }
    ...
    if (!normalizePlayerName(name))
    {
        ...
        SendPacket(&data);
        sLog->OutErrorConsole("Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
        return;
    }
    ...
    */
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 3:
        {
            ...
            sLog->OutCharacters("Account: %d (IP: %s) Create Character:[%s] (GUID: %u)", GetAccountId(), IP_str.c_str(), createInfo->Name.c_str(), newChar.GetGUIDLow());
            sScriptMgr->OnPlayerCreate(&newChar);
			sWorld->AddCharacterNameData(newChar.GetGUIDLow(), std::string(newChar.GetName()), newChar.GetCurrentGender(), newChar.GetCurrentRace(), newChar.GetCurrentClass(), newChar.GetCurrentLevel());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    sLog->OutCharacters("Account: %d, IP: %s deleted character: %s, GUID: %u, Level: %u", accountId, IP_str.c_str(), name.c_str(), GUID_LOPART(guid), level);
	sScriptMgr->OnPlayerDelete(guid, name.c_str(), this);
    sWorld->DeleteCharaceterNameData(GUID_LOPART(guid));

    if (sLog->IsOutCharactersDump())                                // optimize GetPlayerDump call
    {
        std::string dump;
        if (PlayerDumpWriter().GetDump(GUID_LOPART(guid), dump))
            sLog->OutCharactersDump(dump.c_str(), accountId, GUID_LOPART(guid), name.c_str());
    }
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("Player tryes to login again, AccountId = %d", GetAccountId());
        return;
    }
    ...
    {
        sLog->OutErrorConsole("Account (%u) can't login with that character (%u).", GetAccountId(), GUID_LOPART(playerGuid));
        KickPlayer();
        return;
    }
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    if (pCurrChar->GetGuildId() != 0)
    {
        ...
        else
        {
            ...
            sLog->OutErrorConsole("Player %s (GUID: %u) marked as member of not existing guild (id: %u), removing guild membership for player.", pCurrChar->GetName(), pCurrChar->GetGUIDLow(), pCurrChar->GetGuildId());
            pCurrChar->SetInGuild(0);
        }
    }
    ...
    if (!pCurrChar->getCinematic())
    {
        ...
        if (ChrClassesEntry const* cEntry = sChrClassesStore.LookupEntry(pCurrChar->GetCurrentClass()))
        {
            if (cEntry->CinematicSequence)
                pCurrChar->SendCinematicStart(cEntry->CinematicSequence);
            else if (ChrRacesEntry const* rEntry = sChrRacesStore.LookupEntry(pCurrChar->GetCurrentRace()))
    ...
    if (pCurrChar->m_deathState != ALIVE)
    {
        ...
        if (pCurrChar->GetCurrentRace() == RACE_NIGHT_ELF)
    ...
    std::string IP_str = GetRemoteAddress();
	sLog->OutCharacters("Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d",
		GetAccountId(), IP_str.c_str(), pCurrChar->GetName(), pCurrChar->GetGUIDLow(), pCurrChar->GetCurrentLevel());
...
{
    sLog->OutErrorConsole("WORLD SESSION: HandleSetFactionCheat, not expected call, please report.");
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string newName)
{
    ...
    sLog->OutCharacters("Account: %d (IP: %s) Character:[%s] (guid:%u) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), guidLow, newName.c_str());
...
void WorldSession::HandleAlterAppearance(WorldPacket& recvData)
{
    ...
    if (!bs_hair || bs_hair->type != 0 || bs_hair->race != _player->GetCurrentRace() || bs_hair->gender != _player->GetCurrentGender())
        return;

    BarberShopStyleEntry const* bs_facialHair = sBarberShopStyleStore.LookupEntry(FacialHair);

    if (!bs_facialHair || bs_facialHair->type != 2 || bs_facialHair->race != _player->GetCurrentRace() || bs_facialHair->gender != _player->GetCurrentGender())
        return;

    BarberShopStyleEntry const* bs_skinColor = sBarberShopStyleStore.LookupEntry(SkinColor);

    if (bs_skinColor && (bs_skinColor->type != 3 || bs_skinColor->race != _player->GetCurrentRace() || bs_skinColor->gender != _player->GetCurrentGender()))
        return;
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
	...
	if (result)
    {
		...
		sLog->OutCharacters("Account: %d (IP: %s), Character[%s] (guid:%u) Customized to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldname.c_str(), GUID_LOPART(guid), newName.c_str());
    }
