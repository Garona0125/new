...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        uint8 lvl = itr->second->GetCurrentLevel();
        if (lvl < level_min || lvl > level_max)
            continue;

        // check if class matches classmask
        uint32 class_ = itr->second->GetCurrentClass();
        if (!(classmask & (1 << class_)))
            continue;

        // check if race matches racemask
        uint32 race = itr->second->GetCurrentRace();
        if (!(racemask & (1 << race)))
            continue;

        uint32 pzoneid = itr->second->GetZoneId();
        uint8 gender = itr->second->GetCurrentGender();
...
void WorldSession::HandleAddFriendOpcode(WorldPacket& recvData)
{
    ...
    std::string friendName = GetQuantumSystemText(LANG_FRIEND_IGNORE_UNKNOWN);
...
void WorldSession::HandleAddIgnoreOpcode(WorldPacket& recvData)
{
    ...
    std::string ignoreName = GetQuantumSystemText(LANG_FRIEND_IGNORE_UNKNOWN);
...
void WorldSession::HandleUpdateAccountData(WorldPacket &recvData)
{
    ...
    if (decompressedSize > 0xFFFF)
    {
        ...
        sLog->OutErrorConsole("UAD: Account data packet too big, size %u", decompressedSize);
        return;
    }
    ...
    {
        recvData.rfinish();                   // unnneded warning spam in this case
        sLog->OutErrorConsole("UAD: Failed to decompress account data");
        return;
    }
    ...
    else
    {
        switch (type)
        {
            ...
            case ACTION_BUTTON_ITEM:
                ...
                break;
            default:
                sLog->OutErrorConsole("MISC: Unknown action button type %u for action %u into button %u for player %s (GUID: %u)", type, action, button, _player->GetName(), _player->GetGUIDLow());
                return;
        }
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recvData)
{
    ...
    {
        if (ActionBar != 0)
            sLog->OutErrorConsole("WorldSession::HandleSetActionBarToggles in not logged state with value: %u, ignored", uint32(ActionBar));
        return;
    }
...
void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
    ...
    if (apply)
    {
        ...
        if (WorldObject* target = _player->GetViewpoint())
            ...
            sLog->outDebug(LOG_FILTER_NETWORKIO, "Player %s requests non-existing seer " UI64FMTD, _player->GetName(), _player->GetUInt64Value(PLAYER_FARSIGHT));
    }
    else
    {
...
void WorldSession::HandleSetDungeonDifficultyOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("WorldSession::HandleSetDungeonDifficultyOpcode: player %d sent an invalid instance mode %d!", _player->GetGUIDLow(), mode);
        return;
    }
    ...
    {
        sLog->OutErrorConsole("WorldSession::HandleSetDungeonDifficultyOpcode: player (Name: %s, GUID: %u) tried to reset the instance while player is inside!", _player->GetName(), _player->GetGUIDLow());
        return;
    }
    ...
    if (group)
    {
        if (group->IsLeader(_player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
					sLog->OutErrorConsole("WorldSession::HandleSetDungeonDifficultyOpcode: player %d tried to reset the instance while group member (Name: %s, GUID: %u) is inside!", _player->GetGUIDLow(), groupGuy->GetName(), groupGuy->GetGUIDLow());
                    return;
                }
            }
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("WorldSession::HandleSetRaidDifficultyOpcode: player %d sent an invalid instance mode %d!", _player->GetGUIDLow(), mode);
        return;
    }
    ...
    if (map && map->IsDungeon())
    {
        sLog->OutErrorConsole("WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while inside!", _player->GetGUIDLow());
        return;
    }
...
{
	sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: CMSG_SET_TAXI_BENCHMARK_MODE");
...
void WorldSession::HandleUpdateMissileTrajectory(WorldPacket& recvPacket)
{
	...
	Position pos = *spell->m_targets.GetSrc();
    pos.Relocate(curX, curY, curZ);
    spell->m_targets.ModSrc(pos);

    pos = *spell->m_targets.GetDst();
