...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket& recvData)
{
    ...
    else
    {
        ...
        if (_player->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& recvData)
{
    ...
    if (!result)
    {
        sLog->OutErrorConsole("Petition %u is not found for player %u %s", GUID_LOPART(petitionGuid), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket& recvData)
{
    ...
    if (type != GUILD_CHARTER_TYPE)
    {
        if (player->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("Player %s (guid: %u) tried to turn in petition (guid: %u) that is not present in the database", _player->GetName(), _player->GetGUIDLow(), GUID_LOPART(petitionGuid));
        return;
    }
