...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("Battleground: invalid bgtype (%u) received. possible cheater? player guid %u", bgTypeId_, _player->GetGUIDLow());
        return;
    }
    ...
    PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(bg->GetMapId(), _player->GetCurrentLevel());
    if (!bracketEntry)
        return;
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    {
        sLog->OutErrorConsole("BattlegroundHandler: itrplayerstatus not found.");
        return;
    }
    // if action == 1, then instanceId is required
    if (!ginfo.IsInvitedToBGInstanceGUID && action == 1)
    {
        sLog->OutErrorConsole("BattlegroundHandler: instance not found.");
        return;
    }
    ...
    if (!bg)
    {
        sLog->OutErrorConsole("BattlegroundHandler: bg_template not found for type id %u.", bgTypeId);
        return;
    }
    ...
    if (action == 1 && ginfo.ArenaType == 0)
    {
        ...
        if (_player->GetCurrentLevel() > bg->GetMaxLevel())
        {
            sLog->OutErrorConsole("Battleground: Player %s (%u) has level (%u) higher than maxlevel (%u) of battleground (%u)! Do not port him to battleground!",
                _player->GetName(), _player->GetGUIDLow(), _player->GetCurrentLevel(), bg->GetMaxLevel(), bg->GetTypeID());
            action = 0;
        }
    }
    ...
    switch (action)
    {
		...
		case 0:                                         // leave queue
            ...
            break;
        default:
            sLog->OutErrorConsole("Battleground port: unknown action %u", action);
            break;
    }
}
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recvData)
{
    ...
    switch (arenaslot)
    {
        ...
        case 2:
            ...
            break;
        default:
            sLog->OutErrorConsole("Unknown arena slot %u at HandleBattlemasterJoinArena()", arenaslot);
            return;
    }
    ...
    if (!bg)
    {
        sLog->OutErrorConsole("Battleground: template bg (all arenas) not found");
        return;
    }
