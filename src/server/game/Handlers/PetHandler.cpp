...
void WorldSession::HandleDismissCritter(WorldPacket &recvData)
{
    ...
    {
         if (pet->GetTypeId() == TYPE_ID_UNIT && pet->ToCreature()->IsSummon())
...
void WorldSession::HandlePetAction(WorldPacket& recvData)
{
    ...
    if (!pet)
    {
        sLog->OutErrorConsole("HandlePetAction: Pet (GUID: %u) doesn't exist for player '%s'", uint32(GUID_LOPART(guid1)), GetPlayer()->GetName());
        return;
    }

    if (pet != GetPlayer()->GetFirstControlled())
    {
        sLog->OutErrorConsole("HandlePetAction: Pet (GUID: %u) does not belong to player '%s'", uint32(GUID_LOPART(guid1)), GetPlayer()->GetName());
        return;
    }
    ...
    if (pet->GetTypeId() == TYPE_ID_PLAYER && !(flag == ACT_COMMAND && spellid == COMMAND_ATTACK))
        return;
...
void WorldSession::HandlePetStopAttack(WorldPacket &recvData)
{
    ...
    if (!pet)
    {
        sLog->OutErrorConsole("HandlePetStopAttack: Pet %u does not exist", uint32(GUID_LOPART(guid)));
        return;
    }

    if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharm())
    {
        sLog->OutErrorConsole("HandlePetStopAttack: Pet GUID %u isn't a pet or charmed creature of player %s", uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
        return;
    }
...
void WorldSession::HandlePetActionHelper(Unit* pet, uint64 guid1, uint16 spellid, uint16 flag, uint64 guid2)
{
    ...
    if (!charmInfo)
    {
        //sLog->OutErrorConsole("WorldSession::HandlePetAction(petGuid: " UI64FMTD ", tagGuid: " UI64FMTD ", spellId: %u, flag: %u): object (entry: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
		sLog->OutConsole("WorldSession::HandlePetAction(petGuid: " UI64FMTD ", tagGuid: " UI64FMTD ", spellId: %u, flag: %u): object (entry: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
    ...
    switch (flag)
    {
            ...
            switch (spellid)
            {
                ...
                case COMMAND_ATTACK: // spellid=1792  //ATTACK
                {
                    ...
                    if (pet->GetVictim() != TargetUnit || (pet->GetVictim() == TargetUnit && !pet->GetCharmInfo()->IsCommandAttack()))
                    {
                        ...
                        if (pet->GetTypeId() != TYPE_ID_PLAYER && pet->ToCreature()->IsAIEnabled)
                        {
                ...
                case COMMAND_ABANDON:                       // abandon (hunter pet) or dismiss (summoned pet)
                    ...
                    {
                        ASSERT(pet->GetTypeId() == TYPE_ID_UNIT);
                        if (pet->IsPet())
                        {
                    ...
                    break;
                default:
                    sLog->OutErrorConsole("WORLD: unknown PET flag Action %i and spellid %i.", uint32(flag), spellid);
            }
            break;
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            if (!spellInfo)
            {
                sLog->OutErrorConsole("WORLD: unknown PET spell id %i", spellid);
                return;
            }
            ...
            if (result == SPELL_FAILED_UNIT_NOT_INFRONT && !pet->IsPossessed() && !pet->IsVehicle())
            {
                if (unit_target)
                {
                    ...
                    if (unit_target->GetTypeId() == TYPE_ID_PLAYER)
                        pet->SendUpdateToPlayer((Player*)unit_target);
                }
                else if (Unit* unit_target2 = spell->m_targets.GetUnitTarget())
                {
                    pet->SetInFront(unit_target2);
                    if (unit_target2->GetTypeId() == TYPE_ID_PLAYER)
                        pet->SendUpdateToPlayer((Player*)unit_target2);
                }

                if (Unit* powner = pet->GetCharmerOrOwner())
                    if (powner->GetTypeId() == TYPE_ID_PLAYER)
...
void WorldSession::SendPetNameQuery(uint64 petguid, uint32 petnumber)
{
    ...
    data << uint32(petnumber);
    data << name.c_str();
    data << uint32(pet->GetUInt32Value(UNIT_FIELD_PET_NAME_TIME_STAMP));
...
bool WorldSession::CheckStableMaster(uint64 guid)
{
    ...
    {
        if (!GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_STABLE_MASTER))
        {
...
void WorldSession::HandlePetSetAction(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("HandlePetSetAction: Unknown pet (GUID: %u) or pet owner (GUID: %u)", GUID_LOPART(petguid), _player->GetGUIDLow());
        return;
    }
    ...
    if (!charmInfo)
    {
        sLog->OutErrorConsole("WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        if (!((act_state == ACT_ENABLED || act_state == ACT_DISABLED || act_state == ACT_PASSIVE) && spell_id && !pet->HasSpell(spell_id)))
        {
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell_id))
            {
                ...
                {
                    if (pet->GetTypeId() == TYPE_ID_UNIT && pet->ToCreature()->IsPet())
...
void WorldSession::HandlePetRename(WorldPacket& recvData)
{
    ...
    if (isdeclined)
    {
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_PET_DECLINED_NAME);
        stmt->setUInt32(0, pet->GetCharmInfo()->GetPetNumber());
        trans->Append(stmt);

        stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_PET_DECLINED_NAME);
    ...
    pet->SetUInt32Value(UNIT_FIELD_PET_NAME_TIME_STAMP, uint32(time(NULL))); // cast can't be helped
}
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPacket& recvPacket)
{
    ...
    {
        sLog->OutErrorConsole("HandlePetSpellAutocastOpcode.Pet %u isn't pet of player %s .", uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
        return;
    }
    ...
    if (!charmInfo)
    {
        sLog->OutErrorConsole("WorldSession::HandlePetSpellAutocastOpcod: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!", pet->GetGUIDLow(), pet->GetTypeId());
        return;
    }
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    {
        sLog->OutErrorConsole("HandlePetCastSpellOpcode: Pet %u isn't pet of player %s .", uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
        return;
    }
    ...
    if (spellInfo->StartRecoveryCategory > 0) // Check if spell is affected by GCD
        if (caster->GetTypeId() == TYPE_ID_UNIT && caster->GetCharmInfo() && caster->GetCharmInfo()->GetGlobalCooldownMgr().HasGlobalCooldown(spellInfo))
        {
