...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    if (!GetPlayer()->GetGameObjectIfCanInteractWith(mailbox, GAMEOBJECT_TYPE_MAIL_BOX))
        return;
    ...
    if (player->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ))
    {
        SendNotification(GetQuantumSystemText(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    if (receive)
    {
        ...
        receiveLevel = receive->GetCurrentLevel();
    }
    else
    {
    ...
    {
        SendNotification(GetQuantumSystemText(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
            ...
            if (!AccountMgr::IsPlayerAccount(GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sObjectMgr->GetPlayerNameByGUID(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_UNKNOWN);
                }
