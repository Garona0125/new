...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("CHAT: Wrong message type received: %u", type);
        recvData.rfinish();
        return;
    }
    ...
    if (lang == LANG_ADDON)
    {
        ...
        switch (type)
        {
            ...
            case CHAT_MSG_WHISPER:
                ...
                break;
            default:
                sLog->outDebug(LOG_FILTER_NETWORKIO, "Player %s (GUID: %u) sent a chatmessage with an invalid language/message type combination", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
                recvData.rfinish();
                return;
        }
    }
    ...
    else
    {
        ...
        if (!sender->CanSpeak())
        {
            ...
            SendNotification(GetQuantumSystemText(LANG_WAIT_BEFORE_SPEAKING), timeStr.c_str());
    ...
    {
		SendNotification(GetQuantumSystemText(LANG_GM_SILENCE), sender->GetName());
		recvData.rfinish();
        return;
    }
    ...
    if (!ignoreChecks)
    {
        ...
        if (lang != LANG_ADDON)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_NETWORKIO, "Player %s (GUID: %u) sent a chatmessage with an invalid link: %s", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow(), msg.c_str());
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
			if (sender->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ))
			{
				SendNotification(GetQuantumSystemText(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
				return;
			}

			// Antispam
			if (sender->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_SAY_TIME_REQ) && !_player->IsGameMaster())
			{
				SendNotification(GetQuantumSystemText(LANG_CHANNEL_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_TIME_REQ)/60);
				return;
			} // Antispam End
        ...
        case CHAT_MSG_WHISPER:
        {
            if (sender->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ))
            {
                SendNotification(GetQuantumSystemText(LANG_WHISPER_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }

			// Antispam
			if (sender->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_WHISPER_TIME_REQ) && !_player->IsGameMaster())
			{
				SendNotification(GetQuantumSystemText(LANG_CHANNEL_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_TIME_REQ)/60);
				return;
			} // End Antispam
            ...
            {
				SendNotification(GetQuantumSystemText(LANG_GM_SILENCE), GetPlayer()->GetName());
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            ...
            {
                if (_player->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ))
                {
                    SendNotification(GetQuantumSystemText(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                    return;
                }
				...
				{
					SendNotification(GetQuantumSystemText(LANG_CHANNEL_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_TIME_REQ)/60);
					return;
				} // End Antispam
			}
		...
		case CHAT_MSG_AFK:
        {
            if (!_player->IsInCombatActive())
            {
                ...
                {
                    _player->autoReplyMsg = msg.empty() ? GetQuantumSystemText(LANG_PLAYER_AFK_DEFAULT) : msg;
		...
		case CHAT_MSG_DND:
        {
            ...
            {
                _player->autoReplyMsg = msg.empty() ? GetQuantumSystemText(LANG_PLAYER_DND_DEFAULT) : msg;
		...
		default:
            sLog->OutErrorConsole("CHAT: unknown message type %u, lang: %u", type, lang);
            break;
    }
...
namespace Trinity
{
    ...
    {
        public:
            EmoteChatBuilder(Player const& player, uint32 text_emote, uint32 emote_num, Unit const* target): i_player(player), i_text_emote(text_emote), i_emote_num(emote_num), i_target(target) {}
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData)
{
    ...
    Trinity::PlayerDistWorker<Trinity::LocalizedPacketDo<Trinity::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXT_EMOTE), emote_do);
    TypeContainerVisitor<Trinity::PlayerDistWorker<Trinity::LocalizedPacketDo<Trinity::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
    cell.Visit(p, message, *GetPlayer()->GetMap(), *GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXT_EMOTE));

    GetPlayer()->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_DO_EMOTE, text_emote, 0, unit);

    //Send scripted event call
    if (unit && unit->GetTypeId() == TYPE_ID_UNIT && ((Creature*)unit)->AI())
