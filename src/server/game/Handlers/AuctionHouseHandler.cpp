...
{
    if (GetPlayer()->GetCurrentLevel() < sWorld->getIntConfig(CONFIG_AUCTION_LEVEL_REQ))
    {
        SendNotification(GetQuantumSystemText(LANG_AUCTION_REQ), sWorld->getIntConfig(CONFIG_AUCTION_LEVEL_REQ));
        return;
    }
...
void WorldSession::HandleAuctionSellItem(WorldPacket& recvData)
{
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        else // Required stack size of auction does not match to current item stack size, clone item and set correct stack size
        {
            ...
            if (!newItem)
            {
                sLog->OutErrorConsole("CMSG_AUCTION_SELL_ITEM: Could not create clone of item %u", item->GetEntry());
                SendAuctionCommandResult(0, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
                return;
            }
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket& recvData)
{
    ...
    if (auction && auction->owner == player->GetGUIDLow())
    {
        ...
        {
            sLog->OutErrorConsole("Auction id: %u has non-existed item (item guid : %u)!!!", auction->Id, auction->itemGUIDLow);
    ...
    else
    {
        ...
        sLog->OutErrorConsole("CHEATER : %u, he tried to cancel auction (id: %u) of another player, or auction is NULL", player->GetGUIDLow(), auctionId);
        return;
    }
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("Client sent bad opcode!!! with count: %u and size : %lu (must be: %u)", outbiddedCount, (unsigned long)recvData.size(), (16 + outbiddedCount * 4));
        outbiddedCount = 0;
    }
