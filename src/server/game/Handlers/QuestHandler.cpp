...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket& recvData)
{
    ...
    Object* questgiver = ObjectAccessor::GetObjectByTypeMask(*_player, guid, TYPE_MASK_UNIT | TYPE_MASK_GAMEOBJECT);
    if (!questgiver)
    {
    ...
    switch (questgiver->GetTypeId())
    {
        ...
        default:
            sLog->OutErrorConsole("QuestGiver called for unexpected type %u", questgiver->GetTypeId());
            break;
    }
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket& recvData)
{
    ...
    Object* object = ObjectAccessor::GetObjectByTypeMask(*_player, guid, TYPE_MASK_UNIT | TYPE_MASK_GAMEOBJECT | TYPE_MASK_ITEM | TYPE_MASK_PLAYER);

    // no or incorrect quest giver
    if (!object || (object->GetTypeId() != TYPE_ID_PLAYER && !object->HasQuest(questId)) ||
        (object->GetTypeId() == TYPE_ID_PLAYER && object != _player && !object->ToPlayer()->CanShareQuest(questId)))
    {
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    Object* object = ObjectAccessor::GetObjectByTypeMask(*_player, guid, TYPE_MASK_UNIT | TYPE_MASK_GAMEOBJECT | TYPE_MASK_ITEM);
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutErrorConsole("Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player %s (guid %d) tried to get invalid reward (%u) (probably packet hacking)", _player->GetName(), _player->GetGUIDLow(), reward);
        return;
    }
    ...
    Object* object = ObjectAccessor::GetObjectByTypeMask(*_player, guid, TYPE_MASK_UNIT | TYPE_MASK_GAMEOBJECT);
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        ...
        {
            sLog->OutErrorConsole("HACK ALERT: Player %s (guid: %u) is trying to complete quest (id: %u) but he has no right to do it!",
...
void WorldSession::HandleQuestConfirmAccept(WorldPacket& recvData)
{
    ...
    if (const Quest* quest = sObjectMgr->GetQuestTemplate(questId))
    {
		...
		if (!pOriginalPlayer->CanShareQuest(questId))
			return;
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recvData)
{
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        if (!_player->CanSeeStartQuest(quest) && _player->GetQuestStatus(questId) == QUEST_STATUS_NONE)
        {
            sLog->OutErrorConsole("Possible hacking attempt: Player %s [guid: %u] tried to complete quest [entry: %u] without being in possession of the quest!",
                          _player->GetName(), _player->GetGUIDLow(), questId);
            return;
        }
...
uint32 WorldSession::getDialogStatus(Player* player, Object* questgiver, uint32 defstatus)
{
    ...
    switch (questgiver->GetTypeId())
    {
        ...
        default:
            ...
            sLog->OutErrorConsole("Warning: GetDialogStatus called for unexpected type %u", questgiver->GetTypeId());
            ...
            if (player->CanSeeStartQuest(quest))
            {
                if (player->SatisfyQuestLevel(quest, false))
                {
                    if (quest->IsAutoComplete() || (quest->IsRepeatable() && player->IsQuestRewarded(quest_id)))
                        result2 = DIALOG_STATUS_REWARD_REP;
                    else if (player->GetCurrentLevel() <= ((player->GetQuestLevel(quest) == -1) ? player->GetCurrentLevel() : player->GetQuestLevel(quest) + sWorld->getIntConfig(CONFIG_QUEST_LOW_LEVEL_HIDE_DIFF)))
                    {
    ...
    for (Player::ClientGUIDs::const_iterator itr = _player->m_clientGUIDs.begin(); itr != _player->m_clientGUIDs.end(); ++itr)
    {
        ...
        if (IS_CRE_OR_VEH_OR_PET_GUID(*itr))
        {
            ...
            if (!questgiver || questgiver->IsHostileTo(_player))
                continue;
            if (!questgiver->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUEST_GIVER))
                continue;
        ...
        else if (IS_GAMEOBJECT_GUID(*itr))
        {
            ...
            if (!questgiver)
                continue;
            if (questgiver->GetGoType() != GAMEOBJECT_TYPE_QUEST_GIVER)
                continue;
            questStatus = sScriptMgr->GetDialogStatus(_player, questgiver);
            if (questStatus > 6)
                questStatus = getDialogStatus(_player, questgiver, defstatus);
