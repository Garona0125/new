...
namespace Trinity
{
    ...
    struct AIRelocationNotifier
    {
        Unit &i_unit;
        bool isCreature;
        explicit AIRelocationNotifier(Unit &unit) : i_unit(unit), isCreature(unit.GetTypeId() == TYPE_ID_UNIT)  {}
        template<class T> void Visit(GridRefManager<T> &) {}
        void Visit(CreatureMapType &);
    };
    ...
    struct MessageDistDeliverer
    {
        ...
        MessageDistDeliverer(WorldObject* src, WorldPacket* msg, float dist, bool own_team_only = false, Player const* skipped = NULL)
            ...
            , team((own_team_only && src->GetTypeId() == TYPE_ID_PLAYER) ? ((Player*)src)->GetTeam() : 0)
    ...
    class RaiseDeadObjectCheck
    {
        public:
            ...
            {
				if (_source->GetTypeId() != TYPE_ID_PLAYER || !((Player*)_source)->IsHonorOrXPTarget(u) ||
			...
			{
				if (_source == u || _source->GetTypeId() != TYPE_ID_PLAYER || !((Player*)_source)->IsHonorOrXPTarget(u) ||
			...
			{
				if (_source->GetTypeId() != TYPE_ID_PLAYER || u->GetType() == CORPSE_BONES)
					return false;
    ...
    class ExplodeCorpseObjectCheck
    {
        public:
            ...
            bool operator()(Creature* u)
            {
                if (u->getDeathState() != CORPSE || u->IsInFlight() ||
                    (u->GetDisplayId() != u->GetNativeDisplayId()) ||
                    (u->GetCreatureTypeMask() & CREATURE_TYPE_MASK_MECHANICAL_OR_ELEMENTAL) != 0)
                    return false;
    ...
    class NearestGameObjectFishingHole
    {
        public:
            ...
            {
                if (go->GetGOInfo()->type == GAMEOBJECT_TYPE_FISHING_HOLE && go->isSpawned() && i_obj.IsWithinDistInMap(go, i_range) && i_obj.IsWithinDistInMap(go, (float)go->GetGOInfo()->fishinghole.radius))
                {
    ...
    class MostHPMissingInRange
    {
        public:
            ...
            {
                if (u->IsAlive() && u->IsInCombatActive() && !i_obj->IsHostileTo(u) && i_obj->IsWithinDistInMap(u, i_range) && u->GetMaxHealth() - u->GetHealth() > i_hp)
                {
    ...
    class FriendlyCCedInRange
    {
        public:
            ...
            bool operator()(Unit* u)
            {
                if (u->IsAlive() && u->IsInCombatActive() && !i_obj->IsHostileTo(u) && i_obj->IsWithinDistInMap(u, i_range) &&
    ...
    class AnyUnfriendlyNoTotemUnitInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Unit* u)
            {
                ...
                if (u->GetTypeId() == TYPE_ID_UNIT && ((Creature*)u)->IsTotem())
                    return false;
    ...
    class AnyAoETargetUnitInObjectRangeCheck
    {
        public:
				...
				: i_obj(obj), i_funit(funit), _spellInfo(NULL), i_range(range)
            {
                ...
                if (owner)
                    check = owner;
                i_targetForPlayer = (check->GetTypeId() == TYPE_ID_PLAYER);
				if (i_obj->GetTypeId() == TYPE_ID_DYNAMIC_OBJECT)
            ...
            bool operator()(Unit* u)
            {
                ...
                if (i_funit->_IsValidAttackTarget(u, _spellInfo,i_obj->GetTypeId() == TYPE_ID_DYNAMIC_OBJECT ? i_obj : NULL) && i_obj->IsWithinDistInMap(u, i_range))
                    return true;
