...
void WardenCheckMgr::LoadWardenChecks()
{
    ...
    {
        sLog->OutConsole(">> Warden disabled, loading checks skipped.");
        sLog->OutConsole();
        return;
    }
    ...
    {
		sLog->OutConsole("=============================================================");
        sLog->OutConsole(">> Loaded 0 Warden checks. DB table `warden_checks` is empty!");
		sLog->OutConsole("=============================================================");
        sLog->OutConsole();
        return;
    }
	...
	sLog->OutConsole("=====================================");
    sLog->OutConsole(">> Loaded %u warden checks in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=====================================");
    sLog->OutConsole();
}
...
void WardenCheckMgr::LoadWardenOverrides()
{
	...
	if (!sWorld->getBoolConfig(CONFIG_WARDEN_ENABLED))
	{
		sLog->OutConsole(">> Warden disabled, loading check overrides skipped.");
		sLog->OutConsole();
		return;
	}
    ...
    {
		sLog->OutConsole("=======================================================================");
        sLog->OutConsole(">> Loaded 0 Warden action overrides. DB table `warden_action` is empty!");
		sLog->OutConsole("=======================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (action > WARDEN_ACTION_BAN)
            sLog->OutErrorConsole("Warden check override action out of range (ID: %u, action: %u)", checkId, action);
        // Check if check actually exists before accessing the CheckStore vector
        else if (checkId > CheckStore.size())
            sLog->OutErrorConsole("Warden check action override for non-existing check (ID: %u, action: %u), skipped", checkId, action);
        else
        {
	...
	sLog->OutConsole("===============================================");
    sLog->OutConsole(">> Loaded %u warden action overrides in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===============================================");
    sLog->OutConsole();
}
