...
void OutdoorPvPMgr::InitOutdoorPvP()
{
    ...
    //                                                 0       1
    QueryResult result = WorldDatabase.Query("SELECT TypeId, ScriptName FROM outdoor_pvp_template");

    if (!result)
    {
		sLog->OutConsole("==============================================================================");
        sLog->OutConsole(">> Loaded 0 outdoor PvP definitions. DB table `outdoor_pvp_template` is empty.");
		sLog->OutConsole("==============================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->OutErrorDatabase("Invalid OutdoorPvPTypes value %u in outdoor_pvp_template; skipped.", typeId);
            continue;
        }
    ...
    for (uint8 i = 1; i < MAX_OUTDOORPVP_TYPES; ++i)
    {
        ...
        {
            sLog->OutErrorDatabase("Could not initialize OutdoorPvP object for type ID %u; no entry in database.", uint32(i));
            continue;
        }
        ...
        if (!pvp)
        {
            sLog->OutErrorConsole("Could not initialize OutdoorPvP object for type ID %u; got NULL pointer from script.", uint32(i));
            continue;
        }

        if (!pvp->SetupOutdoorPvP())
        {
            sLog->OutErrorConsole("Could not initialize OutdoorPvP object for type ID %u; SetupOutdoorPvP failed.", uint32(i));
            delete pvp;
            continue;
        }
	...
	sLog->OutConsole("===============================================");
    sLog->OutConsole(">> Loaded %u outdoor PvP definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("===============================================");
    sLog->OutConsole();
}
