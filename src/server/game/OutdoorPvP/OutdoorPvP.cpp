...
m_value(0), m_team(TEAM_NEUTRAL), m_OldState(OBJECTIVE_STATE_NEUTRAL),
m_State(OBJECTIVE_STATE_NEUTRAL), m_neutralValuePct(0), m_PvP(pvp)
{
}
...
{
    if (uint32 guid = sObjectMgr->AddGameobjectData(entry, map, x, y, z, o, 0, rotation0, rotation1, rotation2, rotation3))
    {
...
{
    if (uint32 guid = sObjectMgr->AddCreatureData(entry, team, map, x, y, z, o, spawntimedelay))
    {
...
bool OPvPCapturePoint::SetCapturePointData(uint32 entry, uint32 map, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3)
{
    ...
    {
        sLog->OutErrorConsole("OutdoorPvP: GO %u is not capture point!", entry);
        return false;
    }

    m_capturePointGUID = sObjectMgr->AddGameobjectData(entry, map, x, y, z, o, 0, rotation0, rotation1, rotation2, rotation3);
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
    ...
    Creature* creature = HashMapHolder<Creature>::Find(m_Creatures[type]);
    if (!creature)
    {
    ...
    //if (Map* map = sMapMgr->FindMap(creature->GetMapId()))
    //    map->Remove(creature, false);
    // delete respawn time for this creature
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CREATURE_RESPAWN);
    stmt->setUInt32(0, guid);
	stmt->setUInt16(1, creature->GetMapId());
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    if (fact_diff < 0)
    {
        ...
        if (m_State == OBJECTIVE_STATE_HORDE && m_value <= -m_maxValue)
            return false;
    ...
    else
    {
        ...
        if (m_State == OBJECTIVE_STATE_ALLIANCE && m_value >= m_maxValue)
            return false;
    ...
    else if (oldValue * m_value <= 0) // grey, go through mid point
    {
        ...
        if (Challenger == ALLIANCE)
            m_State = OBJECTIVE_STATE_NEUTRAL_ALLIANCE_CHALLENGE;
        // if challenger is horde, then n->h challenge
        else if (Challenger == HORDE)
            m_State = OBJECTIVE_STATE_NEUTRAL_HORDE_CHALLENGE;
        m_team = TEAM_NEUTRAL;
    }
    else // grey, did not go through mid point
    {
        // old phase and current are on the same side, so one team challenges the other
        if (Challenger == ALLIANCE && (m_OldState == OBJECTIVE_STATE_HORDE || m_OldState == OBJECTIVE_STATE_NEUTRAL_HORDE_CHALLENGE))
            m_State = OBJECTIVE_STATE_HORDE_ALLIANCE_CHALLENGE;
        else if (Challenger == HORDE && (m_OldState == OBJECTIVE_STATE_ALLIANCE || m_OldState == OBJECTIVE_STATE_NEUTRAL_ALLIANCE_CHALLENGE))
            m_State = OBJECTIVE_STATE_ALLIANCE_HORDE_CHALLENGE;
        m_team = TEAM_NEUTRAL;
    }
    ...
    {
        //sLog->OutErrorConsole("%u->%u", m_OldState, m_State);
        if (oldTeam != m_team)
            ChangeTeam(oldTeam);
        ChangeState();
        return true;
...
void OutdoorPvP::HandleKill(Player* killer, Unit* killed)
{
    if (Group* group = killer->GetGroup())
    {
        for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            if ((groupGuy->IsOutdoorPvPActive() && IsInsideObjective(groupGuy)) || killed->GetTypeId() == TYPE_ID_UNIT)
            {
    ...
    {
        // creature kills must be notified, even if not inside objective / not outdoor pvp active
        if (killer && ((killer->IsOutdoorPvPActive() && IsInsideObjective(killer)) || killed->GetTypeId() == TYPE_ID_UNIT))
        {
