...
InstanceSave* InstanceSaveManager::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, time_t resetTime, bool canReset, bool load)
{
    ...
    if (!entry)
    {
        sLog->OutErrorConsole("InstanceSaveManager::AddInstanceSave: wrong mapid = %d, instanceid = %d!", mapId, instanceId);
        return NULL;
    }

    if (instanceId == 0)
    {
        sLog->OutErrorConsole("InstanceSaveManager::AddInstanceSave: mapid = %d, wrong instanceid = %d!", mapId, instanceId);
        return NULL;
    }

    if (difficulty >= (entry->IsRaid() ? MAX_RAID_DIFFICULTY : MAX_DUNGEON_DIFFICULTY))
    {
        sLog->OutErrorConsole("InstanceSaveManager::AddInstanceSave: mapid = %d, instanceid = %d, wrong dificalty %u!", mapId, instanceId, difficulty);
        return NULL;
    }
...
void InstanceSaveManager::RemoveInstanceSave(uint32 InstanceId)
{
    ...
    if (itr != m_instanceSaveById.end())
    {
        ...
        {
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_INSTANCE_RESET_TIME);
...
void InstanceSaveManager::LoadInstances()
{
	...
	sLog->OutConsole("==============================");
    sLog->OutConsole(">> Loaded instances in %u ms", GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("==============================");
    sLog->OutConsole();
}
...
void InstanceSaveManager::LoadResetTimes()
{
    ...
    if (result)
    {
        do
        {
            ...
            if (!mapDiff)
            {
                sLog->OutErrorConsole("InstanceSaveManager::LoadResetTimes: invalid mapid(%u)/difficulty(%u) pair in instance_reset!", mapid, difficulty);
    ...
    if (!add)
    {
        ...
        if (itr == range.second)
        {
            ...
            if (itr == m_resetTimeQueue.end())
                sLog->OutErrorConsole("InstanceSaveManager::ScheduleReset: cannot cancel the reset, the event(%d, %d, %d) was not found!", event.type, event.mapid, event.instanceId);
        }
    }
    else
...
void InstanceSaveManager::_ResetOrWarnAll(uint32 mapid, Difficulty difficulty, bool warn, time_t resetTime)
{
    ...
    if (!warn)
    {
        ...
        {
            sLog->OutErrorConsole("InstanceSaveManager::ResetOrWarnAll: not valid difficulty or no reset delay for map %d", mapid);
            return;
        }
        ...
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GLOBAL_INSTANCE_RESET_TIME);
