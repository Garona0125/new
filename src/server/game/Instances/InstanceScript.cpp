...
void InstanceScript::HandleGameObject(uint64 GUID, bool open, GameObject* go)
{
    ...
    if (go && go->GetGoState() != (open ? GO_STATE_ACTIVE : GO_STATE_READY))
        ...
        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: InstanceScript: HandleGameObject failed");
}
...
void InstanceScript::LoadMinionData(const MinionData* data)
{
    ...
    sLog->outDebug(LOG_FILTER_QCSCR, "InstanceScript::LoadMinionData: " UI64FMTD " minions loaded.", uint64(minions.size()));
}
...
void InstanceScript::LoadDoorData(const DoorData* data)
{
    ...
    sLog->outDebug(LOG_FILTER_QCSCR, "InstanceScript::LoadDoorData: " UI64FMTD " doors loaded.", uint64(doors.size()));
}
...
void InstanceScript::UpdateMinionState(Creature* minion, EncounterState state)
{
    switch (state)
    {
        case NOT_STARTED:
            if (!minion->IsAlive())
                minion->Respawn();
            else if (minion->IsInCombatActive())
...
bool InstanceScript::SetBossState(uint32 id, EncounterState state)
{
    if (id < bosses.size())
    {
        ...
        if (bossInfo->state == TO_BE_DECIDED)
        {
            ...
            //sLog->OutErrorConsole("Inialize boss %u state as %u.", id, (uint32)state);
            return false;
        }
        else
        {
...
void InstanceScript::DoUseDoorOrButton(uint64 uiGuid, uint32 uiWithRestoreTime, bool bUseAlternativeState)
{
    ...
    if (go)
    {
        if (go->GetGoType() == GAMEOBJECT_TYPE_DOOR || go->GetGoType() == GAMEOBJECT_TYPE_BUTTON)
        {
            ...
            sLog->OutErrorConsole("QUANTUM CORE SCRIPTS: Script call DoUseDoorOrButton, but gameobject entry %u is type %u.", go->GetEntry(), go->GetGoType());
    }
}
...
void InstanceScript::DoRespawnGameObject(uint64 uiGuid, uint32 uiTimeToDespawn)
{
    ...
    {
        if (go->GetGoType() == GAMEOBJECT_TYPE_FISHING_NODE || go->GetGoType() == GAMEOBJECT_TYPE_DOOR || go->GetGoType() == GAMEOBJECT_TYPE_BUTTON || go->GetGoType() == GAMEOBJECT_TYPE_TRAP)
            return;
...
void InstanceScript::DoUpdateWorldState(uint32 uiStateId, uint32 uiStateData)
{
    ...
    if (!lPlayers.isEmpty())
    {
        ...
        sLog->outDebug(LOG_FILTER_QCSCR, "QUANTUM CORE SCRIPTS: DoUpdateWorldState attempt send data but no players in map.");
}
        ...
        for (Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
		{
            if (Player* player = i->getSource())
			{
                if (WorldSession* session = player->GetSession())
				{
					...
					session->SendNotification("Welcome! Needs to be testing", buff);
				}
			}
		}
    }
...
{
    sLog->OutErrorConsole("Achievement system call InstanceScript::CheckAchievementCriteriaMeet but instance script for map %u not have implementation for achievement criteria %u",
...
void InstanceScript::UpdateEncounterState(EncounterCreditType type, uint32 creditEntry, Unit* source)
{
    ...
    for (DungeonEncounterList::const_iterator itr = encounters->begin(); itr != encounters->end(); ++itr)
    {
        if ((*itr)->creditType == type && (*itr)->creditEntry == creditEntry)
        {
            ...
            sLog->outDebug(LOG_FILTER_QCSCR, "Instance %s (instanceId %u) completed encounter %s", instance->GetMapName(), instance->GetInstanceId(), (*itr)->dbcEntry->encounterName[0]);
...
void InstanceScript::DoCompleteAchievement(uint32 achievement_id)
{
   AchievementEntry const* pAE = sAchievementStore.LookupEntry(achievement_id);
   Map::PlayerList const &PlayerList = instance->GetPlayers();

   if (!pAE)
   {
       sLog->OutErrorConsole("QUANTUMCORE SCRIPT: DoCompleteAchievement called for not existing achievement %u", achievement_id);
       return;
   }
