    ...
    if (sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
    {
        ...
        else
        {
            ...
            if (!AccountMgr::IsPlayerAccount(bidder_security)) // not do redundant DB requests
            {
                if (!sObjectMgr->GetPlayerNameByGUID(bidder_guid, bidder_name))
                    bidder_name = sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_UNKNOWN);
            }
        }
        if (!AccountMgr::IsPlayerAccount(bidder_security))
        {
            ...
            if (!sObjectMgr->GetPlayerNameByGUID(auction->owner, owner_name))
                owner_name = sObjectMgr->GetQuantumSystemTextForDBCLocale(LANG_UNKNOWN);
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    {
		sLog->OutConsole("===============================================================================");
        sLog->OutConsole(">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
		sLog->OutConsole("===============================================================================");
        sLog->OutConsole();
        return;
    }
    ...
    do
    {
        ...
        if (!proto)
        {
            sLog->OutErrorConsole("AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u id: #%u) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
	...
	sLog->OutConsole("=====================================");
    sLog->OutConsole(">> Loaded %u auction items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("=====================================");
    sLog->OutConsole();
}
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    {
		sLog->OutConsole("=======================================================");
        sLog->OutConsole(">> Loaded 0 auctions. DB table `auctionhouse` is empty!");
		sLog->OutConsole("=======================================================");
        sLog->OutConsole();
        return;
    }
	...
	sLog->OutConsole("================================");
    sLog->OutConsole(">> Loaded %u auctions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
	sLog->OutConsole("================================");
    sLog->OutConsole();
}
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    if (!item)
    {
        sLog->OutErrorConsole("AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        sLog->OutErrorConsole("Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        sLog->OutErrorConsole("Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    {
        sLog->OutErrorConsole("Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
    ...
    {
        sLog->OutErrorConsole("Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
    return true;
}
...
void AuctionHouseMgr::DeleteExpiredAuctionsAtStartup()
{
    ...
    if (!expAuctions)
    {
        sLog->OutConsole(">> No expired auctions to delete");
        sLog->OutConsole();
        return;
    }
    ...
    sLog->OutConsole(">> Deleted %u expired auctions in %u ms", expirecount, GetMSTimeDiffToNow(oldMSTime));
    sLog->OutConsole();
...
bool AuctionEntry::LoadFromFieldList(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        sLog->OutErrorConsole("AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        sLog->OutErrorConsole("AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    if (!auctionHouseEntry)
    {
        sLog->OutErrorConsole("AuctionEntry::LoadFromFieldList() - Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
