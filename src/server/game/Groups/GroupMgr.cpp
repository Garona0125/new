...
uint32 GroupMgr::GenerateNewGroupDbStoreId()
{
    ...
    {
        sLog->OutErrorConsole("Group storage ID overflow!! Can't continue, shutting down server. ");
...
uint32 GroupMgr::GenerateGroupId()
{
    ...
    {
        sLog->OutErrorConsole("Group guid overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
    return NextGroupId++;
}
...
{
    {
        ...
        {
			sLog->OutConsole("==========================================================");
            sLog->OutConsole(">> Loaded 0 group definitions. DB table `groups` is empty!");
			sLog->OutConsole("==========================================================");
            sLog->OutConsole();
            return;
        }
		...
		sLog->OutConsole("=========================================");
        sLog->OutConsole(">> Loaded %u group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("=========================================");
        sLog->OutConsole();
    }

    sLog->OutConsole("Loading Group members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("============================================================");
            sLog->OutConsole(">> Loaded 0 group members. DB table `group_member` is empty!");
			sLog->OutConsole("============================================================");
            sLog->OutConsole();
            return;
        }
        ...
        do
        {
            ...
            if (group)
                ...
                sLog->OutErrorConsole("GroupMgr::LoadGroups: Consistency failed, can't find group (storage id: %u)", fields[0].GetUInt32());
		...
		sLog->OutConsole("=====================================");
        sLog->OutConsole(">> Loaded %u group members in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("=====================================");
        sLog->OutConsole();
    }

    sLog->OutConsole("Loading Group instance saves...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                   0           1        2              3             4             5            6
        ...
        {
			sLog->OutConsole("=====================================================================");
            sLog->OutConsole(">> Loaded 0 group-instance saves. DB table `group_instance` is empty!");
			sLog->OutConsole("=====================================================================");
            sLog->OutConsole();
            return;
        }
        ...
        do
        {
            ...
            {
                sLog->OutErrorDatabase("Incorrect entry in group_instance table : no dungeon map %d", fields[1].GetUInt16());
                continue;
            }
            ...
            {
                sLog->OutErrorDatabase("Wrong dungeon difficulty use in group_instance table: %d", diff + 1);
		...
		sLog->OutConsole("============================================");
        sLog->OutConsole(">> Loaded %u group-instance saves in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("============================================");
        sLog->OutConsole();
    }
}