...
Group::Group() : m_leaderGuid(0), m_leaderName(""), m_groupType(GROUP_TYPE_NORMAL),
...
Group::~Group()
{
    if (m_bgGroup)
    {
        ...
        else sLog->OutErrorConsole("Group::~Group: battleground group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while (!RollId.empty())
    {
...
bool Group::Create(Player* leader)
{
    ...
    m_groupType  = isBGGroup() ? GROUP_TYPE_BGRAID : GROUP_TYPE_NORMAL;
...
{
    m_groupType = GroupType(m_groupType | GROUP_TYPE_LFG | GROUP_TYPE_LFG_RESTRICTED);
...
{
    m_groupType = GroupType(m_groupType | GROUP_TYPE_RAID);
...
bool Group::AddMember(Player* player)
{
    ...
    if (player)
    {
        if (!IsLeader(player->GetGUID()) && !isBGGroup())
        {
            ...
            if (player->GetCurrentLevel() >= LEVELREQUIREMENT_HEROIC)
            {
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(Battleground const* bgOrTemplate, BattlegroundQueueTypeId bgQueueTypeId, uint32 MinPlayerCount, uint32 /*MaxPlayerCount*/, bool isRated, uint32 arenaSlot)
{
    ...
    PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(bgOrTemplate->GetMapId(), reference->GetCurrentLevel());
    if (!bracketEntry)
        return ERR_BATTLEGROUND_JOIN_FAILED;
    ...
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next(), ++memberscount)
    {
        ...
        PvPDifficultyEntry const* memberBracketEntry = GetBattlegroundBracketByLevel(bracketEntry->mapId, member->GetCurrentLevel());
    ...
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (player && !player->getAttackers().empty() && player->GetInstanceId() == instanceId && (player->GetMap()->IsRaidOrHeroicDungeon()))
            for (std::set<Unit*>::const_iterator i = player->getAttackers().begin(); i != player->getAttackers().end(); ++i)
                if ((*i) && (*i)->GetTypeId() == TYPE_ID_UNIT && (*i)->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_INSTANCE_BIND)
                    return true;
    }
    return false;
}
...
{
    return m_groupType & GROUP_TYPE_LFG;
}

bool Group::isRaidGroup() const
{
    return m_groupType & GROUP_TYPE_RAID;
}
...
void Group::SetGroupMemberFlag(uint64 guid, bool apply, GroupMemberFlags flag)
{
    ...
    switch (flag) {
        case MEMBER_FLAG_MAIN_ASSIST:
            RemoveUniqueGroupMemberFlag(MEMBER_FLAG_MAIN_ASSIST);         // Remove main assist flag from current if any.
            break;
        case MEMBER_FLAG_MAIN_TANK:
            RemoveUniqueGroupMemberFlag(MEMBER_FLAG_MAIN_TANK);           // Remove main tank flag from current if any.
            break;
