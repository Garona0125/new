...
namespace Trinity
{
    ...
    namespace XP
    {
        ...
        inline uint32 BaseGain(uint8 pl_level, uint8 mob_level, ContentLevels content)
        {
            ...
            switch (content)
            {
                ...
                case CONTENT_71_80:
                    ...
                    break;
                default:
                    sLog->OutErrorConsole("BaseGain: Unsupported content level %u", content);
                    nBaseExp = 45;
                    break;
            }
        ...
        inline uint32 Gain(Player* player, Unit* u, bool isBattleGround = false)
        {
            ...
            {
                gain = BaseGain(player->GetCurrentLevel(), u->GetCurrentLevel(), GetContentLevelsForMapAndZone(u->GetMapId(), u->GetZoneId()));

                if (gain != 0 && u->GetTypeId() == TYPE_ID_UNIT && ((Creature*)u)->isElite())
                {
                ...
                float premium_rate = player->GetSession()->IsPremiumAccount() ? sWorld->getRate(RATE_XP_KILL_PREMIUM) : 1.0f;

				return uint32(gain* isBattleGround ? sWorld->getRate(RATE_XP_BG_KILL)*premium_rate : sWorld->getRate(RATE_XP_KILL)*premium_rate);

				//return uint32(gain*sWorld->getRate(RATE_XP_KILL)*premium_rate);

				// Guild-Level-System (Bonus: Monster Kill Xp)
