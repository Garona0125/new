...
void PoolGroup<Quest>::SpawnObject(ActivePoolData& spawns, uint32 limit, uint32 triggerFrom)
{
    ...
    if (limit > newQuests.size() && !currentQuests.empty())
    {
        do
        {
            uint32 questId = Quantum::DataPackets::SelectRandomContainerElement(currentQuests);
    ...
    do
    {
        uint32 questId = Quantum::DataPackets::SelectRandomContainerElement(newQuests);
        spawns.ActivateObject<Quest>(questId, poolId);
        PoolObject tempObj(questId, 0.0f);
        Spawn1Object(&tempObj);
...
void PoolMgr::LoadFromDB()
{
    // Pool templates
    {
        ...
        if (!result)
        {
            mPoolTemplate.clear();
			sLog->OutConsole("============================================================");
            sLog->OutConsole(">> Loaded 0 object pools. DB table `pool_template` is empty.");
			sLog->OutConsole("============================================================");
            sLog->OutConsole();
            return;
        }
		...
		sLog->OutConsole("=====================================");
        sLog->OutConsole(">> Loaded %u objects pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
		sLog->OutConsole("=====================================");
        sLog->OutConsole();
    }

    // Creatures
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Creatures Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("=====================================================================");
            sLog->OutConsole(">> Loaded 0 creatures in  pools. DB table `pool_creature` is empty.");
			sLog->OutConsole("=====================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    sLog->OutErrorDatabase("`pool_creature` has a non existing creature spawn (GUID: %u) defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                if (pool_id > max_pool_id)
                {
                    sLog->OutErrorDatabase("`pool_creature` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100)
                {
                    sLog->OutErrorDatabase("`pool_creature` has an invalid chance (%f) for creature guid (%u) in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
                PoolTemplateData* pPoolTemplate = &mPoolTemplate[pool_id];
			...
			sLog->OutConsole("==========================================");
            sLog->OutConsole(">> Loaded %u creatures in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("==========================================");
            sLog->OutConsole();
        }
    }
    ...
    // Gameobjects
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Gameobject Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("=======================================================================");
            sLog->OutConsole(">> Loaded 0 gameobjects in  pools. DB table `pool_gameobject` is empty.");
			sLog->OutConsole("=======================================================================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    sLog->OutErrorDatabase("`pool_gameobject` has a non existing gameobject spawn (GUID: %u) defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                ...
                if (goinfo->type != GAMEOBJECT_TYPE_CHEST &&
                    ...
                    goinfo->type != GAMEOBJECT_TYPE_FISHING_HOLE)
                {
                    sLog->OutErrorDatabase("`pool_gameobject` has a not lootable gameobject spawn (GUID: %u, type: %u) defined for pool id (%u), skipped.", guid, goinfo->type, pool_id);
                    continue;
                }

                if (pool_id > max_pool_id)
                {
                    sLog->OutErrorDatabase("`pool_gameobject` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }

                if (chance < 0 || chance > 100)
                {
                    sLog->OutErrorDatabase("`pool_gameobject` has an invalid chance (%f) for gameobject guid (%u) in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
			...
			sLog->OutConsole("===========================================");
            sLog->OutConsole(">> Loaded %u gameobject in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("===========================================");
            sLog->OutConsole();
        }
    }
    ...
    // Pool of pools
    sLog->OutConsole("QUANTUMCORE SERVER: Loading Mother Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("==========================");
            sLog->OutConsole(">> Loaded 0 pools in pools");
			sLog->OutConsole("==========================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->OutErrorDatabase("`pool_pool` mother_pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", mother_pool_id);
                    continue;
                }
                if (child_pool_id > max_pool_id)
                {
                    sLog->OutErrorDatabase("`pool_pool` included pool_id (%u) is out of range compared to max pool id in `pool_template`, skipped.", child_pool_id);
                    continue;
                }
                if (mother_pool_id == child_pool_id)
                {
                    sLog->OutErrorDatabase("`pool_pool` pool_id (%u) includes itself, dead-lock detected, skipped.", child_pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100)
                {
                    sLog->OutErrorDatabase("`pool_pool` has an invalid chance (%f) for pool id (%u) in mother pool id (%u), skipped.", chance, child_pool_id, mother_pool_id);
                    continue;
                }
            ...
            for (uint32 i=0; i < max_pool_id; ++i)
            {
                ...
                for (SearchMap::iterator poolItr = mPoolSearchMap.find(i); poolItr != mPoolSearchMap.end(); poolItr = mPoolSearchMap.find(poolItr->second))
                {
                    ...
                    if (checkedPools.find(poolItr->second) != checkedPools.end())
                    {
                        ...
                        sLog->OutErrorDatabase("%s", ss.str().c_str());
			...
			sLog->OutConsole("=============================================");
            sLog->OutConsole(">> Loaded %u pools in mother pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=============================================");
            sLog->OutConsole();
        }
    }

    sLog->OutConsole("QUANTUMCORE SERVER: Loading Quest Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("===========================");
            sLog->OutConsole(">> Loaded 0 quests in pools");
			sLog->OutConsole("===========================");
            sLog->OutConsole();
        }
        else
        {
            ...
            do
            {
                ...
                if (!quest)
                {
                    sLog->OutErrorDatabase("`pool_quest` has a non existing quest template (Entry: %u) defined for pool id (%u), skipped.", entry, pool_id);
                    continue;
                }

                if (pool_id > max_pool_id)
                {
                    sLog->OutErrorDatabase("`pool_quest` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }

                if (!quest->IsDailyOrWeekly())
                {
                    sLog->OutErrorDatabase("`pool_quest` has an quest (%u) which is not daily or weekly in pool id (%u), use ExclusiveGroup instead, skipped.", entry, pool_id);
                    continue;
                }
                ...
                {
                    sLog->OutErrorDatabase("`pool_quest` quest %u is %s but pool (%u) is specified for %s, mixing not allowed, skipped.",
                ...
                {
                    sLog->OutErrorDatabase("`pool_quest` lists entry (%u) as member of pool (%u) but is not started anywhere, skipped.", entry, pool_id);
                    continue;
                }
			...
			sLog->OutConsole("=======================================");
            sLog->OutConsole(">> Loaded %u quests in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
			sLog->OutConsole("=======================================");
            sLog->OutConsole();
        }
    }
    ...
    sLog->OutConsole("Starting objects pooling system...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        {
			sLog->OutConsole("=====================================================");
            sLog->OutConsole(">> Pool handling system initialized, 0 pools spawned.");
			sLog->OutConsole("=====================================================");
            sLog->OutConsole();
        }
        else
            ...
            do
            {
                ...
                if (!CheckPool(pool_entry))
                {
                    if (pool_pool_id)
                        ...
                        sLog->OutErrorDatabase("Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. This broken pool is a child pool of Id %u and cannot be safely removed.", pool_entry, fields[2].GetUInt32());
                    else
                        sLog->OutErrorDatabase("Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. The pool will not be spawned.", pool_entry);
                    continue;
                }
