...
WorldSocketMgr::WorldSocketMgr() : m_NetThreads(0), m_NetThreadsCount(0), m_SockOutKBuff(-1), m_SockOutUBuff(65536), m_UseNoDelay(true), m_Acceptor (0){}
...
{
    m_UseNoDelay = GetConfigSettings::GetBoolState("Network.TcpNodelay", true);

    int num_threads = GetConfigSettings::GetIntState("Network.Threads", 1);

    if (num_threads <= 0)
    {
        sLog->OutErrorConsole("Network.Threads is wrong in your config file");
        return -1;
    }
    ...
    // -1 means use default
    m_SockOutKBuff = GetConfigSettings::GetIntState("Network.OutKBuff", -1);

    m_SockOutUBuff = GetConfigSettings::GetIntState("Network.OutUBuff", 65536);

    if (m_SockOutUBuff <= 0)
    {
        sLog->OutErrorConsole("Network.OutUBuff is wrong in your config file");
        return -1;
    }
    ...
    {
        sLog->OutErrorConsole("Failed to open acceptor, check if the port is free");
        return -1;
    }
...
WorldSocketMgr::OnSocketOpen (WorldSocket* sock)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            sLog->OutErrorConsole("WorldSocketMgr::OnSocketOpen set_option SO_SNDBUF");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (sock->peer().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            sLog->OutErrorConsole("WorldSocketMgr::OnSocketOpen: peer().set_option TCP_NODELAY errno = %s", ACE_OS::strerror (errno));
            return -1;
        }
    }
