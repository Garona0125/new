...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
    ...
    else
    {
        ...
        {
            sLog->OutErrorConsole("WorldSocket::SendPacket enqueue_tail failed");
            mb->release();
            return -1;
        }
    }
...
int WorldSocket::open (void *a)
{
    ...
    {
        sLog->OutErrorConsole("WorldSocket::open: peer().get_remote_addr errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
    ...
    {
        sLog->OutErrorConsole("WorldSocket::open: unable to register client handler errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
...
int WorldSocket::handle_output_queue (GuardType& g)
{
    ...
    {
        sLog->OutErrorConsole("WorldSocket::handle_output_queue dequeue_head");
        return -1;
    }
    ...
    else if (n < (ssize_t)send_len) //now n > 0
    {
        ...
        {
            sLog->OutErrorConsole("WorldSocket::handle_output_queue enqueue_head");
            mblk->release();
            return -1;
        }
...
int WorldSocket::handle_input_header (void)
{
    ...
    if ((header.size < 4) || (header.size > 10240) || (header.cmd  > 10240))
    {
        ...
        sLog->OutErrorConsole("WorldSocket::handle_input_header(): client (account: %u, char [GUID: %u, name: %s]) sent malformed packet (size: %d, cmd: %d)",
...
int WorldSocket::handle_input_missing_data (void)
{
    ...
    while (message_block.length() > 0)
    {
        ...
        if (!m_RecvWPct)
        {
            sLog->OutErrorConsole("Forcing close on input m_RecvWPct = NULL");
            errno = EINVAL;
            return -1;
        }
...
int WorldSocket::cancel_wakeup_output (GuardType& g)
{
        ...
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        ...
        sLog->OutErrorConsole("WorldSocket::cancel_wakeup_output");
        return -1;
    }
...
int WorldSocket::schedule_wakeup_output (GuardType& g)
{
    ...
    if (reactor()->schedule_wakeup
        (this, ACE_Event_Handler::WRITE_MASK) == -1)
    {
        sLog->OutErrorConsole("WorldSocket::schedule_wakeup_output");
        return -1;
    }
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    try
    {
        switch (opcode)
        {
            ...
            case CMSG_AUTH_SESSION:
                if (m_Session)
                {
                    sLog->OutErrorConsole("WorldSocket::ProcessIncoming: Player send CMSG_AUTH_SESSION again");
                    return -1;
                }
            ...
            default:
            {
                ...
                {
                    sLog->OutErrorConsole("WorldSocket::ProcessIncoming: Client not authed opcode = %u", uint32(opcode));
                    return -1;
                }
            }
        }
    ...
    {
        sLog->OutErrorConsole("WorldSocket::ProcessIncoming ByteBufferException occured while parsing an instant handled packet (opcode: %u) from client %s, accountid=%i. Disconnected client.",
...
int WorldSocket::HandleAuthSession (WorldPacket& recvPacket)
{
    ...
    if (sWorld->IsClosed())
    {
        ...
        sLog->OutErrorConsole("WorldSocket::HandleAuthSession: World closed, denying client (%s).", GetRemoteAddress().c_str());
        return -1;
    }
    ...
    if (!result)
    {
        ...
        sLog->OutErrorConsole("WorldSocket::HandleAuthSession: Sent Auth Response (unknown account).");
        return -1;
    }
    ...
    if (sWorld->getBoolConfig(CONFIG_WARDEN_ENABLED) && os != "Win" && os != "OSX")
    {
		...
		sLog->outDebug(LOG_FILTER_NETWORKIO, "WorldSocket::HandleAuthSession: Client %s attempted to log in using invalid client OS (%s).", GetRemoteAddress().c_str(), os.c_str());
        return -1;
    }

    // Checks gmlevel per Realm
    stmt = LoginDatabase.GetPreparedStatement(LOGIN_GET_ACCOUNT_SECURITY_GM_LEVEL_BY_REALM_ID);
    ...
    if (banresult) // if account banned
    {
        ...
        sLog->OutErrorConsole("WorldSocket::HandleAuthSession: Sent Auth Response (Account banned).");
        return -1;
    }

	// Check premium
	stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_PREMIUM);
	stmt->setUInt32(0, id);
    ...
    if (memcmp (sha.GetDigest(), digest, 20))
    {
		...
		sLog->OutErrorConsole("WorldSocket::HandleAuthSession: Authentication failed for account: %u ('%s') address: %s", id, account.c_str(), address.c_str());
        return -1;
    }
    ...
    ACE_NEW_RETURN (m_Session, WorldSession (id, this, AccountTypes(security), expansion, isPremium, mutetime, locale, recruiter, isRecruiter), -1);
...
int WorldSocket::HandlePing (WorldPacket& recvPacket)
{
        ...
        if (diff_time < ACE_Time_Value (27))
        {
            ...
            if (max_count && m_OverSpeedPings > max_count)
            {
                ...
                if (m_Session && AccountMgr::IsPlayerAccount(m_Session->GetSecurity()))
                {
                    ...
                    sLog->OutErrorConsole("WorldSocket::HandlePing: Player (account: %u, GUID: %u, name: %s) kicked for over-speed pings (address: %s)",
    ...
    // critical section
    {
        ...
        {
            sLog->OutErrorConsole("WorldSocket::HandlePing: peer sent CMSG_PING, "
