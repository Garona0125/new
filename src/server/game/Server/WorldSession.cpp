...
WorldSession::WorldSession(uint32 id, WorldSocket* sock, AccountTypes sec, uint8 expansion, bool IsPremiumAccount, time_t mute_time, LocaleConstant locale, uint32 recruiter, bool isARecruiter):
...
{
    sLog->OutErrorConsole("Client (account %u) send packet %s (%u) with size " SIZEFMTD " but expected %u (attempt to crash server?), skipped",
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
            ...
            _recvQueue.next(packet, updater))
    {
        ...
        {
            sLog->OutErrorConsole("SESSION: received non-existed opcode %s (0x%.4X)", LookupOpcodeName(packet->GetOpcode()), packet->GetOpcode());
        ...
        else
        {
            ...
            {
                sLog->OutErrorConsole("WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%i. Skipped packet.",
    ...
    if (_player)
    {
        ...
        else if (!_player->getAttackers().empty())
        {
            ...
            for (Unit::AttackerSet::const_iterator itr = _player->getAttackers().begin(); itr != _player->getAttackers().end(); ++itr)
            {
                ...
                else if ((*itr)->GetTypeId() == TYPE_ID_PLAYER)
...
{
    char const* format = GetQuantumSystemText(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
{
    sLog->OutErrorConsole("SESSION: received unhandled opcode %s (0x%.4X)", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}

void WorldSession::Handle_EarlyProccess(WorldPacket& recvPacket)
{
    sLog->OutErrorConsole("SESSION: received opcode %s (0x%.4X) that must be processed in WorldSocket::OnRead", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}

void WorldSession::Handle_ServerSide(WorldPacket& recvPacket)
{
    sLog->OutErrorConsole("SESSION: received server-side opcode %s (0x%.4X)", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}

void WorldSession::Handle_Deprecated(WorldPacket& recvPacket)
{
    sLog->OutErrorConsole("SESSION: received deprecated opcode %s (0x%.4X)", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask)
{
    ...
    do
    {
        ...
        {
            sLog->OutErrorConsole("Table `%s` have invalid account data type (%u), ignore.", mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }

        if ((mask & (1 << type)) == 0)
        {
            sLog->OutErrorConsole("Table `%s` have non appropriate for table  account data type (%u), ignore.", mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::ReadMovementInfo(WorldPacket &data, MovementInfo* mi)
{
    ...
    if (mi->HasMovementFlag(MOVEMENTFLAG_ON_TRANSPORT))
    {
    ...
    */
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENT_FLAG_HOVER) && !GetPlayer()->HasAuraType(SPELL_AURA_HOVER),
        MOVEMENT_FLAG_HOVER);
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_WATER_WALKING) && !GetPlayer()->HasAuraType(SPELL_AURA_WATER_WALK),
        MOVEMENTFLAG_WATER_WALKING);
...
void WorldSession::ReadAddonsInfo(WorldPacket &data)
{
    ...
    if (size > 0xFFFFF)
    {
        sLog->OutErrorConsole("WorldSession::ReadAddonsInfo addon info too big, size %u", size);
        return;
    }
    ...
    if (uncompress(const_cast<uint8*>(addonInfo.contents()), &uSize, const_cast<uint8*>(data.contents() + pos), data.size() - pos) == Z_OK)
    {
        ...
        sLog->OutErrorConsole("Addon packet uncompress error!");
}
