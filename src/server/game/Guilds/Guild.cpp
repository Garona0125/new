...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode, std::string const& param)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_COMMAND_RESULT [%s]: Type: %u, code: %u, param: %s" , type, errCode, param.c_str());
}
...
void Guild::SendSaveEmblemResult(WorldSession* session, GuildEmblemError errCode)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "MSG_SAVE_GUILD_EMBLEM [%s] Code: %u", errCode);
}
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(uint8 tabs, SQLTransaction& trans, bool logOnCreate /* = false */)
{
    for (uint8 i = 0; i < tabs; ++i)
    {
        ...
        if (logOnCreate)
            sLog->outDebug(LOG_FILTER_GUILD, "Guild %u has broken Tab %u for rank %u. Created default tab.", m_guildId, i, m_rankId);
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    {
        sLog->OutErrorConsole("Invalid slot for item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    if (!proto)
    {
        sLog->OutErrorConsole("Unknown item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    {
        sLog->OutErrorConsole("Item (GUID %u, id: %u) not found in item_instance, deleting from guild bank!", itemGuid, itemEntry);
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const
{
    ...
    if (session)
    {
        sLog->outDebug(LOG_FILTER_GUILD, "MSG_QUERY_GUILD_BANK_TEXT [%s]: Tabid: %u, Text: %s" , m_tabId, m_text.c_str());
        session->SendPacket(&data);
    }
    else
    {
...
void Guild::Member::SetStats(Player* player)
{
    ...
    m_level = player->GetCurrentLevel();
    m_class = player->GetCurrentClass();
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    if (!m_zoneId)
    {
        sLog->OutErrorConsole("Player (GUID: %u) has broken zone-data", GUID_LOPART(m_guid));
...
bool Guild::Member::CheckStats() const
{
    if (m_level < 1)
    {
        sLog->OutErrorConsole("Player (GUID: %u) has a broken data in field `characters`.`level`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }

    if (m_class < CLASS_WARRIOR || m_class >= MAX_CLASSES)
    {
        sLog->OutErrorConsole("Player (GUID: %u) has a broken data in field `characters`.`class`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }
    return true;
}
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    if (!pFrom->IsBank() && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE) && !AccountMgr::IsPlayerAccount(m_pPlayer->GetSession()->GetSecurity())) // TODO: move to scripts
	{
		...
		sLog->outCommand(m_pPlayer->GetSession()->GetAccountId(),
			"GM %s (Guid: %u) (Account: %u) deposit item: %s (Entry: %d Count: %u) to guild bank named: %s (Guild ID: %u)",
			m_pPlayer->GetName(), m_pPlayer->GetGUIDLow(), m_pPlayer->GetSession()->GetAccountId(),
			pFrom->GetItem()->GetTemplate()->Name1.c_str(), pFrom->GetItem()->GetEntry(), pFrom->GetItem()->GetCount(),
			m_pGuild->GetName().c_str(), m_pGuild->GetId());
	}
}
    ...
    if (Member* member = GetMember(player->GetGUID()))
    {
        switch (dataid)
        {
            ...
            case GUILD_MEMBER_DATA_LEVEL:
                ...
                break;
            default:
                sLog->OutErrorConsole("Guild::UpdateMemberData: Called with incorrect DATAID %u (value %u)", dataid, value);
                return;
        }
        //HandleRoster();
    }
...
void Guild::HandleRoster(WorldSession* session /*= NULL*/)
{
    ...
    if (session)
    {
        sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_ROSTER [%s]");
        session->SendPacket(&data);
    }
    else
    {
...
void Guild::HandleQuery(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_QUERY_RESPONSE [%s]");
}
...
void Guild::HandleSetBankTabInfo(WorldSession* session, uint8 tabId, std::string const& name, std::string const& icon)
{
    ...
    if (!tab)
    {
        sLog->OutErrorConsole("Guild::HandleSetBankTabInfo: Player %s trying to change bank tab info from unexisting tab %d.", tabId);
        return;
    }
...
void Guild::SendInfo(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_INFO [%s]");
}
...
void Guild::SendEventLog(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    sLog->outDebug(LOG_FILTER_GUILD, "MSG_GUILD_EVENT_LOG_QUERY [%s]");
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        session->SendPacket(&data);
        sLog->outDebug(LOG_FILTER_GUILD, "MSG_GUILD_BANK_LOG_QUERY [%s]");
    }
}
...
void Guild::SendPermissions(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    sLog->outDebug(LOG_FILTER_GUILD, "MSG_GUILD_PERMISSIONS [%s] Rank: %u", rankId);
}
...
void Guild::SendMoneyInfo(WorldSession* session) const
{
    ...
    data << int32(amount);
    session->SendPacket(&data);
    sLog->outDebug(LOG_FILTER_GUILD, "MSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: %u", amount);
}
...
void Guild::SendLoginInfo(WorldSession* session)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_EVENT [%s] MOTD");
    ...
    if (dbTabId < _GetPurchasedTabsSize() || isMoneyTab)
    {
        ...
        if (pLog->CanInsert())
        {
            ...
            if (BankEventLogEntry::IsMoneyEvent(eventType))
            {
                if (!isMoneyTab)
                {
                    sLog->OutErrorConsole("GuildBankEventLog ERROR: MoneyEvent(LogGuid: %u, Guild: %u) does not belong to money tab (%u), ignoring...", guid, m_id, dbTabId);
                    return false;
                }
            }
            else if (isMoneyTab)
            {
                sLog->OutErrorConsole("GuildBankEventLog ERROR: non-money event (LogGuid: %u, Guild: %u) belongs to money tab, ignoring...", guid, m_id);
                return false;
            }
...
void Guild::LoadBankTabFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
        sLog->OutErrorConsole("Invalid tab (tabId: %u) in guild bank, skipped.", tabId);
    else
...
bool Guild::LoadBankItemFromDB(Field* fields)
{
    ...
    if (tabId >= _GetPurchasedTabsSize())
    {
        sLog->OutErrorConsole("Invalid tab for item (GUID: %u, id: #%u) in guild bank, skipped.",
...
bool Guild::Validate()
{
    ...
    {
        sLog->OutErrorConsole("Guild %u has invalid number of ranks, creating new...", m_id);
        broken_ranks = true;
    }
    else
    {
        for (uint8 rankId = 0; rankId < ranks; ++rankId)
        {
            ...
            {
                sLog->OutErrorConsole("Guild %u has broken rank id %u, creating default set of ranks...", m_id, rankId);
                broken_ranks = true;
            }
            else
            {
    ...
    if (!GetConfigSettings::GetBoolState("Guild.AllowMultipleGuildMaster", 0))
	{
        for (Members::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
		{
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc)
{
    ...
    _CreateRank(sObjectMgr->GetQuantumSystemText(LANG_GUILD_MASTER,   loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetQuantumSystemText(LANG_GUILD_OFFICER,  loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetQuantumSystemText(LANG_GUILD_VETERAN,  loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetQuantumSystemText(LANG_GUILD_MEMBER,   loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetQuantumSystemText(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
...
void Guild::_SendBankList(WorldSession* session /* = NULL*/, uint8 tabId /*= 0*/, bool sendAllSlots /*= false*/, SlotIds *slots /*= NULL*/) const
{
    ...
    if (session)
    {
        ...
        session->SendPacket(&data);
        sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_BANK_LIST [%s]: TabId: %u, FullSlots: %u, slots: %d", tabId, sendAllSlots, numSlots);
    }
    else // TODO - Probably this is just sent to session + those that have sent CMSG_GUILD_BANKER_ACTIVATE
    {
        for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
        {
            ...
            data.put<uint32>(rempos, numSlots);
            player->GetSession()->SendPacket(&data);
            sLog->outDebug(LOG_FILTER_GUILD, "SMSG_GUILD_BANK_LIST [%s]: TabId: %u, FullSlots: %u, slots: %u", player->GetName(), tabId, sendAllSlots, numSlots);
        }
    }
}
...
bool Guild::HasLevelForBonus(uint8 guildBonus)
{
	switch (guildBonus)
	{
	    case GUILD_BONUS_GOLD_BANK_1:
			return m_guild_level >= sWorld->GetReqGuildLevelForBonus(GUILD_BONUS_GOLD_BANK_1);
			break;
		case GUILD_BONUS_GOLD_BANK_2:
			return m_guild_level >= sWorld->GetReqGuildLevelForBonus(GUILD_BONUS_GOLD_BANK_2);
			break;
		case GUILD_BONUS_FOR_REPAIR_1:
			return m_guild_level >= sWorld->GetReqGuildLevelForBonus(GUILD_BONUS_FOR_REPAIR_1);
			break;
		case GUILD_BONUS_FOR_REPAIR_2:
			return m_guild_level >= sWorld->GetReqGuildLevelForBonus(GUILD_BONUS_FOR_REPAIR_2);
			break;
		...
		case GUILD_BONUS_MOUNT_SPEED_1:
			return m_guild_level >= sWorld->GetReqGuildLevelForBonus(GUILD_BONUS_MOUNT_SPEED_1);
			break;
		case GUILD_BONUS_MOUNT_SPEED_2:
			return m_guild_level >= sWorld->GetReqGuildLevelForBonus(GUILD_BONUS_MOUNT_SPEED_2);
			break;
